...
void CDownloadClientsCtrl::Init()
{
	...
	InsertColumn(i++, GetResString(IDS_CD_CSOFT),			LVCFMT_LEFT,  DFLT_CLIENTSOFT_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_FILE),				LVCFMT_LEFT,  DFLT_FILENAME_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_DL_SPEED),			LVCFMT_RIGHT, DFLT_DATARATE_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_AVAILABLEPARTS),	LVCFMT_LEFT,  DFLT_PARTSTATUS_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_CL_TRANSFDOWN),	LVCFMT_RIGHT, DFLT_SIZE_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_CL_TRANSFUP),		LVCFMT_RIGHT, DFLT_SIZE_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_META_SRCTYPE),		LVCFMT_LEFT,  100);
	...
	switch (iSubItem)
	{
		...
		case 1:
			...
			if ( client->GetClientModVer().IsEmpty() )
				...
				_stprintf(pszText, _T("%s [%s]"), client->GetClientSoftVer(), client->GetClientModVer()); 
			if (pszText[0] == _T('\0'))
...
int CDownloadClientsCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theApp.emuledlg->transferwnd->GetDownloadClientsList()->GetNextSortOrder(static_cast<int>(lParamSort))) != (-1))
