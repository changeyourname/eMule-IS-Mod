...
public:
	...
	void	SetNextSearchID(uint32_t uNextID)				{m_nEd2kSearchID = uNextID;}
	...
	bool	CanDeleteSearch(uint32_t nSearchID) const;
	bool	CanDeleteAllSearches() const;
	void	DeleteSearch(uint32_t nSearchID);
	...
	uint32_t	GetFilterColumn() const				{ return m_nFilterColumn; }
