...
public:
	...
	virtual void	StartDiscovery(uint16_t nTCPPort, uint16_t nUDPPort, uint16_t nTCPWebPort);
	...
	class CStartDiscoveryThread : public CSafeThread
	{
		DECLARE_DYNCREATE(CStartDiscoveryThread)
	protected:
		CStartDiscoveryThread();
		bool	OpenPort(uint16_t nPort, bool bTCP, char* pachLANIP, bool bCheckAndRefresh);
...
private:
	uint16_t	m_nOldUDPPort;
	uint16_t	m_nOldTCPPort;
	uint16_t	m_nOldTCPWebPort;
