...
BOOL CFileDetailDialogInfo::OnInitDialog()
{
	...
	AddAnchor(IDC_FD_X8, TOP_LEFT, BOTTOM_RIGHT);
	
	// general labels
	AddAnchor(IDC_FD_X1, TOP_LEFT);
	AddAnchor(IDC_FD_X2, TOP_LEFT);
	AddAnchor(IDC_FD_X3, TOP_LEFT);
	AddAnchor(IDC_FD_X4, TOP_LEFT);
	AddAnchor(IDC_FD_X10, TOP_LEFT);
	AddAnchor(IDC_FD_X9, TOP_LEFT);
	AddAnchor(IDC_FD_X5, TOP_LEFT);
	AddAnchor(IDC_HSAV, TOP_LEFT);
	...
	AddAnchor(IDC_PFSTATUS, TOP_LEFT);
	AddAnchor(IDC_HASHSET, TOP_LEFT, TOP_RIGHT);

	// transfer labels
	AddAnchor(IDC_FD_X7, TOP_LEFT);
	AddAnchor(IDC_FD_X13, TOP_LEFT);
	AddAnchor(IDC_FD_X14, TOP_LEFT);
	AddAnchor(IDC_FD_X12, TOP_LEFT);
	AddAnchor(IDC_REMAINING_TEXT, TOP_LEFT);
	AddAnchor(IDC_FD_COMPR, TOP_LEFT);
	AddAnchor(IDC_FD_CORR, TOP_LEFT);
	AddAnchor(IDC_FD_RECOV, TOP_LEFT);

	// transfer values
	AddAnchor(IDC_SOURCECOUNT, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_DATARATE, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_TRANSFERRED, TOP_LEFT);
	AddAnchor(IDC_COMPLSIZE, TOP_LEFT);
	AddAnchor(IDC_REMAINING, TOP_LEFT);
	AddAnchor(IDC_COMPRESSION, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_CORRUPTED, TOP_LEFT, TOP_RIGHT);
	AddAnchor(IDC_RECOVERED, TOP_LEFT, TOP_RIGHT);

	// dates labels
	AddAnchor(IDC_FD_X16, TOP_LEFT);
	AddAnchor(IDC_DL_ACTIVE_TIME_LBL, TOP_LEFT);
	AddAnchor(IDC_FD_X15, TOP_LEFT);
	AddAnchor(IDC_FD_LASTCHANGE, TOP_LEFT);

	// dates values
	AddAnchor(IDC_FILECREATED, TOP_LEFT);
	AddAnchor(IDC_DL_ACTIVE_TIME, TOP_LEFT);
	AddAnchor(IDC_LASTSEENCOMPL, TOP_LEFT);
	AddAnchor(IDC_LASTRECEIVED, TOP_LEFT);
...
void CFileDetailDialogInfo::RefreshData()
{
	...
	if (m_paFiles->GetSize() == 1)
	{
		...
		str.Format(_T("%u;  %s: %u (%.1f%%)"), file->partCount(), GetResString(IDS_AVAILABLE) , file->GetAvailablePartCount(), (float)((file->GetAvailablePartCount()*100)/file->partCount()));
		...
		// last receive
		if (file->GetFileDate() != 0 && file->GetRealFileSize() > (uint64_t)0)
		{
	...
	uint64_t uRealFileSize = 0;
	uint64_t uTransferred = 0;
	uint64_t uCorrupted = 0;
	uint64_t uCompression = 0;
	uint64_t uCompleted = 0;
	int iHashsetAvailable = 0;
	// ismod: no aich
	uint32_t uDataRate = 0;
	UINT uSources = 0;
	...
	for (int i = 0; i < m_paFiles->GetSize(); i++)
	{
		...
		uFileSize += (uint64_t)file->GetFileSize();
		uRealFileSize += (uint64_t)file->GetRealFileSize();
		uTransferred += (uint64_t)file->GetTransferred();
		uCorrupted += file->GetCorruptionLoss();
		uCompression += file->GetCompressionGain();
		uDataRate += file->GetDatarate();
		uCompleted += (uint64_t)file->GetCompletedSize();
		iHashsetAvailable += (file->hashset().count() == file->GetED2KPartCount()) ? 1 : 0;	// SLUGFILLER: SafeHash - use GetED2KPartCount
		// ismod: no aich
