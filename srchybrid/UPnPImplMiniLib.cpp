...
void CUPnPImplMiniLib::StartDiscovery(uint16_t nTCPPort, uint16_t nUDPPort, uint16_t nTCPWebPort){
	...
	CStartDiscoveryThread* pStartDiscoveryThread = (CStartDiscoveryThread*) CSafeThread::BeginThread(RUNTIME_CLASS(CStartDiscoveryThread), THREAD_PRIORITY_NORMAL,0, CREATE_SUSPENDED);
...
IMPLEMENT_DYNCREATE(CStartDiscoveryThread, CSafeThread)
...
bool CUPnPImplMiniLib::CStartDiscoveryThread::OpenPort(uint16_t nPort, bool bTCP, char* pachLANIP, bool bCheckAndRefresh){
	const char achTCP[] = "TCP";
	...
	if (bCheckAndRefresh)
	{
		...
		if (bTCP)
			...
			, achPort, achTCP, achOutIP, achOutPort, 0, 0, 0);
		else
			nResult = UPNP_GetSpecificPortMappingEntry(m_pOwner->m_pURLs->controlURL, m_pOwner->m_pIGDData->first.servicetype
			, achPort, achUDP, achOutIP, achOutPort, 0, 0, 0);
	...
	if (bTCP)
		...
		, achPort, achPort, pachLANIP, achDescTCP, achTCP, NULL, 0);
	else
		nResult = UPNP_AddPortMapping(m_pOwner->m_pURLs->controlURL, m_pOwner->m_pIGDData->first.servicetype
		, achPort, achPort, pachLANIP, achDescUDP, achUDP, NULL, 0);
	...
	if (nResult == UPNPCOMMAND_SUCCESS && achOutIP[0] != 0){
		DebugLog(_T("Successfully added mapping for port %u (%s) on local IP %S"), nPort, bTCP ? _T("TCP") : _T("UDP"), achOutIP);
		return true;
	}
	else {
		DebugLogWarning(_T("Failed to verify mapping for port %u (%s) on local IP %S - considering as failed"), nPort, bTCP ? _T("TCP") : _T("UDP"), achOutIP);
