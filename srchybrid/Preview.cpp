...
IMPLEMENT_DYNCREATE(CPreviewThread, CSafeThread)

BEGIN_MESSAGE_MAP(CPreviewThread, CSafeThread)
END_MESSAGE_MAP()
...
BOOL CPreviewThread::Run()
{
	...
	if (!srcFile.Open(m_pPartfile->GetFilePath(), CFile::modeRead | CFile::shareDenyNone))
		return FALSE;
	try{
		uint64_t nSize = m_pPartfile->GetFileSize();
...
int CPreviewApps::ReadAllApps()
{
		...
		while (!feof(readFile))
		{
			...
			if (!strTitle.IsEmpty())
			{
				...
				if (!strCommandLine.IsEmpty())
				{
					...
					{
						uint64_t ullMinCompletedSize = 0;
						uint64_t ullMinStartOfFile = 0;
	...
	return static_cast<int>(m_aApps.GetCount());
}
...
int CPreviewApps::GetAllMenuEntries(CMenu& rMenu, const CPartFile* file)
{
	...
	for (int i = 0; i < m_aApps.GetCount(); i++)
	{
		...
		if (file)
		{
			if (file->GetCompletedSize() >= (uint64_t)16*1024)
				bEnabled = true;
		}
...
void ExecutePartFile(CPartFile* file, LPCTSTR pszCommand, LPCTSTR pszCommandArgs)
{
	...
	if ( ShellExecuteEx(&sei) ) {
		file->setPreviewProcess(sei.hProcess);
	} else {
		AfxMessageBox(_T("Failed to start preview"), MB_ICONSTOP);
	}
}
