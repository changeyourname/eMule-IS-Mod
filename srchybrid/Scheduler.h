...
struct Schedule_Struct{
   ...
   CString			values[16];
   int				actions[16];
   void ResetActions()	{for (uint8_t index=0;index<16;index++) {actions[index]=0;values[index]=_T("");}}
   ~Schedule_Struct() {  }
};
...
public:
	...
	UINT	GetCount()		{ return (UINT) schedulelist.GetCount();}
	void	SaveOriginals();
	void	RestoreOriginals();
	void	ActivateSchedule(int index,bool makedefault=false);
	
	uint16_t	original_upload;
	uint16_t	original_download;
