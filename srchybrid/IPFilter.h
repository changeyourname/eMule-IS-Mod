...
struct SIPFilter
{
	SIPFilter(uint32_t newStart, uint32_t newEnd, UINT newLevel, const CStringA& newDesc)
		: start(newStart),
		  end(newEnd),
		  level(newLevel),
		  desc(newDesc),
...
public:
	...
	void AddIPRange(uint32_t start, uint32_t end, UINT level, const CStringA& rstrDesc) {
	...
	bool IsFiltered(uint32_t IP) /*const*/;
	bool IsFiltered(uint32_t IP, UINT level) /*const*/;
...
private:
	...
	bool ParseFilterLine1(const CStringA& rstrBuffer, uint32_t& ip1, uint32_t& ip2, UINT& level, CStringA& rstrDesc) const;
	bool ParseFilterLine2(const CStringA& rstrBuffer, uint32_t& ip1, uint32_t& ip2, UINT& level, CStringA& rstrDesc) const;
};
