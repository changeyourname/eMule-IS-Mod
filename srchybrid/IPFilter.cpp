...
bool CIPFilter::ParseFilterLine1(const CStringA& sbuffer, uint32_t& ip1, uint32_t& ip2, UINT& level, CStringA& desc) const
{
	UINT u1, u2, u3, u4, u5, u6, u7, u8, uLevel = DFLT_FILTER_LEVEL;
	int iDescStart = 0;
	int iItems = sscanf_s(sbuffer, "%u.%u.%u.%u - %u.%u.%u.%u , %u , %n", &u1, &u2, &u3, &u4, &u5, &u6, &u7, &u8, &uLevel, &iDescStart);
	if (iItems < 8)
		return false;
...
bool CIPFilter::ParseFilterLine2(const CStringA& sbuffer, uint32_t& ip1, uint32_t& ip2, UINT& level, CStringA& desc) const
{
...
bool CIPFilter::IsFiltered(uint32_t ip) /*const*/
{
...
bool CIPFilter::IsFiltered(uint32_t ip, UINT level) /*const*/
{
	/* ismod
	if (m_iplist.GetCount() == 0 || ip == 0)
	*/
	if (m_iplist.GetCount() == 0 || ip == 0 || ip == theApp.GetServerIP())
		return false;
