...
void DebugSend(LPCTSTR pszMsg, uint32_t uIP, uint16_t uPort);
...
{
	m_bAbortLoading = false;
	CLoadDataThread* pLoadDataThread = (CLoadDataThread*) CSafeThread::BeginThread(RUNTIME_CLASS(CLoadDataThread), THREAD_PRIORITY_BELOW_NORMAL,0, CREATE_SUSPENDED);
	if (pLoadDataThread == NULL)
...
{ 
	if (!m_bDataLoaded){
		...
		try
		{
			uint32_t uTotalSource = 0;
			uint32_t uTotalKey = 0;
			uint32_t uTotalLoad = 0;
			...
			if(fileLoad.Open(m_sLoadFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				fileLoad.WriteUInt32(ConvertToTime32(time(NULL)));
				fileLoad.WriteUInt32(static_cast<uint32_t>(m_mapLoad.GetCount()));
			...
			if (fileSource.Open(m_sSourceFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				fileSource.WriteUInt32(ConvertToTime32(time(NULL)+KADEMLIAREPUBLISHTIMES));
				fileSource.WriteUInt32(static_cast<uint32_t>(m_mapSources.GetCount()));
				POSITION pos1 = m_mapSources.GetStartPosition();
				while( pos1 != NULL )
				{
					CCKey key1;
					...
					CKadSourcePtrList* keyHashSrcMap = &pCurrSrcHash->ptrlistSource;
					fileSource.WriteUInt32(static_cast<uint32_t>(keyHashSrcMap->GetCount()));
					POSITION pos2 = keyHashSrcMap->GetHeadPosition();
					while( pos2 != NULL )
					{
						Source* pCurrSource = keyHashSrcMap->GetNext(pos2);
						...
						CKadEntryPtrList* srcEntryList = &pCurrSource->ptrlEntryList;
						fileSource.WriteUInt32(static_cast<uint32_t>(srcEntryList->GetCount()));
						for(POSITION pos3 = srcEntryList->GetHeadPosition(); pos3 != NULL; )
						{
							CEntry* pCurrName = srcEntryList->GetNext(pos3);
							fileSource.WriteUInt32(ConvertToTime32(pCurrName->m_tLifetime));
			...
			if (fileKey.Open(m_sKeyFileName, CFile::modeWrite | CFile::modeCreate | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				fileKey.WriteUInt32(ConvertToTime32(time(NULL)+KADEMLIAREPUBLISHTIMEK));
				fileKey.WriteUInt128(Kademlia::CKademlia::GetPrefs()->GetKadID());
				fileKey.WriteUInt32(static_cast<uint32_t>(m_mapKeyword.GetCount()));
				...
				while( pos1 != NULL )
				{
					...
					CSourceKeyMap* keySrcKeyMap = &pCurrKeyHash->mapSource;
					fileKey.WriteUInt32(static_cast<uint32_t>(keySrcKeyMap->GetCount()));
					POSITION pos2 = keySrcKeyMap->GetStartPosition();
					while( pos2 != NULL )
					{
						Source* pCurrSource;
						...
						CKadEntryPtrList* srcEntryList = &pCurrSource->ptrlEntryList;
						fileKey.WriteUInt32(static_cast<uint32_t>(srcEntryList->GetCount()));
						for(POSITION pos3 = srcEntryList->GetHeadPosition(); pos3 != NULL; )
						{
							CKeyEntry* pCurrName = (CKeyEntry*)srcEntryList->GetNext(pos3);
							ASSERT( pCurrName->IsKeyEntry() );
							fileKey.WriteUInt32(ConvertToTime32(pCurrName->m_tLifetime));
...
void CIndexed::Clean(void)
{
	...
	try
	{
		...
		uint32_t uRemovedKey = 0;
		uint32_t uRemovedSource = 0;
		uint32_t uTotalSource = 0;
		uint32_t uTotalKey = 0;
...
bool CIndexed::AddKeyword(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CKeyEntry* pEntry, uint8_t& uLoad, bool bIgnoreThreadLock)
{
	...
	{
		uint32_t uIndexTotal = static_cast<uint32_t>(pCurrKeyHash->mapSource.GetCount());
		...
		if(pCurrKeyHash->mapSource.Lookup(CCKey(uSourceID.GetData()), pCurrSource))
		{
			...
			uLoad = (uint8_t)((uIndexTotal*100)/KADEMLIAMAXINDEX);
			pCurrSource->ptrlEntryList.AddHead(pEntry);
			return true;
		}
		else
...
bool CIndexed::AddSources(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CEntry* pEntry, uint8_t& uLoad, bool bIgnoreThreadLock)
{
	// do not access any data while the loading thread is busy;
	// bIgnoreThreadLock should be only used by CLoadDataThread itself
	if (!bIgnoreThreadLock && !m_bDataLoaded) {
	...
	{
		uint32_t uSize = static_cast<uint32_t>(pCurrSrcHash->ptrlistSource.GetSize());
		for(POSITION pos1 = pCurrSrcHash->ptrlistSource.GetHeadPosition(); pos1 != NULL; )
		{
			...
			if( pCurrSource->ptrlEntryList.GetSize() )
			{
				...
				if( pCurrEntry->m_uIP == pEntry->m_uIP && ( pCurrEntry->m_uTCPPort == pEntry->m_uTCPPort || pCurrEntry->m_uUDPPort == pEntry->m_uUDPPort ))
				{
					...
					pCurrSource->ptrlEntryList.AddHead(pEntry);
					uLoad = (uint8_t)((uSize*100)/KADEMLIAMAXSOUCEPERFILE);
					return true;
				}
			}
			else
...
bool CIndexed::AddNotes(const CUInt128& uKeyID, const CUInt128& uSourceID, Kademlia::CEntry* pEntry, uint8_t& uLoad, bool bIgnoreThreadLock)
{
	// do not access any data while the loading thread is busy;
	// bIgnoreThreadLock should be only used by CLoadDataThread itself
	if (!bIgnoreThreadLock && !m_bDataLoaded) {
	...
	{
		uint32_t uSize = static_cast<uint32_t>(pCurrNoteHash->ptrlistSource.GetSize());
		for(POSITION pos1 = pCurrNoteHash->ptrlistSource.GetHeadPosition(); pos1 != NULL; )
		{
			...
			if( pCurrNote->ptrlEntryList.GetSize() )
			{
				...
				if(pCurrEntry->m_uIP == pEntry->m_uIP || pCurrEntry->m_uSourceID == pEntry->m_uSourceID)
				{
					...
					pCurrNote->ptrlEntryList.AddHead(pEntry);
					uLoad = (uint8_t)((uSize*100)/KADEMLIAMAXNOTESPERFILE);
					return true;
				}
			}
			else
...
bool CIndexed::AddLoad(const CUInt128& uKeyID, uint32_t uTime, bool bIgnoreThreadLock)
{
	// do not access any data while the loading thread is busy;
	// bIgnoreThreadLock should be only used by CLoadDataThread itself
	if (!bIgnoreThreadLock && !m_bDataLoaded) {
...
void CIndexed::SendValidKeywordResult(const CUInt128& uKeyID, const SSearchTerm* pSearchTerms, uint32_t uIP, uint16_t uPort, bool bOldClient, bool bKad2, uint16_t uStartPosition, CKadUDPKey senderUDPKey)
{
	// do not access any data while the loading thread is busy;
	if (!m_bDataLoaded) {
		DEBUG_ONLY( DebugLogWarning(_T("CIndexed Memberfunction call failed because the dataloading still in progress")) );
	...
	if(m_mapKeyword.Lookup(CCKey(uKeyID.GetData()), pCurrKeyHash))
	{
		...
		const uint16_t uMaxResults = 300;
			...
			while( pos1 != NULL )
			{
				...
				for(POSITION pos2 = pCurrSource->ptrlEntryList.GetHeadPosition(); pos2 != NULL; )
				{
					...
					if ( (bOnlyTrusted ^ (pCurrName->GetTrustValue() < 1.0f)) && (!pSearchTerms || pCurrName->StartSearchTermsMatch(pSearchTerms)) )
					{
						if( iCount < 0 )
							iCount++;
						else if( (uint16_t)iCount < uMaxResults )
						{
							if((!bOldClient || pCurrName->m_uSize <= OLD_MAX_EMULE_FILE_SIZE))
							{
								...
								{
									uint32_t uLen = sizeof(byPacket)-byIO.GetAvailable();
									PokeUInt16(pbyCountPos, (uint16_t)iUnsentCount);
...
void CIndexed::SendValidSourceResult(const CUInt128& uKeyID, uint32_t uIP, uint16_t uPort, bool bKad2, uint16_t uStartPosition, uint64_t uFileSize, CKadUDPKey senderUDPKey)
{
	// do not access any data while the loading thread is busy;
	if (!m_bDataLoaded) {
		DEBUG_ONLY( DebugLogWarning(_T("CIndexed Memberfunction call failed because the dataloading still in progress")) );
...
void CIndexed::SendValidNoteResult(const CUInt128& uKeyID, uint32_t uIP, uint16_t uPort, bool bKad2, uint64_t uFileSize, CKadUDPKey senderUDPKey)
{
	// do not access any data while the loading thread is busy;
	if (!m_bDataLoaded) {
		DEBUG_ONLY( DebugLogWarning(_T("CIndexed Memberfunction call failed because the dataloading still in progress")) );
...
bool CIndexed::SendStoreRequest(const CUInt128& uKeyID)
{
	...
	if(m_mapLoad.Lookup(CCKey(uKeyID.GetData()), pLoad))
	{
		if(pLoad->uTime < (uint32_t)time(NULL))
		{
...
uint32_t CIndexed::GetFileKeyCount()
{
	// do not access any data while the loading thread is busy;
	if (!m_bDataLoaded) {
		DEBUG_ONLY( DebugLogWarning(_T("CIndexed Memberfunction call failed because the dataloading still in progress")) );
	...
	return static_cast<uint32_t>(m_mapKeyword.GetCount());
}
...
IMPLEMENT_DYNCREATE(CLoadDataThread, CSafeThread)
...
int CIndexed::CLoadDataThread::Run()
{
	...
	try
	{
		...
		if (!m_pOwner->m_bAbortLoading)
		{
			...
			{
				setvbuf(fileLoad.m_pStream, NULL, _IOFBF, 32768);
				uint32_t uVersion = fileLoad.ReadUInt32();
				if(uVersion<2)
				{
					/*time_t tSaveTime = */fileLoad.ReadUInt32();
					uint32_t uNumLoad = fileLoad.ReadUInt32();
		...
		if (!m_pOwner->m_bAbortLoading)
		{
			...
			if (fileKey.Open(m_sKeyFileName, CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				uint32_t uVersion = fileKey.ReadUInt32();
				if( uVersion < 4)
				{
							...
							while( uNumKeys && !m_pOwner->m_bAbortLoading )
							{
								...
								while( uNumSource && !m_pOwner->m_bAbortLoading )
								{
									...
									while( uNumName && !m_pOwner->m_bAbortLoading)
									{
										...
										uint32_t uTotalTags = fileKey.ReadByte();
										while( uTotalTags )
										{
											...
											if(pTag)
											{
												...
												{
													pToAdd->m_uIP = (uint32_t)pTag->GetInt();
													pToAdd->AddTag(pTag);
												}
												else if (!pTag->m_name.Compare(TAG_SOURCEPORT))
												{
													pToAdd->m_uTCPPort = (uint16_t)pTag->GetInt();
													pToAdd->AddTag(pTag);
												}
												else if (!pTag->m_name.Compare(TAG_SOURCEUPORT))
												{
													pToAdd->m_uUDPPort = (uint16_t)pTag->GetInt();
													pToAdd->AddTag(pTag);
												}
												else
												{
		...
		if (!m_pOwner->m_bAbortLoading)
		{
			...
			if (fileSource.Open(m_sSourceFileName, CFile::modeRead | CFile::typeBinary | CFile::shareDenyWrite))
			{
				...
				uint32_t uVersion = fileSource.ReadUInt32();
				if( uVersion < 3 )
				{
					time_t tSaveTime = fileSource.ReadUInt32();
					if( tSaveTime > time(NULL) )
					{
						uint32_t uNumKeys = fileSource.ReadUInt32();
						while( uNumKeys && !m_pOwner->m_bAbortLoading )
						{
							fileSource.ReadUInt128(&uKeyID);
							uint32_t uNumSource = fileSource.ReadUInt32();
							while( uNumSource && !m_pOwner->m_bAbortLoading )
							{
								fileSource.ReadUInt128(&uSourceID);
								uint32_t uNumName = fileSource.ReadUInt32();
								while( uNumName && !m_pOwner->m_bAbortLoading )
								{
									CEntry* pToAdd = new Kademlia::CEntry();
									pToAdd->m_bSource = true;
									pToAdd->m_tLifetime = fileSource.ReadUInt32();
									uint32_t uTotalTags = fileSource.ReadByte();
									while( uTotalTags )
									{
