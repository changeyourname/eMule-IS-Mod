...
void CPrefs::SetIPAddress(uint32_t uVal)
{
...
uint32_t CPrefs::GetIPAddress() const
{
	return m_uIP;
}
...
void CPrefs::SetTotalFile(uint8_t uVal)
{
	m_uTotalFile = uVal;
}

uint8_t CPrefs::GetTotalStoreSrc() const
{
	return m_uTotalStoreSrc;
}

void CPrefs::SetTotalStoreSrc(uint8_t uVal)
{
	m_uTotalStoreSrc = uVal;
}

uint8_t CPrefs::GetTotalStoreKey() const
{
	return m_uTotalStoreKey;
}

void CPrefs::SetTotalStoreKey(uint8_t uVal)
{
	m_uTotalStoreKey = uVal;
}

uint8_t CPrefs::GetTotalSource() const
{
	return m_uTotalSource;
}

void CPrefs::SetTotalSource(uint8_t uVal)
{
	m_uTotalSource = uVal;
}

uint8_t CPrefs::GetTotalNotes() const
{
	return m_uTotalNotes;
}

void CPrefs::SetTotalNotes(uint8_t uVal)
{
	m_uTotalNotes = uVal;
}

uint8_t CPrefs::GetTotalStoreNotes() const
{
	return m_uTotalStoreNotes;
}

void CPrefs::SetTotalStoreNotes(uint8_t uVal)
{
	m_uTotalStoreNotes = uVal;
}

uint32_t CPrefs::GetKademliaUsers() const
{
	return m_uKademliaUsers;
}

void CPrefs::SetKademliaUsers(uint32_t uVal)
{
	m_uKademliaUsers = uVal;
}

uint32_t CPrefs::GetKademliaFiles() const
{
...
uint32_t CPrefs::GetUDPVerifyKey(uint32_t dwTargetIP) {
	uint64_t ui64Buffer = thePrefs.GetKadUDPKey();
	ui64Buffer <<= 32;
	ui64Buffer |= dwTargetIP;
	MD5Sum md5((uchar*)&ui64Buffer, 8);
	return ((uint32_t)(PeekUInt32(md5.GetRawHash() + 0) ^ PeekUInt32(md5.GetRawHash() + 4) ^ PeekUInt32(md5.GetRawHash() + 8) ^ PeekUInt32(md5.GetRawHash() + 12)) % 0xFFFFFFFE) + 1; 
}
...
uint16_t CPrefs::GetExternalKadPort() const
{
	return m_nExternKadPort;
}

#define EXTERNAL_PORT_ASKIPS	3
void CPrefs::SetExternKadPort(uint16_t uVal, uint32_t uFromIP)
{
...
uint16_t CPrefs::GetInternKadPort() const
{
...
uint8_t CPrefs::GetMyConnectOptions(bool bEncryption, bool bCallback){
