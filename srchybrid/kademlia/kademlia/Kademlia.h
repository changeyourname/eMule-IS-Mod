...
namespace Kademlia
{
	...
	class CKademlia
	{
		public:
			...
			static uint32_t GetKademliaUsers(bool bNewMethod = false);
			...
			static void	Bootstrap(uint32_t uIP, uint16_t uPort, bool bKad2);
			static void	Bootstrap(LPCTSTR szHost, uint16_t uPort, bool bKad2);
			static void	ProcessPacket(const byte* pbyData, uint32_t uLenData, uint32_t uIP, uint16_t uPort, bool bValidReceiverKey, CKadUDPKey senderUDPKey);
			...
			static bool	FindNodeIDByIP(CKadClientSearcher& rRequester, uint32_t dwIP, uint16_t nTCPPort, uint16_t nUDPPort);
		...
		private:
			...
			static CList<uint32_t, uint32_t> m_liStatsEstUsersProbes;
			CPrefs *m_pPrefs;
