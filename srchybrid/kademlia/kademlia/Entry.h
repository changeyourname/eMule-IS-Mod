...
namespace Kademlia
{
	...
	class CEntry
	{
		...
		public:
			...
			uint64_t		GetIntTagValue(CKadTagNameString strTagName, bool bIncludeVirtualTags = true) const;
			bool		GetIntTagValue(CKadTagNameString strTagName, uint64_t& rValue, bool bIncludeVirtualTags = true) const;
			CKadTagValueString GetStrTagValue(CKadTagNameString strTagName) const;
			void		AddTag(CKadTag* pTag)			{ m_listTag.push_back(pTag); }
			uint32_t		GetTagCount() const; // Adds filename and size to the count if not empty, even if they are not stored as tags
		...
		protected:
			void		WriteTagListInc(CDataIO* pData, uint32_t nIncreaseTagNumber = 0);
	...
	class CKeyEntry : public CEntry
	{
		...
		protected:
			...
			static void			AdjustGlobalPublishTracking(uint32_t uIP, bool bIncrease, CString strDbgReason);
			
			// netfinity: Rearranged for alignment reasons
			CList<structPublishingIP> *m_pliPublishingIPs;
			static CMap<uint32_t, uint32_t, uint32_t, uint32_t> s_mapGlobalPublishIPs; // tracks count of publishings for each 255.255.255.0/28 subnet
			uint32_t	dwLastTrustValueCalc;			
			float	m_fTrustValue;
