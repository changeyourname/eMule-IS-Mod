...
uint32_t CSearchManager::m_uNextID = 0;
SearchMap CSearchManager::m_mapSearches;

bool CSearchManager::IsSearching(uint32_t uSearchID)
{
...
void CSearchManager::StopSearch(uint32_t uSearchID, bool bDelayDelete)
{
...
CSearch* CSearchManager::PrepareLookup(uint32_t uType, bool bStart, const CUInt128 &uID)
{
...
void CSearchManager::UpdateStats()
{
	...
	uint8_t uTotalFile = 0;
	uint8_t uTotalStoreSrc = 0;
	uint8_t uTotalStoreKey = 0;
	uint8_t uTotalSource = 0;
	uint8_t uTotalNotes = 0;
	uint8_t uTotalStoreNotes = 0;
	for (SearchMap::const_iterator itSearchMap = m_mapSearches.begin(); itSearchMap != m_mapSearches.end(); ++itSearchMap)
	{
		switch(itSearchMap->second->GetSearchTypes())
		{
...
void CSearchManager::ProcessPublishResult(const CUInt128 &uTarget, const uint8_t uLoad, const bool bLoadResponse)
{
...
void CSearchManager::ProcessResponse(const CUInt128 &uTarget, uint32_t uFromIP, uint16_t uFromPort, ContactList *plistResults)
{
