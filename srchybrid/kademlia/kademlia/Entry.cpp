...
CMap<uint32_t, uint32_t, uint32_t, uint32_t> CKeyEntry::s_mapGlobalPublishIPs;
...
uint64_t CEntry::GetIntTagValue(CKadTagNameString strTagName, bool bIncludeVirtualTags) const
{
	uint64_t uResult = 0;
...
bool CEntry::GetIntTagValue(CKadTagNameString strTagName, uint64_t& rValue, bool bIncludeVirtualTags) const
{
	for (TagList::const_iterator itTagList = m_listTag.begin(); itTagList != m_listTag.end(); ++itTagList)
	{
		CKadTag* pTag = *itTagList;
...
CKadTagValueString CEntry::GetCommonFileName() const
{
	...
	uint32_t nHighestPopularityIndex = 0;
	for (POSITION pos = m_listFileNames.GetHeadPosition(); pos != NULL;){
...
uint32_t CEntry::GetTagCount() const // Adds filename and size to the count if not empty, even if they are not stored as tags
{
	return m_listTag.size() + ((m_uSize != 0) ? 1 : 0) + (GetCommonFileName().IsEmpty() ? 0 : 1);
}

void CEntry::WriteTagListInc(CDataIO* pData, uint32_t nIncreaseTagNumber){
	...
	uint32_t uCount = GetTagCount() + nIncreaseTagNumber; // will include name and size tag in the count if needed
	ASSERT( uCount <= 0xFF );
	pData->WriteByte((uint8_t)uCount);
...
bool CKeyEntry::SearchTermsMatch(const SSearchTerm* pSearchTerm) const
{
	...
	{
		int iStrSearchTerms = static_cast<int>(pSearchTerm->m_pastr->GetCount());
...
void CKeyEntry::AdjustGlobalPublishTracking(uint32_t uIP, bool bIncrease, CString strDbgReason){
	uint32_t nCount = 0;
...
void CKeyEntry::MergeIPsAndFilenames(CKeyEntry* pFromEntry){
	...
	if (!bRefresh){
		ASSERT( m_uIP != 0 );
		structPublishingIP add = { time(NULL), m_uIP };
...
void CKeyEntry::WritePublishTrackingDataToFile(CDataIO* pData){
	...
	pData->WriteUInt32((uint32_t)m_listFileNames.GetCount());
	for (POSITION pos = m_listFileNames.GetHeadPosition(); pos != NULL;){
		const structFileNameEntry& rCur = m_listFileNames.GetNext(pos);
	...
	if (m_pliPublishingIPs != NULL){
		pData->WriteUInt32((uint32_t)m_pliPublishingIPs->GetCount());
		for (POSITION pos = m_pliPublishingIPs->GetHeadPosition(); pos != NULL;){
			...
			pData->WriteUInt32((uint32_t)rCur.m_tLastPublish);
		}
	}
	else{
		ASSERT( false );
...
void CKeyEntry::ReadPublishTrackingDataFromFile(CDataIO* pData){
	// format: <Names_Count 4><{<Name string><PopularityIndex 4>} Names_Count><PublisherCount 4><{<IP 4><Time 4>} PublisherCount>
	ASSERT( m_listFileNames.IsEmpty() );
	uint32_t nNameCount = pData->ReadUInt32();
	for (uint32_t i = 0; i < nNameCount; i++){
	...
	m_pliPublishingIPs = new CList<structPublishingIP>();
	uint32_t nIPCount = pData->ReadUInt32();
	time_t nDbgLastTime = 0;
	for (uint32_t i = 0; i < nIPCount; i++){
		structPublishingIP sToAdd;
		sToAdd.m_uIP = pData->ReadUInt32();
		ASSERT( sToAdd.m_uIP != 0 );
		sToAdd.m_tLastPublish = pData->ReadUInt32();
		ASSERT( nDbgLastTime <= (uint32_t)sToAdd.m_tLastPublish ); // shoudl always be sorted oldest first
...
void CKeyEntry::WriteTagListWithPublishInfo(CDataIO* pData){
	...
	uint32_t uTrust = (uint16_t)(GetTrustValue() * 100);
	uint32_t uPublishers = static_cast<uint32_t>(m_pliPublishingIPs->GetCount()) % 256;
	uint32_t uNames = static_cast<uint32_t>(m_listFileNames.GetCount()) % 256;
	// 32 bit tag: <namecount uint8><publishers uint8><trustvalue*100 uint16>
	uint32_t uTagValue = (uNames << 24) | (uPublishers << 16) | (uTrust << 0);
