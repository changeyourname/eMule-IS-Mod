...
CList<uint32_t, uint32_t> CKademlia::m_liStatsEstUsersProbes;
...
uint32_t CKademlia::GetKademliaUsers(bool bNewMethod)
{
	if( m_pInstance && m_pInstance->m_pPrefs ){
		if (bNewMethod)
...
uint32_t CKademlia::GetKademliaFiles()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint32_t CKademlia::GetTotalStoreKey()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint32_t CKademlia::GetTotalStoreSrc()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint32_t CKademlia::GetTotalStoreNotes()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint32_t CKademlia::GetTotalFile()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
uint32_t CKademlia::GetIPAddress()
{
	if( m_pInstance && m_pInstance->m_pPrefs )
...
void CKademlia::ProcessPacket(const byte *pbyData, uint32_t uLenData, uint32_t uIP, uint16_t uPort, bool bValidReceiverKey, CKadUDPKey senderUDPKey)
{
...
void CKademlia::Bootstrap(LPCTSTR szHost, uint16_t uPort, bool bKad2)
{
...
void CKademlia::Bootstrap(uint32_t uIP, uint16_t uPort, bool bKad2)
{
	if( m_pInstance && m_pInstance->m_pUDPListener && !IsConnected() && time(NULL) - m_tBootstrap > 10 ){
		m_tBootstrap = time(NULL);
