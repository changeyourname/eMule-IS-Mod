...
namespace Kademlia
{
	...
	class CSearchManager
	{
		...
		public:
			static bool IsSearching(uint32_t uSearchID);
			static void StopSearch(uint32_t uSearchID, bool bDelayDelete);
			static void StopAllSearches();
			// Search for a particular file
			// Will return unique search id, returns zero if already searching for this file.
			static CSearch* PrepareLookup(uint32_t uType, bool bStart, const CUInt128 &uID);
			...
			static void ProcessResponse(const CUInt128 &uTarget, uint32_t uFromIP, uint16_t uFromPort, ContactList *plistResults);
			static void ProcessResult(const CUInt128 &uTarget, const CUInt128 &uAnswer, TagList *plistInfo);
			static void ProcessPublishResult(const CUInt128 &uTarget, const uint8_t uLoad, const bool bLoadResponse);
			...
			static void SetNextSearchID(uint32_t uNextID)				{m_uNextID = uNextID;}
		private:
