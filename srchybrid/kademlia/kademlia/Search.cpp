...
void DebugSend(LPCTSTR pszMsg, uint32_t uIP, uint16_t uUDPPort);
...
void CSearch::ProcessResponse(uint32_t uFromIP, uint16_t uFromPort, ContactList *plistResults)
{
		...
		case KEYWORD:
			{
				...
				if (pFromContact->GetVersion() >= 3/*47b*/)
				{
					if (m_uSearchTermsDataSize == 0)
					{
						...
						// Start position range (0x0 to 0x7FFF)
						m_pfileSearchTerms.WriteUInt16((uint16_t)0x0000);
					}
					else
					{
						// JOHNTODO - Need to add ability to change start position.
						// Start position range (0x8000 to 0xFFFF)
						m_pfileSearchTerms.WriteUInt16((uint16_t)0x8000);
		...
		case STOREKEYWORD:
			{
				...
				uint16_t iCount = (uint16_t)m_listFileIDs.size();
				...
				while(iCount && (itListFileID != m_listFileIDs.end()))
				{
					...
					// Correct file count.
					uint32_t current_pos = byIO.GetUsed();
					byIO.Seek(16);
...
void CSearch::ProcessResultFile(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	for (TagList::const_iterator itTagList = plistInfo->begin(); itTagList != plistInfo->end(); ++itTagList)
	{
		...
		else if (!pTag->m_name.Compare(TAG_BUDDYHASH))
		{
			...
			byCryptOptions = (uint8_t)pTag->GetInt();
...
void CSearch::ProcessResultNotes(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	for (TagList::const_iterator itTagList = plistInfo->begin(); itTagList != plistInfo->end(); ++itTagList)
	{
		...
		{
			pEntry->m_uIP = (uint32_t)pTag->GetInt();
			delete pTag;
		}
		else if (!pTag->m_name.Compare(TAG_SOURCEPORT))
		{
...
void CSearch::ProcessResultKeyword(const CUInt128 &uAnswer, TagList *plistInfo)
{
	...
	uint32_t uBitrate = 0;
	uint32_t uAvailability = 0;
	uint32_t uPublishInfo = 0;
	...
	for (TagList::const_iterator itTagList = plistInfo->begin(); itTagList != plistInfo->end(); ++itTagList)
	{
		...
		else if (!pTag->m_name.Compare(TAG_FILESIZE))
		{
			...
			uLength = (uint32_t)pTag->GetInt();
		else if (!pTag->m_name.Compare(TAG_MEDIA_BITRATE))
			uBitrate = (uint32_t)pTag->GetInt();
		...
		else if (!pTag->m_name.Compare(TAG_SOURCES))
		{
			...
			uAvailability = (uint32_t)pTag->GetInt();
		...
		else if (!pTag->m_name.Compare(TAG_PUBLISHINFO))
		{
			...
			uPublishInfo = (uint32_t)pTag->GetInt();
/*#ifdef _DEBUG
...
void CSearch::SendFindValue(CContact* pContact)
{
	...
	try
	{
		...
		uint8_t byContactCount = GetRequestContactCount();
		if (byContactCount > 0)
	...
	try
	{
		if (pFile && byIO)
		{
			...
			if (pFile->GetFileSize() > OLD_MAX_EMULE_FILE_SIZE)
			{
				...
				byte byValue[8];
				*((uint64_t*)byValue) = pFile->GetFileSize();
			...
			if (pFile->GetMetaDataVer() > 0)
			{
				...
				for (int iIndex = 0; iIndex < _countof(_aMetaTags); iIndex++)
				{
...
uint32_t CSearch::GetNodeLoad() const
{
...
uint32_t CSearch::GetSearchID() const
{
	return m_uSearchID;
}
uint32_t CSearch::GetSearchTypes() const
{
	return m_uType;
}
void CSearch::SetSearchTypes( uint32_t uVal )
{
	m_uType = uVal;
}
...
uint32_t CSearch::GetAnswers() const
{
...
uint32_t CSearch::GetKadPacketSent() const
{
	return m_uKadPacketSent;
}
uint32_t CSearch::GetRequestAnswer() const
{
...
uint32_t CSearch::GetNodeLoadResonse() const
{
	return m_uTotalLoadResponses;
}
uint32_t CSearch::GetNodeLoadTotal() const
{
	return m_uTotalLoad;
}
void CSearch::UpdateNodeLoad( uint8_t uLoad )
{
...
void CSearch::SetSearchTermData( uint32_t uSearchTermDataSize, LPBYTE pucSearchTermsData )
{
