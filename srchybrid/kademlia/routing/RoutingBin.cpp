...
CMap<uint32_t, uint32_t, uint32_t, uint32_t> CRoutingBin::s_mapGlobalContactIPs;
CMap<uint32_t, uint32_t, uint32_t, uint32_t> CRoutingBin::s_mapGlobalContactSubnets;
...
void CRoutingBin::SetTCPPort(uint32_t uIP, uint16_t uUDPPort, uint16_t uTCPPort)
{
...
CContact* CRoutingBin::GetContact(uint32_t uIP, uint16_t nPort, bool bTCPPort){
	// Find contact with IP/Port
	for (ContactList::iterator itContactList = m_listEntries.begin(); itContactList != m_listEntries.end(); ++itContactList)
	{
		CContact* pContact = *itContactList;
...
void CRoutingBin::GetNumContacts(uint32_t& nInOutContacts, uint32_t& nInOutFilteredContacts, uint8_t byMinVersion) const
{
...
void CRoutingBin::GetClosestTo(uint32_t uMaxType, const CUInt128 &uTarget, uint32_t uMaxRequired, ContactMap *pmapResult, bool bEmptyFirst, bool bInUse)
{
...
void CRoutingBin::AdjustGlobalTracking(uint32_t uIP, bool bIncrease){
	// IP
	uint32_t nSameIPCount = 0;
...
bool CRoutingBin::ChangeContactIPAddress(CContact* pContact, uint32_t uNewIP)
{
	...
	if ((pContact->GetIPAddress() & 0xFFFFFF00) != (uNewIP & 0xFFFFFF00)){
		...
		uint32_t nSameSubnetGlobalCount = 0;
...
CContact* CRoutingBin::GetRandomContact(uint32_t nMaxType, uint32_t nMinKadVersion)
{
	if (m_listEntries.empty())
		return NULL;
	// Find contact with IP/Port
	CContact* pLastFit = NULL;
	uint32_t nRandomStartPos = GetRandomUInt16() % m_listEntries.size();
	uint32_t nIndex = 0;
	for (ContactList::iterator itContactList = m_listEntries.begin(); itContactList != m_listEntries.end(); ++itContactList)
	{
		CContact* pContact = *itContactList;
...
bool CRoutingBin::CheckGlobalIPLimits(uint32_t uIP, uint16_t uPort, bool bLog)
{
	// no more than 1 KadID per IP
	uint32_t nSameIPCount = 0;
	s_mapGlobalContactIPs.Lookup(uIP, nSameIPCount);
