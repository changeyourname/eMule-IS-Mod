...
bool CRoutingZone::Add(const CUInt128 &uID, uint32_t uIP, uint16_t uUDPPort, uint16_t uTCPPort, uint8_t uVersion, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, bool bFromNodesDat, bool bFromHello)
{
	uint32_t uhostIP = ntohl(uIP);
	if (::IsGoodIPPort(uhostIP, uUDPPort))
	{
...
CContact* CRoutingZone::GetContact(uint32_t uIP, uint16_t nPort, bool bTCPPort) const
{
	if (IsLeaf())
...
CContact* CRoutingZone::GetRandomContact(uint32_t nMaxType, uint32_t nMinKadVersion) const
{
	if (IsLeaf())
		return m_pBin->GetRandomContact(nMaxType, nMinKadVersion);
	else{
		uint32_t nZone = GetRandomUInt16() % 2;
...
void CRoutingZone::GetClosestTo(uint32_t uMaxType, const CUInt128 &uTarget, const CUInt128 &uDistance, uint32_t uMaxRequired, ContactMap *pmapResult, bool bEmptyFirst, bool bInUse) const
{
...
uint32_t CRoutingZone::GetMaxDepth() const
{
	if (IsLeaf())
		return 0;
...
uint32_t CRoutingZone::GetNumContacts() const
{
	if (IsLeaf())
...
void CRoutingZone::GetNumContacts(uint32_t& nInOutContacts, uint32_t& nInOutFilteredContacts, uint8_t byMinVersion) const
{
	if (IsLeaf())
...
uint32_t CRoutingZone::GetBootstrapContacts(ContactList *plistResult, uint32_t uMaxRequired)
{
