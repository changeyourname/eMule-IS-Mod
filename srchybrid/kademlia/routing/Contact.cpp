...
CContact::CContact(const CUInt128 &uClientID, uint32_t uIp, uint16_t uUdpPort, uint16_t uTcpPort, uint8_t uVersion, CKadUDPKey cUDPKey, bool bIPVerified)
{
...
CContact::CContact(const CUInt128 &uClientID, uint32_t uIp, uint16_t uUdpPort, uint16_t uTcpPort, const CUInt128 &uTarget, uint8_t uVersion, CKadUDPKey cUDPKey, bool bIPVerified)
{
	m_uClientID = uClientID;
...
uint32_t CContact::GetIPAddress() const
{
	return m_uIp;
}
...
void CContact::SetIPAddress(uint32_t uIp)
{
	if (m_uIp != uIp){
...
uint16_t CContact::GetTCPPort() const
{
	return m_uTcpPort;
}
...
void CContact::SetTCPPort(uint16_t uPort)
{
	m_uTcpPort = uPort;
}

uint16_t CContact::GetUDPPort() const
{
	return m_uUdpPort;
}
...
void CContact::SetUDPPort(uint16_t uPort)
{
	m_uUdpPort = uPort;
}
...
{
	time_t uHours = (time(NULL)-m_tCreated)/HR2S(1);
	switch(uHours)
	{
		case 0:
			m_byType = 2;
...
uint8_t CContact::GetVersion() const
{
	return m_uVersion;
}

void CContact::SetVersion(uint8_t uVersion)
{
	m_uVersion = uVersion;
}
