...
namespace Kademlia
{
	...
	class CRoutingZone
	{
		...
		public:
			...
			bool		OnBigTimer();
			void		OnSmallTimer();
			bool		Add(const CUInt128 &uID, uint32_t uIP, uint16_t uUDPPort, uint16_t uTCPPort, uint8_t uVersion, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, bool bFromNodesDat, bool bFromHello);
			bool		AddUnfiltered(const CUInt128 &uID, uint32_t uIP, uint16_t uUDPPort, uint16_t uTCPPort, uint8_t uVersion, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, bool bFromNodesDat, bool bFromHello);
			bool		Add(CContact* pContact, bool& bUpdate, bool& bOutIPVerified);
			void		ReadFile(CString strSpecialNodesdate = _T(""));
			bool		VerifyContact(const CUInt128 &uID, uint32_t uIP);
			CContact*	GetContact(const CUInt128 &uID) const;
			CContact*	GetContact(uint32_t uIP, uint16_t nPort, bool bTCPPort) const;
			CContact*	GetRandomContact(uint32_t nMaxType, uint32_t nMinKadVersion) const;
			...
			void		GetClosestTo(uint32_t uMaxType, const CUInt128 &uTarget, const CUInt128 &uDistance, uint32_t uMaxRequired, ContactMap *plistResult, bool bEmptyFirst = true, bool bSetInUse = false) const;
