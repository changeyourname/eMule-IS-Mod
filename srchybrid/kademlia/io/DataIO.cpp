...
uint8_t CDataIO::ReadUInt8()
{
	uint8_t uRetVal;
	ReadArray(&uRetVal, sizeof(uint8_t));
	return uRetVal;
}

uint16_t CDataIO::ReadUInt16()
{
	uint16_t uRetVal;
	ReadArray(&uRetVal, sizeof(uint16_t));
	return uRetVal;
}

uint32_t CDataIO::ReadUInt32()
{
	uint32_t uRetVal;
	ReadArray(&uRetVal, sizeof(uint32_t));
	return uRetVal;
}

uint64_t CDataIO::ReadUInt64()
{
	uint64_t uRetVal;
	ReadArray(&uRetVal, sizeof(uint64_t));
	return uRetVal;
}

void CDataIO::ReadUInt128(CUInt128* puValue)
{
	ReadArray(puValue->GetDataPtr(), sizeof(uint32_t)*4);
}
...
BYTE* CDataIO::ReadBsob(uint8_t* puSize)
{
	*puSize = ReadUInt8();
...
void CDataIO::WriteUInt8(uint8_t uVal)
{
	WriteArray(&uVal, sizeof(uint8_t));
}

void CDataIO::WriteUInt16(uint16_t uVal)
{
	WriteArray(&uVal, sizeof(uint16_t));
}

void CDataIO::WriteUInt32(uint32_t uVal)
{
	WriteArray(&uVal, sizeof(uint32_t));
}

void CDataIO::WriteUInt64(uint64_t uVal)
{
	WriteArray(&uVal, sizeof(uint64_t));
}

void CDataIO::WriteUInt128(const CUInt128& uVal)
{
	WriteArray(uVal.GetData(), sizeof(uint32_t)*4);
}
...
void CDataIO::WriteBsob(const BYTE* pbyValue, uint8_t uSize)
{
	WriteUInt8(uSize);
...
void CDataIO::WriteTag(const CKadTag* pTag)
{
	try
	{
		...
		WriteUInt16((uint16_t)name.GetLength());
		...
		switch (uType)
		{
			...
			case TAGTYPE_STRING:
				{
					...
					WriteUInt16((uint16_t)utf8.GetLength());
			...
			case TAGTYPE_UINT64:
				...
				break;
			case TAGTYPE_UINT32:
				WriteUInt32((uint32_t)pTag->GetInt());
				break;
			case TAGTYPE_UINT16:
				WriteUInt16((uint16_t)pTag->GetInt());
				break;
			case TAGTYPE_UINT8:
				WriteUInt8((uint8_t)pTag->GetInt());
				break;
			case TAGTYPE_FLOAT32:
...
void CDataIO::WriteTag(LPCSTR szName, uint64_t uValue)
{
	CKadTagUInt64 tag(szName, uValue);
	WriteTag(&tag);
}

void CDataIO::WriteTag(LPCSTR szName, uint32_t uValue)
{
	CKadTagUInt32 tag(szName, uValue);
	WriteTag(&tag);
}

void CDataIO::WriteTag(LPCSTR szName, uint16_t uValue)
{
	CKadTagUInt16 tag(szName, uValue);
	WriteTag(&tag);
}

void CDataIO::WriteTag(LPCSTR szName, uint8_t uValue)
{
...
{
	uint32_t uCount = (uint32_t)tagList.size();
	ASSERT( uCount <= 0xFF );
	WriteByte((uint8_t)uCount);
