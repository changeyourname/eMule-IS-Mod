...
CByteIO::CByteIO(byte* pbyBuffer, uint32_t uAvailable)
{
	m_bReadOnly = false;
...
CByteIO::CByteIO(const byte* pbyBuffer, uint32_t uAvailable)
{
	m_bReadOnly = true;
...
void CByteIO::ReadArray(LPVOID lpResult, uint32_t uByteCount)
{
...
void CByteIO::WriteArray(LPCVOID lpVal, uint32_t uByteCount)
{
	if (m_bReadOnly)
...
void CByteIO::Seek(uint32_t newpos)
{
