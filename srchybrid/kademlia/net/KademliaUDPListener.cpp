...
void CKademliaUDPListener::Bootstrap(LPCTSTR szHost, uint16_t uUDPPort, bool bKad2)
{
	uint32_t uRetVal = 0;
...
// Used by Kad1.0 and Kad 2.0
void CKademliaUDPListener::Bootstrap(uint32_t uIP, uint16_t uUDPPort, bool bKad2, uint8_t byKadVersion, const CUInt128* uCryptTargetID)
{
	if (bKad2)
	{
...
// Used by Kad1.0 and Kad 2.0
void CKademliaUDPListener::SendMyDetails(byte byOpcode, uint32_t uIP, uint16_t uUDPPort, uint8_t byKadVersion, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID, bool bRequestAckPackage)
{
	if (byKadVersion > 0)
	{
		byte byPacket[1024];
			...
			&& (bRequestAckPackage || CKademlia::GetPrefs()->GetFirewalled() ||  CUDPFirewallTester::IsFirewalledUDP(true)))
		{
			...
			// 1 - TCP firewalled
			// 1 - UDP firewalled
			const uint8_t uUDPFirewalled	= CUDPFirewallTester::IsFirewalledUDP(true) ? 1 : 0;
			const uint8_t uTCPFirewalled	= CKademlia::GetPrefs()->GetFirewalled() ? 1 : 0;
			const uint8_t uRequestACK		= bRequestAckPackage ? 1 : 0;
			const uint8_t byMiscOptions =  (uRequestACK << 2) | (uTCPFirewalled << 1) | (uUDPFirewalled << 0);
		...
		uint32_t uLen = sizeof(byPacket) - byteIOResponse.GetAvailable();
		if (byKadVersion >= KADEMLIA_VERSION6_49aBETA){
...
void CKademliaUDPListener::FirewalledCheck(uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey, uint8_t byKadVersion)
{
	if (byKadVersion > KADEMLIA_VERSION6_49aBETA) {
...
void CKademliaUDPListener::SendNullPacket(byte byOpcode,uint32_t uIP, uint16_t uUDPPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
	CSafeMemFile fileIO(0);
...
void CKademliaUDPListener::SendPublishSourcePacket(const CContact* pContact, const CUInt128 &uTargetID, const CUInt128 &uContactID, const TagList& tags)
{
	...
	uint32_t uLen = sizeof(byPacket) - byteIO.GetAvailable();
...
void CKademliaUDPListener::ProcessPacket(const byte* pbyData, uint32_t uLenData, uint32_t uIP, uint16_t uUDPPort, bool bValidReceiverKey, CKadUDPKey senderUDPKey)
{
	...
	uint32_t uLenPacket = uLenData - 2;
...
bool CKademliaUDPListener::AddContact(const byte *pbyData, uint32_t uLenData, uint32_t uIP, uint16_t uUDPPort, uint16_t uTCPPort, CKadUDPKey cUDPKey, bool& bIPVerified, bool bUpdate, CUInt128* puOutContactID)
{
...
bool CKademliaUDPListener::AddContact_KADEMLIA2(const byte* pbyData, uint32_t uLenData, uint32_t uIP, uint16_t& uUDPPort, uint8_t* pnOutVersion, CKadUDPKey cUDPKey, bool& rbIPVerified, bool bUpdate, bool bFromHelloReq, bool* pbOutRequestsACK, CUInt128* puOutContactID)
{
	...
	if (puOutContactID != NULL)
		*puOutContactID = uID;
	uint16_t uTCPPort = byteIO.ReadUInt16();
	uint8_t uVersion = byteIO.ReadByte();
	if (pnOutVersion != NULL)
		*pnOutVersion = uVersion;

	bool bUDPFirewalled = false;
	bool bTCPFirewalled = false;
	uint8_t uTags = byteIO.ReadByte();
	while (uTags)
	{
		...
		{
			if (pTag->IsInt() && (uint16_t)pTag->GetInt() > 0)
				uUDPPort = (uint16_t)pTag->GetInt();
			else
				ASSERT( false );
		}
...
// Used only for Kad1.0
void CKademliaUDPListener::AddContacts( const byte *pbyData, uint32_t uLenData, uint16_t uNumContacts, bool bUpdate)
{
	CSafeMemFile fileIO( pbyData, uLenData );
	CRoutingZone *pRoutingZone = CKademlia::GetRoutingZone();
	CUInt128 uID;
	for (uint16_t iIndex=0; iIndex<uNumContacts; iIndex++)
	{
		fileIO.ReadUInt128(&uID);
		uint32_t uIP = fileIO.ReadUInt32();
		uint16_t uUDPPort = fileIO.ReadUInt16();
		uint16_t uTCPPort = fileIO.ReadUInt16();
		fileIO.ReadUInt8();
...
// Used only for Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_BOOTSTRAP_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	...
	uint16_t uNumContacts = 1 + (uint16_t)CKademlia::GetRoutingZone()->GetBootstrapContacts(&contacts, 20);
...
// Used only for Kad2.0
void CKademliaUDPListener::Process_KADEMLIA2_BOOTSTRAP_REQ (uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// Get some contacts to return
	ContactList contacts;
	uint16_t uNumContacts = (uint16_t)CKademlia::GetRoutingZone()->GetBootstrapContacts(&contacts, 20);
...
// Used only for Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_BOOTSTRAP_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP)
{
	// Verify packet is expected size
	if (uLenPacket < 27)
	{
	...
	uint16_t uNumContacts = fileIO.ReadUInt16();
...
// Used only for Kad2.0
void CKademliaUDPListener::Process_KADEMLIA2_BOOTSTRAP_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey, bool bValidReceiverKey)
{
	...
	uint16_t uTCPPort = fileIO.ReadUInt16();
	uint8_t uVersion = fileIO.ReadUInt8();
...
// Used in Kad1.0 only.
void CKademliaUDPListener::Process_KADEMLIA_HELLO_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket != 25)
	{
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_HELLO_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey, bool bValidReceiverKey)
{
	//uint16 dbgOldUDPPort = uUDPPort;
	uint8_t byContactVersion = 0;
	CUInt128 uContactID;
...
// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_HELLO_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_HELLO_RES_ACK (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, bool bValidReceiverKey)
{
	if (uLenPacket < 17)
	{
		CString strError;
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_HELLO_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey, bool bValidReceiverKey)
{
...
// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket != 33)
	{
	...
	if(CKademlia::GetPrefs()->GetKadID() == uCheck)
	{
		...
		uint16_t uCount = (uint16_t)results.size();
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// Get target and type
	CSafeMemFile fileIO( pbyPacketData, uLenPacket);
	byte byType = fileIO.ReadUInt8();
	...
	if(CKademlia::GetPrefs()->GetKadID() == uCheck)
	{
		// Get required number closest to target
		ContactMap results;
		CKademlia::GetRoutingZone()->GetClosestTo(2, uTarget, uDistance, (uint32_t)byType, &results);
		uint8_t uCount = (uint8_t)results.size();
...
// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket < 17)
	{
	...
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
	uint16_t uNumContacts = fileIO.ReadUInt8();
	...
	try
	{
		for (uint16_t iIndex=0; iIndex<uNumContacts; iIndex++)
		{
			...
			uint32_t uIPResult = fileIO.ReadUInt32();
			uint16_t uUDPPortResult = fileIO.ReadUInt16();
			uint16_t uTCPPortResult = fileIO.ReadUInt16();
			fileIO.ReadUInt8();
			uint32_t uhostIPResult = ntohl(uIPResult);
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey /*senderUDPKey*/)
{
	...
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
	uint8_t uNumContacts = fileIO.ReadUInt8();
	...
	try
	{
		for (uint8_t iIndex=0; iIndex<uNumContacts; iIndex++)
		{
			fileIO.ReadUInt128(&uIDResult);
			uint32_t uIPResult = fileIO.ReadUInt32();
			uint16_t uUDPPortResult = fileIO.ReadUInt16();
...
SSearchTerm* CKademliaUDPListener::CreateSearchExpressionTree(CSafeMemFile& fileIO, int iLevel)
{
	...
	uint8_t uOp = fileIO.ReadUInt8();
	if (uOp == 0x00)
	{
		uint8_t uBoolOp = fileIO.ReadUInt8();
		if (uBoolOp == 0x00) // AND
		{
	...
	else if (uOp == 0x02) // Meta tag
	{
		...
		// read tag name
		CStringA strTagName;
		uint16_t lenTagName = fileIO.ReadUInt16();
	...
	else if (uOp == 0x03 || uOp == 0x08) // Numeric Relation (0x03=32-bit or 0x08=64-bit)
	{
		...
		// read tag value
		uint64_t ullValue = (uOp == 0x03) ? fileIO.ReadUInt32() : fileIO.ReadUInt64();

		// read integer operator
		uint8_t mmop = fileIO.ReadUInt8();
		...
		// read tag name
		CStringA strTagName;
		uint16_t uLenTagName = fileIO.ReadUInt16();
...
// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_SEARCH_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket < 17)
	{
	...
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
	uint8_t uRestrictive = fileIO.ReadUInt8();
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_SEARCH_KEY_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	CSafeMemFile fileIO( pbyPacketData, uLenPacket);
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
	uint16_t uStartPosition = fileIO.ReadUInt16();
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_SEARCH_SOURCE_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	CSafeMemFile fileIO( pbyPacketData, uLenPacket);
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
	uint16_t uStartPosition = (fileIO.ReadUInt16() & 0x7FFF);
	uint64_t uFileSize = fileIO.ReadUInt64();
	CKademlia::GetIndexed()->SendValidSourceResult(uTarget, uIP, uUDPPort, true, uStartPosition, uFileSize, senderUDPKey);
}

// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_SEARCH_RES (const byte *pbyPacketData, uint32_t uLenPacket)
{
	// Verify packet is expected size
	if (uLenPacket < 37)
	{
	...
	uint16_t uCount = byteIO.ReadUInt16();
	CUInt128 uAnswer;
	while( uCount > 0 )
	{
		// What is the answer
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_SEARCH_RES (const byte *pbyPacketData, uint32_t uLenPacket, CKadUDPKey /*senderUDPKey*/)
{
	CByteIO byteIO(pbyPacketData, uLenPacket);
...
// Used in Kad1.0 only
void CKademliaUDPListener::Process_KADEMLIA_PUBLISH_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	...
	while( uCount > 0 )
	{
		...
		try
		{
			...
			uint32_t uTags = byteIO.ReadByte();
			while(uTags > 0)
			{
				CKadTag* pTag = byteIO.ReadTag();
				if(pTag)
				{
					...
					else if (!pTag->m_name.Compare(TAG_FILESIZE))
					{
						if( pEntry->m_uSize == 0 )
						{
							...
							{
								pEntry->m_uSize = *((uint64_t*)pTag->GetBsob());
							}
							else
					...
					else if (!pTag->m_name.Compare(TAG_SOURCEPORT))
					{
						...
						{
							pEntry->m_uTCPPort = (uint16_t)pTag->GetInt();
							pEntry->AddTag(pTag);
						}
						else
						{
		...
		{
			pEntry->m_tLifetime = (uint32_t)time(NULL)+KADEMLIAREPUBLISHTIMES;
		...
		{
			pEntry->m_tLifetime = (uint32_t)time(NULL)+KADEMLIAREPUBLISHTIMEK;
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_KEY_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	//Used Pointers
	CIndexed *pIndexed = CKademlia::GetIndexed();
...
// Used in Kad2.0 only
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_SOURCE_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	//Used Pointers
	CIndexed *pIndexed = CKademlia::GetIndexed();
		...
		while(uTags > 0)
		{
			...
			if(pTag)
			{
				...
				else if (!pTag->m_name.Compare(TAG_SOURCEPORT))
				{
					if( pEntry->m_uTCPPort == 0 )
					{
						pEntry->m_uTCPPort = static_cast<uint16_t>(pTag->GetInt());
						pEntry->AddTag(pTag);
					}
					else
					{
				...
				else if (!pTag->m_name.Compare(TAG_SOURCEUPORT))
				{
					...
					{
						pEntry->m_uUDPPort = (uint16_t)pTag->GetInt();
						pEntry->AddTag(pTag);
...
// Used only by Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_PUBLISH_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP)
{
	// Verify packet is expected size
	if (uLenPacket < 16)
	{
...
// Used only by Kad2.0
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	...
	CUInt128 uFile;
	fileIO.ReadUInt128(&uFile);
	uint8_t uLoad = fileIO.ReadUInt8();
	CSearchManager::ProcessPublishResult(uFile, uLoad, true);
	if (fileIO.GetLength() > fileIO.GetPosition()){
		// for future use
		uint8_t byOptions = fileIO.ReadUInt8();
...
// Used only by Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_SEARCH_NOTES_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket < 32)
	{
...
// Used only by Kad2.0
void CKademliaUDPListener::Process_KADEMLIA2_SEARCH_NOTES_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	CSafeMemFile fileIO(pbyPacketData, uLenPacket);
	CUInt128 uTarget;
	fileIO.ReadUInt128(&uTarget);
...
// Used only by Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_SEARCH_NOTES_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP)
{
	// Verify packet is expected size
	if (uLenPacket < 37)
	{
...
// Used only by Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_PUBLISH_NOTES_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort)
{
	// Verify packet is expected size
	if (uLenPacket < 37)
	{
...
// Used only by Kad2.0
void CKademliaUDPListener::Process_KADEMLIA2_PUBLISH_NOTES_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// check if we are UDP firewalled
	if (CUDPFirewallTester::IsFirewalledUDP(true))
	{
...
// Used only by Kad1.0
void CKademliaUDPListener::Process_KADEMLIA_PUBLISH_NOTES_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP)
{
	// Verify packet is expected size
	if (uLenPacket < 16)
	{
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// Verify packet is expected size
	if (uLenPacket != 2)
	{
...
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED2_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// Verify packet is expected size
	if (uLenPacket < 19)
	{
	...
	uint8_t byConnectOptions = fileIO.ReadUInt8();
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, CKadUDPKey /*senderUDPKey*/)
{
	// Verify packet is expected size
	if (uLenPacket != 4)
	{
	...
	CSafeMemFile fileIO(pbyPacketData, uLenPacket);
	uint32_t uFirewalledIP = fileIO.ReadUInt32();
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FIREWALLED_ACK_RES (uint32_t uLenPacket)
{
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FINDBUDDY_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey)
{
	// Verify packet is expected size
	if (uLenPacket < 34)
	{
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_FINDBUDDY_RES (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t uUDPPort, CKadUDPKey /*senderUDPKey*/)
{
	// Verify packet is expected size
	if (uLenPacket < 34)
	{
...
// Used by Kad1.0 and Kad2.0
void CKademliaUDPListener::Process_KADEMLIA_CALLBACK_REQ (const byte *pbyPacketData, uint32_t uLenPacket, uint32_t uIP, CKadUDPKey /*senderUDPKey*/)
{
	// Verify packet is expected size
	if (uLenPacket < 34)
	{
	...
	if (pBuddy != NULL)
	{
		...
		CUInt128 uFile;
		fileIO.ReadUInt128(&uFile);
		uint16_t uTCP = fileIO.ReadUInt16();
...
void CKademliaUDPListener::Process_KADEMLIA2_PING(uint32_t uIP, uint16_t uUDPPort, CKadUDPKey senderUDPKey){
...
void CKademliaUDPListener::Process_KADEMLIA2_PONG (const byte* pbyPacketData, uint32_t uLenPacket, uint32_t uIP, uint16_t /*uUDPPort*/, CKadUDPKey /*senderUDPKey*/){
	if (uLenPacket < 2)
	{
		CString strError;
		strError.Format(_T("***NOTE: Received wrong size (%u) packet in %hs"), uLenPacket, __FUNCTION__);
...
void CKademliaUDPListener::Process_KADEMLIA2_FIREWALLUDP(const byte *pbyPacketData, uint32_t uLenPacket,uint32_t uIP, CKadUDPKey /*senderUDPKey*/)
{
	// Verify packet is expected size
	if (uLenPacket < 3)
	{
	...
	uint8_t byErrorCode = PeekUInt8(pbyPacketData);
	uint16_t nIncomingPort = PeekUInt16(pbyPacketData + 1);
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint32_t uLenData, uint32_t uDestinationHost, uint16_t uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
	if (uLenData < 2) {
		ASSERT(0);
		return;
...
void CKademliaUDPListener::SendPacket(const byte *pbyData, uint32_t uLenData, byte byOpcode, uint32_t uDestinationHost, uint16_t uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
...
void CKademliaUDPListener::SendPacket(CSafeMemFile *pbyData, byte byOpcode, uint32_t uDestinationHost, uint16_t uDestinationPort, CKadUDPKey targetUDPKey, const CUInt128* uCryptTargetID)
{
	AddTrackedOutPacket(uDestinationHost, byOpcode);
...
bool CKademliaUDPListener::FindNodeIDByIP(CKadClientSearcher* pRequester, uint32_t dwIP, uint16_t nTCPPort, uint16_t nUDPPort){
...
void CKademliaUDPListener::SendLegacyChallenge(uint32_t uIP, uint16_t uUDPPort, const CUInt128& uContactID, bool bKad2){
