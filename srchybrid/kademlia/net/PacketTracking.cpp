...
void CPacketTracking::AddTrackedOutPacket(uint32_t dwIP, uint8_t byOpcode){
...
bool CPacketTracking::IsTrackedOutListRequestPacket(uint8_t byOpcode) const
{
	switch(byOpcode){
...
bool CPacketTracking::IsOnOutTrackList(uint32_t dwIP, uint8_t byOpcode, bool bDontRemove){
#ifdef _DEBUG
		if (!IsTrackedOutListRequestPacket(byOpcode))
...
bool CPacketTracking::InTrackListIsAllowedPacket(uint32_t uIP, uint8_t byOpcode, bool /*bValidSenderkey*/){
	...
	uint32_t iAllowedPacketsPerMinute;
	...
	const uint32_t iSecondsPerPacket = 60 / iAllowedPacketsPerMinute;
	const uint32_t dwCurrentTick = ::GetTickCount();
	...
	for (int i = 0; i < pTrackEntry->m_aTrackedRequests.GetCount(); i++){
		if (pTrackEntry->m_aTrackedRequests[i].m_byOpcode == byOpcode){
			...
			{
				uint32_t nRemoveCount = (dwCurrentTick - rCurTrackedRequest.m_dwFirstAdded) / SEC2MS(iSecondsPerPacket);
...
void CPacketTracking::InTrackListCleanup(){
	const uint32_t dwCurrentTick = ::GetTickCount();
	const uint32_t dbgOldSize = static_cast<uint32_t>(m_liTrackPacketsIn.GetCount());
...
void CPacketTracking::AddLegacyChallenge(CUInt128 uContactID, CUInt128 uChallengeID, uint32_t uIP, uint8_t byOpcode){
...
bool CPacketTracking::IsLegacyChallenge(CUInt128 uChallengeID, uint32_t uIP, uint8_t byOpcode, CUInt128& ruContactID){
...
bool CPacketTracking::HasActiveLegacyChallenge(uint32_t uIP) const
{
	for (POSITION pos = listChallengeRequests.GetHeadPosition(); pos != NULL; listChallengeRequests.GetNext(pos)){
