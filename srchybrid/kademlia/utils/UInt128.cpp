...
{
#if (defined(_M_IX86) || defined(_M_AMD64)) && (_MSC_FULL_VER > 13009037)
	((uint32_t*)pby)[0] = _byteswap_ulong(m_uData[0]);
	((uint32_t*)pby)[1] = _byteswap_ulong(m_uData[1]);
	((uint32_t*)pby)[2] = _byteswap_ulong(m_uData[2]);
	((uint32_t*)pby)[3] = _byteswap_ulong(m_uData[3]);
#else

	for (int iIndex=0; iIndex<16; iIndex++)
		pby[iIndex] = static_cast<byte>((m_uData[iIndex/4] >> (8*(3-(iIndex%4)))) & 0xFF);
#endif
}
