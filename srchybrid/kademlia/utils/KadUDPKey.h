...
namespace Kademlia
{
	class CKadUDPKey {
	public:
		CKadUDPKey(uint32_t uZero = 0)											{ASSERT(uZero == 0); m_dwKey = uZero; m_dwIP = 0;}
		CKadUDPKey(uint32_t dwKey, uint32_t dwIP)									{m_dwKey = dwKey; m_dwIP = dwIP;}
		...
		CKadUDPKey& operator=(const uint32_t uZero)								{ASSERT(uZero == 0); m_dwKey = uZero; m_dwIP = 0; return *this; }
		friend bool operator==(const CKadUDPKey& k1,const CKadUDPKey& k2)		{return k1.GetKeyValue(k1.m_dwIP) == k2.GetKeyValue(k2.m_dwIP);}

		uint32_t	GetKeyValue(uint32_t dwMyIP)	const								{return (dwMyIP == m_dwIP) ? m_dwKey : 0;}
