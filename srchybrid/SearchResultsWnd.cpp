...
void CSearchResultsWnd::OnInitialUpdate()
{
    ...
    m_ctlFilter.SetDelayedEvaluateMessage(UM_DELAYED_EVALUATE);
	    ...
	    if (theApp.serverconnect->IsConnected())
		{
			...
			if (toask)
			{
				...
				if (toask->SupportsLargeFilesUDP() && (toask->GetUDPFlags() & SRV_UDPFLG_EXT_GETFILES))
				{
					...
					Packet* pExtSearchPacket = new Packet(OP_GLOBSEARCHREQ3, searchpacket->size + (uint32_t)data.GetLength());
					data.SeekToBegin();
					data.Read(pExtSearchPacket->pBuffer, (uint32_t)data.GetLength());
					memcpy(pExtSearchPacket->pBuffer+(uint32_t)data.GetLength(), searchpacket->pBuffer, searchpacket->size);
...
{
    // ismod: filter users [start]
    int iSel = searchlistctrl.GetNextItem(-1, LVIS_SELECTED | LVIS_FOCUSED);
    const CSearchFile* file = (CSearchFile*)searchlistctrl.GetItemData(iSel);
    if (file && file->IsUser()) return;
    // ismod [end]
	//start download(s)
	DownloadSelected();
}
...
void CSearchResultsWnd::DownloadSelected(bool bPaused)
{
	...
	while (pos != NULL)
	{
		...
		if (iIndex >= 0)
		{
            ...
            /* ismod: proper download of  files in subfolders
			if (sel_file->GetListParent() != NULL)
            */
            if (sel_file->GetListParent() != NULL && !sel_file->GetListParent()->IsFolder())
...
public:
	...
	void WriteMetaDataSearchParam(UINT uMetaTagID, const CString& rstrValue)
	{
		...
		m_data->WriteString(rstrValue, m_eStrEncode); // string value
		m_data->WriteUInt16(sizeof uint8_t);			// meta tag ID length
		m_data->WriteUInt8((uint8_t)uMetaTagID);		// meta tag ID name
	...
	void WriteMetaDataSearchParam(UINT uMetaTagID, UINT uOperator, uint64_t ullValue)
	{
		...
		if (b64BitValue && m_bSupports64Bit) {
			...
			m_data->WriteUInt32((uint32_t)ullValue);	// numeric value
		}
		m_data->WriteUInt8((uint8_t)uOperator);	// comparison operator
		m_data->WriteUInt16(sizeof uint8_t);		// meta tag ID length
		m_data->WriteUInt8((uint8_t)uMetaTagID);	// meta tag ID name
		m_strDbg.AppendFormat(_T("%s%s%I64u "), DbgGetFileMetaTagName(uMetaTagID), DbgGetSearchOperatorName(uOperator), ullValue);
	}

	void WriteMetaDataSearchParam(LPCSTR pszMetaTagID, UINT uOperator, uint64_t ullValue)
	{
		bool b64BitValue = ullValue > 0xFFFFFFFFui64;
		if (b64BitValue && m_bSupports64Bit) {
			if (m_pbPacketUsing64Bit)
...
static void AddAndAttr(UINT uTag, UINT uOpr, uint64_t ullVal)
{
    ...
    */
    {
        ...
        if (pParams->bSearchUsers)
            pParams->strBooleanExpr = _T("+++USERNICK+++ ");
        if(pParams->strBooleanExpr.Find(_T(' ')) != -1)
            pParams->strBooleanExpr = _T('\'') + pParams->strBooleanExpr + _T('\'');
        if(pParams->strBooleanExpr.Right(1) != _T(" "))
            pParams->strBooleanExpr += _T(" ");

        pParams->strBooleanExpr += pParams->strExpression;
        if(pParams->bFilterPPPlus)
            pParams->strBooleanExpr += _T(" NOT +++");
    }
    // filter folders
    if(!pParams->bSearchFolders && pParams->strFileType == ED2KFTSTR_EMULECOLLECTION)
        pParams->strBooleanExpr += _T(" NOT ED2K:\\");
    // ismod [end]
...
bool CSearchResultsWnd::CanDeleteSearch(uint32_t /*nSearchID*/) const
{
	return (searchselect.GetItemCount() > 0);
}

void CSearchResultsWnd::DeleteSearch(uint32_t nSearchID)
{
...
LRESULT CSearchResultsWnd::OnCloseTab(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	{
		uint32_t nSearchID = ((const SSearchParams*)item.lParam)->dwSearchID;
...
LRESULT CSearchResultsWnd::OnChangeFilter(WPARAM wParam, LPARAM lParam)
{
	...
	bool bColumnDiff = (m_nFilterColumn != (uint32_t)wParam);
	m_nFilterColumn = (uint32_t)wParam;
