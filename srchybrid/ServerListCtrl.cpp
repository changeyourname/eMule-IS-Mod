...
{ 
	...
	*/
	ServerMenu.AppendMenu(MF_STRING | MF_GRAYED, MP_REMOVE, GetResString(IDS_REMOVETHIS), _T("DELETESELECTED"));
	ServerMenu.AppendMenu(MF_STRING | MF_GRAYED, MP_REMOVEALL, GetResString(IDS_REMOVEALL), _T("DELETE"));
...
{
//* ismod: simplify connetion to one server
...
#define UNDEFINED_INT_AT_BOTTOM(i1, i2) \
	...
	switch (LOWORD(lParamSort))
	{
	  ...
	  case 1:
		  if (item1->HasDynIP() && item2->HasDynIP())
			  ...
			  uint32_t uIP1 = htonl(item1->GetIP());
			  uint32_t uIP2 = htonl(item2->GetIP());
			  if (uIP1 < uIP2)
				  iResult = -1;
			  else if (uIP1 > uIP2)
				  iResult = 1;
	...
	int dwNextSort;
	if (iResult == 0 && (dwNextSort = theApp.emuledlg->serverwnd->serverlistctrl.GetNextSortOrder(static_cast<int>(lParamSort))) != (-1))
	{
...
void CServerListCtrl::OnNmCustomDraw(NMHDR *pNMHDR, LRESULT *plResult)
{
	...
	if (pnmlvcd->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
	{
		...
		/* ismod: use hostname instead of ip
		if (pConnectedServer && pConnectedServer->GetIP() == pServer->GetIP() && pConnectedServer->GetPort() == pServer->GetPort())
		*/
		if (pConnectedServer && pConnectedServer->GetAddress() == pServer->GetAddress() && pConnectedServer->GetPort() == pServer->GetPort())
