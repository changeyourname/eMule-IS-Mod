...
int CArchivePreviewDlg::ShowAceResults(int succ, archiveScannerThreadParams_s* tp) {
	...
	if (!tp->ai->ACEdir->IsEmpty())
	{
		...
		while (pos != NULL)
		{
			...
			int iSystemImage = static_cast<int>(bIsDirectory ? theApp.GetFileTypeSystemImageIdx(_T("\\"), 1) : theApp.GetFileTypeSystemImageIdx(temp, temp.GetLength()));
...
static void FreeMemory(void* arg)
{
	...
	for( GapList::iterator it = tp->filled->begin(); it != tp->filled->end(); ++it )
		delete *it;
	tp->filled->clear();
	delete tp->filled;
	tp->filled=NULL;
...
void CArchivePreviewDlg::UpdateArchiveDisplay(bool doscan) {
	...
	if (CSafeThread::BeginThread(RunArchiveScanner, (LPVOID)tp, THREAD_PRIORITY_BELOW_NORMAL) == NULL) {
		FreeMemory(tp);
