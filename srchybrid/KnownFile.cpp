...
void CKnownFile::AssertValid() const
{
	...
	(void)m_strFilePath;
	(void)m_partCount;
	(void)m_ED2KPartCount;
	(void)m_ED2KPartHashCount;
...
void CKnownFile::DrawShareStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
    ...
    if(m_ClientUploadList.GetSize() > 0 || m_nCompleteSourcesCountHi > 1) {
	    ...
	    if (!onlygreyrect) {
            ...
            uint32_t tempCompleteSources = m_nCompleteSourcesCountLo;
            if(tempCompleteSources > 0) {
                tempCompleteSources--;
            }

		    for (UINT i = 0; i < partCount(); i++){
                uint32_t frequency = tempCompleteSources;
                if(!m_AvailPartFrequency.empty()) {
                    frequency = max(m_AvailPartFrequency[i], tempCompleteSources);
                }

			    if(frequency > 0 ){
				    COLORREF color = RGB(0, (22*(frequency-1) >= 210)? 0:210-(22*(frequency-1)), 255);
				    s_ShareStatusBar.FillRange(PARTSIZE*(uint64_t)(i), PARTSIZE*(uint64_t)(i+1), color);
			    }
	        }
	    }
    } else {
...
{
	// Cache part count
	uint16_t partcount = partCount();
	bool flag = (time(NULL) - m_nCompleteSourcesTime) > 0; 
	
	// Reset part counters
	if ( m_AvailPartFrequency.size() < partcount )
		m_AvailPartFrequency.resize(partcount);
...
bool CKnownFile::CreateFromFile( LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam )
{
	...
	if ( (uint64_t)llFileSize > MAX_EMULE_FILE_SIZE ) {
		if ( llFileSize == -1i64 )
	...
	SetFileSize((uint64_t)llFileSize);

	// we are reading the file data later in 16K blocks, adjust the internal file stream buffer accordingly
	...
	m_hasharray.allocate(partCount());
	uint64_t togo = m_nFileSize;
	while( togo >= PARTSIZE )
	{
		// ismod: no aich
		if ( !CreateHash(file, PARTSIZE, m_hasharray.append_v()) ) {
			LogError(_T("Failed to hash file \"%s\" - %s"), strFilePath, _tcserror(errno));
			fclose(file);
			return false;
		}
		...
		if ( pvProgressParam && theApp.IsRunning() ) {
			...
			UINT uProgress = (UINT)(uint64_t)(((uint64_t)(GetFileSize() - togo) * 100) / GetFileSize());
			ASSERT( uProgress <= 100 );
			VERIFY( theApp.emuledlg->PostMessage(TM_FILEOPPROGRESS, (WPARAM)pvProgressParam, (LPARAM)uProgress) );
		}
	}

	// ismod: no aich
	if ( !CreateHash(file, togo, m_hasharray.append_v()) ) {
		LogError(_T("Failed to hash file \"%s\" - %s"), strFilePath, _tcserror(errno));
		fclose(file);
		return false;
	}
	...
	// ismod: no aich
	if ( m_hasharray.count() == 1 ) {
		md4cpy(m_abyFileHash, m_hasharray[0]);
		// SLUGFILLER: SafeHash remove - removed delete
	} else {
		// SLUGFILLER: SafeHash remove - moved up
		CreateHash(m_hasharray.data(), m_hasharray.count()*m_hasharray.hashSize(), m_abyFileHash);
	}
...
void CKnownFile::SetFileSize(EMFileSize nFileSize)
{
	...
	// nr. of data parts
	ASSERT( (uint64_t)(((uint64_t)nFileSize + (PARTSIZE - 1)) / PARTSIZE) <= (UINT)USHRT_MAX );
	m_partCount = (uint16_t)(((uint64_t)nFileSize + (PARTSIZE - 1)) / PARTSIZE);

	// nr. of parts to be used with OP_FILESTATUS
	m_ED2KPartCount = (uint16_t)((uint64_t)nFileSize / PARTSIZE + 1);

	// nr. of parts to be used with OP_HASHSETANSWER
	m_ED2KPartHashCount = (uint16_t)((uint64_t)nFileSize / PARTSIZE);
	if (m_ED2KPartHashCount != 0)
		m_ED2KPartHashCount += 1;
}
...
bool CKnownFile::LoadHashsetFromFile(CFileDataIO* file, bool checkhash){
	...
	m_hasharray.setCount(parts);
	file->Read(m_hasharray.data(), parts*m_hasharray.hashSize());
	...
	} else if (parts != GetED2KPartCount()) {	// SLUGFILLER: SafeHash - use GetED2KPartCount
		// delete hashset
		m_hasharray.release();
		return false;
	}
	...
	if (!m_hasharray.empty()){
		CreateHash(m_hasharray.data(), m_hasharray.count()*m_hasharray.hashSize(), checkid);
	}
...
bool CKnownFile::setHashset(const CHashArray &aHashset)
{
	// delete hashset
	m_hasharray.release();

	m_hasharray.copy(aHashset);
	...
	CreateHash(m_hasharray.data(), m_hasharray.count()*m_hasharray.hashSize(), aucHashsetHash);
...
bool CKnownFile::LoadTagsFromFile(CFileDataIO* file)
{
	...
	for (UINT j = 0; j < tagcount; j++){
		...
		switch (newtag->GetNameID()){
			...
			case FT_ATTRANSFERREDHI:{
				ASSERT( newtag->IsInt() );
				if (newtag->IsInt())
					statistic.SetAllTimeTransferred(((uint64_t)newtag->GetInt() << 32) | (UINT)statistic.GetAllTimeTransferred());
				delete newtag;
				break;
			}
			case FT_ATREQUESTED:{
			...
			case FT_KADLASTPUBLISHSRC:{
				ASSERT( newtag->IsInt() );
				if (newtag->IsInt())
					SetLastPublishTimeKadSrc( ConvertFromTime32(newtag->GetInt()), 0 );
			...
			case FT_KADLASTPUBLISHNOTES:{
				ASSERT( newtag->IsInt() );
				if (newtag->IsInt())
					SetLastPublishTimeKadNotes( ConvertFromTime32(newtag->GetInt()) );
				delete newtag;
				break;
			}
			case FT_FLAGS:
	...
	return m_nFileSize!=(uint64_t)0;		// SLUGFILLER: SafeHash - Must have a filesize tag
}

bool CKnownFile::LoadDateFromFile(CFileDataIO* file){
	m_tUtcLastModified = ConvertFromTime32(file->ReadUInt32());
	return true;
}
...
bool CKnownFile::LoadFromFile(CFileDataIO* file){
	...
	if (GetED2KPartCount() <= 1) {	// ignore loaded hash for 1-chunk files
		...
		ret2 = true;
	} else if (GetED2KPartCount()!=hashset().count())
...
bool CKnownFile::WriteToFile(CFileDataIO* file)
{
	...
	file->WriteUInt16((uint16_t)m_hasharray.count());
	for (UINT i = 0; i < m_hasharray.count(); i++)
		file->WriteHash16(m_hasharray[i]);
	...
	CTag lastSharedTag(FT_LASTSHARED, (uint32_t)timeLastShared);
	...
	if (!ShouldPartiallyPurgeFile())
	{
		...
		if (m_uMetaDataVer > 0)
		{
			...
			uint32_t uFlags = m_uMetaDataVer & 0x0F;
...
void CKnownFile::CreateHash(CFile* pFile, uint64_t Length, uchar* pMd4HashOut)
{
	ASSERT( pFile != NULL );
	ASSERT( pMd4HashOut != NULL );

	uint64_t Required = Length;
	uchar X[1024*16]; // 16kbytes
	CMD4 md4;

	bool bSkipSleeping = false;
	// no sleeping in hashing and GUI thread
	DWORD dwCurrentThreadId = GetCurrentThreadId();
	if ( theApp.m_nThreadID == dwCurrentThreadId || theApp.m_pHashThread->m_nThreadID == dwCurrentThreadId )
	{
		bSkipSleeping = true;
	}
	...
	while ( Required >= 64 ) {
        ...
        if ( (Required / 64) > sizeof(X)/(64 * sizeof(X[0])) ) 
			...
			len = (uint32_t)Required / 64;
		pFile->Read(X, len*64);

		if ( !bSkipSleeping )
			WaitForSingleObject(theApp.m_globalWaitEvent, thePrefs.GetHashingSpeed());
	...
	Required = Length % 64;
	if ( Required != 0 ) {
		pFile->Read(X, (uint32_t)Required);
	}

	md4.Add(X, (uint32_t)Required);
	md4.Finish();
	md4cpy(pMd4HashOut, md4.GetHash());
}

bool CKnownFile::CreateHash(FILE* fp, uint64_t uSize, uchar* pucHash)
{
	bool bResult = false;
	CStdioFile file(fp);
	try
...
bool CKnownFile::CreateHash(const uchar* pucData, uint64_t uSize, uchar* pucHash)
{
	bool bResult = false;
	CMemFile file(const_cast<uchar*>(pucData), static_cast<UINT>(uSize));
	try
	{
		CreateHash(&file, uSize, pucHash);
		bResult = true;
...
Packet*	CKnownFile::CreateSrcInfoPacket(const CUpDownClient* forClient, uint8_t byRequestedVersion, uint16_t nRequestedOptions) const
{
	...
	for (POSITION pos = m_ClientUploadList.GetHeadPosition(); pos != 0; )
	{
		...
		bool bNeeded = false;
		const uint8_t* rcvstatus = forClient->GetUpPartStatus();
		if (rcvstatus)
		{
			ASSERT( forClient->GetUpPartCount() == partCount() );
			const uint8_t* srcstatus = cur_src->GetUpPartStatus();
			if (srcstatus)
			{
				ASSERT( cur_src->GetUpPartCount() == partCount() );
				...
				else
				{
					...
					//if (thePrefs.GetVerbose())
						DEBUG_ONLY( DebugLogError(_T("*** %hs - found source (%s) with wrong part count (%u) attached to file \"%s\" (partcount=%u)"), __FUNCTION__, cur_src->DbgGetClientInfo(), cur_src->GetUpPartCount(), GetFileName(), partCount()));
				}
			}
			else
			{
...
void CKnownFile::SetUpPriority(uint8_t iNewUpPriority, bool bSave)
{
		...
		if (_tmakepathlimit(szFullPath, NULL, GetPath(), GetFileName(), NULL)){
			try{
				...
				if (mp3info)
				{
					// length
					if (mp3info->time){
						CTag* pTag = new CTag(FT_MEDIA_LENGTH, (uint32_t)mp3info->time);
						AddTagUnique(pTag);
						m_uMetaDataVer = META_DATA_VER;
					}
					...
					if (uBitrate){
						CTag* pTag = new CTag(FT_MEDIA_BITRATE, (uint32_t)uBitrate);
						AddTagUnique(pTag);
						m_uMetaDataVer = META_DATA_VER;
					}
				}
		...
		if (_tmakepathlimit(szFullPath, NULL, GetPath(), GetFileName(), NULL))
		{
			...
			try
			{
				...
				if (mi)
				{
					...
					if (uLengthSec) {
						CTag* pTag = new CTag(FT_MEDIA_LENGTH, (uint32_t)uLengthSec);
						AddTagUnique(pTag);
						m_uMetaDataVer = META_DATA_VER;
					}
...
bool CKnownFile::PublishSrc()
{
		...
		(Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) || !Kademlia::CUDPFirewallTester::IsVerified()))
	{
		...
		if( buddy )
		{
			...
			{
				SetLastPublishTimeKadSrc( (uint32_t)time(NULL)+KADEMLIAREPUBLISHTIMES, lastBuddyIP );
				return true;
			}
		}
		else
...
bool CKnownFile::GrabImage(uint8_t nFramesToGrab, double dStartTime, bool bReduceColor, uint16_t nMaxWidth, void* pSender)
{
	return GrabImage(GetPath() + CString(_T("\\")) + GetFileName(), nFramesToGrab,  dStartTime, bReduceColor, nMaxWidth, pSender);
}

bool CKnownFile::GrabImage(LPCTSTR strFileName,uint8_t nFramesToGrab, double dStartTime, bool bReduceColor, uint16_t nMaxWidth, void* pSender)
{
	if (!IsMovie())
		return false;
	CFrameGrabThread* framegrabthread = (CFrameGrabThread*) CSafeThread::BeginThread(RUNTIME_CLASS(CFrameGrabThread), THREAD_PRIORITY_NORMAL,0, CREATE_SUSPENDED);
	if (framegrabthread == NULL)
...
void CKnownFile::GrabbingFinished(CxImage** imgResults, uint8_t nFramesGrabbed, void* pSender)
{
	// continue processing
...
CString CKnownFile::GetInfoSummary(bool bNoFormatCommands) const
{
	...
	// ismod: path has no trailing backslash
