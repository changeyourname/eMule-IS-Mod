...
void CTitleMenu::EnableIcons()
{
	m_bIconMenu = true;
	m_ImageList.DeleteImageList();
	m_ImageList.Create(ICONSIZE, ICONSIZE, ILC_COLOR32 | ILC_MASK, 0, 1);
...
BOOL CTitleMenu::AppendMenu(UINT nFlags, UINT_PTR nIDNewItem, LPCTSTR lpszNewItem, LPCTSTR lpszIconName)
{
	...
	if (bResult)
		SetMenuBitmap(nFlags, static_cast<UINT>(nIDNewItem), lpszNewItem, lpszIconName);
	return bResult;
}
