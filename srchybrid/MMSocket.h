...
{
public:
	CMMPacket(uint8_t byOpcode)	{m_pBuffer = new CMemFile; m_pBuffer->Write(&byOpcode,1); m_bSpecialHeader = false;}
	~CMMPacket()				{delete m_pBuffer;}
	void	WriteByte(uint8_t write)		{m_pBuffer->Write(&write,1);}
	void	WriteShort(uint16_t write)	{m_pBuffer->Write(&write,2);}
	void	WriteInt(uint32_t write)		{m_pBuffer->Write(&write,4);}
	void	WriteInt64(uint64_t write)	{m_pBuffer->Write(&write,8);}
	void	WriteString(CStringA write){
		uint8_t len = (write.GetLength() > 255) ? (uint8_t)255 : (uint8_t)write.GetLength();
		WriteByte(len);
...
{
public:
	CMMData(char* pData,uint32_t nSize):CSafeMemFile((BYTE*)pData,nSize)	{}
