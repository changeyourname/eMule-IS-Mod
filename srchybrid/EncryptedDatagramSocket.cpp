...
int CEncryptedDatagramSocket::DecryptReceivedClient(BYTE* pbyBufIn, int nBufLen, BYTE** ppbyBufOut, uint32_t dwIP, uint32_t* nReceiverVerifyKey, uint32_t* nSenderVerifyKey) const{
	int nResult = nBufLen;
	*ppbyBufOut = pbyBufIn;
...
// else																-> ASSERT
int CEncryptedDatagramSocket::EncryptSendClient(uchar** ppbyBuf, int nBufLen, const uchar* pachClientHashOrKadID, bool bKad, uint32_t nReceiverVerifyKey, uint32_t nSenderVerifyKey) const{
	...
	const uint32_t nCryptHeaderLen = byPadLen + CRYPT_HEADER_WITHOUTPADDING + (bKad ? 8 : 0);
	
	uint32_t nCryptedLen = nBufLen + nCryptHeaderLen;
	uchar* pachCryptedBuffer = new uchar[nCryptedLen];
	bool bKadRecKeyUsed = false;
	
	uint16_t nRandomKeyPart = (uint16_t)cryptRandomGen.GenerateWord32(0x0000, 0xFFFF);
	MD5Sum md5;
	if (bKad){
		...
		uchar achKeyData[23];
		md4cpy(achKeyData, pachClientHashOrKadID);
		uint32_t dwIP = theApp.GetPublicIP();
		memcpy(achKeyData+16, &dwIP, 4);
	...
	uint32_t dwMagicValue = MAGICVALUE_UDP_SYNC_CLIENT;
	...
	for (int j = 0; j < byPadLen; j++){
		uint8_t byRand = (uint8_t)rand();	// they actually dont really need to be random, but it doesn't hurts either
...
int CEncryptedDatagramSocket::DecryptReceivedServer(BYTE* pbyBufIn, int nBufLen, BYTE** ppbyBufOut, uint32_t dwBaseKey, uint32_t dbgIP) const{
	int nResult = nBufLen;
	*ppbyBufOut = pbyBufIn;
	...
	// might be an encrypted packet, try to decrypt
	uchar achKeyData[7];
	memcpy(achKeyData, &dwBaseKey, sizeof(dwBaseKey));
...
int CEncryptedDatagramSocket::EncryptSendServer(uchar** ppbyBuf, int nBufLen, uint32_t dwBaseKey) const{
	...
	uint32_t dwMagicValue = MAGICVALUE_UDP_SYNC_SERVER;
	pachCryptedBuffer[0] = bySemiRandomNotProtocolMarker;
	memcpy(pachCryptedBuffer + 1, &nRandomKeyPart, 2);
	RC4Crypt((uchar*)&dwMagicValue, pachCryptedBuffer + 3, 4, &keySendKey);
	RC4Crypt((uchar*)&byPadLen, pachCryptedBuffer + 7, 1, &keySendKey);
