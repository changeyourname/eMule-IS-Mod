...
BOOL CComboBoxEx2::PreTranslateMessage(MSG* pMsg)
{
	...
	{
		UINT uChar = MapVirtualKey(static_cast<UINT>(pMsg->wParam), 2);
		if (uChar != 0)
		{
...
// ismod [start]
void* CComboBoxEx2::GetCurrentItemData() const
{
    int iItem = GetCurSel();
    if ( CB_ERR != iItem )
        return GetItemDataPtr(iItem);
    else    
        return nullptr;
}

CString CComboBoxEx2::GetCurrentItemText() const 
{
	int iItem = GetCurSel();
	CString strText;
	if ( CB_ERR != iItem )
