...
BEGIN_MESSAGE_MAP(CCollectionViewDialog, CResizableDialog)
	...
	// ismod
	ON_EN_CHANGE(IDC_MOD_EDITCATALOGNAME, OnEnChangeModEditcatalogname)
	ON_BN_CLICKED(IDC_MOD_SELECTOUTPUTDIR, OnBnClickedModSelectoutputdir)
END_MESSAGE_MAP()
...
void CCollectionViewDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	// ismod
	DDX_Control(pDX, IDC_MOD_EDITCATALOGNAME, m_CatalogName);
}
...
BOOL CCollectionViewDialog::OnInitDialog(void)
{
	...
	// ismod
	m_CatalogName.SetWindowText(m_pCollection->m_sCollectionName);
	//SetDlgItemText(IDC_MOD_CATALOGNAME, GetResString(IDS_OPTIONS) );
	...
	while (pos != NULL)
	{
		...
		int iImage = (int) theApp.GetFileTypeSystemImageIdx(pCollectionFile->GetFileName());
...
void CCollectionViewDialog::OnBnClickedModSelectoutputdir()
{
	TCHAR buffer[MAX_PATH] = {0};
	if (SelectDir(GetSafeHwnd(),buffer,_T("Укажите место сохранения каталога '" + m_pCollection->m_sCollectionName + _T("'")))) {
		SetDlgItemText(IDC_MOD_SAVEPATH, buffer);
		m_pCollection->m_sCollectionFolder = buffer;
		m_pCollection->m_sCollectionFolder.TrimRight(_T("\\"));
		CString temp = m_pCollection->m_sCollectionFolder +_T("\\")+ m_pCollection->m_sCollectionName;
		SetDlgItemText(IDC_MOD_SAVEPATH, temp);
		if (m_pCollection->m_sCollectionName == _T("") || temp == _T("")) GetDlgItem(IDC_VIEWCOLLECTIONDL)->EnableWindow(false);
		else GetDlgItem(IDC_VIEWCOLLECTIONDL)->EnableWindow(true);
	}
}
