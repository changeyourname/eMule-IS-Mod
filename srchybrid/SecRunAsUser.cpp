...
#include "SecRunAsUser.h"    // class declaration
#include "otherfunctions.h"  // GetResString,...
#include "Preferences.h"     // thePrefs
...
//_COM_SMARTPTR_TYPEDEF(IADsAccessControlEntry, IID_IADsAccessControlEntry);
//_COM_SMARTPTR_TYPEDEF(IADsSecurityDescriptor, IID_IADsSecurityDescriptor);
_COM_SMARTPTR_TYPEDEF(IADsWinNTSystemInfo, IID_IADsWinNTSystemInfo);
...
CSecRunAsUser::Result CSecRunAsUser::PrepareUser()
{
	CoInitialize(NULL);
	bool bResult = false;
		...
		if ( !bResult ) {
            ...
            if ( SUCCEEDED(pUsers->Create(CComBSTR(L"user"),CString(EMULEACCOUNTW).AllocSysString() ,&pDisp )) ) {
                ...
                if ( SUCCEEDED(pDisp->QueryInterface(&pUser)) ) {
                    ...
                    pUser->SetInfo();
                    m_strPassword = CreateRandomPW();
                    bResult = SUCCEEDED(pUser->SetPassword(m_strPassword.AllocSysString()));
                }
            }
		}
        if ( bResult ) {
...
#define FULLACCESS ADS_RIGHT_GENERIC_ALL
	...
	// we don't check for success here, for emule will also run if one dir fails for some reason
...
bool CSecRunAsUser::SetObjectPermission(CString strDirFile, DWORD lGrantedAccess)
{
	...
	try {
		...
		uint32_t newAceIndex = 0;
		uint32_t CurrentAceIndex = 0;
		if (AclInfo.AceCount) {
...
CSecRunAsUser::Result CSecRunAsUser::RestartAsUser()
{
...
CSecRunAsUser::Result CSecRunAsUser::RestartAsRestricted()
{
	if (m_bRunningRestricted || m_bRunningAsEmule)
		return RES_OK;
	...
	try {
		...
		// cleanup
		MHeapFree((LPVOID)pstructUserToken);
...
CSecRunAsUser::Result CSecRunAsUser::RestartSecure()
{
	Result res;
