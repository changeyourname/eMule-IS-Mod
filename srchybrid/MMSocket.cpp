...
void CMMSocket::OnReceive(int nErrorCode){
	...
	const size_t SIZE_PRESERVE = 0x1000;
	size_t readMax = sizeof(GlobalReadBuffer); 
	size_t dwSize = Receive(GlobalReadBuffer, readMax);
			...
			if ('\n' == m_pBuf[dwPos])
				if (bPrevEndl)
				{
					...
					for (dwPos = 0; dwPos < m_dwHttpHeaderLen; )
					{
						...
						if (!pPtr)
							break;
						ptrdiff_t dwNextPos = pPtr - m_pBuf;
...
bool CMMSocket::SendPacket(CMMPacket* packet, bool bQueueFirst){
	if (m_pSendBuffer != NULL){
		...
		if (!packet->m_bSpecialHeader)
			nLen = _snprintf_s(szBuf, _countof(szBuf), _countof(szBuf), "HTTP/1.1 200 OK\r\nConnection: close\r\nContent-Type: %s\r\nContent-Length: %ld\r\n\r\n",m_pOwner->GetContentType(), packet->m_pBuffer->GetLength());
		else
			nLen = _snprintf_s(szBuf, _countof(szBuf), _countof(szBuf), "Content-Length: %ld\r\n\r\n", packet->m_pBuffer->GetLength());
...
{
	if(m_pSendBuffer != NULL){
		uint32_t res = Send(m_pSendBuffer+m_nSent,m_nSendLen-m_nSent);
		if (res == SOCKET_ERROR){
			if (GetLastError() != WSAEWOULDBLOCK)
				Close();
			return;
