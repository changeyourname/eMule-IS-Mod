...
void CEditDelayed::DoDelayedEvalute( bool bForce )
{
	...
	GetParent()->SendMessage(m_uDelayedEvaluateMessage, (WPARAM)m_nCurrentColumnIdx, (LPARAM)(LPCTSTR)m_strLastEvaluatedContent);
}

void CEditDelayed::OnInit(CHeaderCtrl* pColumnHeader, CArray<int>* paIgnoredColums)
{
	SetEditRect(false);
	CRect rectWindow;
	...
	pImageList->Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 1);
		...
		if (point.x <= ICON_LEFTSPACE)
		{
			...
			for (int i = 0; i < nCount ;i++) {
				...
				if (hdi.cxy > 0 && !bIgnored) // ignore hidden columns
					menu.AppendMenu(MF_STRING | ((m_nCurrentColumnIdx == nIdx) ? MF_CHECKED : MF_UNCHECKED), nIdx, hdi.pszText);
			}
...
{
	const int nNewIdx = (int) LOWORD(wParam);
	if ( nNewIdx != m_nCurrentColumnIdx )
	{
		m_nCurrentColumnIdx = nNewIdx;
		if (m_bShowsColumnText)
			ShowColumnText(true);
