...
void CIrcMain::ParseMessage(CString sRawMessage)
{
	try
	{
			...
			if (sRawMessage.Mid(iIndex, 2) == _T(":\001"))
			{
				...
				{	
					...
					if( (sSound.Right(4) != _T(".wav")) && (sSound.Right(4) != _T(".mp3")) )
						return;
					sSound.Format(_T("%sSounds\\IRC\\%s"), thePrefs.GetMuleDirectory(EMULE_APPDIR), sSound);
				...
				if( sRawMessage.Mid(iIndex, 11) == _T(":\001REPFRIEND") )
				{
					...
					SetVerify();
					uint32_t uNewClientID = _tstoi(sRawMessage.Tokenize(_T(":"), iIndex));
					uint16_t uNewClientPort = (uint16_t)_tstoi(sRawMessage.Tokenize(_T("|"), iIndex));
		...
		{
			sUser = sRawMessage.Tokenize(_T(" "), iIndex);
