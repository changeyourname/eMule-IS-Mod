...
#include "MenuCmds.h"
#include "Log.h"
#include "EMSocket.h" // netfinity: Advanced download throttling
// ismod
...
void CDownloadQueue::AddPartFilesToShare() const
{
...
void CDownloadQueue::AddSearchToDownload(CSearchFile* toadd, uint8_t paused, int cat)
{
	if (toadd->GetFileSize()== (uint64_t)0 || IsFileExisting(toadd->GetFileHash()))
		return;
	...
	if (paused == 2)
		paused = (uint8_t)thePrefs.AddNewFilesPaused();
...
void CDownloadQueue::AddSearchToDownload(const CString &link, uint8_t paused, int cat)
{
...
void CDownloadQueue::StartNextFile(int cat, bool force){
	...
	if (pfile)
		pfile->ResumeFile();
	// ismod
	else if ( thePrefs.GetAutoShutdownWhenDone() ) {
		emUtils::shutdownWindows();
		theApp.SetAutomaticShutdown(true);
	}
}
...
void CDownloadQueue::Process(){
    ...
    uint64_t maxDownload = thePrefs.GetMaxDownloadInBytesPerSec(true);
	...
	// ismod: limit speed when allocating
	for (size_t i=0; i<filelist.size(); ++i)
	{
		if(filelist[i]->IsAllocating())
		{
			maxDownload = 10240; // 10kb max speed per file
			break;
		}
	}

// BEGIN netfinity: Advanced download throttling
	if (maxDownload != UNLIMITED*1024)
		CEMSocket::SetGlobalDownloadLimit((uint32_t) maxDownload);
	else
		CEMSocket::DisableGlobalDownloadLimit();
// END netfinity: Advanced download throttling
	// netfinity: Datarate is now handled by the black box!
	...
	for (size_t i=0; i<filelist.size(); ++i){
		...
		theStats.m_fGlobalDone += (uint64_t)cur_file->GetCompletedSize();
		theStats.m_fGlobalSize += (uint64_t)cur_file->GetFileSize();
		...
		if (cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY){
			datarateX += cur_file->Process(/*downspeed*/ 0, udcounter);
		}
		else{
...
CPartFile* CDownloadQueue::GetFileByKadFileSearchID(uint32_t id) const
{
	for (size_t i=0; i<filelist.size(); ++i)
	{
		CPartFile* cur_file = filelist[i];
...
bool CDownloadQueue::IsTempFile(const CString& , const CString& rstrName) const
{
	...
	for (size_t i=0; i<filelist.size(); ++i)
    {
		if (!othername.CompareNoCase(filelist[i]->GetPartMetFileName()))
			return true;
	}
...
bool CDownloadQueue::CheckAndAddSource(CPartFile* sender,CUpDownClient* source){
	...
	if (theApp.clientlist->AttachToAlreadyKnown(&source,0)){
		...
		if (thePrefs.GetVerbose() && source->GetRequestFile()){
			...
			if (source->GetRequestFile()->partCount() != 0 && source->GetRequestFile()->partCount() != sender->partCount())
...
#ifdef _DEBUG
	if (thePrefs.GetVerbose() && source->GetPartCount()!=0 && source->GetPartCount()!=sender->partCount()){
...
bool CDownloadQueue::CheckAndAddKnownSource(CPartFile* sender,CUpDownClient* source){
	...
	if (!source->HasLowID()){
		uint32_t nClientIP = ntohl(source->GetUserIDHybrid());
...
void CDownloadQueue::RemoveFile(CPartFile* toremove)
{
    ...
    filelist.erase(std::find(filelist.begin(), filelist.end(), toremove));
	ASSERT( filelist.end() == std::find(filelist.begin(), filelist.end(), toremove) );
	SortByPriority();
	CheckDiskspace();
	ExportPartMetFilesOverview();
}
...
bool CDownloadQueue::IsMaxFilesPerUDPServerPacketReached(uint32_t nFiles, uint32_t nIncludedLargeFiles) const
{
...
bool CDownloadQueue::SendGlobGetSourcesUDPPacket(CSafeMemFile* data, bool bExt2Packet, uint32_t nFiles, uint32_t nIncludedLargeFiles)
{
	...
	if (cur_udpserver)
	{
		...
		data = NULL;
		if (bExt2Packet){
			ASSERT( iPacketSize > 0 && (uint32_t)iPacketSize == nFiles*20 + nIncludedLargeFiles*8);
			packet.opcode = OP_GLOBGETSOURCES2;
		}
		else{
			ASSERT( iPacketSize > 0 && (uint32_t)iPacketSize == nFiles*16 && nIncludedLargeFiles == 0);
...
bool CDownloadQueue::SendNextUDPPacket()
{
	...
	while (!IsMaxFilesPerUDPServerPacketReached(iFiles, iLargeFiles) && !bSentPacket)
	{
		...
		while (!bSentPacket && !(nextfile && (nextfile->GetStatus() == PS_READY || nextfile->GetStatus() == PS_EMPTY)))
		{
			...
			{
				auto it = std::find(filelist.begin(), filelist.end(), lastfile);
				if (filelist.end() == it ) // the last file is no longer in the DL-list (may have been finished or canceld)
				{
					// get first file to search sources for
					nextfile = filelist[0];
					lastfile = nextfile;
				...
				{
					++it;
					if (it == filelist.end()) // finished asking the current server for all files
					{
		...
		if (!bSentPacket && nextfile && nextfile->GetSourceCount() < nextfile->GetMaxSourcePerFileUDP() && (bServerSupportsLargeFiles || !nextfile->IsLargeFile()) )
		{
			if (bGetSources2Packet){
				if (nextfile->IsLargeFile()){
					...
					dataGlobGetSources.WriteHash16(nextfile->GetFileHash());
					dataGlobGetSources.WriteUInt32((uint32_t)(uint64_t)nextfile->GetFileSize());
				}
			}
			else{
	...
	if (m_cRequestsSentToServer >= MAX_REQUESTS_PER_SERVER)
	{
		...
		if (filelist.size() >= MAX_REQUESTS_PER_SERVER) {
            int startpos = 0, endpos = filelist.size()-1;
			while (startpos < MAX_REQUESTS_PER_SERVER) {
                CPartFile* file = filelist[startpos];
                filelist[startpos++] = filelist[endpos];
                filelist[endpos--] = file;
            }
		}
...
int CDownloadQueue::CompareParts(void *, const void *file1, const void *file2)
{
    return (int)CPartFile::RightFileHasHigherPrio(*(CPartFile**)file1, *(CPartFile**)file2);
}

void CDownloadQueue::SortByPriority()
{
    qsort_s((void*)filelist.data(), filelist.size(), sizeof(CPartFile*), CompareParts, NULL);
}
...
void CDownloadQueue::CheckDiskspace(bool bNotEnoughSpaceLeft)
{
	...
	uint64_t nTotalAvailableSpaceMain = bNotEnoughSpaceLeft ? 0 : GetFreeDiskSpaceX(thePrefs.GetTempDir());
	...
	if (thePrefs.GetMinFreeDiskSpace() == 0)
	{
		for (size_t i=0; i<filelist.size(); ++i)
		{
			...
			uint64_t nTotalAvailableSpace = bNotEnoughSpaceLeft ? 0 : 
			...
			uint64_t nSpaceToGo = cur_file->GetNeededSpace();
		...
		for (size_t i=0; i<filelist.size(); ++i)
		{
			...
			if (nTotalAvailableSpace < thePrefs.GetMinFreeDiskSpace())
			{
				if (cur_file->IsNormalFile())
				{
					...
					uint64_t nSpaceToGrow = cur_file->GetNeededSpace();
					if (nSpaceToGrow > 0)
						cur_file->PauseFile(true/*bInsufficient*/);
				}
				else
...
void CDownloadQueue::GetDownloadSourcesStats(SDownloadStats& results)
{
	...
	for (size_t i=0; i<filelist.size(); ++i)
	{
		const CPartFile* cur_file = filelist[i];
...
CUpDownClient* CDownloadQueue::GetDownloadClientByIP(uint32_t dwIP)
{
	for ( size_t i=0; i<filelist.size(); ++i ) {
		CPartFile* cur_file = filelist[i];
		for (POSITION pos2 = cur_file->srclist.GetHeadPosition();pos2 != 0; ){
...
CUpDownClient* CDownloadQueue::GetDownloadClientByIP_UDP(uint32_t dwIP, uint16_t nUDPPort, bool bIgnorePortOnUniqueIP, bool* pbMultipleIPs)
{
...
bool CDownloadQueue::IsInList(const CUpDownClient* client) const
{
	for ( size_t i=0; i<filelist.size(); ++i ) {
		CPartFile* cur_file = filelist[i];
		for ( POSITION pos = cur_file->srclist.GetHeadPosition(); pos != 0; /**/ ) {
			if ( cur_file->srclist.GetNext(pos) == client )
				return true;
		}
	}
	return false;
...
void CDownloadQueue::SetCatPrio(UINT cat, uint8_t newprio)
{
	for (size_t i=0; i<filelist.size(); ++i){
		CPartFile* cur_file = filelist[i];
...
// ZZ:DownloadManager -->
void CDownloadQueue::RemoveAutoPrioInCat(UINT cat, uint8_t newprio){
	CPartFile* cur_file;
	for (size_t i=0; i<filelist.size(); ++i){
		cur_file = filelist[i];
...
UINT CDownloadQueue::GetDownloadingFileCount() const
{
	UINT result = 0;
	for (size_t i=0; i<filelist.size(); ++i){
		UINT uStatus = filelist[i]->GetStatus();
...
UINT CDownloadQueue::GetPausedFileCount() const
{
	UINT result = 0;
	for (size_t i=0; i<filelist.size(); ++i){
		if (filelist[i]->GetStatus() == PS_PAUSED)
			result++;
	}
	return result;
}
...
{
    m_localServerReqQueue.remove(pFile);
    pFile->m_bLocalSrcReqQueued = false;
}
...
void CDownloadQueue::ProcessLocalRequests()
{
	if ( !m_localServerReqQueue.empty() && (m_dwNextTCPSrcReq < ::GetTickCount()) )
	{
		...
		while ( !m_localServerReqQueue.empty() && (iFiles < iMaxFilesPerTcpFrame) )
		{
			...
			uint32_t dwBestWaitTime = 0xFFFFFFFF;
			auto bestIter = m_localServerReqQueue.end();
			for ( auto it = m_localServerReqQueue.begin(); it != m_localServerReqQueue.end(); ++it ) {
				CPartFile* cur_file = *it;
				if ( cur_file->GetStatus() == PS_READY || cur_file->GetStatus() == PS_EMPTY ) {
					uint8_t nPriority = cur_file->GetDownPriority();
					...
					m_localServerReqQueue.erase(it);
			...
			if ( bestIter != m_localServerReqQueue.end() )
			{
				CPartFile* cur_file = *bestIter;
				cur_file->m_bLocalSrcReqQueued = false;
				cur_file->m_LastSearchTime = ::GetTickCount();
				m_localServerReqQueue.erase(bestIter);
				...
				if (!cur_file->IsLargeFile()){
					smPacket.WriteUInt32((uint32_t)(uint64_t)cur_file->GetFileSize());
				}
				else{
...
void CDownloadQueue::SendLocalSrcRequest( CPartFile* sender ) {
    ASSERT( m_localServerReqQueue.end() == std::find(m_localServerReqQueue.begin(), m_localServerReqQueue.end(), sender) );
	m_localServerReqQueue.push_back(sender);
}

int CDownloadQueue::GetDownloadFilesStats(uint64_t &rui64TotalFileSize,
									      uint64_t &rui64TotalLeftToTransfer,
									      uint64_t &rui64TotalAdditionalNeededSpace) const
{
	int iActiveFiles = 0;
	for (size_t i=0; i<filelist.size(); ++i)
	{
		...
		if (uState == PS_READY || uState == PS_EMPTY)
		{
			uint64_t ui64LeftToTransfer = 0;
			uint64_t ui64AdditionalNeededSpace = 0;
			cur_file->GetLeftToTransferAndAdditionalNeededSpace(ui64LeftToTransfer, ui64AdditionalNeededSpace);
			rui64TotalFileSize += (uint64_t)cur_file->GetFileSize();
...
void CSourceHostnameResolveWnd::AddToResolve(const uchar* fileid, LPCSTR pszHostname, uint16_t port, LPCTSTR pszURL)
{
...
LRESULT CSourceHostnameResolveWnd::OnHostnameResolved(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			{
				uint32_t nIP = ((LPIN_ADDR)(pHost->h_addr_list[0]))->s_addr;
...
void CDownloadQueue::KademliaSearchFile(uint32_t searchID, const Kademlia::CUInt128* pcontactID, const Kademlia::CUInt128* pbuddyID, uint8_t type, uint32_t ip, uint16_t tcp, uint16_t udp, uint32_t dwBuddyIP, uint16_t dwBuddyPort, uint8_t byCryptOptions)
{
...
void CDownloadQueue::ExportPartMetFilesOverview() const
{
	...
	try
	{
		...
		for (size_t i=0; i<filelist.size(); ++i)
		{
			const CPartFile* pPartFile = filelist[i];
			if (pPartFile->GetStatus(true) != PS_COMPLETE)
			{
				TCHAR szNam[_MAX_FNAME];
				TCHAR szExt[_MAX_EXT];
				_tsplitpath_s(pPartFile->GetFilePath(), NULL, 0, NULL, 0, szNam, _countof(szNam), szExt, _countof(szExt));
				if (thePrefs.GetTempDirCount()==1)
	...
	catch(CFileException* e)
	{
		CString strError;
		TCHAR szError[MAX_CFEXP_ERRORMSG];
		if (e->GetErrorMessage(szError, _countof(szError))){
			strError += _T(" - ");
			strError += szError;
		}
...
void CDownloadQueue::OnConnectionState(bool bConnected)
{
	for (size_t i=0; i<filelist.size(); ++i)
	{
		CPartFile* pPartFile = filelist[i];
...
CString CDownloadQueue::GetOptimalTempDir(UINT nCat, EMFileSize nFileSize){
	...
	CMap<int, int, int64_t, int64_t> mapNeededSpaceOnDrive;
	CMap<int, int, int64_t, int64_t> mapFreeSpaceOnDrive;
	
	int64_t llBuffer = 0;
	int64_t llHighestFreeSpace = 0;
    ...
    for (size_t i=0; i<filelist.size(); ++i){
		const CPartFile* pCurFile =  filelist[i];
		const int nDriveNumber = GetPathDriveNumber(pCurFile->GetTempPath());

		int64_t llNeededForCompletion = 0;
	...
	for (int i = 0; i < thePrefs.tempdir.GetCount(); i++) {
		...
		if (nFileSize <= OLD_MAX_EMULE_FILE_SIZE || !IsFileOnFATVolume(thePrefs.GetTempDir(i))){
			...
			if (GetPathDriveNumber(thePrefs.GetCatPath(nCat)) == nDriveNumber &&
				llAvailableSpace > (int64_t)nFileSize)
			{
				//this one is perfect
			...
			if (llAvailableSpace > (int64_t)nFileSize && llAvailableSpace > llHighestTotalSpace){
	...
	else if (nHighestFreeSpaceDir != (-1)){ // condition 1 could not be met too, take 2
		return thePrefs.GetTempDir(nHighestFreeSpaceDir);
	}
	else if( nAnyAvailableDir != (-1)){
		return thePrefs.GetTempDir(nAnyAvailableDir);
	}
	else{ // so was condition 2 and 3, take 4.. wait there is no 3 - this must be a bug
		ASSERT( false );
		return thePrefs.GetTempDir();
	}
}
...
{
	for ( size_t i=0; i<filelist.size(); ++i ) {
		filelist[i]->RefilterFileComments();
	}
}

/*
 * @param pszEd2kLink ed2k link
 * @param boost boost source ask for this request
 */
void CDownloadQueue::PreviewFileByLink( LPCTSTR pszEd2kLink, bool boost )
{
	CED2KFileLink* pEd2kLink = reinterpret_cast<CED2KFileLink*>( CED2KLink::CreateLinkFromUrl(pszEd2kLink) );

	if ( NULL != pEd2kLink && CED2KLink::kFile == pEd2kLink->GetKind() ) {
		CPartFile* pPartFile = GetFileByID(pEd2kLink->GetHashKey());
		if ( NULL != pPartFile ) {
            if ( boost ) {
                // update last search time for all files
                for( size_t i=0; i<filelist.size(); ++i ) {
                    CPartFile *file = filelist[i];
                    if ( 0 == file->m_LastSearchTime ) {
                        file->m_LastSearchTime = 1;
                    }
                }

                pPartFile->m_LastSearchTime = 0;
                m_dwNextTCPSrcReq = 0;
            }
