...
void CClientList::GetStatistics(uint32_t &ruTotalClients, int stats[NUM_CLIENTLIST_STATS], 
...
{
	ruTotalClients = static_cast<uint32_t>(list.GetCount());
...
CUpDownClient* CClientList::FindClientByIP(uint32_t clientip, UINT port) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
...
CUpDownClient* CClientList::FindClientByUserHash(const uchar* clienthash, uint32_t dwIP, uint16_t nTCPPort) const
{
...
CUpDownClient* CClientList::FindClientByIP(uint32_t clientip) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
...
CUpDownClient* CClientList::FindClientByIP_UDP(uint32_t clientip, UINT nUDPport) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
...
CUpDownClient* CClientList::FindClientByUserID_KadPort(uint32_t clientID, uint16_t kadPort) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
...
CUpDownClient* CClientList::FindClientByIP_KadPort(uint32_t ip, uint16_t port) const
{
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client = list.GetNext(pos);
...
CUpDownClient* CClientList::FindClientByServerID(uint32_t uServerIP, uint32_t uED2KUserID) const
{
	uint32_t uHybridUserID = ntohl(uED2KUserID);
	for (POSITION pos = list.GetHeadPosition(); pos != NULL;)
	{
		CUpDownClient* cur_client =	list.GetNext(pos);
...
void CClientList::AddBannedClient(uint32_t dwIP){
	m_bannedList.SetAt(dwIP, ::GetTickCount());
}

bool CClientList::IsBannedClient(uint32_t dwIP) const
{
	uint32_t dwBantime;
...
void CClientList::RemoveBannedClient(uint32_t dwIP){
...
// false = hash changed
bool CClientList::ComparePriorUserhash(uint32_t dwIP, uint16_t nPort, void* pNewHash){
	CDeletedClient* pResult = 0;
...
UINT CClientList::GetClientsFromIP(uint32_t dwIP) const
{
	CDeletedClient* pResult;
	if (m_trackedClientsList.Lookup(dwIP, pResult))
		return static_cast<UINT>(pResult->m_ItemsList.GetCount());
	return 0;
}
...
uint32_t CClientList::GetBadRequests(const CUpDownClient* upcClient) const{
	CDeletedClient* pResult = NULL;
	if (upcClient->GetIP() == 0){
		ASSERT( false );
		return 0;
...
bool CClientList::RequestTCP(Kademlia::CContact* contact, uint8_t byConnectOptions)
{
	uint32_t nContactIP = ntohl(contact->GetIPAddress());
...
void CClientList::RequestBuddy(Kademlia::CContact* contact, uint8_t byConnectOptions)
{
	uint32_t nContactIP = ntohl(contact->GetIPAddress());
	// don't connect ourself
	if (theApp.serverconnect->GetLocalIP() == nContactIP && thePrefs.GetPort() == contact->GetTCPPort())
...
void CClientList::AddKadFirewallRequest(uint32_t dwIP){
...
bool CClientList::IsKadFirewallCheckIP(uint32_t dwIP) const{
...
void CClientList::AddTrackCallbackRequests(uint32_t dwIP){
	IPANDTICS add = {dwIP, ::GetTickCount()};
...
bool CClientList::AllowCalbackRequest(uint32_t dwIP) const
{
