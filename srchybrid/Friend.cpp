...
#include "Log.h"
// ismod headers
#include "Preferences.h" // friend slot for all friends
...
void CFriend::SetFriendSlot(bool newValue) {
	...
	// ismod: store friend slot
	if ( newValue )
		thePrefs.SetFriendSlotUserHash(m_abyUserhash);
	else
		thePrefs.RemoveFriendSlotUserHash();
...
bool CFriend::GetFriendSlot() const {
    if(GetLinkedClient() != NULL) {
		...
		return m_friendSlot;
		*/
		return thePrefs.GetUseFriendSlotForAllFriends()?true:m_friendSlot;
    }
}
