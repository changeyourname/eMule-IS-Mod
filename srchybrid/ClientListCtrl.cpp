...
void CClientListCtrl::Init()
{
	...
	InsertColumn(i++, GetResString(IDS_CL_UPLOADSTATUS),	LVCFMT_LEFT,  100);
	InsertColumn(i++, GetResString(IDS_CL_TRANSFUP),		LVCFMT_RIGHT, DFLT_SIZE_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_CL_DOWNLSTATUS),	LVCFMT_LEFT,  100);
	...
	InsertColumn(i++, GetResString(IDS_CONNECTED),		LVCFMT_LEFT,   50);

	CString coltemp;
	coltemp = GetResString(IDS_CD_UHASH);
	coltemp.Remove(_T(':'));
	InsertColumn(i++, coltemp,							LVCFMT_LEFT,  DFLT_HASH_COL_WIDTH);
