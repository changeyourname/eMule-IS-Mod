...
CHTRichEditCtrl::CHTRichEditCtrl()
{
	...
	// ismod: right click hyperlink patch
	m_bRBClicked = false;
}
...
void CHTRichEditCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (point.x != -1 && point.y != -1 && lSelStart == lSelEnd)
	{
		...
		// ismod: right clicking on hyperlinks
		m_bRBClicked = true;
		...
		// ismod: patch for right clicking on hyperlinks
		m_bRBClicked = false;
	}
...
BOOL CHTRichEditCtrl::OnEnLink(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*/
	if (pEnLink && pEnLink->msg == WM_LBUTTONDOWN && !m_bRBClicked)
	{
		CString strUrl;
...
void CHTRichEditCtrl::SetFont(CFont* pFont, BOOL bRedraw)
{
	...
	_tcsncpy_s(cf.szFaceName, _countof(cf.szFaceName), lf.lfFaceName, _countof(cf.szFaceName));
...
// если лог чата имеет фокус ввода, то при нажатии любой кнопки
// фокус перейдет на едит бокс набираемого сообщения
BOOL CHTRichEditCtrl::PreTranslateMessage(MSG* pMsg)
{
	if(pMsg->message == WM_CHAR 
		&& !(GetKeyState(VK_CONTROL) & 0x8000) // не зажали ctrl
		&& GetParent()->IsKindOf(RUNTIME_CLASS(CChatSelector)))
	{
		CChatSelector * pChatSelector = (CChatSelector*)GetParent();
		pChatSelector->m_pParent->m_wndMessage.SetFocus();
		pChatSelector->m_pParent->m_wndMessage.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
		return FALSE;
	}
