	...
	, m_ctrlTreeOptions(ILC_COLOR32)
{
	...
	m_bDebug2Disk = false;
	m_iCommitFiles = 0;
	//m_bFilterLANIPs = false; // ismod: do not filter lan ips
	m_bExtControls = false;
	m_uServerKeepAliveTimeout = 0;
	m_iSpaceAllocPolicy = 0;
	...
	//m_htiFilterLANIPs = NULL; // ismod: do not filter lan ips
	m_htiExtControls = NULL;
	m_htiServerKeepAliveTimeout = NULL;
	m_htiSpaceAllocPolicy = NULL; // ismod: space alloc group
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		// File related group
		//
		m_htiSpaceAllocPolicy = m_ctrlTreeOptions.InsertGroup(GetResString(IDS_MOD_SPACEALLOC), iImgBackup, TVI_ROOT); // ismod: space alloc group
		m_htiFullAlloc = m_ctrlTreeOptions.InsertRadioButton(GetResString(IDS_FULLALLOC), m_htiSpaceAllocPolicy, m_iSpaceAllocPolicy==SPACE_ALLOC_FULL);
		m_htiSparsePartFiles = m_ctrlTreeOptions.InsertRadioButton(GetResString(IDS_SPARSEPARTFILES), m_htiSpaceAllocPolicy, m_iSpaceAllocPolicy==SPACE_ALLOC_SPARSE);
		...
		m_ctrlTreeOptions.Expand(m_htiSpaceAllocPolicy, TVE_EXPAND);
	...
	// File related group
	//
	DDX_TreeRadio(pDX, IDC_EXT_OPTS, m_htiSpaceAllocPolicy, m_iSpaceAllocPolicy);
	m_ctrlTreeOptions.SetRadioButtonEnable(m_htiSparsePartFiles, thePrefs.GetSparsePartFiles());
...
BOOL CPPgTweaks::OnInitDialog()
{
	...
	m_iSpaceAllocPolicy = thePrefs.m_iSpaceAllocPolicy;
	...
	m_ctrlTreeOptions.SetImageListColorFlags(ILC_COLOR32);
...
BOOL CPPgTweaks::OnApply()
{
	...
	thePrefs.m_iSpaceAllocPolicy = (ESpaceAllocPolicy)m_iSpaceAllocPolicy;
...
{	
	if(m_hWnd)
	{
		...
		if (m_htiSpaceAllocPolicy) m_ctrlTreeOptions.SetItemText(m_htiSpaceAllocPolicy, GetResString(IDS_MOD_SPACEALLOC)); // ismod: space alloc group
