...
{
	/* ismod: do not update server list
	...
	*/
	...
	// ismod: если список пуст, добавим сами [start]
	if(GetServerCount() == 0)
	{
		CServer *server = new CServer(4661, _T("emule1.is74.ru"));
		server->SetListName(_T("emule1.is74.ru"));
		server->SetIsStaticMember(true);
		theApp.emuledlg->serverwnd->serverlistctrl.AddServer(server, true);

		server = new CServer(4661, _T("emule2.is74.ru"));
		server->SetListName(_T("emule3.is74.ru"));
		server->SetIsStaticMember(true);
		theApp.emuledlg->serverwnd->serverlistctrl.AddServer(server, true);
		
		server = new CServer(4661, _T("emule3.is74.ru"));
		server->SetListName(_T("emule3.is74.ru"));
		server->SetIsStaticMember(true);
		theApp.emuledlg->serverwnd->serverlistctrl.AddServer(server, true);
...
void CServerList::ServerStats()
{
	...
	if (theApp.IsConnected() && theApp.serverconnect->IsUDPSocketAvailable() && list.GetCount() > 0)
	{
		...
		if (!ping_server->GetCryptPingReplyPending() && (tNow - ping_server->GetLastPingedTime()) >= UDPSERVSTATREASKTIME && theApp.GetPublicIP() != 0 && thePrefs.IsServerCryptLayerUDPEnabled()){
			...
			uint32_t nPacketLen = 4 + (uint8_t)(rand() % 16); // max padding 16 bytes
			BYTE* pRawPacket = new BYTE[nPacketLen];
			uint32_t dwChallenge = (rand() << 17) | (rand() << 2) | (rand() & 0x03);
			if (dwChallenge == 0)
				dwChallenge++;
			PokeUInt32(pRawPacket, dwChallenge);
			for (uint32_t i = 4; i < nPacketLen; i++) // fillung up the remaining bytes with random data
				pRawPacket[i] = static_cast<uint8_t>(rand() & 0xFF);

			ping_server->SetChallenge(dwChallenge);
			ping_server->SetLastPinged(GetTickCount());
			ping_server->SetLastPingedTime((tNow - (uint32_t)UDPSERVSTATREASKTIME) + 20); // give it 20 seconds to respond
...
void CServerList::GetStatus(uint32_t& total, uint32_t& failed, 
							uint32_t& user, uint32_t& file, uint32_t& lowiduser,
							uint32_t& totaluser, uint32_t& totalfile, 
							float& occ) const
{
	total = (uint32_t) list.GetCount();
	failed = 0;
	user = 0;
	file = 0;
	totaluser = 0;
	...
	uint32_t maxuserknownmax = 0;
	uint32_t totaluserknownmax = 0;
...
void CServerList::GetAvgFile(uint32_t& average) const
{
...
void CServerList::GetUserFileStatus(uint32_t& user, uint32_t& file) const
{
	user = 0;
	file = 0;
	for (POSITION pos = list.GetHeadPosition(); pos != 0; ){
...
CServer* CServerList::GetServerByAddress(LPCTSTR address, uint16_t port) const
{
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
...
CServer* CServerList::GetServerByIP(uint32_t nIP) const
{
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
        CServer* s = list.GetNext(pos);
		if (s->GetIP() == nIP)
...
CServer* CServerList::GetServerByIPTCP(uint32_t nIP, uint16_t nTCPPort) const
{
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
        CServer* s = list.GetNext(pos);
...
CServer* CServerList::GetServerByIPUDP(uint32_t nIP, uint16_t nUDPPort, bool bObfuscationPorts) const
{
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
        CServer* s = list.GetNext(pos);
...
bool CServerList::SaveServermetToFile()
{
	...
	try{
		...
		UINT fservercount = (UINT) list.GetCount();
...
void CServerList::AddServersFromTextFile(const CString& strFilename)
{
	...
	try	{
		...
		while (f.ReadString(strLine))
		{
			...
			CServer* nsrv = new CServer((uint16_t)_tstoi(strPort), strHost);
			...
			{
				delete nsrv;
				CServer* srvexisting = GetServerByAddress(strHost, (uint16_t)_tstoi(strPort));
				if (srvexisting) {
...
void CServerList::CheckForExpiredUDPKeys() {
	...
	uint32_t cKeysTotal = 0;
	uint32_t cKeysExpired = 0;
	uint32_t cPingDelayed = 0;
	const uint32_t dwIP = theApp.GetPublicIP();
	const uint32_t tNow = (uint32_t)time(NULL);
	ASSERT( dwIP != 0 );
	...
	for (POSITION pos = list.GetHeadPosition();pos != 0;){
		...
		if (pServer->SupportsObfuscationUDP() && pServer->GetServerKeyUDP(true) != 0 && pServer->GetServerKeyUDPIP() != dwIP){
			...
			if (tNow - pServer->GetRealLastPingedTime() < UDPSERVSTATMINREASKTIME){
				...
				pServer->SetLastPingedTime((tNow - (uint32_t)UDPSERVSTATREASKTIME) + (UDPSERVSTATMINREASKTIME - (tNow - pServer->GetRealLastPingedTime())));
			}
			else
