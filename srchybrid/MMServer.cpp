...
bool CMMServer::PreProcessPacket(char* pPacket, uint32_t nSize, CMMSocket* sender){
	if (nSize >= 3){
		uint16_t nSessionID;
...
void CMMServer::ProcessHelloPacket(CMMData* data, CMMSocket* sender){
	...
	if (data->ReadByte() != MM_VERSION){
		...
		if (testValue != thePrefs.GetMMPass() || plainPW.GetLength() == 0 ){
			...
			m_nSessionID = (uint16_t)rand();
			packet->WriteShort(m_nSessionID);
			packet->WriteString(thePrefs.GetUserNick());
			packet->WriteShort((uint16_t)((thePrefs.GetMaxUpload() >= UNLIMITED) ? 0 : thePrefs.GetMaxUpload()));
			packet->WriteShort((uint16_t)((thePrefs.GetMaxDownload() >= UNLIMITED) ? 0 : (uint16_t)thePrefs.GetMaxDownload()));
...
void CMMServer::ProcessStatusRequest(CMMSocket* sender, CMMPacket* packet){
	...
	packet->WriteShort((uint16_t)theApp.uploadqueue->GetDatarate()/100);
	packet->WriteShort((uint16_t)((thePrefs.GetMaxGraphUploadRate(true)*1024)/100));
	packet->WriteShort((uint16_t)theApp.downloadqueue->GetDatarate()/100);
	packet->WriteShort((uint16_t)((thePrefs.GetMaxGraphDownloadRate()*1024)/100));
	packet->WriteByte((uint8_t)theApp.downloadqueue->GetDownloadingFileCount());
	packet->WriteByte((uint8_t)theApp.downloadqueue->GetPausedFileCount());
	packet->WriteInt((uint32_t)(theStats.sessionReceivedBytes/1048576));
	packet->WriteShort((uint16_t)((theStats.GetAvgDownloadRate(0)*1024)/100));
...
void CMMServer::ProcessFileListRequest(CMMSocket* sender, CMMPacket* packet){
	...
	packet->WriteByte((uint8_t)nCount);
	...
	for (int i = 0; i != nCount; i++){
		...
		packet->WriteByte((uint8_t)cur_file->GetCategory());
...
void CMMServer::ProcessFinishedListRequest(CMMSocket* sender){
	...
	nCount = (m_SentFinishedList.GetCount() > 30)? 30 : static_cast<int>(m_SentFinishedList.GetCount());
	packet->WriteByte((uint8_t)nCount);
	for (int i = 0; i != nCount; i++){
		...
		if (cur_file->IsPartFile())
			packet->WriteByte((uint8_t)(((CPartFile*)cur_file)->GetCategory()));
		else
			packet->WriteByte(0);
	}
	sender->SendPacket(packet);
...
void CMMServer::ProcessFileCommand(CMMData* data, CMMSocket* sender){
	uint8_t byCommand = data->ReadByte();
	uint8_t byFileIndex = data->ReadByte();
...
void  CMMServer::ProcessSearchRequest(CMMData* data, CMMSocket* sender){
	...
	char buffer[500];
	int iBuffLen = _snprintf_s(buffer, _countof(buffer), _countof(buffer), "HTTP/1.1 200 OK\r\nConnection: Close\r\nContent-Type: %s\r\n", GetContentType());
	if (iBuffLen > 0)
		sender->Send(buffer,iBuffLen);
}

void  CMMServer::ProcessChangeLimitRequest(CMMData* data, CMMSocket* sender){
	uint16_t nNewUpload = data->ReadShort();
	uint16_t nNewDownload = data->ReadShort();
	thePrefs.SetMaxUpload(nNewUpload);
	thePrefs.SetMaxDownload(nNewDownload);

	CMMPacket* packet = new CMMPacket(MMP_CHANGELIMITANS);
	packet->WriteShort((uint16_t)((thePrefs.GetMaxUpload() >= UNLIMITED) ? 0 : thePrefs.GetMaxUpload()));
	packet->WriteShort((uint16_t)((thePrefs.GetMaxDownload() >= UNLIMITED) ? 0 : thePrefs.GetMaxDownload()));
	sender->SendPacket(packet);
}
...
void  CMMServer::ProcessPreviewRequest(CMMData* data, CMMSocket* sender){
	uint8_t byFileType = data->ReadByte();
	uint8_t byFileIndex = data->ReadByte();
	uint16_t nDisplayWidth = data->ReadShort();
	uint8_t nNumber = data->ReadByte();
...
void CMMServer::PreviewFinished(CxImage** imgFrames, uint8_t nCount){
	if (h_timer != 0){
		KillTimer(0,h_timer);
		h_timer = 0;
	}
...
void  CMMServer::ProcessStatisticsRequest(CMMData* data, CMMSocket* sender){
	uint16_t nWidth = data->ReadShort();
	CArray<UpDown>* rawData = theApp.webserver->GetPointsForWeb();
	int nRawDataSize = static_cast<int>(rawData->GetSize());
	...
	packet->WriteShort((uint16_t)((nRawDataSize-nPos)*thePrefs.GetTrafficOMeterInterval()));
	packet->WriteShort((uint16_t)(min(nWidth, nRawDataSize)));
	while (nPos < nRawDataSize){
		...
		packet->WriteInt((uint32_t)ROUND(nAddUp/i));
		packet->WriteInt((uint32_t)ROUND(nAddDown/i));
		packet->WriteShort((uint16_t)ROUND(nAddCon/i));
	}
...
void CMMServer::WriteFileInfo(CPartFile* selFile, CMMPacket* packet){
	...
	packet->WriteShort((uint16_t)(selFile->GetDatarate()/100));
	packet->WriteShort((uint16_t)(selFile->GetSourceCount()));
	packet->WriteShort((uint16_t)(selFile->GetTransferringSrcCount()));
	...
	uint8_t* parts = selFile->MMCreatePartStatus();
	packet->WriteShort((uint16_t)selFile->partCount());
	for (UINT i = 0; i < selFile->partCount(); i++){
