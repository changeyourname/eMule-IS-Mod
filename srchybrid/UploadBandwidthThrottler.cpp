...
UploadBandwidthThrottler::UploadBandwidthThrottler(void) {
	...
	doRun = true;
	CSafeThread::BeginThread(RunProc, (LPVOID)this);
}
 ...
 */
uint64_t UploadBandwidthThrottler::GetNumberOfSentBytesSinceLastCallAndReset() {
	sendLocker.Lock();

	uint64_t numberOfSentBytesSinceLastCall = m_SentBytesSinceLastCall;
	m_SentBytesSinceLastCall = 0;
 ...
 * @return the number of bytes that has been put on the sockets since the last call
 */
uint64_t UploadBandwidthThrottler::GetNumberOfSentBytesOverheadSinceLastCallAndReset() {
	sendLocker.Lock();

	uint64_t numberOfSentBytesSinceLastCall = m_SentBytesSinceLastCallOverhead;
	m_SentBytesSinceLastCallOverhead = 0;
 ...
 */
uint32_t UploadBandwidthThrottler::GetHighestNumberOfFullyActivatedSlotsSinceLastCallAndReset() {
    sendLocker.Lock();
    ...
    uint32_t highestNumberOfFullyActivatedSlots = m_highestNumberOfFullyActivatedSlots;
    m_highestNumberOfFullyActivatedSlots = 0;
 ...
 */
void UploadBandwidthThrottler::AddToStandardList(uint32_t index, ThrottledFileSocket* socket) {
	if(socket != NULL) {
		sendLocker.Lock();

		RemoveFromStandardListNoLock(socket);
		if(index > (uint32_t)m_StandardOrder_list.GetSize()) {
			index = (uint32_t) m_StandardOrder_list.GetSize();
        }
...
bool UploadBandwidthThrottler::RemoveFromStandardListNoLock(ThrottledFileSocket* socket) {
    ...
    if(foundSocket && m_highestNumberOfFullyActivatedSlots > (uint32_t)m_StandardOrder_list.GetSize()) {
        m_highestNumberOfFullyActivatedSlots = (uint32_t) m_StandardOrder_list.GetSize();
    }
...
uint32_t UploadBandwidthThrottler::GetSlotLimit(uint32_t currentUpSpeed) {
    uint32_t upPerClient = UPLOAD_CLIENT_DATARATE;
	...
	if (currentUpSpeed > 12*1024)
		nMaxSlots = (uint16_t)(((float)currentUpSpeed) / upPerClient);
    ...
    return max<uint32_t>(nMaxSlots, MIN_UP_CLIENTS_ALLOWED);
}

uint32_t UploadBandwidthThrottler::CalculateChangeDelta(uint32_t numberOfConsecutiveChanges) const {
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	int64_t realBytesToSpend = 0;
	uint32_t allowedDataRate = 0;
    uint32_t rememberedSlotCounter = 0;
    DWORD lastTickReachedBandwidth = lastLoopTick;

	uint32_t nEstiminatedLimit = 0;
	int nSlotsBusyLevel = 0;
	DWORD nUploadStartTime = 0;
    uint32_t numberOfConsecutiveUpChanges = 0;
    uint32_t numberOfConsecutiveDownChanges = 0;
	...
	while(doRun) {
		...
		/* ismod: no lastCommonRouteFinder, thePrefs.GetMaxUpload() min 100
		...
		*/
			    ...
			    if (theApp.uploadqueue){
				    if (nEstiminatedLimit == 0){ // no autolimit was set yet
                        ...
                        if (nSlotsBusyLevel > 250){
                            ...
                            uint32_t changeDelta = CalculateChangeDelta(numberOfConsecutiveDownChanges);
                            ...
                            uint32_t changeDelta = CalculateChangeDelta(numberOfConsecutiveUpChanges);
		...
		uint32_t minFragSize = 1300;
        uint32_t doubleSendSize = minFragSize*2; // send two packages at a time so they can share an ACK
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
        ...
        if(allowedDataRate == _UI32_MAX || realBytesToSpend >= 1000 || allowedDataRate == 0 && nEstiminatedLimit == 0) {
            ...
            sleepTime = max<uint32_t>((uint32_t)ceil(((double)doubleSendSize*1000)/nEstiminatedLimit), TIME_BETWEEN_UPLOAD_LOOPS);
        } else {
            // sleep for just as long as we need to get back to having one byte to send
            sleepTime = max<uint32_t>((uint32_t)ceil((double)(-realBytesToSpend + 1000)/allowedDataRate), TIME_BETWEEN_UPLOAD_LOOPS);
        ...
        if(bytesToSpend >= 1 || allowedDataRate == 0) {
		    uint64_t spentBytes = 0;
		    uint64_t spentOverhead = 0;
		    ...
		    while((bytesToSpend > 0 && spentBytes < (uint64_t)bytesToSpend || allowedDataRate == 0 && spentBytes < 500) && (!m_ControlQueueFirst_list.IsEmpty() || !m_ControlQueue_list.IsEmpty())) {
			    ...
			    if(socket != NULL) {
				    ...
				    uint32_t lastSpentBytes = socketSentBytes.sentBytesControlPackets + socketSentBytes.sentBytesStandardPackets;
		    ...
		    for(uint32_t slotCounter = 0; slotCounter < (uint32_t)m_StandardOrder_list.GetSize(); slotCounter++) {
			    ThrottledFileSocket* socket = m_StandardOrder_list.GetAt(slotCounter);
    
			    if(socket != NULL) {
				    if(thisLoopTick-socket->GetLastCalledSend() > SEC2MS(1)) {
					    // trickle
					    uint32_t neededBytes = socket->GetNeededBytes();
            ...
            uint32_t maxSlot = (uint32_t)m_StandardOrder_list.GetSize();
            ...
            for(uint32_t maxCounter = 0; maxCounter < min(maxSlot, (uint32_t)m_StandardOrder_list.GetSize()) && bytesToSpend > 0 && spentBytes < (uint64_t)bytesToSpend; maxCounter++) {
                if(rememberedSlotCounter >= (uint32_t)m_StandardOrder_list.GetSize() ||
				...
				if(socket != NULL) {
					...
					SocketSentBytes socketSentBytes = socket->SendFileAndControlData((uint32_t)min<uint32_t>(doubleSendSize, static_cast<uint32_t>(bytesToSpend-spentBytes)), doubleSendSize);
					uint32_t lastSpentBytes = socketSentBytes.sentBytesControlPackets + socketSentBytes.sentBytesStandardPackets;
			...
			for(uint32_t slotCounter = 0; slotCounter < (uint32_t)m_StandardOrder_list.GetSize() && bytesToSpend > 0 && spentBytes < (uint64_t)bytesToSpend; slotCounter++) {
				ThrottledFileSocket* socket = m_StandardOrder_list.GetAt(slotCounter);

				if(socket != NULL) {
                    uint32_t bytesToSpendTemp = (UINT)(bytesToSpend-spentBytes);
		    ...
		    if(realBytesToSpend < -(((int64_t)m_StandardOrder_list.GetSize()+1)*minFragSize)*1000) {
			    int64_t newRealBytesToSpend = -(((int64_t)m_StandardOrder_list.GetSize()+1)*minFragSize)*1000;
    
			    realBytesToSpend = newRealBytesToSpend;
                lastTickReachedBandwidth = thisLoopTick;
            } else {
                uint64_t bandwidthSavedTolerance = 0;
                if(realBytesToSpend > 0 && (uint64_t)realBytesToSpend > 999+bandwidthSavedTolerance) {
			        int64_t newRealBytesToSpend = 999+bandwidthSavedTolerance;
                    ...
                    if(thisLoopTick-lastTickReachedBandwidth > max<DWORD>(1000, timeSinceLastLoop*2)) {
                        m_highestNumberOfFullyActivatedSlots = (uint32_t) m_StandardOrder_list.GetSize()+1;
                        lastTickReachedBandwidth = thisLoopTick;
