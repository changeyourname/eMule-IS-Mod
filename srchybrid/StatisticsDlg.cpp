...
void CStatisticsDlg::SetAllIcons()
{
	...
	CImageList iml;
	iml.Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 1);
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate) 
{
	...
	{
		uint32_t	statGoodSessions =				0;
		uint32_t	statBadSessions =				0;
		...
		{
			uint64_t	DownOHTotal = 0;
			uint64_t	DownOHTotalPackets = 0;
			CDownloadQueue::SDownloadStats myStats;
			theApp.downloadqueue->GetDownloadSourcesStats(myStats);
			...
			if (forceUpdate || stattree.IsExpanded(h_down_session)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(down_S[0])) 
				{
					...
					{
						int i = 0;
						uint64_t DownDataTotal =		thePrefs.GetDownSessionClientData();
						uint64_t DownDataClient =		thePrefs.GetDownData_EMULE();
					...
					{
						int i = 0;
						uint64_t	PortDataDefault =	thePrefs.GetDownDataPort_4662();
						uint64_t	PortDataOther =		thePrefs.GetDownDataPort_OTHER();
						uint64_t	PortDataTotal =		thePrefs.GetDownSessionDataPort();
				...
				if (forceUpdate || stattree.IsExpanded(down_S[4])) 
				{
					...
					if (statGoodSessions > 0) 
					{
						...
						cbuffer.Format( _T("%s: %s") , GetResString(IDS_STATS_AVGDATADLSES) , CastItoXBytes((uint32_t)0, false, false) );
			...
			{
				// Downloaded Data
				uint64_t ullCumReceived = theStats.sessionReceivedBytes + thePrefs.GetTotalDownloaded();
				...
				if (forceUpdate || stattree.IsExpanded(down_T[0])) 
				{
					...
					{
						int i = 0;
						uint64_t DownDataTotal =		thePrefs.GetDownTotalClientData();
						uint64_t DownDataClient =		thePrefs.GetCumDownData_EMULE();
						double percentClientTransferred = 0;
						if ( DownDataTotal!=0 && DownDataClient!=0 )
							percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
						cbuffer.Format( _T("eMule: %s (%1.1f%%)") , CastItoXBytes( DownDataClient, false, false ) , percentClientTransferred);
					...
					{
						int i = 0;
						uint64_t	PortDataDefault =	thePrefs.GetCumDownDataPort_4662();
						uint64_t	PortDataOther =		thePrefs.GetCumDownDataPort_OTHER();
						uint64_t	PortDataTotal =		thePrefs.GetDownTotalPortData();
						double	percentPortTransferred = 0;
				...
				if (forceUpdate || stattree.IsExpanded(down_T[2])) 
				{
					...
					uint32_t avgDownTime = thePrefs.GetDownS_AvgTime();
					if (thePrefs.GetDownC_AvgTime()<=0) 
						thePrefs.SetDownCAvgTime(avgDownTime);
					avgDownTime = (uint32_t) (avgDownTime+thePrefs.GetDownC_AvgTime())/2;
				...
				uint64_t ullCumCompressed = thePrefs.GetSesSavedFromCompression() + thePrefs.GetCumSavedFromCompression();
				...
				uint64_t ullCumCorrupted = thePrefs.GetSesLostFromCorruption() + thePrefs.GetCumLostFromCorruption();
		...
		{
			uint64_t UpOHTotal =			0;
			uint64_t UpOHTotalPackets =	0;
			...
			if (forceUpdate || stattree.IsExpanded(h_up_session)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(up_S[0])) 
				{
					...
					{
						int i = 0;
						uint64_t UpDataTotal =		thePrefs.GetUpSessionClientData();
						uint64_t UpDataClient =		thePrefs.GetUpData_EMULE();
						double percentClientTransferred = 0;
					...
					{
						int i = 0;
						uint64_t	PortDataDefault =	thePrefs.GetUpDataPort_4662();
						uint64_t	PortDataOther =		thePrefs.GetUpDataPort_OTHER();
						uint64_t	PortDataTotal =		thePrefs.GetUpSessionPortData();
						double	percentPortTransferred = 0;
					...
					{
						int i = 0;
						uint64_t	DataSourceFile =	thePrefs.GetUpData_File();
						uint64_t	DataSourcePF =		thePrefs.GetUpData_Partfile();
						uint64_t	DataSourceTotal =		thePrefs.GetUpSessionDataFile();
			...
			if (forceUpdate || stattree.IsExpanded(h_up_total)) 
			{
				...
				if (forceUpdate || stattree.IsExpanded(up_T[0])) 
				{
					...
					{
						int i = 0;
						uint64_t UpDataTotal =		thePrefs.GetUpTotalClientData();
						uint64_t UpDataClient =		thePrefs.GetCumUpData_EMULE();
						double percentClientTransferred = 0;
						if ( UpDataTotal!=0 && UpDataClient!=0 )
							percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
						cbuffer.Format( _T("eMule: %s (%1.1f%%)") , CastItoXBytes( UpDataClient, false, false ) , percentClientTransferred);
					...
					{
						int i = 0;
						uint64_t	PortDataDefault =	thePrefs.GetCumUpDataPort_4662();
						uint64_t	PortDataOther =		thePrefs.GetCumUpDataPort_OTHER();
						uint64_t	PortDataTotal =		thePrefs.GetUpTotalPortData();
						double	percentPortTransferred = 0;
					...
					{
						int i = 0;
						uint64_t	DataSourceFile =	thePrefs.GetCumUpData_File();
						uint64_t	DataSourcePF =		thePrefs.GetCumUpData_Partfile();
						uint64_t	DataSourceTotal =	thePrefs.GetUpTotalDataFile();
						double	percentFileTransferred = 0;
				...
				if (forceUpdate || stattree.IsExpanded(up_T[1])) 
				{
					...
					// Set Avg Upload time
					uint32_t avguptime = theApp.uploadqueue->GetAverageUpTime();
					if (thePrefs.GetUpAvgTime()<=0) 
						thePrefs.SetUpAvgTime(avguptime);
					avguptime = (uint32_t) (avguptime+thePrefs.GetUpAvgTime())/2;
				...
				if (forceUpdate || stattree.IsExpanded(time_aaph[mx])) 
				{
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hup[mx])) 
					{
						...
						if (forceUpdate || stattree.IsExpanded(time_aap_up[mx][0])) 
						{
							...
							{
								int i = 0;
								uint64_t UpDataTotal = (uint64_t)(thePrefs.GetUpTotalClientData() * avgModifier[mx]);
								uint64_t UpDataClient = (uint64_t)(thePrefs.GetCumUpData_EMULE() * avgModifier[mx]);
								double percentClientTransferred = 0;
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								cbuffer.Format( _T("eMule: %s (%1.1f%%)") , CastItoXBytes( UpDataClient, false, false ) , percentClientTransferred);
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_EDONKEYHYBRID() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_EDONKEY() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_AMULE() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_MLDONKEY() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_SHAREAZA() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
								...
								UpDataClient = (uint64_t)(thePrefs.GetCumUpData_EMULECOMPAT() * avgModifier[mx]);
								if ( UpDataTotal!=0 && UpDataClient!=0 )
									percentClientTransferred = (double) 100 * UpDataClient / UpDataTotal;
								else
									percentClientTransferred = 0;
							...
							{
								int i = 0;
								uint64_t	PortDataDefault =	(uint64_t)(thePrefs.GetCumUpDataPort_4662() * avgModifier[mx]);
								uint64_t	PortDataOther =		(uint64_t)(thePrefs.GetCumUpDataPort_OTHER() * avgModifier[mx]);
								uint64_t	PortDataTotal =		(uint64_t)( thePrefs.GetUpTotalPortData() * avgModifier[mx]);
								double	percentPortTransferred = 0;
							...
							{
								int i = 0;
								uint64_t	DataSourceFile =	(uint64_t)(thePrefs.GetCumUpData_File() * avgModifier[mx]);
								uint64_t	DataSourcePF =		(uint64_t)(thePrefs.GetCumUpData_Partfile() * avgModifier[mx]);
								uint64_t	DataSourceTotal =	(uint64_t)(thePrefs.GetUpTotalDataFile() * avgModifier[mx]);
								double	percentFileTransferred = 0;
						...
						// Upload Sessions
						uint32_t statGoodSessions = (uint32_t)((theApp.uploadqueue->GetSuccessfullUpCount() + thePrefs.GetUpSuccessfulSessions() + theApp.uploadqueue->GetUploadQueueLength()) * avgModifier[mx]);
						uint32_t statBadSessions = (uint32_t)((theApp.uploadqueue->GetFailedUpCount() + thePrefs.GetUpFailedSessions()) * avgModifier[mx]);
						...
						// Calculate Upline OH Totals
						uint64_t UpOHTotal = (uint64_t)((theStats.GetUpDataOverheadFileRequest() + 
													 theStats.GetUpDataOverheadSourceExchange() + 
													 theStats.GetUpDataOverheadServer() + 
													 theStats.GetUpDataOverheadKad() + 
													 theStats.GetUpDataOverheadOther()
												    ) * avgModifier[mx]);
						uint64_t UpOHTotalPackets = (uint64_t)((theStats.GetUpDataOverheadFileRequestPackets() + 
														    theStats.GetUpDataOverheadSourceExchangePackets() + 
														    theStats.GetUpDataOverheadServerPackets() + 
														    theStats.GetUpDataOverheadKadPackets() + 
														    theStats.GetUpDataOverheadOtherPackets()
						...
						// Set Cumulative Total Overhead
						cbuffer.Format(GetResString(IDS_TOVERHEAD),CastItoXBytes(UpOHTotal + ((uint64_t)thePrefs.GetUpOverheadTotal() * avgModifier[mx]), false, false), CastItoIShort((uint64_t)(UpOHTotalPackets + ((uint64_t)thePrefs.GetUpOverheadTotalPackets() * avgModifier[mx]))));
						stattree.SetItemText(time_aap_up[mx][2], cbuffer);
						if (forceUpdate || stattree.IsExpanded(time_aap_up[mx][2])) 
						{
							int i = 0;
							// Set up total file req OH
							cbuffer.Format(GetResString(IDS_FROVERHEAD), CastItoXBytes( (uint64_t)(theStats.GetUpDataOverheadFileRequest() + thePrefs.GetUpOverheadFileReq()) * avgModifier[mx], false, false), CastItoIShort( (uint64_t)(theStats.GetUpDataOverheadFileRequestPackets() + thePrefs.GetUpOverheadFileReqPackets()) * avgModifier[mx]));
							stattree.SetItemText(time_aap_up_oh[mx][i], cbuffer);
							i++;
							// Set up total source exch OH
							cbuffer.Format(GetResString(IDS_SSOVERHEAD), CastItoXBytes( (uint64_t)(theStats.GetUpDataOverheadSourceExchange()+thePrefs.GetUpOverheadSrcEx()) * avgModifier[mx], false, false), CastItoIShort( (uint64_t)(theStats.GetUpDataOverheadSourceExchangePackets()+thePrefs.GetUpOverheadSrcExPackets()) * avgModifier[mx]));
							stattree.SetItemText(time_aap_up_oh[mx][i], cbuffer);
							i++;
							// Set up total server OH
							cbuffer.Format(GetResString(IDS_SOVERHEAD), 
										   ...
										   CastItoIShort((uint64_t)(theStats.GetUpDataOverheadServerPackets() + 
																  thePrefs.GetUpOverheadServerPackets()
																 ) * avgModifier[mx]));
							stattree.SetItemText(time_aap_up_oh[mx][i], cbuffer);
							i++;
							// Set up total Kad OH
							cbuffer.Format(GetResString(IDS_KADOVERHEAD), 
										   CastItoXBytes((uint64_t)(theStats.GetUpDataOverheadKad() + 
																  thePrefs.GetUpOverheadKad()
																 ) * avgModifier[mx], false, false), 
										   CastItoIShort((uint64_t)(theStats.GetUpDataOverheadKadPackets() + 
																  thePrefs.GetUpOverheadKadPackets()
																 ) * avgModifier[mx]));
							stattree.SetItemText(time_aap_up_oh[mx][i], cbuffer);
							i++;
					...
					if (forceUpdate || stattree.IsExpanded(time_aap_hdown[mx])) 
					{
						...
						theApp.downloadqueue->GetDownloadSourcesStats(myStats);
						// Downloaded Data
						cbuffer.Format(GetResString(IDS_STATS_DDATA),CastItoXBytes( (uint64_t)(theStats.sessionReceivedBytes+thePrefs.GetTotalDownloaded()) * avgModifier[mx], false, false ));
						...
						if (forceUpdate || stattree.IsExpanded(time_aap_down[mx][0])) 
						{
							...
							{
								int i = 0;
								uint64_t DownDataTotal = (uint64_t)(thePrefs.GetDownTotalClientData() * avgModifier[mx]);
								uint64_t DownDataClient = (uint64_t)(thePrefs.GetCumDownData_EMULE() * avgModifier[mx]);
								double percentClientTransferred = 0;
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								cbuffer.Format( _T("eMule: %s (%1.1f%%)") , CastItoXBytes( DownDataClient, false, false ) , percentClientTransferred);
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_EDONKEYHYBRID() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_EDONKEY() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_AMULE() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_MLDONKEY() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_SHAREAZA() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_EMULECOMPAT() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
								...
								DownDataClient = (uint64_t)(thePrefs.GetCumDownData_URL() * avgModifier[mx]);
								if ( DownDataTotal!=0 && DownDataClient!=0 )
									percentClientTransferred = (double) 100 * DownDataClient / DownDataTotal;
								else
									percentClientTransferred = 0;
							...
							{
								int i = 0;
								uint64_t	PortDataDefault =	(uint64_t)(thePrefs.GetCumDownDataPort_4662() * avgModifier[mx]);
								uint64_t	PortDataOther =		(uint64_t)(thePrefs.GetCumDownDataPort_OTHER() * avgModifier[mx]);
								uint64_t	PortDataTotal =		(uint64_t)(thePrefs.GetDownTotalPortData() * avgModifier[mx]);
								double	percentPortTransferred = 0;
						...
						// Set Cum Completed Downloads
						cbuffer.Format(_T("%s: %I64u"), GetResString(IDS_STATS_COMPDL), (uint64_t) (thePrefs.GetDownCompletedFiles() * avgModifier[mx]) );
						stattree.SetItemText(time_aap_down[mx][1], cbuffer);
						// Set Cum Download Sessions
						uint32_t	statGoodSessions = (uint32_t)((thePrefs.GetDownC_SuccessfulSessions() + myStats.a[1]) * avgModifier[mx]);
						uint32_t	statBadSessions = (uint32_t)(thePrefs.GetDownC_FailedSessions() * avgModifier[mx]);
						double	percentSessions;
						cbuffer.Format(_T("%s: %u"), GetResString(IDS_STATS_DLSES), statGoodSessions+statBadSessions );
						...
						// Set Cumulative Gained Due To Compression
						cbuffer.Format( GetResString( IDS_STATS_GAINCOMP ) , CastItoXBytes( (uint64_t)(thePrefs.GetSesSavedFromCompression()+ thePrefs.GetCumSavedFromCompression()) * avgModifier[mx], false, false ) );
						stattree.SetItemText( time_aap_down[mx][3] , cbuffer );
						// Set Cumulative Lost Due To Corruption
						cbuffer.Format( GetResString( IDS_STATS_LOSTCORRUPT ) , CastItoXBytes( (uint64_t)(thePrefs.GetSesLostFromCorruption() + thePrefs.GetCumLostFromCorruption()) * avgModifier[mx], false, false ) );
						stattree.SetItemText( time_aap_down[mx][4] , cbuffer );
						// Set Cumulative Saved Due To ICH
						cbuffer.Format(GetResString(IDS_STATS_ICHSAVED), (uint32_t)((thePrefs.GetSesPartsSavedByICH() + thePrefs.GetCumPartsSavedByICH()) * avgModifier[mx]));
						stattree.SetItemText( time_aap_down[mx][5] , cbuffer );

						uint64_t DownOHTotal =	theStats.GetDownDataOverheadFileRequest() + 
												theStats.GetDownDataOverheadSourceExchange() + 
												theStats.GetDownDataOverheadServer() + 
												theStats.GetDownDataOverheadKad() + 
												theStats.GetDownDataOverheadOther();
						uint64_t DownOHTotalPackets = 
												theStats.GetDownDataOverheadFileRequestPackets() + 
												theStats.GetDownDataOverheadSourceExchangePackets() + 
												theStats.GetDownDataOverheadServerPackets() + 
												theStats.GetDownDataOverheadKadPackets() + 
												theStats.GetDownDataOverheadOtherPackets();
						// Total Overhead
						cbuffer.Format(GetResString(IDS_TOVERHEAD),CastItoXBytes( (uint64_t)(DownOHTotal + thePrefs.GetDownOverheadTotal()) * avgModifier[mx], false, false), CastItoIShort((uint64_t)(DownOHTotalPackets + thePrefs.GetDownOverheadTotalPackets()) * avgModifier[mx]));
						stattree.SetItemText(time_aap_down[mx][6], cbuffer);
						if (forceUpdate || stattree.IsExpanded(time_aap_down[mx][6])) 
						{
							int i = 0;
							// File Request Overhead
							cbuffer.Format(GetResString(IDS_FROVERHEAD), CastItoXBytes( (uint64_t)(theStats.GetDownDataOverheadFileRequest() + thePrefs.GetDownOverheadFileReq()) * avgModifier[mx], false, false), CastItoIShort((uint64_t)(theStats.GetDownDataOverheadFileRequestPackets() + thePrefs.GetDownOverheadFileReqPackets()) * avgModifier[mx]));
							stattree.SetItemText(time_aap_down_oh[mx][i], cbuffer);
							i++;
							// Source Exchange Overhead
							cbuffer.Format(GetResString(IDS_SSOVERHEAD), CastItoXBytes( (uint64_t)(theStats.GetDownDataOverheadSourceExchange()+thePrefs.GetDownOverheadSrcEx()) * avgModifier[mx], false, false), CastItoIShort((uint64_t)(theStats.GetDownDataOverheadSourceExchangePackets()+thePrefs.GetDownOverheadSrcExPackets()) * avgModifier[mx]));
							stattree.SetItemText(time_aap_down_oh[mx][i], cbuffer);
							i++;
							// Server Overhead
							cbuffer.Format(GetResString(IDS_SOVERHEAD), 
										   CastItoXBytes((uint64_t)(theStats.GetDownDataOverheadServer() +
													     thePrefs.GetDownOverheadServer()) * avgModifier[mx], false, false), 
										   CastItoIShort((uint64_t)(theStats.GetDownDataOverheadServerPackets() + 
														 thePrefs.GetDownOverheadServerPackets()) * avgModifier[mx]));
							stattree.SetItemText(time_aap_down_oh[mx][i], cbuffer);
							i++;
							// Kad Overhead
							cbuffer.Format(GetResString(IDS_KADOVERHEAD), 
										   CastItoXBytes((uint64_t)(theStats.GetDownDataOverheadKad() +
													              thePrefs.GetDownOverheadKad()
																 ) * avgModifier[mx], false, false), 
										   CastItoIShort((uint64_t)(theStats.GetDownDataOverheadKadPackets() + 
														          thePrefs.GetDownOverheadKadPackets()
																 ) * avgModifier[mx]));
							stattree.SetItemText(time_aap_down_oh[mx][i], cbuffer);
							i++;
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				uint32_t currtopcnt = 0;
				uint32_t currtopver = 0;
				uint32_t totalOther = 0;
				for (uint32_t i = 0; i < MAX_SUB_CLIENT_VERSIONS; i++)
				{
				...
				{
					for (uint32_t i = 0; i < cli_lastCount[0] - verCount; i++) 
					{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[1]) || cli_lastCount[1] == 0) 
			{
				...
				{
					for (uint32_t i = 0; i < cli_lastCount[1] - verCount; i++) 
					{
			...
			{				
				...
				{
					for (uint32_t i = 0; i < cli_lastCount[2] - verCount; i++) 
					{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[3]) || cli_lastCount[3] == 0) 
			{
				...
				{
					for (uint32_t i = 0; i < cli_lastCount[3] - verCount; i++) 
					{
	...
	{		
		...
		uint64_t allsize=theApp.sharedfiles->GetDatasize(bytesLargestFile); // Func returns total share size and sets pointeredd uint64 to largest single filesize
		CString cbuffer2;
		if(theApp.sharedfiles->GetCount() != 0)
			cbuffer2.Format( _T("%s"), CastItoXBytes(allsize/(uint64_t)theApp.sharedfiles->GetCount(), false, false));
		else 
			cbuffer2.Format( _T("%s"), CastItoXBytes((uint32_t)0, false, false) );
	...
	{			
		uint64_t ui64TotalFileSize = 0;
		uint64_t ui64TotalLeftToTransfer = 0;
		uint64_t ui64TotalAdditionalNeededSpace = 0;
		...
		uint64_t ui64TotalTransferred = ui64TotalFileSize - ui64TotalLeftToTransfer;
		double fPercent = 0.0;
		...
		CString buffer2;
		uint64_t ui64TotalFreeSpace = GetFreeTempSpace(-1);
	...
	if (g_pfnPrevCrtAllocHook)
	{
		...
		extern CRBMap<const unsigned char*, UINT> g_allocations;
		...
		pos = g_allocations.GetHeadPosition();
		while (pos != NULL) 
		{
			const unsigned char* pszFileName;
			UINT count;
