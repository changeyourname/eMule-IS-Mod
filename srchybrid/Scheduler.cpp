	...
	while (count<max) {
		...
		if (temp!=_T("")) {
			...
			ini.SerGet(true, news->actions,
				_countof(news->actions), _T("Actions"));
			ini.SerGet(true, news->values,
				_countof(news->values), _T("Values"));
...
void CScheduler::SaveToFile(){
	...
	for (uint8_t i=0; i<GetCount();i++) {
		...
		ini.WriteInt(_T("StartTime"),ConvertToTime32(schedule->time));
		ini.WriteInt(_T("EndTime"),ConvertToTime32(schedule->time2));
		ini.WriteBool(_T("Enabled"),schedule->enabled);

		ini.SerGet(false, schedule->actions,
			_countof(schedule->actions), _T("Actions"));
		ini.SerGet(false, schedule->values,
			_countof(schedule->values), _T("Values"));
	}
}
...
int CScheduler::Check(bool forcecheck){
	...
	for (uint8_t si=0;si<theApp.scheduler->GetCount();si++) {
	...
	for (int ai=0;ai<16;ai++) {
		...
		switch (schedule->actions[ai]) {
			case 1 :
				...
				if (makedefault)
					original_upload=(uint16_t)_tstoi(schedule->values[ai]); 
				break;
			case 2 :
				thePrefs.SetMaxDownload(_tstoi(schedule->values[ai]));
				if (makedefault)
					original_download=(uint16_t)_tstoi(schedule->values[ai]);
				break;
			case 3 :
