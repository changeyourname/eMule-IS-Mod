...
Packet::Packet(uint8_t protocol){
	m_bSplitted = false;
...
Packet::Packet(char* pPacketPart, uint32_t nSize, bool bLast, bool bFromPartFile){// only used for splitted packets!
	m_bFromPF = bFromPartFile;
	m_bSplitted = true;
	m_bPacked = false;
	m_bLastSplitted = bLast;
	tempbuffer = 0;
	pBuffer = pPacketPart+6; // NEO: FIX - [CriticalBugFix] <-- Xanatos --
...
Packet::Packet(uint8_t in_opcode, uint32_t in_size, uint8_t protocol, bool bFromPartFile){
	m_bFromPF = bFromPartFile;
	m_bSplitted = false;
	m_bPacked = false;
	m_bLastSplitted = false;
...
Packet::Packet(CMemFile* datafile, uint8_t protocol, uint8_t ucOpcode){
	m_bSplitted = false;
	m_bPacked = false;
	m_bLastSplitted = false;
	m_bFromPF = false;
...
Packet::Packet(const CStringA& str, uint8_t ucProtocol, uint8_t ucOpcode){
	m_bSplitted = false;
	m_bPacked = false;
	m_bLastSplitted = false;
	m_bFromPF = false;
...
// NEO: FIX - [CriticalBugFix] -- Xanatos -->
...
void Packet::PackPacket(){
	ASSERT (!m_bSplitted);
	// netfinity: Make sure packet isn't already compressed!
	if (m_bPacked)
		return;
...
CTag::CTag(LPCSTR pszName, uint64_t uVal, bool bInt64)
{
...
CTag::CTag(uint8_t uName, uint64_t uVal, bool bInt64)
{
	ASSERT( uVal <= 0xFFFFFFFF || bInt64 );
	if (bInt64){
		m_uType = TAGTYPE_UINT64;
...
CTag::CTag(uint8_t uName, LPCTSTR pszVal)
{
	m_uType = TAGTYPE_STRING;
	m_uName = uName;
	m_pszName = NULL;
...
CTag::CTag(uint8_t uName, const CString& rstrVal)
{
	m_uType = TAGTYPE_STRING;
	m_uName = uName;
	m_pszName = NULL;
...
CTag::CTag(uint8_t uName, const BYTE* pucHash)
{
...
CTag::CTag(uint8_t uName, uint32_t nSize, const BYTE* pucData){
...
bool CTag::WriteNewEd2kTag(CFileDataIO* data, EUtf8Str eStrEncode) const
{
	...
	else if (IsStr())
	{
		...
		if (uStrValLen >= 1 && uStrValLen <= 16)
			uType = (uint8_t)(TAGTYPE_STR1 + uStrValLen - 1);
		else
	...
	if (m_pszName)
	{
		...
		data->WriteUInt16((uint16_t)uTagNameLen);
	...
	{
		data->WriteUInt16((uint16_t)uStrValLen);
	...
	{
		data->WriteUInt32((uint32_t)m_uVal);
	}
	else if (uType == TAGTYPE_UINT16)
	{
		data->WriteUInt16((uint16_t)m_uVal);
	}
	else if (uType == TAGTYPE_UINT8)
	{
		data->WriteUInt8((uint8_t)m_uVal);
	}
...
bool CTag::WriteTagToFile(CFileDataIO* file, EUtf8Str eStrEncode) const
{
	...
	if (IsStr() || IsInt() || IsFloat() || IsBlob() || IsInt64())
	{
		...
		if (m_pszName)
		{
			UINT taglen = static_cast<UINT>(strlen(m_pszName));
			file->WriteUInt16(static_cast<uint16_t>(taglen));
		...
		{
			file->WriteUInt32((uint32_t)m_uVal);
		}
		else if (IsInt64(false))
		{
...
void CTag::SetInt(uint32_t uVal)
{
	ASSERT( IsInt() );
	if (IsInt())
		m_uVal = uVal;
...
void CTag::SetInt64(uint64_t uVal)
{
	ASSERT( IsInt64(true) );
	if (IsInt64(true)){
		m_uVal = uVal;
...
CString CTag::GetFullInfo(CString (*pfnDbgGetFileMetaTagName)(UINT uMetaTagID)) const
{
	...
	{
		strTag.AppendFormat(_T("(Int32)%u"), (uint32_t)m_uVal);
	}
	else if (m_uType == TAGTYPE_UINT64)
	{
