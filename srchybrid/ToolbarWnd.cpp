...
void CToolbarWnd::FillToolbar()
{
	...
	// ismod: moved to first
	atb1[0].iBitmap = 6;
	atb1[0].idCommand = MP_PREVIEW;
	atb1[0].fsState = TBSTATE_WRAP;
	atb1[0].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[0].iString = m_btnBar->AddString(GetResString(IDS_DL_PREVIEW));

	atb1[1].iBitmap = 0;
	atb1[1].idCommand = MP_PRIOLOW;
	atb1[1].fsState = TBSTATE_WRAP;
	atb1[1].fsStyle = BTNS_DROPDOWN  | BTNS_AUTOSIZE;
	atb1[1].iString = m_btnBar->AddString(GetResString(IDS_PRIORITY) + _T(" (") + GetResString(IDS_DOWNLOAD) + _T(")"));

	atb1[2].iBitmap = 1;
	atb1[2].idCommand = MP_PAUSE;
	atb1[2].fsState = TBSTATE_WRAP;
	atb1[2].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[2].iString = m_btnBar->AddString(GetResString(IDS_DL_PAUSE));

	atb1[3].iBitmap = 2;
	atb1[3].idCommand = MP_STOP;
	atb1[3].fsState = TBSTATE_WRAP;
	atb1[3].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[3].iString = m_btnBar->AddString(GetResString(IDS_DL_STOP));

	atb1[4].iBitmap = 3;
	atb1[4].idCommand = MP_RESUME;
	atb1[4].fsState = TBSTATE_WRAP;
	atb1[4].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[4].iString = m_btnBar->AddString(GetResString(IDS_DL_RESUME));

	atb1[5].iBitmap = 4;
	atb1[5].idCommand = MP_CANCEL;
	atb1[5].fsState = TBSTATE_WRAP;
	atb1[5].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[5].iString = m_btnBar->AddString(GetResString(IDS_MAIN_BTN_CANCEL));
	/////////////
	atb1[6].iBitmap = -1;
	atb1[6].idCommand = 0;
	atb1[6].fsState = TBSTATE_WRAP;
	atb1[6].fsStyle = BTNS_SEP;
	atb1[6].iString = -1;

	atb1[7].iBitmap = 5;
	atb1[7].idCommand = MP_OPEN;
	atb1[7].fsState = TBSTATE_WRAP;
	atb1[7].fsStyle = BTNS_BUTTON  | BTNS_AUTOSIZE;
	atb1[7].iString = m_btnBar->AddString(GetResString(IDS_DL_OPEN));
	...
	// ismod [start]
#define BTN_AUTOSHUTDOWN 17
	atb1[17].iBitmap = 14;
	atb1[17].idCommand = MP_AUTOSHUTDOWN;
	atb1[17].fsState = TBSTATE_ENABLED | TBSTATE_WRAP;
	atb1[17].fsStyle = BTNS_CHECK | BTNS_AUTOSIZE;
	atb1[17].iString = m_btnBar->AddString(GetResString(IDS_AUTOSHUTDOWN));
	// ismod [end]
...
LRESULT CToolbarWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	...
	iml.Add(CTempIconLoader(_T("AutoShutdown_ON")));
	/*iml.Add(CTempIconLoader(_T("AutoShutdown_ON")));*/
...
void CToolbarWnd::OnUpdateCmdUI(CFrameWnd* /*pTarget*/, BOOL /*bDisableIfNoHndler*/)
{
	...
	bool bUpdate = true;
	if (m_pCommandTargetWnd != NULL && theApp.IsRunning() )
...
BOOL CToolbarWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	} else if ( LOWORD(wParam) == MP_AUTOSHUTDOWN ) {
		TBBUTTONINFO tbbi = {0};
		tbbi.cbSize = sizeof(tbbi);
		tbbi.dwMask = TBIF_BYINDEX | TBIF_STATE;
		m_btnBar->GetButtonInfo(BTN_AUTOSHUTDOWN, &tbbi);
		thePrefs.SetAutoShutdownWhenDone((tbbi.fsState & TBSTATE_CHECKED) != 0);
	}
