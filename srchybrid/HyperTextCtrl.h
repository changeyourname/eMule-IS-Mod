...
public:
	CHyperLink(); // i_a 
	CHyperLink(int iBegin, uint16_t iEnd, const CString& sTitle, const CString& sCommand, const CString& sDirectory);
	CHyperLink(int iBegin, uint16_t iEnd, const CString& sTitle, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
	...
	void SetBegin( uint16_t m_iInBegin )				{m_iBegin = m_iInBegin;}
	void SetEnd( uint16_t m_iInEnd )					{m_iEnd = m_iInEnd;}
...
public:
	CKeyWord();
	CKeyWord(int iBegin, uint16_t iEnd, COLORREF icolor);
...
public:
	...
	CLinePartInfo(int iBegin, uint16_t iEnd, CHyperLink* pHyperLink = NULL, CKeyWord* pKeyWord = NULL);
	CLinePartInfo(const CLinePartInfo& Src);
	uint16_t Begin()							{return m_xBegin;}
	uint16_t End()							{return m_xEnd;}
	uint16_t Len()							{return ((m_xEnd - m_xBegin) + 1);}
};
...
public:
	...
	CLineInfo(int iBegin, uint16_t iEnd);
...
public:
	...
	CVisPart(const CLinePartInfo& LinePartInfo, const CRect& rcBounds, 
		int iRealBegin, uint16_t iRealLen,CVisPart* pPrev,CVisPart* pNext);
