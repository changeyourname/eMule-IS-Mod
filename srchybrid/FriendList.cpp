...
bool CFriendList::LoadList(){
	...
	try {
		...
		for (UINT i = 0; i < nRecordsNumber; i++) {
			...
			// ismod: restore firend slot
			if ( !md4cmp(Record->m_abyUserhash, thePrefs.GetFriendSlotUserHash()) )
				Record->SetFriendSlot(true);
		}
		file.Close();
	}
...
void CFriendList::SaveList(){
	...
	try{
		...
		file.WriteUInt32((uint32_t) m_listFriends.GetCount());
		for (POSITION pos = m_listFriends.GetHeadPosition();pos != 0;)
...
CFriend* CFriendList::SearchFriend(const uchar* abyUserHash, uint32_t dwIP, uint16_t nPort) const {
...
bool CFriendList::AddFriend(const uchar* abyUserhash, uint32_t dwLastSeen, uint32_t dwLastUsedIP, uint16_t nLastUsedPort, 
...
void CFriendList::RemoveFriend(CFriend* todel){
	...
	/* ismod: moved to m_wndOutput->RemoveFriend(todel)
    todel->SetLinkedClient(NULL);
	*/

	if (m_wndOutput)
		/* ismod: ask before dele friend
		m_wndOutput->RemoveFriend(todel);
		*/
		if(!m_wndOutput->RemoveFriend(todel)) return;
