...
public:
	...
	bool			IsLargeFile() const					{ return m_nFileSize > (uint64_t)OLD_MAX_EMULE_FILE_SIZE; }

	uint32_t GetIntTagValue(uint8_t tagname) const;
	uint32_t GetIntTagValue(LPCSTR tagname) const;
	bool GetIntTagValue(uint8_t tagname, uint32_t& ruValue) const;
	uint64_t GetInt64TagValue(uint8_t tagname) const;
	uint64_t GetInt64TagValue(LPCSTR tagname) const;
	bool GetInt64TagValue(uint8_t tagname, uint64_t& ruValue) const;
	void SetIntTagValue(uint8_t tagname, uint32_t uValue);
	void SetInt64TagValue(uint8_t tagname, uint64_t uValue);
	const CString& GetStrTagValue(uint8_t tagname) const;
	const CString& GetStrTagValue(LPCSTR tagname) const;
	void SetStrTagValue(uint8_t tagname, LPCTSTR);
	CTag* GetTag(uint8_t tagname, uint8_t tagtype) const;
	CTag* GetTag(LPCSTR tagname, uint8_t tagtype) const;
	CTag* GetTag(uint8_t tagname) const;
	CTag* GetTag(LPCSTR tagname) const;
	const CArray<CTag*, CTag*>& GetTags() const { return taglist; }
	void AddTagUnique(CTag* pTag);
	void DeleteTag(uint8_t tagname);
