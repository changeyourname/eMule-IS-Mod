...
CCaptchaGenerator::CCaptchaGenerator(uint32_t nLetterCount)
{
	m_pimgCaptcha = NULL;
...
void CCaptchaGenerator::ReGenerateCaptcha(uint32_t nLetterCount)
{
	Clear();
	...
	imgBlank.Clear();
	for (uint32_t i = 0; i < nLetterCount; i++) {
		CxImage imgLetter(imgBlank);
		
		CString strLetter(schCaptchaContent[GetRandomUInt16() % _countof(schCaptchaContent)]);
		m_strCaptchaText += strLetter;
		
		uint16_t nRandomSize = GetRandomUInt16() % 10;
		uint16_t nRandomOffset = 3 + GetRandomUInt16() % 11;
		...
		uint32_t nOffset = i * CROWDEDSIZE; 
		ASSERT( imgLetter.GetHeight() == pimgResult->GetHeight() && pimgResult->GetWidth() >= nOffset + imgLetter.GetWidth() );
		for (uint32_t j = 0; j < imgLetter.GetHeight(); j++)
			for (uint32_t k = 0; k < imgLetter.GetWidth(); k++)
