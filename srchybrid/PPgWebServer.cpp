...
BOOL CPPgWebServer::OnInitDialog()
{
	...
	SetButtonIcon(::GetDlgItem(m_hWnd, IDC_TMPLBROWSE), m_icoBrowse);
...
{	
	if(m_bModified)
	{
		...
		uint16_t oldPort=thePrefs.GetWSPort();

		GetDlgItem(IDC_WSPASS)->GetWindowText(sBuf);
		if(sBuf != HIDDEN_PASSWORD)
		// ismod: min pass len [start]
		if (sBuf.GetLength() >= 8)
			thePrefs.SetWSPass(sBuf);
		else
		{
			AfxMessageBox(_T("Минимальная длина пароля 8 символов"), MB_OK);
			return FALSE;
		}
		// ismod [end]
		...
		if (_tstoi(sBuf)!=oldPort) {
			thePrefs.SetWSPort((uint16_t)_tstoi(sBuf));
		...
		if (_tstoi(sBuf)!= thePrefs.GetMMPort() ) {
			thePrefs.SetMMPort((uint16_t)_tstoi(sBuf));
