...
// CHyperLink
CHyperLink::CHyperLink(int iBegin, uint16_t iEnd, const CString& sTitle, const CString& sCommand, const CString& sDirectory){
	m_Type = lt_Shell;
	m_iBegin = iBegin;
	m_iEnd = iEnd;
	m_sTitle = sTitle;
...
CHyperLink::CHyperLink(int iBegin, uint16_t iEnd, const CString& sTitle, HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
	m_Type = lt_Message;
	m_iBegin = iBegin;
	m_iEnd = iEnd;
	m_sTitle = sTitle;
...
// CKeyWord
CKeyWord::CKeyWord(int iBegin, uint16_t iEnd, COLORREF icolor){
	color = icolor;
	m_iBegin = iBegin;
	m_iEnd = iEnd;
}
...
//CLinePartInfo
 ...
 CLinePartInfo::CLinePartInfo(int iBegin, uint16_t iEnd, CHyperLink* pHyperLink, CKeyWord* pKeyWord){
	m_xBegin = (uint16_t)iBegin;
	m_xEnd = iEnd;
...
//CLineInfo
 ...
 CLineInfo::CLineInfo(int iBegin, uint16_t iEnd){
	m_iBegin = iBegin;
	m_iEnd = iEnd;
}
...
//CVisPart
 CVisPart::CVisPart(const CLinePartInfo& LinePartInfo, const CRect& rcBounds, int iRealBegin, uint16_t iRealLen,CVisPart* pPrev,CVisPart* pNext) : CLinePartInfo(LinePartInfo)
{
	m_rcBounds = rcBounds;
			...
			for (int i = pl.Begin(); i < pl.End(); i++){
				if (it != m_Text->GetLinks().end() && i >= it->Begin() && it->End() > i){ // i_a 
					...
					if (it->End() > pl.End()){
						...
						CLinePartInfo pln((uint16_t)i, (uint16_t)it->End(), &*it);
						li.push_back(pln);
						i = pl.m_xBegin;
						++it;
					}
					...
					if (ht->End() > pl.End()){
						...
						CLinePartInfo pln((uint16_t)i, (uint16_t)ht->End(),0, &*ht);
						li.push_back(pln);
						i = pl.m_xBegin;
						++ht;
					}
	...
	for(; it != m_Lines.end(); it++)
	{
		...
		while(jt != it->end())
		{
			if(Len > 0)
			{
				...
				vl.push_back(CVisPart(*jt, rcBounds, Offset, (uint16_t)Len, NULL, NULL));
			}
