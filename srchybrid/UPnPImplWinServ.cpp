...
void CUPnPImplWinServ::StartDiscovery(uint16_t nTCPPort, uint16_t nUDPPort, uint16_t nTCPWebPort, bool bSecondTry)
{
...
void CUPnPImplWinServ::AddDevice(DevicePointer device, bool bAddChilds, int nLevel)
{
	...
	//Borschtsch - I could find the only way to get UPnP router address
	//We will use the device URL string
	//This is not a neat solution, but should work for us
	...
	hr = device->QueryInterface(IID_IUPnPDeviceDocumentAccess, (VOID **)&pDeviceDocument);
	if(SUCCEEDED(hr)){
		...
		if(SUCCEEDED(hr))
		{
			...
			CString tempString;
			CString slash = _T("/");
			tempString = bstrDeviceDocumentURL;
			int hostnameOffset = tempString.Find(slash + slash);
			if (hostnameOffset == -1)
			{
				slash = _T("\\");
				hostnameOffset = tempString.Find(slash + slash);
			}
			if (hostnameOffset != -1)
			{
				hostnameOffset = hostnameOffset + 2;
				int folderOffset = tempString.Find(slash, hostnameOffset);

				//Fetching hostname (it could be IP address)
				m_sInternalIP = tempString.Mid(hostnameOffset, folderOffset - hostnameOffset);
				...
				struct hostent * hp = gethostbyname(CT2A((LPCTSTR)m_sInternalIP));
				if (!hp)
				{
					addr.sin_addr.s_addr = inet_addr(CT2A((LPCTSTR)m_sInternalIP));
				}
				if ((!hp)&&(addr.sin_addr.s_addr == INADDR_NONE))
				{
					return;
				}
				if (hp != NULL)
				{
					memcpy((void*)&addr.sin_addr.s_addr, hp->h_addr, hp->h_length);
				}

				//Saving IP address in variable
				m_sInternalIP = CString(inet_ntoa(addr.sin_addr));
			}
...
bool CUPnPImplWinServ::OnSearchComplete()
{
	...
	for ( size_t pos = 0; pos != m_pDevices.size(); ++pos )
	{
...
CString CUPnPImplWinServ::GetLocalRoutableIP(ServicePointer pService)
{
	...
	/* Borschtsch - only internal router IP will help us to correctly determine local IP address,
	 * even with established VPN connections
	CT2CA pszExternalIP(strExternalIP);
	*/
	CT2CA pszExternalIP(m_sInternalIP);
