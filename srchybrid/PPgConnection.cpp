...
void CPPgConnection::OnEnChangePorts(uint8_t istcpport)
{
	// ports unchanged?
	CString buffer;
	GetDlgItem(IDC_PORT)->GetWindowText(buffer);
	uint16_t tcp = (uint16_t)_tstoi(buffer);
	GetDlgItem(IDC_UDPPORT)->GetWindowText(buffer);
	uint16_t udp = (uint16_t)_tstoi(buffer);
...
void CPPgConnection::OnEnChangeUDPDisable()
{
	...
	if (GetDlgItem(IDC_UDPPORT)->GetWindowTextLength())
	{
		...
		tempVal = (uint16_t)_tstoi(buffer);
	}
	else
		buffer[0] = _T('\0');
	...
	{
		tempVal = (uint16_t)_tstoi(buffer) ? (uint16_t)(_tstoi(buffer)+10) : (uint16_t)(thePrefs.port+10);
...
void CPPgConnection::LoadSettings(void)
{
	if (m_hWnd)
	{
		...
		// ismod: 100kb min speed
		m_ctlMaxUp.SetRange(100, thePrefs.GetMaxGraphUploadRate(true));
...
BOOL CPPgConnection::OnApply()
{
	...
	// ismod: 100kb min speed
	m_ctlMaxUp.SetRange(100, thePrefs.GetMaxGraphUploadRate(true), TRUE);
	SetRateSliderTicks(m_ctlMaxUp);

    {
	    ...
	    if (!IsDlgButtonChecked(IDC_ULIMIT_LBL))
		    ...
		    ulSpeed = (uint16_t)m_ctlMaxUp.GetPos();

	    if (thePrefs.GetMaxGraphUploadRate(true) < ulSpeed && ulSpeed != UNLIMITED)
		    ulSpeed = (uint16_t)(thePrefs.GetMaxGraphUploadRate(true) * 0.8);
	...
	if (thePrefs.GetMaxGraphDownloadRate() < thePrefs.GetMaxDownload() && thePrefs.GetMaxDownload() != UNLIMITED)
		thePrefs.SetMaxDownload((uint16_t)(thePrefs.GetMaxGraphDownloadRate() * 0.8));
	...
	if (GetDlgItem(IDC_PORT)->GetWindowTextLength())
	{
		...
		uint16_t nNewPort = ((uint16_t)_tstoi(buffer)) ? (uint16_t)_tstoi(buffer) : (uint16_t)thePrefs.port;
	...
	{
		GetDlgItem(IDC_UDPPORT)->GetWindowText(buffer, 20);
		uint16_t nNewPort = ((uint16_t)_tstoi(buffer) && !IsDlgButtonChecked(IDC_UDPDISABLE)) ? (uint16_t)_tstoi(buffer) : (uint16_t)0;
...
void CPPgConnection::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	// netfinity: Active Ratio - Removed obsolete code
	/*
