...
const CString strDefaultToolbar = _T("1399009901020304050607089909101112");
...
// ismod [start]
#define ISMOD_REGKEY _T("Software\\eMule IS Mod")

enum ESpaceAllocPolicy{
	SPACE_ALLOC_FULL=0,
	SPACE_ALLOC_SPARSE
};

enum EChatSendMethod{
	SEND_ON_DOUBLE_ENTER=0,
	SEND_ON_SINGLE_ENTER
};
// ismod [end]
...
struct Category_Struct {
	...
	bool	care4all;
	bool	ac_regexpeval;
	bool	downloadInAlphabeticalOrder;
};
...
class CPreferences
{
    ...
    friend class CPPgIsMod;

public:
    static bool	portableMode()                      { return m_portableMode; }
    static bool	checkForUpdates()                   { return m_checkForUpdates; }
    static bool	askForUpdateInstall()               { return m_askForUpdateInstall; }
    static uint32_t	versionCheckDays()              { return m_versionCheckDays; }
    static time_t lastVersionCheckTime()            { return m_lastVersionCheckTime; }
    static void	updateLastVersionCheckTime();

private:
    static bool m_portableMode;
    static bool	m_checkForUpdates;
    static bool	m_askForUpdateInstall;
    static uint32_t m_versionCheckDays;
    static time_t m_lastVersionCheckTime;

public:
	static	CString	strNick;
	static	uint16_t	minupload;
	static	uint16_t	maxupload;
	static	uint16_t	maxdownload;
	...
	static	uint16_t	nServerUDPPort;
	...
	static	uint32_t	maxGraphUploadRateEstimated;
	...
	static	uint64_t	cumDownOverheadTotal;
	static	uint64_t	cumDownOverheadFileReq;
	static	uint64_t	cumDownOverheadSrcEx;
	static	uint64_t	cumDownOverheadServer;
	static	uint64_t	cumDownOverheadKad;
	static	uint64_t	cumDownOverheadTotalPackets;
	static	uint64_t	cumDownOverheadFileReqPackets;
	static	uint64_t	cumDownOverheadSrcExPackets;
	static	uint64_t	cumDownOverheadServerPackets;
	static	uint64_t	cumDownOverheadKadPackets;

	// Saved stats for cumulative upline overhead...
	static	uint64_t	cumUpOverheadTotal;
	static	uint64_t	cumUpOverheadFileReq;
	static	uint64_t	cumUpOverheadSrcEx;
	static	uint64_t	cumUpOverheadServer;
	static	uint64_t	cumUpOverheadKad;
	static	uint64_t	cumUpOverheadTotalPackets;
	static	uint64_t	cumUpOverheadFileReqPackets;
	static	uint64_t	cumUpOverheadSrcExPackets;
	static	uint64_t	cumUpOverheadServerPackets;
	static	uint64_t	cumUpOverheadKadPackets;

	// Saved stats for cumulative upline data...
	static	uint32_t	cumUpSuccessfulSessions;
	static	uint32_t	cumUpFailedSessions;
	static	uint32_t	cumUpAvgTime;
	// Cumulative client breakdown stats for sent bytes...
	static	uint64_t	cumUpData_EDONKEY;
	static	uint64_t	cumUpData_EDONKEYHYBRID;
	static	uint64_t	cumUpData_EMULE;
	static	uint64_t	cumUpData_MLDONKEY;
	static	uint64_t	cumUpData_AMULE;
	static	uint64_t	cumUpData_EMULECOMPAT;
	static	uint64_t	cumUpData_SHAREAZA;
	// Session client breakdown stats for sent bytes...
	static	uint64_t	sesUpData_EDONKEY;
	static	uint64_t	sesUpData_EDONKEYHYBRID;
	static	uint64_t	sesUpData_EMULE;
	static	uint64_t	sesUpData_MLDONKEY;
	static	uint64_t	sesUpData_AMULE;
	static	uint64_t	sesUpData_EMULECOMPAT;
	static	uint64_t	sesUpData_SHAREAZA;

	// Cumulative port breakdown stats for sent bytes...
	static	uint64_t	cumUpDataPort_4662;
	static	uint64_t	cumUpDataPort_OTHER;
	// Session port breakdown stats for sent bytes...
	static	uint64_t	sesUpDataPort_4662;
	static	uint64_t	sesUpDataPort_OTHER;

	// Cumulative source breakdown stats for sent bytes...
	static	uint64_t	cumUpData_File;
	static	uint64_t	cumUpData_Partfile;
	// Session source breakdown stats for sent bytes...
	static	uint64_t	sesUpData_File;
	static	uint64_t	sesUpData_Partfile;

	// Saved stats for cumulative downline data...
	static	uint32_t	cumDownCompletedFiles;
	static	uint32_t	cumDownSuccessfulSessions;
	static	uint32_t	cumDownFailedSessions;
	static	uint32_t	cumDownAvgTime;

	// Cumulative statistics for saved due to compression/lost due to corruption
	static	uint64_t	cumLostFromCorruption;
	static	uint64_t	cumSavedFromCompression;
	static	uint32_t	cumPartsSavedByICH;

	// Session statistics for download sessions
	static	uint32_t	sesDownSuccessfulSessions;
	static	uint32_t	sesDownFailedSessions;
	static	uint32_t	sesDownAvgTime;
	static	uint32_t	sesDownCompletedFiles;
	static	uint64_t	sesLostFromCorruption;
	static	uint64_t	sesSavedFromCompression;
	static	uint32_t	sesPartsSavedByICH;

	// Cumulative client breakdown stats for received bytes...
	static	uint64_t	cumDownData_EDONKEY;
	static	uint64_t	cumDownData_EDONKEYHYBRID;
	static	uint64_t	cumDownData_EMULE;
	static	uint64_t	cumDownData_MLDONKEY;
	static	uint64_t	cumDownData_AMULE;
	static	uint64_t	cumDownData_EMULECOMPAT;
	static	uint64_t	cumDownData_SHAREAZA;
	static	uint64_t	cumDownData_URL;
	// Session client breakdown stats for received bytes...
	static	uint64_t	sesDownData_EDONKEY;
	static	uint64_t	sesDownData_EDONKEYHYBRID;
	static	uint64_t	sesDownData_EMULE;
	static	uint64_t	sesDownData_MLDONKEY;
	static	uint64_t	sesDownData_AMULE;
	static	uint64_t	sesDownData_EMULECOMPAT;
	static	uint64_t	sesDownData_SHAREAZA;
	static	uint64_t	sesDownData_URL;

	// Cumulative port breakdown stats for received bytes...
	static	uint64_t	cumDownDataPort_4662;
	static	uint64_t	cumDownDataPort_OTHER;
	// Session port breakdown stats for received bytes...
	static	uint64_t	sesDownDataPort_4662;
	static	uint64_t	sesDownDataPort_OTHER;
	...
	static	uint32_t	cumConnNumReconnects;
	static	uint32_t	cumConnAvgConnections;
	static	uint32_t	cumConnMaxConnLimitReached;
	static	uint32_t	cumConnPeakConnections;
	static	uint32_t	cumConnTransferTime;
	static	uint32_t	cumConnDownloadTime;
	static	uint32_t	cumConnUploadTime;
	static	uint32_t	cumConnServerDuration;

	// Saved records for servers / network...
	static	uint32_t	cumSrvrsMostWorkingServers;
	static	uint32_t	cumSrvrsMostUsersOnline;
	static	uint32_t	cumSrvrsMostFilesAvail;

	// Saved records for shared files...
	static	uint32_t	cumSharedMostFilesShared;
	static	uint64_t	cumSharedLargestShareSize;
	static	uint64_t	cumSharedLargestAvgFileSize;
	static	uint64_t	cumSharedLargestFileSize;
	...
	static	uint64_t	totalDownloadedBytes;
	static	uint64_t	totalUploadedBytes;
	...
	static uint8_t	m_byCryptTCPPaddingLength;
	static uint32_t   m_dwKadUDPKey;
	...
	static	const CString& GetTempDir(int id = 0)				{return tempdir.GetAt((id < tempdir.GetCount()) ? id : 0);}
	static	int		GetTempDirCount()					{return (int) tempdir.GetCount();}
	...
	static	const CString& GetMuleDirectory(EDefaultDirectory eDirectory, bool bCreate = true);
	static	void	SetMuleDirectory(EDefaultDirectory eDirectory, LPCTSTR pszNewDir);

	static	bool	IsTempFile(LPCTSTR pszDirectory, LPCTSTR pszName);
	static	bool	IsShareableDirectory(LPCTSTR pszDirectory);
	static	bool	IsInstallationDirectory(LPCTSTR pszDir);
	...
	static	void	SetUserNick(LPCTSTR pszNick)		{strNick = pszNick; }
	...
	static	uint16_t	GetPort()							{return port;}
	static	uint16_t	GetUDPPort()						{return udpport;}
	static	uint16_t	GetServerUDPPort()					{return nServerUDPPort;}
	static	uchar*	GetUserHash()						{return userhash;}
	// ZZ:UploadSpeedSense -->
	static	uint16_t	GetMinUpload()						{return minupload;}
	// ZZ:UploadSpeedSense <--
	static	uint16_t	GetMaxUpload()						{return maxupload;}
	...
	static	void	Add2SessionTransferData(UINT uClientID, UINT uClientPort, BOOL bFromPF, BOOL bUpDown, uint32_t bytes, bool sentToFriend = false);
	...
	static	void	Add2LostFromCorruption(uint64_t in)	{sesLostFromCorruption += in;}
	static	void	Add2SavedFromCompression(uint64_t in) {sesSavedFromCompression += in;}
	static	void	Add2SessionPartsSavedByICH(int in)	{sesPartsSavedByICH += in;}

	// Saved stats for cumulative downline overhead
	static	uint64_t	GetDownOverheadTotal()				{return cumDownOverheadTotal;}
	static	uint64_t	GetDownOverheadFileReq()			{return cumDownOverheadFileReq;}
	static	uint64_t	GetDownOverheadSrcEx()				{return cumDownOverheadSrcEx;}
	static	uint64_t	GetDownOverheadServer()				{return cumDownOverheadServer;}
	static	uint64_t	GetDownOverheadKad()				{return cumDownOverheadKad;}
	static	uint64_t	GetDownOverheadTotalPackets()		{return cumDownOverheadTotalPackets;}
	static	uint64_t	GetDownOverheadFileReqPackets()		{return cumDownOverheadFileReqPackets;}
	static	uint64_t	GetDownOverheadSrcExPackets()		{return cumDownOverheadSrcExPackets;}
	static	uint64_t	GetDownOverheadServerPackets()		{return cumDownOverheadServerPackets;}
	static	uint64_t	GetDownOverheadKadPackets()			{return cumDownOverheadKadPackets;}

	// Saved stats for cumulative upline overhead
	static	uint64_t	GetUpOverheadTotal()				{return cumUpOverheadTotal;}
	static	uint64_t	GetUpOverheadFileReq()				{return cumUpOverheadFileReq;}
	static	uint64_t	GetUpOverheadSrcEx()				{return cumUpOverheadSrcEx;}
	static	uint64_t	GetUpOverheadServer()				{return cumUpOverheadServer;}
	static	uint64_t	GetUpOverheadKad()					{return cumUpOverheadKad;}
	static	uint64_t	GetUpOverheadTotalPackets()			{return cumUpOverheadTotalPackets;}
	static	uint64_t	GetUpOverheadFileReqPackets()		{return cumUpOverheadFileReqPackets;}
	static	uint64_t	GetUpOverheadSrcExPackets()			{return cumUpOverheadSrcExPackets;}
	static	uint64_t	GetUpOverheadServerPackets()		{return cumUpOverheadServerPackets;}
	static	uint64_t	GetUpOverheadKadPackets()			{return cumUpOverheadKadPackets;}

	// Saved stats for cumulative upline data
	static	uint32_t	GetUpSuccessfulSessions()			{return cumUpSuccessfulSessions;}
	static	uint32_t	GetUpFailedSessions()				{return cumUpFailedSessions;}
	static	uint32_t	GetUpAvgTime()						{return cumUpAvgTime;}

	// Saved stats for cumulative downline data
	static	uint32_t	GetDownCompletedFiles()				{return cumDownCompletedFiles;}
	static	uint32_t	GetDownC_SuccessfulSessions()		{return cumDownSuccessfulSessions;}
	static	uint32_t	GetDownC_FailedSessions()			{return cumDownFailedSessions;}
	static	uint32_t	GetDownC_AvgTime()					{return cumDownAvgTime;}
	
	// Session download stats
	static	uint32_t	GetDownSessionCompletedFiles()		{return sesDownCompletedFiles;}
	static	uint32_t	GetDownS_SuccessfulSessions()		{return sesDownSuccessfulSessions;}
	static	uint32_t	GetDownS_FailedSessions()			{return sesDownFailedSessions;}
	static	uint32_t	GetDownS_AvgTime()					{return GetDownS_SuccessfulSessions() ? sesDownAvgTime / GetDownS_SuccessfulSessions() : 0;}

	// Saved stats for corruption/compression
	static	uint64_t	GetCumLostFromCorruption()			{return cumLostFromCorruption;}
	static	uint64_t	GetCumSavedFromCompression()		{return cumSavedFromCompression;}
	static	uint64_t	GetSesLostFromCorruption()			{return sesLostFromCorruption;}
	static	uint64_t	GetSesSavedFromCompression()		{return sesSavedFromCompression;}
	static	uint32_t	GetCumPartsSavedByICH()				{return cumPartsSavedByICH;}
	static	uint32_t	GetSesPartsSavedByICH()				{return sesPartsSavedByICH;}

	// Cumulative client breakdown stats for sent bytes
	static	uint64_t	GetUpTotalClientData()				{return   GetCumUpData_EDONKEY()
	...
	static	uint64_t	GetCumUpData_EDONKEY()				{return (cumUpData_EDONKEY +		sesUpData_EDONKEY );}
	static	uint64_t	GetCumUpData_EDONKEYHYBRID()		{return (cumUpData_EDONKEYHYBRID +	sesUpData_EDONKEYHYBRID );}
	static	uint64_t	GetCumUpData_EMULE()				{return (cumUpData_EMULE +			sesUpData_EMULE );}
	static	uint64_t	GetCumUpData_MLDONKEY()				{return (cumUpData_MLDONKEY +		sesUpData_MLDONKEY );}
	static	uint64_t	GetCumUpData_AMULE()				{return (cumUpData_AMULE +			sesUpData_AMULE );}
	static	uint64_t	GetCumUpData_EMULECOMPAT()			{return (cumUpData_EMULECOMPAT +	sesUpData_EMULECOMPAT );}
	static	uint64_t	GetCumUpData_SHAREAZA()				{return (cumUpData_SHAREAZA +		sesUpData_SHAREAZA );}
	
	// Session client breakdown stats for sent bytes
	static	uint64_t	GetUpSessionClientData()			{return   sesUpData_EDONKEY 
																...
																+ sesUpData_SHAREAZA;}
	static	uint64_t	GetUpData_EDONKEY()					{return sesUpData_EDONKEY;}
	static	uint64_t	GetUpData_EDONKEYHYBRID()			{return sesUpData_EDONKEYHYBRID;}
	static	uint64_t	GetUpData_EMULE()					{return sesUpData_EMULE;}
	static	uint64_t	GetUpData_MLDONKEY()				{return sesUpData_MLDONKEY;}
	static	uint64_t	GetUpData_AMULE()					{return sesUpData_AMULE;}
	static	uint64_t	GetUpData_EMULECOMPAT()				{return sesUpData_EMULECOMPAT;}
	static	uint64_t	GetUpData_SHAREAZA()				{return sesUpData_SHAREAZA;}

	// Cumulative port breakdown stats for sent bytes...
	static	uint64_t	GetUpTotalPortData()				{return   GetCumUpDataPort_4662() 
																+ GetCumUpDataPort_OTHER();}
	static	uint64_t	GetCumUpDataPort_4662()				{return (cumUpDataPort_4662 +		sesUpDataPort_4662 );}
	static	uint64_t	GetCumUpDataPort_OTHER()			{return (cumUpDataPort_OTHER +		sesUpDataPort_OTHER );}

	// Session port breakdown stats for sent bytes...
	static	uint64_t	GetUpSessionPortData()				{return   sesUpDataPort_4662 
																+ sesUpDataPort_OTHER;}
	static	uint64_t	GetUpDataPort_4662()				{return sesUpDataPort_4662;}
	static	uint64_t	GetUpDataPort_OTHER()				{return sesUpDataPort_OTHER;}

	// Cumulative DS breakdown stats for sent bytes...
	static	uint64_t	GetUpTotalDataFile()				{return (GetCumUpData_File() +		GetCumUpData_Partfile() );}
	static	uint64_t	GetCumUpData_File()					{return (cumUpData_File +			sesUpData_File );}
	static	uint64_t	GetCumUpData_Partfile()				{return (cumUpData_Partfile +		sesUpData_Partfile );}
	// Session DS breakdown stats for sent bytes...
	static	uint64_t	GetUpSessionDataFile()				{return (sesUpData_File +			sesUpData_Partfile );}
	static	uint64_t	GetUpData_File()					{return sesUpData_File;}
	static	uint64_t	GetUpData_Partfile()				{return sesUpData_Partfile;}

	// Cumulative client breakdown stats for received bytes
	static	uint64_t	GetDownTotalClientData()			{return   GetCumDownData_EDONKEY() 
																...
																+ GetCumDownData_URL();}
	static	uint64_t	GetCumDownData_EDONKEY()			{return (cumDownData_EDONKEY +			sesDownData_EDONKEY);}
	static	uint64_t	GetCumDownData_EDONKEYHYBRID()		{return (cumDownData_EDONKEYHYBRID +	sesDownData_EDONKEYHYBRID);}
	static	uint64_t	GetCumDownData_EMULE()				{return (cumDownData_EMULE +			sesDownData_EMULE);}
	static	uint64_t	GetCumDownData_MLDONKEY()			{return (cumDownData_MLDONKEY +			sesDownData_MLDONKEY);}
	static	uint64_t	GetCumDownData_AMULE()				{return (cumDownData_AMULE +			sesDownData_AMULE);}
	static	uint64_t	GetCumDownData_EMULECOMPAT()		{return (cumDownData_EMULECOMPAT +		sesDownData_EMULECOMPAT);}
	static	uint64_t	GetCumDownData_SHAREAZA()			{return (cumDownData_SHAREAZA +			sesDownData_SHAREAZA);}
	static	uint64_t	GetCumDownData_URL()				{return (cumDownData_URL +				sesDownData_URL);}
	
	// Session client breakdown stats for received bytes
	static	uint64_t	GetDownSessionClientData()			{return   sesDownData_EDONKEY 
																...
																+ sesDownData_URL;}
	static	uint64_t	GetDownData_EDONKEY()				{return sesDownData_EDONKEY;}
	static	uint64_t	GetDownData_EDONKEYHYBRID()			{return sesDownData_EDONKEYHYBRID;}
	static	uint64_t	GetDownData_EMULE()					{return sesDownData_EMULE;}
	static	uint64_t	GetDownData_MLDONKEY()				{return sesDownData_MLDONKEY;}
	static	uint64_t	GetDownData_AMULE()					{return sesDownData_AMULE;}
	static	uint64_t	GetDownData_EMULECOMPAT()			{return sesDownData_EMULECOMPAT;}
	static	uint64_t	GetDownData_SHAREAZA()				{return sesDownData_SHAREAZA;}
	static	uint64_t	GetDownData_URL()					{return sesDownData_URL;}

	// Cumulative port breakdown stats for received bytes...
	static	uint64_t	GetDownTotalPortData()				{return   GetCumDownDataPort_4662() 
																+ GetCumDownDataPort_OTHER();}
	static	uint64_t	GetCumDownDataPort_4662()			{return cumDownDataPort_4662		+ sesDownDataPort_4662;}
	static	uint64_t	GetCumDownDataPort_OTHER()			{return cumDownDataPort_OTHER		+ sesDownDataPort_OTHER;}

	// Session port breakdown stats for received bytes...
	static	uint64_t	GetDownSessionDataPort()			{return   sesDownDataPort_4662 
																+ sesDownDataPort_OTHER;}
	static	uint64_t	GetDownDataPort_4662()				{return sesDownDataPort_4662;}
	static	uint64_t	GetDownDataPort_OTHER()				{return sesDownDataPort_OTHER;}
	...
	static	uint32_t	GetConnNumReconnects()				{return cumConnNumReconnects;}
	static	uint32_t	GetConnAvgConnections()				{return cumConnAvgConnections;}
	static	uint32_t	GetConnMaxConnLimitReached()		{return cumConnMaxConnLimitReached;}
	static	uint32_t	GetConnPeakConnections()			{return cumConnPeakConnections;}
	static	uint32_t	GetConnTransferTime()				{return cumConnTransferTime;}
	static	uint32_t	GetConnDownloadTime()				{return cumConnDownloadTime;}
	static	uint32_t	GetConnUploadTime()					{return cumConnUploadTime;}
	static	uint32_t	GetConnServerDuration()				{return cumConnServerDuration;}

	// Saved records for servers / network
	static	uint32_t	GetSrvrsMostWorkingServers()		{return cumSrvrsMostWorkingServers;}
	static	uint32_t	GetSrvrsMostUsersOnline()			{return cumSrvrsMostUsersOnline;}
	static	uint32_t	GetSrvrsMostFilesAvail()			{return cumSrvrsMostFilesAvail;}

	// Saved records for shared files
	static	uint32_t	GetSharedMostFilesShared()			{return cumSharedMostFilesShared;}
	static	uint64_t	GetSharedLargestShareSize()			{return cumSharedLargestShareSize;}
	static	uint64_t	GetSharedLargestAvgFileSize()		{return cumSharedLargestAvgFileSize;}
	static	uint64_t	GetSharedLargestFileSize()			{return cumSharedLargestFileSize;}
	...
	static	void	SetExpandedTreeItems(LPCTSTR in)	{m_strStatsExpandedTreeItems = in;}
	static	const CString& GetExpandedTreeItems()		{return m_strStatsExpandedTreeItems;}

	static	uint64_t	GetTotalDownloaded()				{return totalDownloadedBytes;}
	static	uint64_t	GetTotalUploaded()					{return totalUploadedBytes;}
	...
	static	uint16_t	GetMaxDownload();
	static	uint64_t	GetMaxDownloadInBytesPerSec(bool dynamic = false);
	...
	static	uint8_t	GetSmartIdState()					{return smartidstate;}
	static	void	SetSmartIdState(uint8_t in_smartidstate) {smartidstate = in_smartidstate;}
	...
	static	void	SetCommentFilter(LPCTSTR pszFilter) {commentFilter = pszFilter;}
	...
	static	int		AddCat(Category_Struct* cat)		{catMap.Add(cat); return (int) catMap.GetCount()-1;}
	static	bool	MoveCat(UINT from, UINT to);
	static	void	RemoveCat(int index);
	static	int		GetCatCount()						{return (int) catMap.GetCount();}
	...
	// WebServer
	static	uint16_t	GetWSPort()							{return m_nWebPort;}
	static	bool	GetWSUseUPnP()						{return m_bWebUseUPnP && GetWSIsEnabled();}
	static	void	SetWSPort(uint16_t uPort)				{m_nWebPort=uPort;}
	static	const CString& GetWSPass()					{return m_strWebPassword;}
	static	void	SetWSPass(LPCTSTR strNewPass);
	...
	static	void	SetWSLowPass(LPCTSTR strNewPass);
	static  const CUIntArray& GetAllowedRemoteAccessIPs(){return m_aAllowedRemoteAccessIPs;}
	static	uint32_t	GetMaxWebUploadFileSizeMB()			{return m_iWebFileUploadSizeLimitMB;}
	...
	static	void	SetTemplate(LPCTSTR in)				{m_strTemplateFile = in;}
	...
	static	void	SetMMPass(LPCTSTR pszNewPass);
	static	bool	IsMMServerEnabled()					{return m_bMMEnabled;}
	static	void	SetMMIsEnabled(bool bEnable)		{m_bMMEnabled=bEnable;}
	static	uint16_t	GetMMPort()							{return m_nMMPort;}
	static	void	SetMMPort(uint16_t uPort)				{m_nMMPort=uPort;}
	...
	static	void	SetFilenameCleanups(LPCTSTR in)		{filenameCleanups=in;}
	...
	static	void	SetToolbarSettings(LPCTSTR in)		{m_sToolbarSettings = in;}
	static	const CString& GetToolbarBitmapSettings()			{return m_sToolbarBitmap;}
	static	void	SetToolbarBitmapSettings(LPCTSTR path){m_sToolbarBitmap = path;}
	//Borschtsch
	static	const CString& GetToolbarBitmapFolderSettings()		{return m_sToolbarBitmapFolder;}
	static	void	SetToolbarBitmapFolderSettings(LPCTSTR path){m_sToolbarBitmapFolder = path;}
	...
	static	const CString& GetNotifierMailServer()				{return m_strNotifierMailServer;}
	static	const CString& GetNotifierMailSender()				{return m_strNotifierMailSender;}
	static	const CString& GetNotifierMailReceiver()			{return m_strNotifierMailReceiver;}
	...
	static  void	IniCopy(LPCTSTR si, LPCTSTR di);

	static	void	EstimateMaxUploadCap(uint32_t nCurrentUpload);
	...
	static uint32_t	GetKadUDPKey()						{return m_dwKadUDPKey;}
	static uint8_t	GetCryptTCPPaddingLength()			{return m_byCryptTCPPaddingLength;}
	...
	static uint16_t	GetRandomTCPPort();
	static uint16_t	GetRandomUDPPort();

protected:
	static	CString m_strFileCommentsFilePath;
	static	Preferences_Ext_Struct* prefsExt;
	static	WORD m_wWinVer;
	static	CArray<Category_Struct*> catMap;
	static	CString	m_defaultDirs[EMULE_ENUM_END];
	static	bool	m_abDefaultDirsCreated[EMULE_ENUM_END];
	...
	static const CString& GetDefaultDirectory(EDefaultDirectory eDirectory, bool bCreate = true);

// ismod [start]
public:
	static void SavePreferencesReg();
	static void LoadPreferencesReg();

	static void	SetBindAddr(uint32_t lAddr);

	// IS74 Authorization
	static CString m_strAuthServ; // not used now
	static CString m_strAuthScript;
	static CString m_strAuthLogin;
	static CString m_strAuthPasswd;
	static const CString& GetAuthServer() { return m_strAuthServ; }
	static const CString& GetAuthScript() { return m_strAuthScript; }
	static const CString& GetAuthLogin()  { return m_strAuthLogin; }
	static const CString& GetAuthPasswd() { return m_strAuthPasswd; }

	// Skin and toolbar stuff
	static bool	m_bUseRandomSkins;
	static void LoadRandomSkinAndToolbar();
	static const CString& GetSkinProfileDir() { return m_strSkinProfileDir; }
	static void SetSkinProfileDir(LPCTSTR dir) { m_strSkinProfileDir = dir; }

	// Save search params
	static bool m_bSaveSearchParams;
	static bool GetSaveSearchParams() { return m_bSaveSearchParams; }
	...
	// Delay at startup
	static int m_iStartDelay;
	
	// Show extended info in title
	static bool	m_bShowExtendedInfoInTitle;
	static bool GetShowExtendedInfoInTitle() { return m_bShowExtendedInfoInTitle; }

	// Word filters
	static bool				m_bWordFilterLocked;
	static CString			m_strWordFilterPass;
	static CStringList		m_strBadWordsList;
	static CSortStringArray	m_strAdultWordsArray;
	static void SaveWordFilter();
	static bool HasBadWords(LPCTSTR szName);
	static bool LoadWordFilter(LPCTSTR szFileName);
	static bool HasFilteredWords(CString strString);

	// Server packet logger
	static bool	m_bServerPacketLogger;
	static bool IsServerPacketLoggerEnabled() { return m_bServerPacketLogger; }

	// Friend slot for all friends
	static bool	m_bFriendSlotForAllFriends;
	static bool GetUseFriendSlotForAllFriends() { return m_bFriendSlotForAllFriends; }

	// Chat send method
	static EChatSendMethod m_iChatSendMethod;
	static EChatSendMethod GetChatSendMethod() { return m_iChatSendMethod; }

	// Windows user name
	static CString m_strWindowsUserName;
	static LPCTSTR GetWindowsUserName() { return m_strWindowsUserName; }

	// Hashing speed
	static int m_iHashingSpeed;
	static int GetHashingSpeed() { return m_iHashingSpeed; }

	// Use colors for clients versions
	static bool	m_bUseClientColors;
	static bool GetUseClientColors() { return m_bUseClientColors; }

	// Always show tray icon
	static bool	m_bAlwaysShowTrayIcon;
	static bool GetAlwaysShowTrayIcon() { return m_bAlwaysShowTrayIcon; }

	// Friend slot userhash
	static uchar m_friendSlotUserHash[16];
	static const uchar* GetFriendSlotUserHash() { return m_friendSlotUserHash; }
	static void SetFriendSlotUserHash( const uchar* hash );
	static void RemoveFriendSlotUserHash() { FillMemory(m_friendSlotUserHash, 16, 0xFF); }
	
	// Disable IS74 authorization
	static bool	m_bDontUseAuth;
	static bool IsAuthDisabled() { return m_bDontUseAuth; }
	
	// Space allocating policy
	static ESpaceAllocPolicy m_iSpaceAllocPolicy;
	static ESpaceAllocPolicy GetSpaceAllocPolicy() { return m_iSpaceAllocPolicy; }
	static void SetSpaceAllocPolicy(ESpaceAllocPolicy policy) { m_iSpaceAllocPolicy = policy; }

	// Automatic shutdown PC when done
	static bool	m_bAutoShutdownWhenDone;
	static bool GetAutoShutdownWhenDone(){ return m_bAutoShutdownWhenDone; }
	static void SetAutoShutdownWhenDone(bool state) { m_bAutoShutdownWhenDone = state; }

	// FunnyNick tag
	static bool	m_bUseFunnyNickTag;
	static void SetFunnyNickTag(bool state) { m_bUseFunnyNickTag = state; }
	static bool GetFunnyNickTag() { return m_bUseFunnyNickTag; }
// ismod [end]
};
