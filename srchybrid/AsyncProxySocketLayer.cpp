...
extern CStringA ipstrA(uint32_t nIP);
...
void CAsyncProxySocketLayer::OnConnect(int nErrorCode)
{
	...
	if (m_nProxyOpID == PROXYOP_CONNECT || m_nProxyOpID == PROXYOP_BIND)
	{
		...
		else if (m_ProxyData.nProxyType == PROXYTYPE_HTTP10 || m_ProxyData.nProxyType == PROXYTYPE_HTTP11)
		{
			...
			{
				char szUserPass[512];
				_snprintf(szUserPass, _countof(szUserPass), "%s:%s", m_ProxyData.strProxyUser, m_ProxyData.strProxyPass);

				CStringA strUserPassBase64 = CBase64Coding::Encode(szUserPass);
				...
				if (m_ProxyData.nProxyType == PROXYTYPE_HTTP10) {
					...
					iHttpReqLen = _snprintf(szHttpReq, _countof(szHttpReq), 
						...
						"\r\n",
						pszHost, nProxyPeerPort, strUserPassBase64, strUserPassBase64);
				}
				else {
					// "Host" field is a MUST for HTTP/1.1 according RFC 2161
					iHttpReqLen = _snprintf(szHttpReq, _countof(szHttpReq), 
						...
						"Proxy-Authorization: Basic %s\r\n"
						"\r\n",
						pszHost, nProxyPeerPort, pszHost, nProxyPeerPort, strUserPassBase64, strUserPassBase64);
				}
			}
