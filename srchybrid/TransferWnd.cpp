...
BEGIN_MESSAGE_MAP(CTransferWnd, CResizableFormView)
	...
	ON_WM_PAINT()
	ON_WM_SYSCOMMAND()
	// ismod: Added by SiRoB, Fix
	ON_NOTIFY(LVN_HOTTRACK, IDC_DOWNLOADCLIENTS, OnHoverUploadList)
	ON_WM_DRAWITEM()
END_MESSAGE_MAP()

CTransferWnd::CTransferWnd(CWnd* /*pParent =NULL*/)
	: CResizableFormView(IDD_TRANSFER)
{
...
void CTransferWnd::OnInitialUpdate()
{
	...
	AddAnchor(IDC_MOD_DOWNLOADBAR, TOP_LEFT, TOP_RIGHT);
...
void CTransferWnd::ShowQueueCount(uint32_t number)
{
	TCHAR buffer[100];
	_sntprintf_s(buffer, _countof(buffer), _countof(buffer), _T("%u (%u ") + GetResString(IDS_BANNED).MakeLower() + _T(")"), number, theApp.clientlist->GetBannedCount());
...
void CTransferWnd::UpdateListCount(EWnd2 listindex, int iCount /*=-1*/)
{
	switch (m_dwShowListIDC)
	{
		case IDC_DOWNLOADLIST + IDC_UPLOADLIST: {
			...
			{
				case wnd2Uploading: {
					uint32_t itemCount = iCount == -1 ? uploadlistctrl.GetItemCount() : iCount;
					uint32_t activeCount = theApp.uploadqueue->GetActiveUploadsCount();
...
void CTransferWnd::OnLvnBeginDragDownloadList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (m_pDragImage == NULL)
	{
		...
		m_pDragImage->Create(16, 16, ILC_COLOR32 | ILC_MASK, 0, 0);
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (wParam>=MP_CAT_SET0 && wParam<=MP_CAT_SET0+99)
	{
		...
		else // set the view filter
		{
			...
			thePrefs.SetCatFilter(m_isetcatmenu,static_cast<int>(wParam)-MP_CAT_SET0);
			m_nLastCatTT=-1;
		}
	...
	{ 
		...
		case MP_HM_OPENINC:
			...
			theDesktop.ShellExecute(_T("open"), thePrefs.GetCategory(m_isetcatmenu)->strIncomingPath);
			break;
...
int CTransferWnd::AddCategory(LPCTSTR newtitle, LPCTSTR newincoming, LPCTSTR newcomment, LPCTSTR newautocat, bool addTab)
{
...
CString CTransferWnd::GetTabStatistic(int tab)
{
	...
	for (int i = 0; i < theApp.downloadqueue->GetFileCount(); i++)
	{
		...
		if (cur_file->CheckShowItemInGivenCat(tab))
		{
			...
			size += (uint64_t)cur_file->GetFileSize();
			trsize += (uint64_t)cur_file->GetCompletedSize();
			if (!cur_file->IsAllocating())
				disksize += (uint64_t)cur_file->GetRealFileSize();
...
void CTransferWnd::OnTabMovement(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	theApp.downloadqueue->MoveCat((uint8_t)from,(uint8_t)to);

	// move category of completed files
	downloadlistctrl.MoveCompletedfilesCat((uint8_t)from,(uint8_t)to);
...
void CTransferWnd::ShowList(uint32_t dwListIDC)
{
	CRect rcWnd;
	GetWindowRect(rcWnd);
...
// ismod [start]
void CTransferWnd::GotoFile( const CPartFile *pFile )
{
	ShowSplitWindow();
	downloadlistctrl.ChangeCategory(0);
	downloadlistctrl.GotoFile(pFile);
}


void CTransferWnd::OnDrawItem( int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct )
{
	if ( IDC_MOD_DOWNLOADBAR == nIDCtl ) {
		CDC dc;
		...
		if ( m_downBarFile ) {
			if ( (HBITMAP)NULL == m_downBarBmp )
				VERIFY(m_downBarBmp.CreateBitmap(1, 1, 1, 8, NULL));
			CDC cdcStatus;
			HGDIOBJ hOldBitmap;
			cdcStatus.CreateCompatibleDC(&dc);
			int cx = m_downBarBmp.GetBitmapDimension().cx; 
			if ( cx != rc.Width() || m_bRedrawDownBar ) {
				m_downBarBmp.DeleteObject();
				m_downBarBmp.CreateCompatibleBitmap(&dc, iWidth, iHeight);
				m_downBarBmp.SetBitmapDimension(iWidth, iHeight);
				hOldBitmap = cdcStatus.SelectObject(m_downBarBmp);
				...
				m_downBarFile->DrawStatusBar(&cdcStatus,  &rec_status, thePrefs.UseFlatBar());
				m_bRedrawDownBar = false;
			} else {
				hOldBitmap = cdcStatus.SelectObject(m_downBarBmp);
			}
			dc.BitBlt(rc.left, rc.top, iWidth, iHeight,  &cdcStatus, 0, 0, SRCCOPY);
	...
	CResizableFormView::OnDrawItem(nIDCtl, lpDrawItemStruct);
}

void CTransferWnd::UpdateDownloadBar()
{
	m_bRedrawDownBar = true;
	GetDlgItem(IDC_MOD_DOWNLOADBAR)->Invalidate(TRUE);
}