...
TEMPLATE void CDialogMinTrayBtn<BASE>::MinTrayBtnDraw()
{
	...
	else
	{
		...
		if (m_bmMinTrayBtnBitmap.m_hObject)
		{
			...
			{
				TransparentBlt(pDC->m_hDC, btn.left, btn.top, btn.Width(), btn.Height(), dcMem.m_hDC, 0, BMP_TRAYBTN_HEIGHT * (iState - 1), BMP_TRAYBTN_WIDTH, BMP_TRAYBTN_HEIGHT, BMP_TRAYBTN_TRANSCOLOR);
