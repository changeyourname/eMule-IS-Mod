...
void CSearchListCtrl::SetAllIcons()
{
    ...
    // ismod: search users,folders [start]
    m_ImageList.Add(CTempIconLoader(_T("SRCCONNECTING"))); // index=9
    m_ImageList.Add(CTempIconLoader(_T("STATSCLIENTS")));
    m_ImageList.Add(CTempIconLoader(_T("FRIEND")));
    m_ImageList.Add(CTempIconLoader(_T("CANCEL")));
    m_ImageList.Add(CTempIconLoader(_T("OPENFOLDER")));
    // ismod [end]
...
{
    // ismod: do not add items with collapsed parents [start]
    if(toshow->GetListParent() && !toshow->GetListParent()->IsListExpanded())
        return;
    // ismod [end]
		...
		for (int iItem = 0; iItem < iTabItems; iItem++)
		{
			if (searchselect.GetItem(iItem, &ti) && ti.lParam != NULL)
			{
				...
				if (pSearchParams->dwSearchID == toshow->GetSearchID())
				{
                    ...
                    // ismod: total found size [start]
                    uint64_t ulTotalSize = searchlist->GetTotalSize(toshow->GetSearchID());
                    if (ulTotalSize > 0)
                    {
                        strTabLabel += _T(" - ") + CastItoXBytes(ulTotalSize, false, false);
                    }
                    // ismod [end]
...
void CSearchListCtrl::UpdateSources(const CSearchFile* toupdate)
{
	...
	{
		CString strBuffer;
		uint32_t nSources = toupdate->GetSourceCount();	
...
// ismod: result changed to bool
bool CSearchListCtrl::UpdateSearch(CSearchFile* toupdate)
{
	if (!toupdate || !theApp.IsRunning())
		return false;
	LVFINDINFO find;
	find.flags = LVFI_PARAM;
	find.lParam = (LPARAM)toupdate;
...
CString CSearchListCtrl::GetCompleteSourcesDisplayString(const CSearchFile* pFile, UINT uSources, bool* pbComplete) const
{
	...
	if (iComplete < 0 && pFile->GetFileSize() <= (uint64_t)PARTSIZE) {
		iComplete = 1;
...
void CSearchListCtrl::ShowResults(uint32_t nResultsID)
{
...
//		thePrefs.SetColumnSortAscending(CPreferences::tableSearch, pNewState->m_bSortAscending);
    ...
    // ismod: single select for users search [start]
    SSearchParams* pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(nResultsID);
...
int CSearchListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int orgSort=static_cast<int>(lParamSort);
	int sortMod = (int) 1;
	if (lParamSort >= 100) {
		sortMod = -1;
		lParamSort -= 100;
...
void CSearchListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
    ...
    const CSearchFile* pFile = (CSearchFile*)GetItemData(iSel);

    if(pFile && pFile->IsUser())
    {
        const CUpDownClient* client = theApp.clientlist->FindClientByUserHash(pFile->GetFileHash());

        CTitleMenu ClientMenu;
        ClientMenu.CreatePopupMenu();
        ClientMenu.AddMenuTitle(GetResString(IDS_CLIENTS), true);
        if (client == NULL) ClientMenu.AppendMenu(MF_STRING, MP_CHECK, _T("Обновить клиента"), _T("SEARCHPARAMS"));
	...
	// ismod: copy bbcode link
	m_SearchFileMenu.EnableMenuItem(MP_GETBBCODEED2KLINK, iSelected > 0 ? MF_ENABLED : MF_GRAYED);
	...
	// ismod: go to file
	m_SearchFileMenu.EnableMenuItem(MP_GOTOFILE, (iSelected == 1 && theApp.sharedfiles->GetFileByID(pFile->GetFileHash()))? MF_ENABLED : MF_GRAYED);
...
BOOL CSearchListCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (selectedList.GetCount() > 0)
	{
        ...
        // ismod [start]
        if (file->IsUser()) //search users menu
        {
            CUpDownClient* client = theApp.clientlist->FindClientByUserHash(file->GetFileHash());
            switch (wParam)
            {
            case MP_DETAIL:
            case MPG_ALTENTER:
                ...
                return TRUE;
            case MP_SHOWLIST:
                if (client) client->RequestSharedFileList();
                return TRUE;
            case MP_MESSAGE:
                if (client) theApp.emuledlg->chatwnd->StartSession(client);
                return TRUE;
            case MP_ADDFRIEND:
                if (client && theApp.friendlist->AddFriend(client))
                {
                    POSITION pos = GetFirstSelectedItemPosition();
                    int index = GetNextSelectedItem(pos);
                    Update(index);
        ...
        } else if (file->IsFolder()) // search folders menu
        {
            /*
            switch (wParam)
            {
        ...
        switch (wParam)
		{
			...
			case MP_GETBBCODEED2KLINK:{
				...
				while (pos != NULL)
				{
					...
					if (!clpbrd.IsEmpty())
						clpbrd += _T("\r\n");
					clpbrd += CreateBBCODEED2kLink(file);
				}
				theApp.CopyTextToClipboard(clpbrd);
				return TRUE;
			}
			...
			// ismod: go to file [start]
			case MP_GOTOFILE:
				if (file) {
					const CKnownFile* temp = theApp.sharedfiles->GetFileByID(file->GetFileHash());
					if (temp)
						if (temp->IsPartFile()) {
							theApp.emuledlg->transferwnd->GetDownloadList()->GotoFile((CPartFile *)temp);						
							theApp.emuledlg->SetActiveDialog(theApp.emuledlg->transferwnd);
						}
						else {
...
void CSearchListCtrl::CreateMenues()
{
	...
	// ismod: go to file
	m_SearchFileMenu.AppendMenu(MF_STRING, MP_GOTOFILE, GetResString(IDS_MOD_GOTOFILE), _T("SEARCHFILETYPE_ANY"));
	...
	// ismod: bbcode copy link
	m_SearchFileMenu.AppendMenu(MF_STRING, MP_GETBBCODEED2KLINK, GetResString(IDS_MOD_DL_LINK3), _T("ED2KLink"));
	...
	if (pGetInfoTip->iSubItem == 0)
	{
			...
			if (file && pGetInfoTip->pszText && pGetInfoTip->cchTextMax > 0){
			    ...
			    if (file->GetClientsCount()){
				    ...
				    if (file->GetClientID() && file->GetClientPort()){
					    uint32_t uClientIP = file->GetClientID();
					    uint32_t uServerIP = file->GetClientServerIP();
						CString strSource;
						if (bFirst){
							bFirst = false;
						...
						strSource.AppendFormat(_T(": %u.%u.%u.%u:%u  Server: %u.%u.%u.%u:%u"),
						    (uint8_t)uClientIP,(uint8_t)(uClientIP>>8),(uint8_t)(uClientIP>>16),(uint8_t)(uClientIP>>24), file->GetClientPort(),
						    (uint8_t)uServerIP,(uint8_t)(uServerIP>>8),(uint8_t)(uServerIP>>16),(uint8_t)(uServerIP>>24), file->GetClientServerPort());
					    if (!strInfo.IsEmpty())
						    strInfo += _T("\n");
					    strInfo += strSource;
				    }
				    ...
				    for (int i = 0; i < aClients.GetSize(); i++){
					    uint32_t uClientIP = aClients[i].m_nIP;
					    uint32_t uServerIP = aClients[i].m_nServerIP;
						CString strSource;
						if (bFirst){
							bFirst = false;
							strSource = _T("Sources");
						...
						strSource.AppendFormat(_T(": %u.%u.%u.%u:%u  Server: %u.%u.%u.%u:%u"),
						    (uint8_t)uClientIP,(uint8_t)(uClientIP>>8),(uint8_t)(uClientIP>>16),(uint8_t)(uClientIP>>24), aClients[i].m_nPort,
						    (uint8_t)uServerIP,(uint8_t)(uServerIP>>8),(uint8_t)(uServerIP>>16),(uint8_t)(uServerIP>>24), aClients[i].m_nServerPort);
					    if (!strInfo.IsEmpty())
						    strInfo += _T("\n");
					    strInfo += strSource;
					    if (strInfo.GetLength() >= pGetInfoTip->cchTextMax)
			    ...
			    if (file->GetServers().GetSize()){
				    ...
				    for (int i = 0; i < aServers.GetSize(); i++){
					    uint32_t uServerIP = aServers[i].m_nIP;
						CString strServer;
						if (i == 0)
							strServer.Format(_T("Servers"));
						strServer.AppendFormat(_T(": %u.%u.%u.%u:%u  Avail: %u"),
						    (uint8_t)uServerIP,(uint8_t)(uServerIP>>8),(uint8_t)(uServerIP>>16),(uint8_t)(uServerIP>>24), aServers[i].m_nPort, aServers[i].m_uAvail);
					    if (!strInfo.IsEmpty())
						    strInfo += _T("\n");
					    strInfo += strServer;
					    if (strInfo.GetLength() >= pGetInfoTip->cchTextMax)
		...
		else
		{
			...
			while (pos)
			{
				...
				{
					iSelected++;
					ulTotalSize += (uint64_t)pFile->GetFileSize();
				}
			}
...
void CSearchListCtrl::ExpandCollapseItem(int iItem, int iAction)
{
	...
	if (!searchfile->IsListExpanded())
	{
		...
		{
            // ismod: folder search [start]
            SSearchParams* pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(searchfile->GetSearchID());
            if(searchfile->IsFolder() && pParams->bSearchFolders && searchfile->GetListChildCount() < 2) {
                searchfile->SetListExpanded(true);
                Update(iItem);
                TryToConnect(searchfile);
                return;
            } else
            // ismod [end]
...
void CSearchListCtrl::OnNmDblClk(NMHDR* /*pNMHDR*/, LRESULT* /*pResult*/)
{
	...
	if (pt.x > TREE_WIDTH)
	{
        ...
        /* ismod: users,folders search [start]
        else
        ...
        */
		else
        {
			...
			if (iSel != -1){
                ...
                {
					if (file->IsFolder())
						ExpandCollapseItem(iSel, EXPAND_COLLAPSE);
					else if (file->IsUser()) {
						CUpDownClient* client = theApp.clientlist->FindClientByUserHash(file->GetFileHash());
						if (client)
                        {
...
void CSearchListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
    ...
    // ismod [start]
    if(!content->IsUser() && !content->IsFolder())
    {
    // ismod [end]
	...
	int iImage = (int) theApp.GetFileTypeSystemImageIdx(content->GetFileName());
	    ImageList_Draw(theApp.GetSystemImageList(), iImage, dc, cur_rec.left + iIndent + TREE_WIDTH, cur_rec.top + iIconPosY, ILD_TRANSPARENT);
    // ismod [start]
    } else if(content->IsUser()) // user
    {
        int iImage;
        const CUpDownClient* cur_client = theApp.clientlist->FindClientByUserHash(content->GetFileHash());
        if (cur_client == NULL)
            iImage = 9;
            ...
            iImage = 10;
        m_ImageList.Draw(dc, iImage, CPoint(cur_rec.left + iIndent + TREE_WIDTH, cur_rec.top + iIconPosY), ILD_TRANSPARENT);
    } else // folder
        m_ImageList.Draw(dc, 13, CPoint(cur_rec.left + iIndent + TREE_WIDTH, cur_rec.top + iIconPosY), ILD_TRANSPARENT);
    // ismod [end]
	...
	if (content->GetListParent() == NULL)
	{
		for (int iCurrent = 0; iCurrent < iCount; iCurrent++)
		{
			...
			if (!IsColumnHidden(iColumn))
			{
                ...
                // ismod: align to left for column sources when searching users
                if (iColumn==2 && content->IsUser()) uDrawTextAlignment = DT_LEFT;
				if (iColumn == 0)
				{
	...
	if(tree_start < tree_end)
	{
        ...
        // ismod: folders
            || ( content->IsFolder() && (content->GetListChildCount()>1 || content->GetFileSize()>(uint64_t)0)))
		{
			//draw box
			RECT circle_rec;
...
{
    // ismod: users has black text color
    if (src->IsUser()) return m_crShades[0];
	...
	// ismod: do not colorize folders [start]
	if ( src->IsFolder() )
		return m_crShades[0];
	// ismod [end]
	...
	uint32_t srccnt = src->GetSourceCount();
	if (srccnt > 0)
		--srccnt;
...
void CSearchListCtrl::DrawSourceChild(CDC *dc, int nColumn, LPRECT lpRect, UINT uDrawTextAlignment, const CSearchFile *src)
{
	...
	switch (nColumn)
	{
		...
		case 4:			// file type
		case 5:			// file hash
            // ismod: do not skip for folders search
            if(!src->GetListParent()->IsFolder())
			break;
...
void CSearchListCtrl::DrawSourceParent(CDC *dc, int nColumn, LPRECT lpRect, UINT uDrawTextAlignment, const CSearchFile *src)
{
	...
	switch (nColumn)
	{
		...
		case 3:{		// complete sources
            // ismod: no text for folders
            if (src->IsFolder()) break;
...
void CSearchListCtrl::ClearResultViewState(uint32_t nResultsID){
...
void CSearchListCtrl::GetItemDisplayText(const CSearchFile *src, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
	...
	{
		case 0:			// file name
			_tcsncpy_s(pszText, cchTextMax, src->GetFileName(), cchTextMax);
			break;
		
		case 1:			// file size
			if (   src->GetListParent() == NULL
                // ismod: children of folders
                || ( src->GetListParent() && src->GetListParent()->IsFolder())
		...
		case 2:			// avail
            // ismod: users search shows subnet [start]
            if (src->IsUser())
                _tcsncpy_s(pszText, cchTextMax, _T(" "), cchTextMax);
			else
            // ismod [end]
            if (src->GetListParent() == NULL)
			{
				...
				if (thePrefs.IsExtControlsEnabled()) {
					if (src->IsKademlia()) {
						uint32_t nKnownPublisher = (src->GetKadPublishInfo() & 0x00FF0000) >> 16;
		...
		case 6:
			_tcsncpy_s(pszText, cchTextMax, src->GetStrTagValue(FT_MEDIA_ARTIST), cchTextMax);
			break;
		
		case 7:
			_tcsncpy_s(pszText, cchTextMax, src->GetStrTagValue(FT_MEDIA_ALBUM), cchTextMax);
			break;
		
		case 8:
			_tcsncpy_s(pszText, cchTextMax, src->GetStrTagValue(FT_MEDIA_TITLE), cchTextMax);
			break;
		
		case 9:{
			uint32_t nMediaLength = src->GetIntTagValue(FT_MEDIA_LENGTH);
			if (nMediaLength){
				CString buffer;
				SecToTimeLength(nMediaLength, buffer);
				_tcsncpy_s(pszText, cchTextMax, buffer, cchTextMax);
			}
			break;
		}
		
		case 10:{
			uint32_t nBitrate = src->GetIntTagValue(FT_MEDIA_BITRATE);
			if (nBitrate)
				_sntprintf_s(pszText, cchTextMax, cchTextMax, _T("%u %s"), nBitrate, GetResString(IDS_KBITSSEC));
			break;
		}
		...
		case 12:		// dir
			if (src->GetDirectory())
				_tcsncpy_s(pszText, cchTextMax, src->GetDirectory(), cchTextMax);
			break;
		
		case 13:
			if (src->m_eKnown == CSearchFile::Shared)
				_tcsncpy_s(pszText, cchTextMax, GetResString(IDS_SHARED), cchTextMax);
			else if (src->m_eKnown == CSearchFile::Downloading)
				_tcsncpy_s(pszText, cchTextMax, GetResString(IDS_DOWNLOADING), cchTextMax);
			else if (src->m_eKnown == CSearchFile::Downloaded)
				_tcsncpy_s(pszText, cchTextMax, GetResString(IDS_DOWNLOADED), cchTextMax);
			else if (src->m_eKnown == CSearchFile::Cancelled)
				_tcsncpy_s(pszText, cchTextMax, GetResString(IDS_CANCELLED), cchTextMax);
...
CString	CSearchListCtrl::FormatFileSize(ULONGLONG ullFileSize) const
{
	if (m_eFileSizeFormat == fsizeKByte) {
		...
		if (fFileSize < 0.01)
			fFileSize = 0.01;
		TCHAR szVal[40];
		_sntprintf_s(szVal, _countof(szVal), _countof(szVal), _T("%.2f"), fFileSize);
...
// ismod [start]
void CSearchListCtrl::TryToConnect(CSearchFile *file)
{
    if (file)
    {
        uint32_t dwIP = file->GetClientID();
        uint16_t port = file->GetClientPort();

        if (theApp.ipfilter->IsFiltered(dwIP)){
            if (thePrefs.GetLogFilteredIPs())
                AddDebugLogLine(true, _T("Ignored request (IP=%s) - IP filter (%s)"), ipstr(dwIP), theApp.ipfilter->GetLastHit());
            return;
        }
        ...
        if (theApp.clientlist->IsBannedClient(dwIP)){
            if (thePrefs.GetLogBannedClients()){
                ...
                AddDebugLogLine(true, _T("Ignored request to banned client %s; %s"), ipstr(dwIP), pClient->DbgGetClientInfo());
            }
            return;
        }

        CUpDownClient* client = theApp.clientlist->FindClientByIP(dwIP, port);
        if (client == NULL) {
            if (dwIP != 0) {
                client = new CUpDownClient(0, port, dwIP, file->GetClientServerIP(), file->GetClientServerPort(), true);
                theApp.clientlist->AddClient(client);
			}else {
				AddDebugLogLine(true, _T("Ignored request to client '%s' with null ip"), file->GetFileName());
				return;
			}
        }
        client->SetSearchFile(file);
        client->TryToConnect(true);
    }
}

void CSearchListCtrl::ExpandCollapseAll(int iAction)
{
	CSearchFile* sel_file;
	CSearchFile* cur_file;
	int iSelItem = GetNextItem(-1, LVNI_SELECTED | LVNI_FOCUSED);
	int iItem;

	if (iAction != COLLAPSE_ONLY && iAction != EXPAND_ONLY)
		return;

	// Remember selection
	sel_file = (CSearchFile*)GetItemData(iSelItem);
	if (iAction == COLLAPSE_ONLY && sel_file->GetListParent() != NULL)
		sel_file = sel_file->GetListParent();

	// Expand all or Collapse all
	for (iItem = GetItemCount() - 1; iItem >= 0; --iItem)
	{
		cur_file = (CSearchFile*)GetItemData(iItem);

		if ((iAction == COLLAPSE_ONLY && cur_file->IsFolder() && cur_file->IsListExpanded() && cur_file->GetListChildCount() > 0) ||
			(iAction == EXPAND_ONLY && cur_file->IsFolder() && !cur_file->IsListExpanded() && cur_file->GetListChildCount() > 0))
		{
			ExpandCollapseItem(iItem, iAction);
		}
	}
	...
	LVFINDINFO find;
	find.flags = LVFI_PARAM;
	find.lParam = (LPARAM)sel_file;
	iSelItem = FindItem(&find);
	if (iSelItem == -1)
		return;
	...
	if (iAction == COLLAPSE_ONLY)
	{
		...
		// Select initial listview entry
		SetItemState(iSelItem, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
		SetSelectionMark(iSelItem);
	}

	EnsureVisible(iSelItem, FALSE/*bPartialOK*/);
	SetFocus();
}

void CSearchListCtrl::ResortList()
{
	SortItems(SortProc, GetSortItem() + (GetSortAscending() ? 0 : 100));
}

void CSearchListCtrl::AddCatalogToDownload(const CSearchFile *file)
{
	LVFINDINFO find;
	find.flags = LVFI_PARAM;
	...
	if (index != -1)
	{
		...
		const SearchList* list = theApp.searchlist->GetSearchListForID(file->GetSearchID());
		for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
		{
			CSearchFile* cur_file = list->GetNext(pos);
			if (cur_file->GetListParent() == file)
			{
				collection.AddFileToCollection(cur_file, true);
			}
		}
		collection.m_sCollectionName = file->GetFileName();
