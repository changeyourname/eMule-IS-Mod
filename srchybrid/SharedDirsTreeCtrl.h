...
public:
	CDirectoryItem(LPCTSTR pszFullPath, HTREEITEM htItem = TVI_ROOT, ESpecialDirectoryItems eItemType = SDI_NO, int m_nCatFilter = -1);
	~CDirectoryItem();
	CDirectoryItem*		CloneContent() { return new CDirectoryItem(m_strFullPath, 0, m_eItemType, m_nCatFilter); }
	HTREEITEM			FindItem(const CDirectoryItem* pContentToFind) const;
...
public:
	...
	bool			ShowDirectory(const CString& strDir, ESpecialDirectoryItems eParentItemType); // ismod
...
private:
	...
	bool	FileSystemTreeIsShared(const CString& rstrDir);
