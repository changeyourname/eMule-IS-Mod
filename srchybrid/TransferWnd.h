...
public:
	...
	int	 AddCategory( LPCTSTR newtitle, LPCTSTR newincoming, LPCTSTR newcomment, LPCTSTR newautocat, bool addTab=true );
...
protected:
	...
	void	ShowList(uint32_t dwListIDC);
	...
	afx_msg void OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct); // ismod

// ismod [start]
public:
	void GotoFile(const CPartFile *pFile);
	void UpdateDownloadBar();
	void SetDownloadBarFile( CPartFile *file ) { m_downBarFile = file; UpdateDownloadBar(); }
	CPartFile* GetDownloadBarFile() { return m_downBarFile; }
private:
	CPartFile *m_downBarFile;
	CBitmap m_downBarBmp;
	bool m_bRedrawDownBar;
public:
