...
*/
#define URLINDICATOR	_T(".ru|http:|www.|.de |.net |.com |.org |.to |.tk |.cc |.fr |ftp:|ed2k:|https:|ftp.|.info|.biz|.uk|.eu|.es|.tv|.cn|.tw|.ws|.nu|.jp")
...
CUpDownClient::CUpDownClient(CPartFile* in_reqfile, uint16_t in_port, uint32_t in_userid,uint32_t in_serverip, uint16_t in_serverport, bool ed2kID)
{
...
//	m_nDownDatarate = 0;
//	m_nDownDataRateMS = 0;
	// netfinity: Transfer Rate BlackBox - Calculate download average for a 25s period
	m_nDownDatarate.SetSamplePeriod(500);
	m_nDownDatarate.SetSamples(50);
...
//	m_nSumForAvgDownDataRate = 0;
	...
	m_bHelloAnswerPending = true; // SiRoB: Hello-Handshake fix
	m_fFailedDownload = 0; // SiRoB: Download Collision fix
	m_fNoViewSharedFiles = 0;
	m_bMultiPacket = 0;
	md4clr(requpfileid);
	m_nTotalUDPPackets = 0;
	m_nFailedUDPPackets = 0;
	m_nUrlStartPos = (uint64_t)-1;
	// ismod: no PeerCache
	m_fNeedOurPublicIP = 0;
	m_random_update_wait = (uint32_t)(rand()/(RAND_MAX/1000));
	m_bSourceExchangeSwapped = false; // ZZ:DownloadManager
	m_dwLastTriedToConnect = ::GetTickCount()-FILEREASKTIME; // ZZ:DownloadManager
...
CUpDownClient::~CUpDownClient()
{
	...
	ASSERT( m_nConnectingState == CCS_NONE || !theApp.IsRunning() );
    ...
    m_fileReaskTimes.RemoveAll(); // ZZ:DownloadManager (one reask timestamp for each file)
...
bool CUpDownClient::ProcessHelloPacket(const uchar* pachPacket, uint32_t nSize)
{
...
bool CUpDownClient::ProcessHelloAnswer(const uchar* pachPacket, uint32_t nSize)
{
	CSafeMemFile data(pachPacket, nSize);
...
bool CUpDownClient::ProcessHelloTypePacket(CSafeMemFile* data)
{
	...
	// ismod: better IP resolution [start]
	SOCKADDR_IN sockAddr = {0};
	int nSockAddrLen = sizeof(sockAddr);
	socket->GetPeerName((SOCKADDR*)&sockAddr, &nSockAddrLen);
	SetIP(sockAddr.sin_addr.S_un.S_addr);
	...
	uint16_t nUserPort = data->ReadUInt16(); // hmm clientport is sent twice - why?
	if (bDbgInfo)
		...
		switch (temptag.GetNameID())
		{
			...
			case CT_PORT:
				if (temptag.IsInt()) {
					...
					nUserPort = (uint16_t)temptag.GetInt();
				}
				else if (bDbgInfo)
					m_strHelloInfo.AppendFormat(_T("\n  ***UnkType=%s"), temptag.GetFullInfo());
				break;
			...
			case CT_EMULE_UDPPORTS:
				...
				// 16 UDP Port
				if (temptag.IsInt()) {
					m_nKadPort = static_cast<uint16_t>(temptag.GetInt() >> 16);
					m_nUDPPort = static_cast<uint16_t>(temptag.GetInt() & 0xFFFF);
					if (bDbgInfo)
			...
			case CT_EMULE_BUDDYUDP:
				...
				// 16 BUDDY Port
				if (temptag.IsInt()) {
					m_nBuddyPort = (uint16_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case CT_EMULE_MISCOPTIONS1:
				...
				if (temptag.IsInt()) {
					...
					m_byUDPVer				= (uint8_t)((temptag.GetInt() >> 24) & 0x0f);
					m_byDataCompVer			= (uint8_t)((temptag.GetInt() >> 20) & 0x0f);
					m_bySupportSecIdent		= (uint8_t)((temptag.GetInt() >> 16) & 0x0f);
					m_bySourceExchange1Ver	= (uint8_t)((temptag.GetInt() >> 12) & 0x0f);
					m_byExtendedRequestsVer	= (uint8_t)((temptag.GetInt() >>  8) & 0x0f);
					m_byAcceptCommentVer	= (uint8_t)((temptag.GetInt() >>  4) & 0x0f);
			...
			case CT_EMULE_MISCOPTIONS2:
				...
				if (temptag.IsInt()) {
					...
					m_byKadVersion			= (uint8_t)((temptag.GetInt() >>  0) & 0x0f);
					dwEmuleTags |= 8;
					if (bDbgInfo)
			...
			case CT_EMULE_VERSION:
				...
				if (temptag.IsInt()) {
					m_byCompatibleClient = (uint8_t)((temptag.GetInt() >> 24));
	...
	//
	if (data->GetLength() - data->GetPosition() == sizeof(uint32_t)){
		uint32_t test = data->ReadUInt32();
		if (test == 'KDLM'){
			m_bIsML = true;
			if (bDbgInfo)
				m_strHelloInfo += _T("\n  ***AddData: \"MLDK\"");
		...
		if (uAddHelloDataSize == sizeof(uint32_t)){
			DWORD dwAddHelloInt32 = data->ReadUInt32();
			m_strHelloInfo.AppendFormat(_T("\n  ***AddData: uint32=%u (0x%08x)"), dwAddHelloInt32, dwAddHelloInt32);
		}
		else if (uAddHelloDataSize == sizeof(uint32_t)+sizeof(uint16_t)){
			DWORD dwAddHelloInt32 = data->ReadUInt32();
...
// ismod: set mod version [start]
    if ( m_strModVersion.Find(_T("IS Mod")) != -1 ) {
		m_bSameMod = true;

        // версия меньше чем у нас
        if ( m_strModVersion < theApp.m_strCurVersionLongMod )
            m_ModVersionFlags |= MOD_VERSION_OLDER;
        // такая же версия как у нас
        else if ( m_strModVersion == theApp.m_strCurVersionLongMod )
            m_ModVersionFlags |= MOD_VERSION_SAME;
        // версия больше чем у нас
        else if ( m_strModVersion > theApp.m_strCurVersionLongMod )
            m_ModVersionFlags |= MOD_VERSION_NEWER;

        // нестабильные версии
        if ( (m_strModVersion.Find(_T("DEBUG")) != -1) ||
            (m_strModVersion.Find(_T("alpha")) != -1) ||
            (m_strModVersion.Find(_T("beta")) != -1) ) 
            m_ModVersionFlags = MOD_VERSION_UNSTABLE;
    } else {
        m_bSameMod = false;
        m_ModVersionFlags = 0;
    }
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	CSafeMemFile data(128);
	data.WriteUInt8((uint8_t)theApp.m_uCurVersionShort);
	...
	uint32_t dwTagValue = (theApp.clientcredits->CryptoAvailable() ? 3 : 0);
...
void CUpDownClient::ProcessMuleInfoPacket(const uchar* pachPacket, uint32_t nSize)
{
	bool bDbgInfo = thePrefs.GetUseDebugDevice();
	m_strMuleInfo.Empty();
	CSafeMemFile data(pachPacket, nSize);
	...
	uint8_t protversion = data.ReadUInt8();
	if (bDbgInfo)
	...
	for (uint32_t i = 0; i < tagcount; i++)
	{
		...
		switch (temptag.GetNameID())
		{
			case ET_COMPRESSION:
				...
				if (temptag.IsInt()) {
					m_byDataCompVer = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_UDPPORT:
				...
				// Bits 15- 0: UDP port
				if (temptag.IsInt()) {
					m_nUDPPort = (uint16_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_UDPVER:
				...
				if (temptag.IsInt()) {
					m_byUDPVer = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_SOURCEEXCHANGE:
				...
				if (temptag.IsInt()) {
					m_bySourceExchange1Ver = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_COMMENTS:
				...
				if (temptag.IsInt()) {
					m_byAcceptCommentVer = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_EXTENDEDREQUEST:
				...
				if (temptag.IsInt()) {
					m_byExtendedRequestsVer = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_COMPATIBLECLIENT:
				...
				if (temptag.IsInt()) {
					m_byCompatibleClient = (uint8_t)temptag.GetInt();
					if (bDbgInfo)
			...
			case ET_FEATURES:
				...
				if (temptag.IsInt()) {
					m_bySupportSecIdent = (uint8_t)((temptag.GetInt()) & 3);
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile* data)
{
	...
	CTag tagModVersion(CT_MOD_VERSION,theApp.m_strCurVersionLongMod);
	tagModVersion.WriteTagToFile(data);
	// ismod [end]

	// eMule UDP Ports
	uint32_t kadUDPPort = 0;
	...
	CTag tagUdpPorts(CT_EMULE_UDPPORTS, 
				((uint32_t)kadUDPPort			   << 16) |
				((uint32_t)thePrefs.GetUDPPort() <<  0)
				); 
...
//					( RESERVED												)
					((uint32_t)theApp.clientlist->GetBuddy()->GetUDPPort()  ) 
					);
	...
	const UINT uPeerCache			= 0; // ismod: no PeerCache
	const UINT uUnicodeSupport		= 1;
	const UINT nAICHVer				= 0; // ismod: no aich
...
void CUpDownClient::ProcessMuleCommentPacket(const uchar* pachPacket, uint32_t nSize)
{
	if (reqfile && reqfile->IsPartFile())
	{
		CSafeMemFile data(pachPacket, nSize);
		uint8_t uRating = data.ReadUInt8();
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
	...
	if (GetDownloadState() == DS_DOWNLOADING){
		...
		if (GetDownloadState() == DS_CONNECTED){ // successfully connected, but probably didn't responsed to our filerequest
			/* ismod: do not use dead sources list
	...
	if (((GetDownloadState() == DS_REQHASHSET) || m_fHashsetRequesting) && (reqfile != NULL))
		reqfile->setHashestState(PFHS_REQUIRED);
...
bool CUpDownClient::TryToConnect(bool bIgnoreMaxCon, bool bNoCallbacks, CRuntimeClass* pClassSocket)
{
	...
	uint32_t uClientIP = (GetIP() != 0) ? GetIP() : GetConnectIP();
...
void CUpDownClient::ConnectionEstablished()
{
    ...
    // ismod: search users,folders stuff [start]
    if (m_pSearchFile && theApp.searchlist->UpdateSearch(m_pSearchFile))
    {
        if (m_pSearchFile->IsFolder() && !m_pSearchFile->GetListChildCount())
        {
            m_bDirContentsRequested = true;
            Packet* pPacket = new Packet(OP_ASKDIRCONTENTS, 16);
            md4cpy(pPacket->pBuffer, m_pSearchFile->GetFileHash());
...
void CUpDownClient::InitClientSoftwareVersion()
{
	...
	if (m_bEmuleProtocol || iHashType == SO_EMULE){
		...
		if (m_byEmuleVersion == 0){
			...
			iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s"), pszSoftware);
		}
			...
			iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v0.%u"), pszSoftware, nClientMinVersion);
		}
		else{
			...
			if (m_clientSoft == SO_EMULE)
				iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v%u.%u%c"), pszSoftware, nClientMajVersion, nClientMinVersion, _T('a') + nClientUpVersion);
			else if (m_clientSoft == SO_AMULE || nClientUpVersion != 0)
				iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v%u.%u.%u"), pszSoftware, nClientMajVersion, nClientMinVersion, nClientUpVersion);
			else if (m_clientSoft == SO_LPHANT)
			{
				if (nClientMinVersion < 10)
				    iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v%u.0%u"), pszSoftware, (nClientMajVersion-1), nClientMinVersion);
				else
					iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v%u.%u"), pszSoftware, (nClientMajVersion-1), nClientMinVersion);
			}
			else
				iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("%s v%u.%u"), pszSoftware, nClientMajVersion, nClientMinVersion);
		}
		if (iLen > 0){
	...
	if (m_bIsHybrid){
		...
		TCHAR szSoftware[128];
		if (nClientUpVersion)
			iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("eDonkeyHybrid v%u.%u.%u"), nClientMajVersion, nClientMinVersion, nClientUpVersion);
		else
			iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("eDonkeyHybrid v%u.%u"), nClientMajVersion, nClientMinVersion);
		if (iLen > 0){
			memcpy(m_strClientSoftware.GetBuffer(iLen), szSoftware, iLen*sizeof(TCHAR));
			m_strClientSoftware.ReleaseBuffer(iLen);
		}
	...
	if (m_bIsML || iHashType == SO_MLDONKEY){
		...
		m_nClientVersion = MAKE_CLIENT_VERSION(0, nClientMinVersion, 0);
		TCHAR szSoftware[128];
		int iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("MLdonkey v0.%u"), nClientMinVersion);
		if (iLen > 0){
			memcpy(m_strClientSoftware.GetBuffer(iLen), szSoftware, iLen*sizeof(TCHAR));
			m_strClientSoftware.ReleaseBuffer(iLen);
		}
	...
	if (iHashType == SO_OLDEMULE){
		...
		m_nClientVersion = MAKE_CLIENT_VERSION(0, nClientMinVersion, 0);
		TCHAR szSoftware[128];
		int iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("Old eMule v0.%u"), nClientMinVersion);
		if (iLen > 0){
			memcpy(m_strClientSoftware.GetBuffer(iLen), szSoftware, iLen*sizeof(TCHAR));
			m_strClientSoftware.ReleaseBuffer(iLen);
		}
	...
	m_nClientVersion = MAKE_CLIENT_VERSION(0, nClientMinVersion, 0);
	TCHAR szSoftware[128];
	int iLen = _sntprintf_s(szSoftware, _countof(szSoftware), _countof(szSoftware), _T("eDonkey v0.%u"), nClientMinVersion);
	if (iLen > 0){
		memcpy(m_strClientSoftware.GetBuffer(iLen), szSoftware, iLen*sizeof(TCHAR));
		m_strClientSoftware.ReleaseBuffer(iLen);
	}
...
void CUpDownClient::ProcessSharedFileList(const uchar* pachPacket, uint32_t nSize, LPCTSTR pszDirectory)
{
	if (m_iFileListRequested > 0) {
        m_iFileListRequested--;
		theApp.searchlist->ProcessSearchAnswer(pachPacket,nSize,this,NULL,pszDirectory);
		// ismod: stupid way to resort searchlist
		if(!m_iFileListRequested)
			theApp.searchlist->ResortResults();
...
void CUpDownClient::SendSignaturePacket()
{
	...
	//end v2
	uchar achBuffer[250];
	uint8_t siglen = theApp.clientcredits->CreateSignature(credits, achBuffer,  250, ChallengeIP, byChaIPKind );
	if (siglen == 0){
		ASSERT ( false );
		return;
	}
...
void CUpDownClient::ProcessPublicKeyPacket(const uchar* pachPacket, uint32_t nSize)
{
...
void CUpDownClient::ProcessSignaturePacket(const uchar* pachPacket, uint32_t nSize)
{
...
void CUpDownClient::ProcessSecIdentStatePacket(const uchar* pachPacket, uint32_t nSize)
{
	if (nSize != 5)
		return;
	if (!credits){
...
void CUpDownClient::ResetFileStatusInfo()
{
	...
	// ismod: no aich
	// ismod: for faster A4AF jump
	m_dwLastTriedToConnect = ::GetTickCount()-FILEREASKTIME;
}
...
void CUpDownClient::SendPreviewAnswer(const CKnownFile* pForFile, CxImage** imgFrames, uint8_t nCount)
{
	m_fPreviewAnsPending = 0;
...
void CUpDownClient::ProcessPreviewReq(const uchar* pachPacket, uint32_t nSize)
{
	if (nSize < 16)
...
void CUpDownClient::ProcessPreviewAnswer(const uchar* pachPacket, uint32_t nSize)
{
	if (m_fPreviewReqPending == 0)
		return;
	m_fPreviewReqPending = 0;
	...
	try{
		for (int i = 0; i != nCount; i++){
			uint32_t nImgSize = data.ReadUInt32();
			if (nImgSize > nSize)
...
void CUpDownClient::AssertValid() const
{
	...
	// ismod [start]
	(void)m_ModExtVersion;
	(void)m_bSameMod;
	(void)m_ModVersionFlags;
	// ismod [end]
...
LPCTSTR CUpDownClient::DbgGetDownloadState() const
{
	...
	if (GetDownloadState() >= _countof(apszState))
		return _T("*Unknown*");
...
LPCTSTR CUpDownClient::DbgGetUploadState() const
{
	...
	if (GetUploadState() >= _countof(apszState))
		return _T("*Unknown*");
...
LPCTSTR CUpDownClient::DbgGetKadState() const
{
	...
	if (GetKadState() >= _countof(apszState))
		return _T("*Unknown*");
...
void CUpDownClient::ProcessPublicIPAnswer(const BYTE* pbyData, UINT uSize){
	if (uSize != 4)
		throw GetResString(IDS_ERR_WRONGPACKAGESIZE);
	uint32_t dwIP = PeekUInt32(pbyData);
...
void CUpDownClient::ProcessChatMessage(CSafeMemFile* data, uint32_t nLength)
{
	//filter me?
...
void CUpDownClient::ProcessCaptchaRequest(CSafeMemFile* data){
		...
		&& theApp.emuledlg->chatwnd->chatselector.GetItemByClient(this) != NULL)
	{
		...
		uint8_t nTagCount = data->ReadUInt8();
		for (uint32_t i = 0; i < nTagCount; i++)
			CTag tag(data, true);
		// sanitize checks - we want a small captcha not a wallpaper
		uint32_t nSize = (uint32_t)(data->GetLength() - data->GetPosition());
...
void CUpDownClient::ProcessCaptchaReqRes(uint8_t nStatus)
{
...
void CUpDownClient::ProcessFirewallCheckUDPRequest(CSafeMemFile* data){
	...
	uint16_t nRemoteInternPort = data->ReadUInt16();
	uint16_t nRemoteExternPort = data->ReadUInt16();
	uint32_t dwSenderKey = data->ReadUInt32();
...
void CUpDownClient::SetConnectOptions(uint8_t byOptions, bool bEncryption, bool bCallback)
{
...
// ismod [start]
COLORREF CUpDownClient::GetClientColor() const
{
	if (thePrefs.GetUseClientColors())
	{
		// старая версия или такая же, но не стабильная
        if ( ( GetModVersionFlags() & MOD_VERSION_OLDER ) ||
             ( GetModVersionFlags() & (MOD_VERSION_SAME | MOD_VERSION_UNSTABLE) ) )
            return RGB(60,140,100); // green
        // такая же версия
		else if ( GetModVersionFlags() & MOD_VERSION_SAME )
            return RGB(0,0,0); // black
        // новая нестабильная версия
        else if ( GetModVersionFlags() & (MOD_VERSION_NEWER | MOD_VERSION_UNSTABLE) )
            return RGB(100,60,140); // purple
        // новая версия
		else if ( GetModVersionFlags() & MOD_VERSION_NEWER )
            return RGB(200,30,30); // red
        // не такой же мод
        else 
            return RGB(80,80,80); // grey
	}
	return RGB(0,0,0);
}

void CUpDownClient::ProcessDirContentsList(const uchar* pachPacket, uint32_t nSize)
{
    if (m_bDirContentsRequested)
    {
        m_bDirContentsRequested = false;
        theApp.searchlist->ProcessDirContentsAnswer(pachPacket, nSize, this);
    }
}