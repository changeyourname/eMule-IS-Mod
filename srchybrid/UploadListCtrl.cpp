...
void CUploadListCtrl::Init()
{
	...
	InsertColumn(i++, GetResString(IDS_DL_TRANSF),	LVCFMT_RIGHT, DFLT_DATARATE_COL_WIDTH);
	InsertColumn(i++, GetResString(IDS_WAITED),		LVCFMT_LEFT,   60);
	InsertColumn(i++, GetResString(IDS_UPLOADTIME),	LVCFMT_LEFT,   80);
	InsertColumn(i++, GetResString(IDS_STATUS),		LVCFMT_LEFT,  100);
...
int CUploadListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	switch (iColumn)
	{
		...
		case 4:
			iResult = CompareUnsigned64(item1->GetWaitTime(), item2->GetWaitTime());
			break;
...
void CUploadListCtrl::AddClient(const CUpDownClient *client)
{
	...
	Update(iItem);
	theApp.emuledlg->transferwnd->UpdateListCount(CTransferDlg::wnd2Uploading, iItemCount + 1);
}
...
void CUploadListCtrl::RemoveClient(const CUpDownClient *client)
{
	...
	if (result != -1) {
		DeleteItem(result);
		theApp.emuledlg->transferwnd->UpdateListCount(CTransferDlg::wnd2Uploading);
	}
}
