...
CDirectoryItem::CDirectoryItem(LPCTSTR pszFullPath, HTREEITEM htItem, ESpecialDirectoryItems eItemType, int nCatFilter){
	m_htItem = htItem;
...
HTREEITEM CDirectoryItem::FindItem(const CDirectoryItem* pContentToFind) const
{
...
void CSharedDirsTreeCtrl::SetAllIcons()
{
	...
	if (pCurImageList != NULL && pCurImageList->GetImageCount() >= 7)
	{
		...
		if (strTempDir.Right(1) != _T("\\"))
			strTempDir += _T("\\");
		int nImage = static_cast<int>(theApp.GetFileTypeSystemImageIdx(strTempDir));			// 5: System Folder Icon
		...
		pCurImageList->Replace(15, CTempIconLoader(_T("SearchFileType_EmuleCollection")));
		// ismod [end] shared files by type
	}
	else
	{
		...
		iml.Add(CTempIconLoader(_T("AABCOLLECTIONFILETYPE"), 16, 16));
		// ismod [end] shared files by type
		...
		switch( pCurrent->m_eItemType ){
			...
			case SDI_ALL:
				...
				for ( int i = 0; i < _countof(_aEd2kTypeView); i++ ) {
					CDirectoryItem *pEd2kType = new CDirectoryItem(_T(""), 0, SDI_ED2KFILETYPE, _aEd2kTypeView[i].eType);
					pEd2kType->m_htItem = InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, GetResString(_aEd2kTypeView[i].uStringId), i+8, i+8, 0, 0, (LPARAM)pEd2kType, pCurrent->m_htItem, TVI_LAST);
...
{	
	...
	if (pSelectedDir != NULL && pSelectedDir->m_eItemType != SDI_UNSHAREDDIRECTORY && pSelectedDir->m_eItemType != SDI_FILESYSTEMPARENT){
		...
		m_SharedFilesMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_PrioMenu.m_hMenu) & UINT_MAX), iSelectedItems > 0 ? MF_ENABLED : MF_GRAYED);
		...
		switch (wParam){
			case MP_OPENFOLDER:
				if (pSelectedDir && !pSelectedDir->m_strFullPath.IsEmpty() /*&& pSelectedDir->m_eItemType == SDI_NO*/){
					...
					theDesktop.ShellExecute(_T("open"), pSelectedDir->m_strFullPath);
				}
...
bool CSharedDirsTreeCtrl::FileSystemTreeIsShared(const CString& rstrDir)
{
	for (POSITION pos = m_strliSharedDirs.GetHeadPosition(); pos != NULL; )
	{
		if (CompareDirectories(m_strliSharedDirs.GetNext(pos), rstrDir) == 0)
			return true;
	}

	// ismod: incoming and categories
	for(int i = 0; i<thePrefs.GetCatCount(); ++i) {
		if ( CompareDirectories(thePrefs.GetCategory(i)->strIncomingPath, rstrDir) == 0 )
			return true;
	}
...
// ismod: renamed
bool CSharedDirsTreeCtrl::ShowDirectory(const CString& strDir, ESpecialDirectoryItems eParentItemType)
{
	// expand directories untill we find our target directory and select it
	POSITION pos = m_pRootDirectoryItem->liSubDirectories.GetHeadPosition();
	while (pos != NULL)
	{
		...
		if (pTemp->m_eItemType == eParentItemType)
		{
			...
			else // search for the fitting sub dir
			{
				...
				while (pos2 != NULL)
				{
					...
					if (strDir.CompareNoCase(pTemp2->m_strFullPath) == 0)
					{
						...
						return true;							
					}else if (strDir.Find(pTemp2->m_strFullPath + _T('\\')) == 0)
					{
						Select(pTemp2->m_htItem, TVGN_CARET);
						EnsureVisible(pTemp2->m_htItem);
						pTemp = pTemp2;
						pos2 = pTemp->liSubDirectories.GetHeadPosition();
					}
				}
				return false;
			}
