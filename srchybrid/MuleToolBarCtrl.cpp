...
void CMuleToolbarCtrl::Init(void)
{
	...
	_tcscpy_s(cButtonStrings, _countof(cButtonStrings), GetResString(IDS_MAIN_BTN_CONNECT));
    ...
    // ismod
    lLen2 = _tcslen(GetResString(IDS_MOD_EM_SCT)) + 1;
    memcpy(cButtonStrings+lLen, GetResString(IDS_MOD_EM_SCT), lLen2*sizeof(TCHAR));
    lLen += lLen2;
    ++m_buttoncount;

	lLen2 = _tcslen(GetResString(IDS_MOD_EM_XCAT)) + 1;
	memcpy(cButtonStrings+lLen, GetResString(IDS_MOD_EM_XCAT), lLen2*sizeof(TCHAR));
	lLen += lLen2;
	++m_buttoncount;
	...
	for (int i = 0; i < config.GetLength(); i += 2)
	{
        ...
        else if( TBButtons[index].idCommand == TBBTN_KAD)
            continue;
...
void CMuleToolbarCtrl::SetAllButtonsStrings()
{
	...
	_sntprintf_s(TBStrings[0], _countof(TBStrings[0]), _countof(TBStrings[0]), _T("%s"), buffer);
	...
	{
		_sntprintf_s(TBStrings[i], _countof(TBStrings[0]), _countof(TBStrings[0]), _T("%s"), GetResString(TBStringIDs[i-1]));
...
void CMuleToolbarCtrl::SetAllButtonsWidth()
{
	...
	{
		int iSmallIconsButtonHeight = GetSystemMetrics(SM_CYSCREEN) <= 600 ? 16 : 28;
...
void CMuleToolbarCtrl::ChangeToolbarBitmap(const CString& path, bool bRefresh)
{
	...
	if (!path.IsEmpty() && Bitmap.LoadImage(path))
	{
		...
		if (bm.bmWidth == (NUM_BUTTON_BITMAPS-2)*m_sizBtnBmp.cx && bm.bmHeight == m_sizBtnBmp.cy)
		{
			bool bAlpha = bm.bmBitsPixel > 24;
			if (ImageList.Create(m_sizBtnBmp.cx, bm.bmHeight, bAlpha ? ILC_COLOR32 : (ILC_COLOR32 | ILC_MASK), 0, 1))
			{
				ImageList.Add(&Bitmap,  bAlpha ? 0xFF000000 : RGB(255, 0, 255));
                ImageList.Add(CTempIconLoader(_T("SCT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy)); // add sct
				ImageList.Add(CTempIconLoader(_T("XCAT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy)); // add xcatalog
	...
	if (!bResult)
	{
		...
		ImageList.Create(m_sizBtnBmp.cx, m_sizBtnBmp.cy, ILC_COLOR32 | ILC_MASK, 0, 1);
        ...
        ImageList.Add(CTempIconLoader(_T("SCT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
		ImageList.Add(CTempIconLoader(_T("XCAT"), m_sizBtnBmp.cx, m_sizBtnBmp.cy));
...
BOOL CMuleToolbarCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	switch (wParam)
	{
		case MP_SELECTTOOLBARBITMAPDIR:{
			...
			_sntprintf_s(buffer, _countof(buffer), _countof(buffer), _T("%s"), thePrefs.GetMuleDirectory(EMULE_TOOLBARDIR));
		...
		case MP_SELECT_SKIN_DIR:{
			TCHAR buffer[MAX_PATH];
			_sntprintf_s(buffer, _countof(buffer), _countof(buffer), _T("%s"), thePrefs.GetMuleDirectory(EMULE_SKINDIR, false));
			buffer[_countof(buffer) - 1] = _T('\0');
...
void CMuleToolbarCtrl::Refresh()
{
	...
	else
	{
		...
		CWnd* wnds[] =
		{
            ...
            (CWnd*)theApp.emuledlg->sctwnd,
			(CWnd*)theApp.emuledlg->xcatwnd
		};
