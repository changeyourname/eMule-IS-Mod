...
#define MAX_FILES_PER_PACKET	198
...
public:
	...
	int GetCount() const { return (int) m_lstKeywords.GetCount(); }
...
IMPLEMENT_DYNCREATE(CAddFileThread, CSafeThread)
...
int CAddFileThread::Run()
{
	...
	if ( !(m_pOwner || m_partfile) || m_strFilename.IsEmpty() || !theApp.IsRunning() )
		return 0;
	...
	CSingleLock lock(&theApp.hashing_mut, TRUE); // only one filehash at a time
	...
	if ( theApp.IsRunning() && newrecord->CreateFromFile(m_strDirectory, m_strFilename, m_partfile) ) // in case of shutdown while still hashing
	{
	...
	else
	{
		...
		// inform main program of hash failure
		if ( m_pOwner && theApp.IsRunning() )
		{
...
CSharedFileList::CSharedFileList(CServerConnect* in_server)
{
	...
	m_currentlyHashingFile = NULL;
    // ismod [start]
    m_bUserPublished = false;
	m_uTotalShareSize = 0ui64;
    // ismod [end]
...
CSharedFileList::~CSharedFileList(){
	while ( !m_waitingForHashList.empty() ) {
		UnknownFile_Struct* nextfile = m_waitingForHashList.back();
		m_waitingForHashList.pop_back();
		delete nextfile;
	}
	if ( NULL != m_currentlyHashingFile ) {
		delete m_currentlyHashingFile;
		m_currentlyHashingFile = NULL;
	}
	delete m_keywords;

	// ismod: delete shared dirs [start]
	for ( auto it = sharedDirs.begin(); it != sharedDirs.end(); ++it ) {
		// don't delete collection file here
		delete it->second;
	}
	sharedDirs.clear();
	sharedDirsPseudo.clear();
	// ismod [end]

#ifdef IS_VERSION_BETA
...
void CSharedFileList::CopySharedFileMap(CRBMap<CCKey,CKnownFile*,CCKeyTrait> &Files_Map)
{
	if (!m_Files_map.IsEmpty())
	{
		POSITION pos = m_Files_map.GetHeadPosition();
		while (pos)
		{
			CCKey key;
			CKnownFile* cur_file;
...
void CSharedFileList::FindSharedFiles()
{
	...
	// ismod: clear shared dirs [start]
	CFileFind dirsFf;
	CString strCollectionCache = thePrefs.GetMuleDirectory(EMULE_COLLECTIONDIR) + _T("\\*");
	bool end = !dirsFf.FindFile(strCollectionCache);
	while (!end)
	{
		end = !dirsFf.FindNextFile();
		DeleteFile(dirsFf.GetFilePath());
	}
	dirsFf.Close();

	SharedDirsMap::iterator sharedDirIter = sharedDirs.begin();
	while ( sharedDirIter != sharedDirs.end() ) {
		delete sharedDirIter->second;
		++sharedDirIter;
	}
	sharedDirs.clear();
	sharedDirsPseudo.clear();
	...
	*/
	...
	// ismod: add shared dirs (collections dir) [start]
	sharedDirIter = sharedDirs.begin();
	while ( sharedDirIter != sharedDirs.end() ) {
		sharedDirIter->second->createCollection();
		sharedDirsPseudo[sharedDirIter->second->collectionFileName()] = sharedDirIter->second;
		++sharedDirIter;
	}
	AddFilesFromDirectory(thePrefs.GetMuleDirectory(EMULE_COLLECTIONDIR, true));
	// ismod [end]

	// khaos::kmod-
	if ( m_waitingForHashList.empty() )
		AddLogLine(false,GetResString(IDS_SHAREDFOUND), m_Files_map.GetCount());
	else
		AddLogLine(false,GetResString(IDS_SHAREDFOUNDHASHING), m_Files_map.GetCount(), m_waitingForHashList.size());
...
{
	ASSERT( pFile->hashset().count() == pFile->GetED2KPartCount() );	// SLUGFILLER: SafeHash - use GetED2KPartCount
	ASSERT( !pFile->IsKindOf(RUNTIME_CLASS(CPartFile)) || STATIC_DOWNCAST(CPartFile, pFile)->hashsetShate()==PFHS_OK );
	...
	// ismod: shared dirs [start]
	if( !pFile->IsPartFile() &&
		(thePrefs.GetMuleDirectory(EMULE_COLLECTIONDIR) != pFile->GetPath()) 
	  )
	{
		SharedDirsMap::const_iterator iter = sharedDirs.find(pFile->GetPath());
		SharedDir* sharedDir = NULL;
		if ( iter == sharedDirs.end() ) {
			sharedDir = new SharedDir(pFile->GetPath());
			sharedDirs[pFile->GetPath()] = sharedDir;
		} else {
			sharedDir = iter->second;
		}
		sharedDir->addFile(pFile);
	}
	// increase total share size
	m_uTotalShareSize += (uint64_t)pFile->GetFileSize();
	// ismod [end]
...
bool CSharedFileList::RemoveFile(CKnownFile* pFile, bool bDeleted)
{
	...
	// ismod: shared dirs [start]
	SharedDirsMap::const_iterator iter = sharedDirs.find(pFile->GetFilePath());
	if ( iter != sharedDirs.end() ) {
		SharedDir* sharedDir = iter->second;
		sharedDir->removeFile(pFile);
		if ( !sharedDir->filesCount() )
		{
			sharedDirs.erase(sharedDirs.find(pFile->GetFilePath()));
			if ( !sharedDir->collectionFileName().IsEmpty() )
				sharedDirsPseudo.erase(sharedDir->collectionFileName());
			delete sharedDir; 
		}
	}
	// deacease total share size
	m_uTotalShareSize -= (uint64_t)pFile->GetFileSize();
	// ismod [end]
 ...
 * 4 => 0
 */
uint8_t GetRealPrio( uint8_t in )
{
    static uint8_t real_prio[] = { 1, 2, 3, 4, 0 };
    ASSERT(in < _countof(real_prio));
    return real_prio[in];
}

void CSharedFileList::SendListToServer()
{
    if ( (m_Files_map.IsEmpty() && m_bUserPublished) || !server->IsConnected() ) {
		return;
	}
	
	CServer* pCurServer = server->GetCurrentServer();
	CSafeMemFile files(1024);
	std::list<CKnownFile*> sortedList;

	for ( POSITION pos = m_Files_map.GetHeadPosition(); pos!=0; ) {
        CCKey bufKey;
        CKnownFile* cur_file = NULL;
		bool added = false;
		m_Files_map.GetNextAssoc(pos, bufKey, cur_file);

		// insert sort into sortedList
		if ( !cur_file->GetPublishedED2K() &&
             (!cur_file->IsLargeFile() || (pCurServer != NULL && pCurServer->SupportsLargeFilesTCP())) )
		{
            for ( auto it = sortedList.begin(); it != sortedList.end(); ++it ) {
                if ( GetRealPrio((*it)->GetUpPriority()) <= GetRealPrio(cur_file->GetUpPriority()) ) {
                    sortedList.insert(it, cur_file);
                    added = true;
                    break;
                }
            }
            if ( !added ) {
                sortedList.push_back(cur_file);
            }
		}
	}
	
	// add to packet
	size_t limit = pCurServer ? pCurServer->GetSoftFiles() : 0;
	if ( limit == 0 || limit > MAX_FILES_PER_PACKET ) {
		limit = MAX_FILES_PER_PACKET;
	}
	if ( limit > sortedList.size() ) {
		limit = sortedList.size();
        if ( 0 == limit && m_bUserPublished ) {
			m_lastPublishED2KFlag = false;
			return;
		}
	}
	// ismod: publish user
	if (!m_bUserPublished) limit++;
	files.WriteUInt32(limit);
	uint32_t count = 0;
    // ismod: publish user [start]
    if ( !m_bUserPublished ) {
        count++;
        CreateOfferedUserPacket(&files, pCurServer);
        m_bUserPublished = true;
    }
    // ismod [end]
	for ( auto it = sortedList.begin(); it != sortedList.end() && count < limit; ++it,++count ) {
		CKnownFile* file = *it;
											  ...
											  CServer* pServer, CUpDownClient* pClient)
{
	...
	// ismod: shared dirs [start]
	bool isSharedDirCollection = false;
	CString strFileName = cur_file->GetFileName();
    Utils::fixNationalChars(strFileName);
	if ( thePrefs.GetMuleDirectory(EMULE_COLLECTIONDIR) == cur_file->GetPath() ) {
		isSharedDirCollection = true;
		strFileName.Replace(_T('-'), _T('\\'));
	}
	// ismod [end]

    tags.Add(new CTag(FT_FILENAME, strFileName));

	if (!cur_file->IsLargeFile()){
		tags.Add(new CTag(FT_FILESIZE, (uint32_t)(uint64_t)cur_file->GetFileSize()));
	}
	else{
		...
		if (pServer != NULL){
			if (!pServer->SupportsLargeFilesTCP()){
				...
				tags.Add(new CTag(FT_FILESIZE, (uint32_t)(uint64_t)cur_file->GetFileSize()));
				tags.Add(new CTag(FT_FILESIZE_HI, (uint32_t)((uint64_t)cur_file->GetFileSize() >> 32)));
			}
		}
		else{
	...
	if (cur_file->GetFileRating()) {
		uint32_t uRatingVal = cur_file->GetFileRating();
		if (pClient) {
	...
	// ismod: shared dirs [start]
	if ( isSharedDirCollection && 
		 pServer && (pServer->GetTCPFlags() & SRV_TCPFLG_COMPRESSION) )
	{
		SharedDir* sharedDir = sharedDirsPseudo[cur_file->GetFileName()];
		uint64_t totalsize = sharedDir->totalSize();

		if ((pServer->GetTCPFlags() & SRV_TCPFLG_NEWTAGS)) {
			tags.Add(new CTag(FT_MEDIA_LENGTH, ((uint32_t*)&totalsize)[0]));
			tags.Add(new CTag(FT_MEDIA_BITRATE, ((uint32_t*)&totalsize)[1]));
		} else {
			tags.Add(new CTag(FT_ED2K_MEDIA_LENGTH, ((uint32_t*)&totalsize)[0]));
			tags.Add(new CTag(FT_ED2K_MEDIA_BITRATE, ((uint32_t*)&totalsize)[1]));
		}
	} else
	// ismod: shared dirs [end]
	...
	if (cur_file->GetMetaDataVer() > 0)
	{
		...
		for (int i = 0; i < _countof(_aMetaTags); i++)
		{
...
uint64_t CSharedFileList::GetDatasize(uint64_t &pbytesLargest) const
{
	pbytesLargest=0;
	// <-----khaos-
	uint64_t fsize;
	...
	for (POSITION pos = m_Files_map.GetHeadPosition();pos != 0;){
		m_Files_map.GetNextAssoc(pos,bufKey,cur_file);
		fsize += (uint64_t)cur_file->GetFileSize();
...
void CSharedFileList::HashNextFile()
{
	if ( !theApp.emuledlg || !::IsWindow(theApp.emuledlg->m_hWnd) ) // wait for the dialog to open
		return;
	if ( theApp.emuledlg && theApp.IsRunning() )
		theApp.emuledlg->sharedfileswnd->sharedfilesctrl.ShowFilesCount();
	if ( NULL != m_currentlyHashingFile ) // one hash at a time
		return;
	if ( m_waitingForHashList.empty() )
		return;
	m_currentlyHashingFile = m_waitingForHashList.front();
	m_waitingForHashList.pop_front();
	...
	if ( NULL == addfilethread )
		throw CString(_T(__FUNCTION__) _T(": Failed to create worker thread for CAddFileThread!"));
	addfilethread->SetValues(this, m_currentlyHashingFile->strDirectory, m_currentlyHashingFile->strName, m_currentlyHashingFile->strSharedDirectory);
...
{
	for (auto it = m_waitingForHashList.begin(); it != m_waitingForHashList.end(); ++it ) {
		const UnknownFile_Struct* pFile = *it;
		if (!pFile->strName.CompareNoCase(rstrName) && !CompareDirectories(pFile->strDirectory, rstrDirectory))
			return true;
	}
	if ( NULL != m_currentlyHashingFile
		&& !m_currentlyHashingFile->strName.CompareNoCase(rstrName)
		&& !CompareDirectories(m_currentlyHashingFile->strDirectory, rstrDirectory) ) {
			return true;
	}
	return false;
}
...
{
	if ( NULL != m_currentlyHashingFile
		&& !m_currentlyHashingFile->strName.CompareNoCase(hashed->GetFileName())
		&& !CompareDirectories(m_currentlyHashingFile->strDirectory, hashed->GetPath()) ) {
			delete m_currentlyHashingFile;
			m_currentlyHashingFile = NULL;
...
{
	if ( NULL != m_currentlyHashingFile
		&& !m_currentlyHashingFile->strName.CompareNoCase(hashed->strName)
		&& !CompareDirectories(m_currentlyHashingFile->strDirectory, hashed->strDirectory) ) {
			delete m_currentlyHashingFile;
			m_currentlyHashingFile = NULL;
			HashNextFile(); // start next hash if possible, but only if a previous hash finished
	}
...
//								ASSERT( IsFilePtrInList(aFiles[f]) );
								...
								{
									count++;
									pSearch->AddFileID(aFiles[f]->GetFileHash());
									if( count > 150 )
									{
...
bool CSharedFileList::ShouldBeShared(const CString &strPath, const CString &strFilePath, bool bMustBeShared) const
{
	...
	if (CompareDirectories(strPath, thePrefs.GetMuleDirectory(EMULE_COLLECTIONDIR)) == 0)
		return true;
...
bool CSharedFileList::ExcludeFile(CString strFilePath)
{
	...
	CKnownFile* cur_file;
	CCKey bufKey;
	for (POSITION pos = m_Files_map.GetHeadPosition();pos != NULL;)
	{
		m_Files_map.GetNextAssoc(pos,bufKey,cur_file);
...
void CSharedFileList::CheckAndAddSingleFile(const CFileFind& ff){
	...
	// ismod: ignore *.!ut files
	if ( ff.GetFileName().Right(4) == _T(".!ut") )
		return;
	...
	time_t fdate = tFoundFileTime.GetTime();
	if (fdate == 0)
		fdate = -1;
	if (fdate == -1){
		if (thePrefs.GetVerbose())
	...
	else
	{
		...
		if (!IsHashing(strFoundDirectory, strFoundFileName) && !thePrefs.IsTempFile(strFoundDirectory, strFoundFileName)){
			...
			m_waitingForHashList.push_back(tohash);
		}
		else
...
CString CSharedFileList::GetPseudoDirName(const CString& strDirectoryName)
{
	...
	for (POSITION pos = m_mapPseudoDirNames.GetHeadPosition(); pos != NULL;)
	{
...
bool CSharedFileList::GetPopularityRank(const CKnownFile* pFile, uint32_t& rnOutSession, uint32_t& rnOutTotal) const
{
	rnOutSession = 0;
	rnOutTotal = 0;
	if (GetFileByID(pFile->GetFileHash()) == NULL)
	{
		ASSERT( false );
		return false;
	}
...
// ismod [start]
void CSharedFileList::CreateOfferedUserPacket(CSafeMemFile* files, CServer* pServer) const
{
	files->WriteHash16(thePrefs.GetUserHash());
	uint32_t nClientID = 0;
	uint16_t nClientPort = 0;
	if (pServer)
	{
	...
	tags.Add(new CTag(FT_FILENAME, _T("+++USERNICK+++ ") + thePrefs.GetUserNick()));
	
	// file size is client id
	if (theApp.serverconnect->IsConnected())
		tags.Add(new CTag(FT_FILESIZE, theApp.serverconnect->GetClientID()));
	else
        tags.Add(new CTag(FT_FILESIZE, 1));

	// MAKE_UINT64(FT_MEDIA_LENGTH, FT_MEDIA_BITRATE) - total share size
	uint64_t largest = 0;
	uint64_t totalsize = theApp.sharedfiles->GetDatasize(largest);
	...
	for (int i = 0; i < tags.GetSize(); i++)
	{
		const CTag* pTag = tags[i];
		if (pServer && (pServer->GetTCPFlags() & SRV_TCPFLG_NEWTAGS))
			pTag->WriteNewEd2kTag(files, utf8strRaw);
		else
			pTag->WriteTagToFile(files, utf8strRaw);
		delete pTag;
	}
}