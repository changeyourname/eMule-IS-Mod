...
BOOL CPPgStats::OnApply()
{
	...
	if (m_bModified)
	{
		...
		if (statsMax > thePrefs.GetMaxConnections() + 5)
		{
			...
			_sntprintf_s(buffer, _countof(buffer), _countof(buffer), _T("%d"), thePrefs.GetStatsMax());
...
void CPPgStats::Localize(void)
{
	if (m_hWnd)
	{
		...
		iItem = m_colors.AddString(GetResString(IDS_MOD_SP_ICONBAR_UP));	m_colors.SetItemData(iItem, 15);
