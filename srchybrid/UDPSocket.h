...
public:
	...
	void SendPacket(Packet* packet, CServer* pServer, uint16_t nSpecialPort = 0, BYTE* pRawPacket = 0, uint32_t nRawLen = 0);
...
private:
	...
	void SendBuffer(uint32_t nIP, uint16_t nPort, BYTE* pPacket, UINT uSize);
	bool ProcessPacket(const BYTE* packet, UINT size, UINT opcode, uint32_t nIP, uint16_t nUDPPort);
	void ProcessPacketError(UINT size, UINT opcode, uint32_t nIP, uint16_t nUDPPort, LPCTSTR pszError);
	bool IsBusy() const { return m_bWouldBlock; }
	int SendTo(BYTE* lpBuf, int nBufLen, uint32_t dwIP, uint16_t nPort);
