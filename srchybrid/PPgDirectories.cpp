...
BOOL CPPgDirectories::OnInitDialog()
{
	...
	((CEdit*)GetDlgItem(IDC_TEMPFILES))->SetLimitText(MAX_PATH);

    AddBuddyButton(GetDlgItem(IDC_INCFILES)->m_hWnd, ::GetDlgItem(m_hWnd, IDC_SELINCDIR));
	SetButtonIcon(::GetDlgItem(m_hWnd, IDC_SELINCDIR), m_icoBrowse);
	
	AddBuddyButton(GetDlgItem(IDC_TEMPFILES)->m_hWnd, ::GetDlgItem(m_hWnd, IDC_SELTEMPDIR));
	SetButtonIcon(::GetDlgItem(m_hWnd, IDC_SELTEMPDIR), m_icoBrowse);
	/* ismod: no UNC, no shareselector
