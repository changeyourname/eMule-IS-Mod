...
void CPerfLog::Startup()
{
	...
	if (m_eMode != None)
	{
		...
		CString strDefFilePath = thePrefs.GetMuleDirectory(EMULE_LOGDIR);
...
void CPerfLog::LogSamples()
{
	...
	uint64_t nDnOHTotal = theStats.GetDownDataOverheadFileRequest() + 
	...
	uint64_t nUpOHTotal = theStats.GetUpDataOverheadFileRequest() + 
