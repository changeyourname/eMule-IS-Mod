...
void CTrayDialog::TraySetToolTip(LPCTSTR lpszToolTip)
{
	...
	_tcsncpy_s(m_nidIconData.szTip, _countof(m_nidIconData.szTip), lpszToolTip, NOTIFYICONDATA_V2_TIP_SIZE);
...
BOOL CTrayDialog::TrayHide()
{
	...
	*/
	if (!thePrefs.GetAlwaysShowTrayIcon() && m_bTrayIconVisible)
	{
		bSuccess = Shell_NotifyIcon(NIM_DELETE, &m_nidIconData);
		if (bSuccess)
			m_bTrayIconVisible = FALSE;
...
void CTrayDialog::OnTimer(WPARAM nIDEvent)
{
