...
BOOL CTaskbarNotifier::LoadConfiguration(LPCTSTR pszFilePath)
{
	...
	_tcsncpy_s(szConfigDir, _countof(szConfigDir), pszFilePath, _countof(szConfigDir));
...
BOOL CTaskbarNotifier::SetBitmap(UINT nBitmapID, int red, int green, int blue)
{
	...
	return SetBitmap(&bitmap, red, green, blue);
}
...
BOOL CTaskbarNotifier::SetBitmap(LPCTSTR pszFileName, int red, int green, int blue)
{
	...
	return SetBitmap((CBitmap*)&Bitmap, red, green, blue);
}
...
void CTaskbarNotifier::Show(LPCTSTR pszCaption, int nMsgType, LPCTSTR pszLink, BOOL bAutoClose)
{
	...
	if (dwTimeToShow > m_dwTimerPrecision) {
		nEvents = max<UINT>(min<UINT>((dwTimeToShow / m_dwTimerPrecision) / 2, nBitmapSize), 1); //<<-- enkeyDEV(Ottavio84) -Reduced frames of a half-
	...
	if (dwTimeToHide > m_dwTimerPrecision) {
		nEvents = max<UINT>(min<UINT>((dwTimeToHide / m_dwTimerPrecision / 2), nBitmapSize), 1); //<<-- enkeyDEV(Ottavio84) -Reduced frames of a half-
