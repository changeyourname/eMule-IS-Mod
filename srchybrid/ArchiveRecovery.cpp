...
void CArchiveRecovery::recover(CPartFile *partFile, bool preview, bool bCreatePartFileCopy)
{
	...
	GapList *filled = new GapList;
	partFile->GetFilledList(filled);
#ifdef _DEBUG
	{
		int i = 0;
		TRACE("%s: filled\n", __FUNCTION__);
		for ( GapList::iterator it = filled->begin(); it != filled->end(); ++it ) {
			const Gap_Struct* gap = *it;
	...
	if (!CSafeThread::BeginThread(run, (LPVOID)tp)){
...
bool CArchiveRecovery::performRecovery(CPartFile *partFile, GapList *filled, 
									   bool preview, bool bCreatePartFileCopy)
{
	...
	try
	{
		...
		if (success)
		{
			...
			theApp.QueueDebugLogLine(false, _T("Archive recovery: Part file size: %s, temp. archive file size: %s (%.1f%%)"), CastItoXBytes(partFile->GetFileSize()), CastItoXBytes(ulTempFileSize), partFile->GetFileSize() > (uint64_t)0 ? (ulTempFileSize * 100.0 / (uint64_t)partFile->GetFileSize()) : 0.0);
								  ...
								  GapList *filled, bool fullSize)
{
	...
	try
	{
		...
		if (fullSize && readZipCentralDirectory(zipInput, centralDirectoryEntries, filled))
		{
			...
			bool deleteCD;
			for (int i= (int) centralDirectoryEntries->GetCount(); i>0; i--)
			{
				deleteCD = false;
				cdEntry = centralDirectoryEntries->GetAt(pos);
				uint32_t lenEntry = sizeof(ZIP_Entry) + cdEntry->lenFilename + cdEntry->lenExtraField + cdEntry->lenCompressed;
		...
		{		
			...
			{
				fill = *it;
				uint32_t filePos = (UINT)zipInput->GetPosition();
				...
				for (;;)
				{
					...
					if (!scanForZipMarker(zipInput, aitp, (uint32_t)ZIP_LOCAL_HEADER_MAGIC, (UINT)(fill->end - zipInput->GetPosition() + 1)))
						break;
		...
		uint32_t startOffset = (UINT)zipOutput->GetPosition();

		// Write all central directory entries
		fileCount = (long) centralDirectoryEntries->GetCount();
		if (fileCount > 0)
		{
			ZIP_CentralDirectory *cdEntry;
			POSITION pos = centralDirectoryEntries->GetHeadPosition();
			...
			// Remember offset before CD entries
			uint32_t endOffset = (UINT)zipOutput->GetPosition();
			...
			writeUInt16(zipOutput, (uint16_t)fileCount);
			writeUInt16(zipOutput, (uint16_t)fileCount);
			writeUInt32(zipOutput, endOffset - startOffset);
			writeUInt32(zipOutput, startOffset);
			writeUInt16(zipOutput, (uint16_t)strlen(ZIP_COMMENT));
...
bool CArchiveRecovery::readZipCentralDirectory(CFile *zipInput, CTypedPtrList<CPtrList, ZIP_CentralDirectory*> *centralDirectoryEntries, GapList *filled)
{
	bool retVal = false;
	try
	{
		...
		if (!(readUInt32(zipInput) == ZIP_END_CD_MAGIC))
		{
			...
			if (!scanForZipMarker(zipInput, NULL, (uint32_t)ZIP_END_CD_MAGIC, 1046))
				return false;
			// Skip it again
			readUInt32(zipInput);
		}
		...
		uint32_t startOffset = readUInt32(zipInput);
...
bool CArchiveRecovery::processZipEntry(CFile *zipInput, CFile *zipOutput, uint32_t available, CTypedPtrList<CPtrList, ZIP_CentralDirectory*> *centralDirectoryEntries)
{
	if (available < 26)
		return false;
	...
	try
	{
		...
		if (centralDirectoryEntries != NULL)
		{
			...
			if (zipOutput!=NULL) {
				cdEntry->lenComment = (uint16_t)strlen(ZIP_COMMENT);
				cdEntry->comment = new BYTE[cdEntry->lenComment];
...
bool CArchiveRecovery::CopyFile(CPartFile *partFile, GapList *filled, CString tempFileName)
{
	bool retVal = false;
	try
	{
		...
		destFile.SetLength( filled->back()->end );
...
bool CArchiveRecovery::IsFilled(uint32_t start, uint32_t end, GapList *filled)
{
	for ( GapList::iterator it = filled->begin(); it != filled->begin(); ++it ) {
		Gap_Struct *fill = *it;
		if (fill->start > start)
			return false;
		if (fill->end >= end)
...
bool CArchiveRecovery::scanForZipMarker(CFile *input, archiveScannerThreadParams_s* aitp, uint32_t marker, uint32_t available)
{
	try
	{
			...
			while (foundPos != NULL)
			{
				...
				if ((crc & 0xFFFF) == headCRC)
				{
					...
					if (validateRarFileBlock(retVal))
					{
						...
						uint32_t dataLength = retVal->PACK_SIZE;
...
void CArchiveRecovery::writeRarBlock(CFile *input, CFile *output, RAR_BlockFile *block)
{
	...
	try
	{
		...
		uint32_t lenToCopy = block->dataLength;
		if (lenToCopy > 0)
		{
								  ...
								  GapList *filled)
{
	...
	try
	{
		...
		if (IsFilled(0,32,filled)) {
			...
			hdrread+=aceInput->Read((void*)acehdr, sizeof(ACE_ARCHIVEHEADER) - (3*sizeof(char*)) - sizeof(uint16_t) );
			...
			if (memcmp(acehdr->HEAD_SIGN , ACE_ID, sizeof(ACE_ID) )!=0 || acehdr->HEAD_TYPE!=0 ||
				...
				hdrread-= 2*sizeof(uint16_t);		// care for the size that is specified with HEADER_SIZE
				...
				if (acehdr->HEAD_CRC == (headcrc & 0xffff)) {
					...
					filesearchstart=acehdr->HEAD_SIZE + 2*sizeof(uint16_t);
					if (aceOutput)
...
void CArchiveRecovery::writeAceHeader(CFile *output, ACE_ARCHIVEHEADER* hdr) {
	...
	int rest= hdr->HEAD_SIZE - (sizeof(ACE_ARCHIVEHEADER)-	(3*sizeof(uint16_t)) -  (3*sizeof(char*)));
...
//		block->data_offset = offsetStart;
		...
		// Now copy compressed data from input file
		uint32_t lenToCopy = block->PACK_SIZE;
		if (lenToCopy > 0)
		{
...
void CArchiveRecovery::ISOReadDirectory(archiveScannerThreadParams_s* aitp, UINT32 startSec, CFile* isoInput, CString currentDirName)
{
	...
	while (aitp->m_bIsValid)
	{
		...
		if (aitp->ai->isoInfos.iJolietUnicode){
			for (unsigned int i = 0; i < file->nameLen/sizeof(uint16_t); i++)
...
uint16_t CArchiveRecovery::readUInt16(CFile *input)
{
	uint16_t retVal = 0;
	BYTE b[2];
	if (input->Read(b, 2) > 0)
...
uint32_t CArchiveRecovery::readUInt32(CFile *input)
{
	uint32_t retVal = 0;
	BYTE b[4];
	if (input->Read(b, 4) > 0)
...
uint16_t CArchiveRecovery::calcUInt16(BYTE *input)
{
	return (((uint16_t)input[1]) << 8) + ((uint16_t)input[0]);
}

uint32_t CArchiveRecovery::calcUInt32(BYTE *input)
{
	return (((uint32_t)input[3]) << 24) + (((uint32_t)input[2]) << 16) + (((uint32_t)input[1]) << 8) + ((uint32_t)input[0]);
}

void CArchiveRecovery::writeUInt16(CFile *output, uint16_t val)
{
	BYTE b[2];
...
void CArchiveRecovery::writeUInt32(CFile *output, uint32_t val)
{
	BYTE b[4];
	b[0] = (BYTE)(val & 0x000000ff);
	b[1] = (BYTE)((val & 0x0000ff00) >>  8);
...
*/
	...
	int nNewProgress = 0;
	if ((uint64_t)aitp->file->GetFileSize() > 0)
		nNewProgress = (int)(uint64_t)((pos*1000) / aitp->file->GetFileSize());
