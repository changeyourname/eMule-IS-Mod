...
protected:
	...
	virtual BOOL MakeMimeHeader(CStringA& header, LPCSTR szBoundary, LPCTSTR szFileName) throw()
	{
		...
		_tsplitpath_s(szFileName, NULL, 0, NULL, 0, szFile, _countof(szFile), szExt, _countof(szExt));
		_tcscat_s(szFile, _countof(szFile), szExt);
...
class CNotifierMailThread : public CSafeThread
{
...
IMPLEMENT_DYNCREATE(CNotifierMailThread, CSafeThread)
...
#else
				if (AtlMimeCharsetFromCodePage(szCharset, uCodePage, spMultiLanguage, _countof(szCharset)))
#endif
...
void CemuleDlg::SendNotificationMail(int iMsgType, LPCTSTR pszText)
{
	...
	CNotifierMailThread* pThread = (CNotifierMailThread*)CSafeThread::BeginThread(RUNTIME_CLASS(CNotifierMailThread), THREAD_PRIORITY_BELOW_NORMAL, 0, CREATE_SUSPENDED);
	if (pThread)
	{
