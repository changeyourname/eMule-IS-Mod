...
void CSearchList::RemoveResults(uint32_t nSearchID)
{
    ...
    // ismod: remove key from map
    m_totalFileSize.RemoveKey(nSearchID);
}

void CSearchList::ShowResults(uint32_t nSearchID)
{
	ASSERT( outputwnd );
...
void CSearchList::RemoveResult(CSearchFile* todel)
{
	...
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
		...
		if (cur_file == todel)
		{
            ...
            // ismod: decrease total size [start]
            uint64_t ulTotalSize;
            m_totalFileSize.Lookup(todel->GetSearchID(), ulTotalSize);
            ulTotalSize -= (uint64_t)todel->GetFileSize();
            m_totalFileSize.SetAt(todel->GetSearchID(), ulTotalSize);
            // ismod [end]
			delete todel;
			return;
		}
...
void CSearchList::NewSearch(CSearchListCtrl* pWnd, CStringA strResultFileType, uint32_t nSearchID, ESearchType eSearchType, CString strSearchExpression, bool bMobilMuleSearch)
{
	if (pWnd)
		outputwnd = pWnd;
...
UINT CSearchList::ProcessSearchAnswer(const uchar* in_packet, uint32_t size,
									  CUpDownClient* Sender, bool* pbMoreResultsAvailable, LPCTSTR pszDirectory)
{
	ASSERT( Sender != NULL );
	uint32_t nSearchID = PtrToUlong(Sender); // netfinity: truncate ptr to 32bit
	SSearchParams* pParams = new SSearchParams;
	pParams->strExpression = Sender ? Sender->GetUserName() : _T("?Unknown?");
	pParams->dwSearchID = nSearchID;
	pParams->bClientSharedFiles = true;
    // ismod: show own files for view shared files
    pParams->bShowOwnFiles = true;
	...
	// ismod: create folder for user shared files [start]
    //if(!pParams) pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(m_nCurED2KSearchID);
    // try to find dir
    CSearchFile* pSearchDir = NULL;
    SearchList* list = GetSearchListForID(nSearchID);
    for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
    {
        CSearchFile* file = list->GetNext(pos);
        if (file->GetFileName() == pszDirectory) 
        {
            pSearchDir = file;
            break;
        }
    ...
    if(!pSearchDir)
    {
        pSearchDir = new CSearchFile(pszDirectory, nSearchID);
        if( !AddToList(pSearchDir, true) )
			pSearchDir = NULL;
    }
    // ismod [end]
	...
	for (UINT i = 0; i < results; i++){
		...
		if (toadd->IsLargeFile() && (Sender == NULL || !Sender->SupportsLargeFiles())){
            ...
            delete toadd; // ismod: memory leak
			continue;
		}
		if (Sender){
        ...
        // ismod: user shared files in folders [start]
		if ( pSearchDir ) {
	        toadd->SetListParent(pSearchDir);
			pSearchDir->SetFileSize(pSearchDir->GetFileSize()+toadd->GetFileSize());
		}
        // ismod [end]
    ...
    // ismod: update total folder size an file count [start]
	if ( pSearchDir ) {
		pSearchDir->AddListChildCount(results);
		pSearchDir->AddSources(results);
	}
    // ismod [end]
...
UINT CSearchList::ProcessSearchAnswer(const uchar* in_packet, uint32_t size, bool bOptUTF8,
									  uint32_t nServerIP, uint16_t nServerPort, bool* pbMoreResultsAvailable)
{
    // ismod no result for closed searches [start]
    SSearchParams* pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(m_nCurED2KSearchID);
    if(!pParams)
        return 0;
    // ismod [end]
	...
	for (UINT i = 0; i < results; i++){
        ...
        // ismod: skip empty folders [start]
        if(toadd->IsFolder() && toadd->GetFileSize()==(uint64_t)0)
        {
            delete toadd;
            continue;
        }
...
UINT CSearchList::ProcessUDPSearchAnswer(CFileDataIO& packet, bool bOptUTF8, uint32_t nServerIP, uint16_t nServerPort)
{
...
UINT CSearchList::GetResultCount(uint32_t nSearchID) const
{
	UINT nSources = 0;
...
void CSearchList::GetWebList(CQArray<SearchFileStruct, SearchFileStruct> *SearchFileArray, int iSortBy) const
{
	for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL;)
	{
		...
		for(POSITION pos2 = listCur->m_listSearchFiles.GetHeadPosition(); pos2 != NULL; )
		{
			...
			if (pFile == NULL || pFile->GetListParent() != NULL || pFile->GetFileSize() == (uint64_t)0 || pFile->GetFileName().IsEmpty())
				continue;
...
// mobilemule
CSearchFile* CSearchList::DetachNextFile(uint32_t nSearchID)
{
...
bool CSearchList::AddToList(CSearchFile* toadd, bool bClientResponse, uint32_t dwFromUDPServerIP)
{
    // ismod [start]
    SSearchParams* pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(toadd->GetSearchID());
    // show own files
    if (!pParams->bShowOwnFiles && theApp.sharedfiles->GetFileByID(toadd->GetFileHash())
        // word filter
        || thePrefs.HasFilteredWords(toadd->GetFileName()) 
        // don't show user on normal search, when filter +++ is off
        ||(!pParams->bFilterPPPlus && !toadd->IsUser() && toadd->GetFileName().Find(_T("+++USERNICK+++"))==0)
        // don't show subnet files, when filter +++ is off
        ||(!pParams->bFilterPPPlus && HasSubnetMark(toadd->GetFileName()))
		)
    {
        delete toadd;
        return false;
    ...
    // ismod [end]
    /* ismod: do not check file type for folders
	if (!bClientResponse && !m_strResultFileType.IsEmpty() && _tcscmp(m_strResultFileType, toadd->GetFileType()) != 0)
    */
    if (!toadd->IsFolder() && !bClientResponse && !m_strResultFileType.IsEmpty() && _tcscmp(m_strResultFileType, toadd->GetFileType()) != 0)
	{
		delete toadd;
		return false;
	}
    ...
    // ismod: increase total search size [start]
    uint64_t ulTotalSize = 0;
    m_totalFileSize.Lookup(toadd->GetSearchID(), ulTotalSize);
    ulTotalSize += (uint64_t)toadd->GetFileSize();
    m_totalFileSize.SetAt(toadd->GetSearchID(), ulTotalSize);
    // ismod [end]
    ...
    // ismod: don't search parent for user response for dir contents result
    if (!toadd->IsFolder() && !toadd->GetListParent())
	for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
	{
			...
			&& md4cmp(parent->GetFileHash(), toadd->GetFileHash()) == 0)
		{
			...
			uint32_t uCompleteSources = (uint32_t)-1;
			...
			if (toadd->GetClientID() && toadd->GetClientPort())
			{
				...
				else
				{
					...
					{
						uint32_t nIP = toadd->GetClientID();
        ...
        // ismod: files in folders [start]
        else if (toadd->GetDirectory() && parent->GetFileName() == toadd->GetDirectory())
        {
            toadd->SetListParent(parent);
            parent->AddListChildCount(1);
			if (parent->GetListChildCount() < 2 )
				parent->AddListChildCount(1);
			parent->SetFileSize(parent->GetFileSize()+toadd->GetFileSize());
            break;
        }
        // ismod [end]
	}
    ...
    */
	...
	if (list->AddTail(toadd))
	{
        ...
        /* ismod: do not add sources to folders
		if (bClientResponse) {
        */
        if (!toadd->IsFolder() && bClientResponse) {
			// If this is a response from a client ("View Shared Files"), we set the "Availability" at least to 1.
			if (!toadd->GetIntTagValue(FT_SOURCES, uAvail) || uAvail==0)
				uAvail = 1;
...
void CSearchList::AddResultCount(uint32_t nSearchID, const uchar* hash, UINT nCount, bool bSpam)
{
	...
	m_foundSourcesCount.SetAt(nSearchID, tempValue 
		+ ( (bSpam && thePrefs.IsSearchSpamFilterEnabled()) ? min<UINT>(nCount, 5) : nCount) );
}
// FIXME LARGE FILES
void CSearchList::KademliaSearchKeyword(uint32_t searchID, const Kademlia::CUInt128* fileID, LPCTSTR name,
										uint64_t size, LPCTSTR type, UINT uKadPublishInfo
	...
	while (numProperties-- > 0)
	{
		...
		if (uPropType == 2 /*TAGTYPE_STRING*/)
		{
			if ((LPCTSTR)pvPropValue != NULL && ((LPCTSTR)pvPropValue)[0] != _T('\0'))
			{
				...
				{
					CTag tagProp((uint8_t)*pszPropName, (LPCTSTR)pvPropValue);
		...
		{
			if ((uint32_t)pvPropValue != 0)
			{	
				CTag tagProp(pszPropName, (uint32_t)pvPropValue);
				tagProp.WriteTagToFile(temp, eStrEncode);
				tagcount++;
				verifierEntry.AddTag(new Kademlia::CKadTagUInt(pszPropName, (uint32_t)pvPropValue));
			}
		}
		else
		{
...
void CSearchList::DoSpamRating(CSearchFile* pSearchFile, bool bIsClientFile, bool bMarkAsNoSpam, bool bRecalculateAll, bool bUpdate, uint32_t dwFromUDPServerIP){
	...
	*/
	...
	else if (bSureNegative && !bMarkAsNoSpam)
	{
		...
		if (pParent != NULL){
			...
			for (POSITION pos = list->GetHeadPosition(); pos != NULL; )
			{
				...
				if (pCurFile->GetListParent() == pParent){
					uint32_t nRating = GetSpamFilenameRatings(pCurFile, bMarkAsNoSpam);
		...
		for (int i = 0; i < m_aui64KnownSpamSizes.GetCount(); i++){
			if ((uint64_t)pSearchFile->GetFileSize() != 0 && _abs64((uint64_t)pSearchFile->GetFileSize() - m_aui64KnownSpamSizes[i]) < 5242880 &&
				((_abs64((uint64_t)pSearchFile->GetFileSize() - m_aui64KnownSpamSizes[i]) * 100) / (uint64_t)pSearchFile->GetFileSize()) < 5)
			{
				if (!bMarkAsNoSpam){
		...
		// 7 Heuristic (UDP Results)
				...
				&& (nResponses >= 3 || nRequests >= 5) && pTempFile->GetSourceCount() > 100)		
			{
				...
				if (((GetED2KFileTypeID(pTempFile->GetFileName()) == ED2KFT_PROGRAM || GetED2KFileTypeID(pTempFile->GetFileName()) ==  ED2KFT_ARCHIVE)
					&& (uint64_t)pTempFile->GetFileSize() > 102400 && (uint64_t)pTempFile->GetFileSize() < 10485760
					&& !bMarkAsNoSpam)
					|| bSourceServer)
				{
...
uint32_t CSearchList::GetSpamFilenameRatings(const CSearchFile* pSearchFile, bool bMarkAsNoSpam){
	...
	if (!m_astrKnownSimilarSpamNames.IsEmpty() && !pSearchFile->GetNameWithoutKeyword().IsEmpty()){
		...
		for (int i = 0; i < m_astrKnownSimilarSpamNames.GetCount(); i++){
			...
			{
				uint32_t nStringComp = LevenshteinDistance(pSearchFile->GetNameWithoutKeyword(), m_astrKnownSimilarSpamNames[i]);
				if (nStringComp != 0)
				...
				if (nStringComp >= 3){
					if (!bMarkAsNoSpam){
						if (nStringComp >= 6)
							...
							nResult = max<uint32_t>(nResult, SPAM_SIMILARNAME_FARHIT); 
					}
					else
						bRemove = true;
				}
...
SearchList* CSearchList::GetSearchListForID(uint32_t nSearchID){
	for (POSITION pos = m_listFileLists.GetHeadPosition(); pos != NULL; )
	{
...
void CSearchList::SentUDPRequestNotification(uint32_t nSearchID, uint32_t dwServerIP){
...
void CSearchList::MarkFileAsSpam(CSearchFile* pSpamFile, bool bRecalculateAll, bool bUpdate){
	...
	m_aui64KnownSpamSizes.Add((uint64_t)pSpamFile->GetFileSize());
...
void CSearchList::RecalculateSpamRatings(uint32_t nSearchID, bool bExpectHigher, bool bExpectLower, bool bUpdate){
...
void CSearchList::SaveSpamFilter(){
	...
	try{
		...
		while( pos != NULL )
		{
			...
			CTag tag(SP_UDPSERVERSPAMRATIO, (uint32_t) sizeof(abyBuffer), abyBuffer);
			tag.WriteNewEd2kTag(&file);
			nCount++;
		}
...
void CSearchList::StoreSearches(){
	...
	try{
		file.WriteUInt8(MET_HEADER_I64TAGS);
		uint8_t byVersion = STOREDSEARCHES_VERSION;
		...
		if (nCount > 0){
			...
			for (pos = m_listFileLists.GetTailPosition(); pos != NULL; ){
				...
				if (pParams != NULL){
					...
					file.WriteUInt32(static_cast<uint32_t>(pSl->m_listSearchFiles.GetCount()));
					POSITION pos2;
...
void CSearchList::LoadSearches(){
	...
	try {
		...
		uint8_t byVersion = file.ReadUInt8();
		if (byVersion != STOREDSEARCHES_VERSION){
			file.Close();
			return;
		}

		uint32_t nHighestKadSearchID = 0xFFFFFFFF;
		uint32_t nHighestEd2kSearchID = 0xFFFFFFFF;
		uint16_t nCount = file.ReadUInt16();
		for (int i = 0; i < nCount; i++){
			SSearchParams* pParams = new SSearchParams(file);
			uint32_t nFileCount = file.ReadUInt32();

			/* ismod: no automatic choose of search method
			...
			*/
			...
			if (pParams->eType == SearchTypeKademlia && (nHighestKadSearchID == 0xFFFFFFFF || nHighestKadSearchID < pParams->dwSearchID)){
                ...
                /* ismod: conflicts with saved view shared files
				ASSERT( pParams->dwSearchID >= 0x80000000 );
                */
            ...
            // ismod: set restore flag in params
            pParams->bRestoringSearch = true;
            
            // fill the list with stored results
			for (uint32_t j = 0; j < nFileCount; j++) {
            ...
            // ismod: unset restore flag in params
            pParams->bRestoringSearch = false;
...
// ismod [start]
bool CSearchList::UpdateSearch(CSearchFile *file)
{
    return outputwnd->UpdateSearch(file);
}

UINT CSearchList::ProcessDirContentsAnswer(const uchar* in_packet, uint32_t size, CUpDownClient* Sender)
{
    ASSERT( Sender != NULL );
    uchar dirHash[16];
    CSafeMemFile packet(in_packet, size);
    packet.ReadHash16(dirHash);
    CSearchFile* pSearchDir = GetSearchFileByHash(dirHash);
    uint32_t nSearchID = pSearchDir->GetSearchID();
    LPCTSTR pszDirectory = pSearchDir->GetFileName();

    SSearchParams* pParams = theApp.emuledlg->searchwnd->GetSearchParamsBySearchID(nSearchID);
    UINT results = packet.ReadUInt32();
    for (UINT i = 0; i < results; i++){
        pParams->bSearchFolders = false;
        CSearchFile* toadd = new CSearchFile(&packet, Sender ? Sender->GetUnicodeSupport()!=utf8strNone : false, nSearchID, 0, 0, pszDirectory);
        pParams->bSearchFolders = true;
        if (toadd->IsLargeFile() && (Sender == NULL || !Sender->SupportsLargeFiles())){
            DebugLogWarning(_T("Client offers large file (%s) but doesn't announced support for it - ignoring file"), toadd->GetFileName());
            delete toadd;
            continue;
        ...
        toadd->SetListParent(pSearchDir);
        pSearchDir->AddListChildCount(1);
        AddToList(toadd, true);
    }
