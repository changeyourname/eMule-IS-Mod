...
typedef struct
{
	...
	CString sFileType;
	uint64_t	m_qwFileSize;
	uint64_t	nFileTransferred;
	uint64_t	nFileAllTimeTransferred;
	UINT	nFileRequests;
	uint32_t	nFileAllTimeRequests;
	UINT	nFileAccepts;
	uint32_t	nFileAllTimeAccepts;
...
typedef struct
{
	...
	CString	sUserName;
	CString	sFileName;
	uint32_t	nTransferredDown;
	uint32_t	nTransferredUp;
	int		nDataRate;
...
typedef struct
{
	...
	bool    bServerStatic;
	uint32_t		nServerUsers;
	uint32_t		nServerMaxUsers;
	uint32_t		nServerFiles;
	CString sServerPriority;
	byte   nServerPriority;
	int		nServerPing;
	int		nServerFailed;
	uint32_t		nServerSoftLimit;
	uint32_t		nServerHardLimit;
...
// Common data
	...
	int				m_iSearchSortby;
	bool			m_bSearchAsc;
	uint16_t			m_nIntruderDetect;
	bool			m_bIsTempDisabled;
	uint32_t			m_nStartTempDisabledTime;
