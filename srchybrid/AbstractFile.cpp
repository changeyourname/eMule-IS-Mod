...
uint32_t CAbstractFile::GetIntTagValue(uint8_t tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
...
bool CAbstractFile::GetIntTagValue(uint8_t tagname, uint32_t& ruValue) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname && pTag->IsInt()){
...
uint64_t CAbstractFile::GetInt64TagValue(LPCSTR tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
...
uint64_t CAbstractFile::GetInt64TagValue(uint8_t tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
...
bool CAbstractFile::GetInt64TagValue(uint8_t tagname, uint64_t& ruValue) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname && pTag->IsInt64(true)){
...
uint32_t CAbstractFile::GetIntTagValue(LPCSTR tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
...
void CAbstractFile::SetIntTagValue(uint8_t tagname, uint32_t uValue)
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname && pTag->IsInt()){
...
void CAbstractFile::SetInt64TagValue(uint8_t tagname, uint64_t uValue)
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname && pTag->IsInt64(true)){
...
const CString& CAbstractFile::GetStrTagValue(uint8_t tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		const CTag* pTag = taglist[i];
...
void CAbstractFile::SetStrTagValue(uint8_t tagname, LPCTSTR pszValue)
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname && pTag->IsStr()){
...
CTag* CAbstractFile::GetTag(uint8_t tagname, uint8_t tagtype) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
...
CTag* CAbstractFile::GetTag(LPCSTR tagname, uint8_t tagtype) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
...
CTag* CAbstractFile::GetTag(uint8_t tagname) const
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
...
void CAbstractFile::DeleteTag(uint8_t tagname)
{
	for (int i = 0; i < taglist.GetSize(); i++){
		CTag* pTag = taglist[i];
		if (pTag->GetNameID()==tagname){
