...
CSearchParamsWnd::CSearchParamsWnd()
{
    ...
    // ismod [start]
    m_rcShowOwnFiles.SetRectEmpty();
    m_rcFilterPPPlus.SetRectEmpty();
    // ismod [end]
}
...
LRESULT CSearchParamsWnd::OnInitDialog(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
    ...
    // ismod [start]
	// show own files
	CheckDlgButton(IDC_MOD_SHOWOWNFILES, BST_CHECKED);
    GetDlgItem(IDC_MOD_SHOWOWNFILES)->GetWindowRect(&m_rcShowOwnFiles);
    ScreenToClient(&m_rcShowOwnFiles);

    // filter +++
    GetDlgItem(IDC_MOD_FILTERPPPLUS)->GetWindowRect(&m_rcFilterPPPlus);
    ScreenToClient(&m_rcFilterPPPlus);
    // ismod [end]
...
*/
	...
	// ismod [start]
	if ( thePrefs.GetSaveSearchParams() )
	{
		CIni ini( thePrefs.GetConfigFile() , _T("IS Mod") );
		
		m_ctlFileType.SelectString(ini.GetString(_T("SEARCH_strFileType"), GetResString(IDS_SEARCH_ANY)));
		CheckDlgButton(IDC_MOD_SHOWOWNFILES, ini.GetBool(_T("SEARCH_bFilterOwnFiles"),true) ? BST_CHECKED:BST_UNCHECKED );
		
		m_ctlOpts.SetItemText(orMinSize, 1, ini.GetString(_T("SEARCH_strMinSize"), _T("")));
		m_ctlOpts.SetItemText(orMaxSize, 1, ini.GetString(_T("SEARCH_strMaxSize"), _T("")));
		m_ctlOpts.SetItemText(orAvailability, 1, ini.GetString(_T("SEARCH_uAvailability"), _T("")));
		m_ctlOpts.SetItemText(orExtension, 1, ini.GetString(_T("SEARCH_strExtension"), _T("")));
		m_ctlOpts.SetItemText(orCompleteSources, 1, ini.GetString(_T("SEARCH_uComplete"), _T("")));
	}
	// ismod [end]
...
void CSearchParamsWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	if (cx >= MIN_HORZ_WIDTH)
	{
		...
		*/
		...
		if (hdwp)
		{
            ...
            // ismod [start]
            // don't show own files
            hdwp = DeferWindowPos(hdwp, *GetDlgItem(IDC_MOD_SHOWOWNFILES), NULL, rcClient.left + m_rcShowOwnFiles.left, rcClient.top + m_rcShowOwnFiles.top, m_rcShowOwnFiles.Width(), m_rcShowOwnFiles.Height(), uFlags);
            // filter +++
            hdwp = DeferWindowPos(hdwp, *GetDlgItem(IDC_MOD_FILTERPPPLUS), NULL, rcClient.left + m_rcFilterPPPlus.left, rcClient.top + m_rcFilterPPPlus.top, m_rcFilterPPPlus.Width(), m_rcFilterPPPlus.Height(), uFlags);
            // ismod [end]
			VERIFY( EndDeferWindowPos(hdwp) );
		}
	...
	else if (cx < MIN_HORZ_WIDTH)
	{
        ...
        // ismod [start]
        CRect rcShowOwnFiles;
        GetDlgItem(IDC_MOD_SHOWOWNFILES)->GetWindowRect(&rcShowOwnFiles);
        ScreenToClient(&rcShowOwnFiles);
        GetDlgItem(IDC_MOD_SHOWOWNFILES)->MoveWindow(rcClient.left, y, rcClient.Width(), rcShowOwnFiles.Height());

        CRect rcFilterPPPlus;
        GetDlgItem(IDC_MOD_SHOWOWNFILES)->GetWindowRect(&rcFilterPPPlus);
        ScreenToClient(&rcFilterPPPlus);
        GetDlgItem(IDC_MOD_SHOWOWNFILES)->MoveWindow(rcClient.left, y, rcClient.Width(), rcFilterPPPlus.Height());
        // ismod [end]
...
{
    /* ismod: only server search
	int iMethod = m_ctlMethod.GetCurSel();
    */
    int iMethod = SearchTypeEd2kGlobal;
	if (iMethod != CB_ERR)
	{
...
void CSearchParamsWnd::SetAllIcons()
{
    ...
    */
    ...
    // ismod [start]
    iml.Add(CTempIconLoader(_T("OPENFOLDER"))); // folder
    iml.Add(CTempIconLoader(_T("STATSCLIENTS"))); // user
    // ismod [end]
...
void CSearchParamsWnd::OnDestroy()
{
	...
	if ( thePrefs.GetSaveSearchParams() )
	{
		...
		ini.WriteString(_T("SEARCH_strFileType"), m_ctlFileType.GetCurrentItemText());
		ini.WriteString(_T("SEARCH_strMinSize"), m_ctlOpts.GetItemText(orMinSize, 1));
		ini.WriteString(_T("SEARCH_strMaxSize"), m_ctlOpts.GetItemText(orMaxSize, 1));
		ini.WriteString(_T("SEARCH_uAvailability"), m_ctlOpts.GetItemText(orAvailability, 1));
		ini.WriteString(_T("SEARCH_strExtension"),m_ctlOpts.GetItemText(orExtension, 1));
		ini.WriteString(_T("SEARCH_uComplete"), m_ctlOpts.GetItemText(orCompleteSources, 1));
		ini.WriteBool(_T("SEARCH_bFilterOwnFiles"), IsDlgButtonChecked(IDC_MOD_SHOWOWNFILES)==BST_CHECKED);
	}
	// ismod [end]
...
void CSearchParamsWnd::InitMethodsCtrl()
{
    ...
    m_ctlMethod.SetCurSel(iMethod != CB_ERR ? iMethod : SearchTypeEd2kGlobal); // select global search
}
...
void CSearchParamsWnd::InitFileTypesCtrl()
{
    ...
    // ismod [start]
    lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_MOD_SEARCH_FOLDER), ED2KFTSTR_FOLDER, 9));
    lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_MOD_SEARCH_USERS), ED2KFTSTR_USER, 10));
    // ismod [end]
...
void CSearchParamsWnd::Localize()
{
    ...
    GetDlgItem(IDC_MOD_SHOWOWNFILES)->SetWindowText(GetResString(IDS_MOD_SHOWOWNFILES));
	GetDlgItem(IDC_MOD_FILTERPPPLUS)->SetWindowText(GetResString(IDS_MOD_FILTERPPPLUS));
...
void CSearchParamsWnd::OnBnClickedSearchReset()
{
    ...
    m_ctlMethod.SetCurSel(0);
    CheckDlgButton(IDC_MOD_SHOWOWNFILES, BST_CHECKED);
    CheckDlgButton(IDC_MOD_FILTERPPPLUS, BST_UNCHECKED);
    // ismod [end]

	m_ctlName.SetWindowText(_T(""));

    /* ismod: no needed anymore
	if (!m_ctlFileType.SelectItemDataStringA(ED2KFTSTR_ANY))
    */
		m_ctlFileType.SetCurSel(0);
...
void CSearchParamsWnd::SetParameters(const SSearchParams* pParams)
{
	if (!pParams->bClientSharedFiles)
	{
		...
		// ismod: users and folders search [start]
		if ( pParams->bSearchUsers )
			m_ctlFileType.SelectItemDataStringA(ED2KFTSTR_USER);
		else if ( pParams->bSearchFolders )
			m_ctlFileType.SelectItemDataStringA(ED2KFTSTR_FOLDER);
		else
		// ismod [end]
...
uint64_t CSearchParamsWnd::GetSearchAttrSize(const CString& rstrExpr)
{
	...
	if (endptr && *endptr != _T('\0'))
	{
		...
		if (chModifier == _T('b'))
			return (uint64_t)(dbl + 0.5);
		else if (chModifier == _T('k'))
			return (uint64_t)(dbl*1024 + 0.5);
		else if (chModifier == _T('\0') || chModifier == _T('m'))
			;
		else if (chModifier == _T('g'))
			return (uint64_t)(dbl*1024*1024*1024 + 0.5);
		else 
			return (uint64_t)-1;
	}
	return (uint64_t)(dbl*1024*1024 + 0.5); // Default = MBytes
}
...
SSearchParams* CSearchParamsWnd::GetParameters()
{
	...
	uint64_t ullMinSize = GetSearchAttrSize(strMinSize);
	if (ullMinSize == (uint64_t)-1) {
		CString strError;
	...
	uint64_t ullMaxSize = GetSearchAttrSize(strMaxSize);
	if (ullMaxSize == (uint64_t)-1) {
		CString strError;
    ...
    {
        pParams->bSearchUsers = true;
        strFileType = ED2KFTSTR_ANY;
    }
    else if (strFileType == ED2KFTSTR_FOLDER)
    {
        pParams->bSearchFolders = true;
        strFileType = ED2KFTSTR_EMULECOLLECTION;
    }

    // show own files
    if (!pParams->bSearchUsers)
        pParams->bShowOwnFiles = IsDlgButtonChecked(IDC_MOD_SHOWOWNFILES) != 0;

    // create 'system' info part
    if (pParams->bSearchUsers)
        pParams->strSysInfoPart = _T("+++USERNICK+++ ");

    // filter +++
    if( !pParams->bSearchUsers )
        pParams->bFilterPPPlus = IsDlgButtonChecked(IDC_MOD_FILTERPPPLUS) != 0;
    // ismod [end]
	pParams->strExpression += strExpression; // ismod: append
	/* ismod: only server search
    pParams->eType = (ESearchType)m_ctlMethod.GetCurSel();
    */
    pParams->eType = SearchTypeEd2kGlobal;
	pParams->strFileType = strFileType;
    // ismod: no params for users and folders [start]
...
void CSearchParamsWnd::ProcessEd2kSearchLinkRequest(CString strSearchTerm)
{
	...
	{
		if ((!theApp.IsConnected() && (ESearchType)m_ctlMethod.GetCurSel() == SearchTypeKademlia)
			|| (!theApp.IsConnected() && ((ESearchType)m_ctlMethod.GetCurSel() == SearchTypeEd2kServer || (ESearchType)m_ctlMethod.GetCurSel() == SearchTypeEd2kGlobal)))
		{
			m_ctlMethod.SetCurSel(0);
		}
		OnBnClickedStart();
...
// ismod [start]
/*
 * filter +++ must be disabled when file type is user
 */
void CSearchParamsWnd::OnEnChangeType()
{
    LPCSTR pszED2KFileType = (LPCSTR)m_ctlFileType.GetCurrentItemData();
    ASSERT( pszED2KFileType != NULL );
    if ( pszED2KFileType ) {
    	bool enabled = lstrcmpA(ED2KFTSTR_USER, pszED2KFileType) != 0;
        GetDlgItem(IDC_MOD_SHOWOWNFILES)->EnableWindow(enabled);
        GetDlgItem(IDC_MOD_FILTERPPPLUS)->EnableWindow(enabled);
    }
}