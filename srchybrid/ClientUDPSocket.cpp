...
void CClientUDPSocket::OnReceive(int nErrorCode)
{
	...
	if (nRealLen == SOCKET_ERROR)
	{
		...
		if (dwError == WSAECONNRESET)
		{
			...
			DebugLog(_T("Client UDP socket unreachable, ICMP message%s"), strClientInfo);				
		}
	...
	// netfinity: Added IsGoodIP check so we don't have to bother with packets with bogus source IPs (just in case!)
	if (!(!IsGoodIP(sockAddr.sin_addr.S_un.S_addr) || theApp.ipfilter->IsFiltered(sockAddr.sin_addr.S_un.S_addr) || theApp.clientlist->IsBannedClient(sockAddr.sin_addr.S_un.S_addr)))
	{
		BYTE* pBuffer;
		uint32_t nReceiverVerifyKey;
		uint32_t nSenderVerifyKey;
			...
			try
			{
				switch (pBuffer[0])
				{
					...
					case OP_KADEMLIAPACKEDPROT:
					{
						...
						if (nPacketLen >= 2)
						{
							uint32_t nNewSize = nPacketLen*10+300;
							BYTE* unpack = NULL;
...
bool CClientUDPSocket::ProcessPacket(const BYTE* packet, UINT size, uint8_t opcode, uint32_t ip, uint16_t port)
{
	switch(opcode)
	{
		...
		case OP_REASKACK:
		{
			...
			if (sender && sender->UDPPacketPending()){
				...
				uint16_t nRank = data_in.ReadUInt16();
		...
		case OP_DIRECTCALLBACKREQ:
		{
			...
			if (Kademlia::CKademlia::IsRunning() && Kademlia::CKademlia::IsFirewalled())
			{
				...
				uint16_t nRemoteTCPPort = data.ReadUInt16();
				uchar uchUserHash[16];
				data.ReadHash16(uchUserHash);
				uint8_t byConnectOptions = data.ReadUInt8();
...
void CClientUDPSocket::OnClose(int nErrorCode){
	if (nErrorCode){
		if (nErrorCode == WSAECONNRESET)
		{
			...
			//if (iSockAddrLen > 0 && sockAddr.sin_addr.S_un.S_addr != 0 && sockAddr.sin_addr.S_un.S_addr != INADDR_NONE)
			//	strClientInfo.Format(_T(" from %s:%u"), ipstr(sockAddr.sin_addr), ntohs(sockAddr.sin_port));
			DebugLog(_T("CClientUDPSocket::OnClose: Client UDP socket unreachable, ICMP message%s"), strClientInfo);				
		}
		else
			DebugLogError(_T("Error: Client UDP socket, close event!?: %s"), GetErrorMessage(nErrorCode, 1));
	}
}

SocketSentBytes CClientUDPSocket::SendControlData(uint32_t maxNumberOfBytesToSend, uint32_t /*minFragSize*/){ // ZZ:UploadBandWithThrottler (UDP)
// ZZ:UploadBandWithThrottler (UDP) -->
...
// <-- ZZ:UploadBandWithThrottler (UDP)
	...
	while (!controlpacket_queue.IsEmpty() && !IsBusy() && sentBytes < maxNumberOfBytesToSend){ // ZZ:UploadBandWithThrottler (UDP)
		...
		{
			uint32_t nLen = cur_packet->packet->size+2;
			...
			if (cur_packet->bEncrypt && (theApp.GetPublicIP() > 0 || cur_packet->bKad)){
				nLen = EncryptSendClient(&sendbuffer, nLen, cur_packet->pachTargetClientHashORKadID, cur_packet->bKad,  cur_packet->nReceiverVerifyKey, (cur_packet->bKad ? Kademlia::CPrefs::GetUDPVerifyKey(cur_packet->dwIP) : (uint16_t)0));
...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32_t dwIP, uint16_t nPort){
	// NOTE: *** This function is invoked from a *different* thread!
...
bool CClientUDPSocket::SendPacket(Packet* packet, uint32_t dwIP, uint16_t nPort, bool bEncrypt, const uchar* pachTargetClientHashORKadID, bool bKad, uint32_t nReceiverVerifyKey){
...
bool CClientUDPSocket::Create()
{
	...
	{
		ret = CAsyncSocket::Create(thePrefs.GetUDPPort(), SOCK_DGRAM, FD_READ | FD_WRITE | FD_CLOSE, thePrefs.GetBindAddrW()) != FALSE;
		if (ret)
		{
			BOOL optval = TRUE;
			DWORD bytesReturned;
