...
void CChatSelector::Init(CChatWnd *pParent)
{
	...
	/* ismod: activate only when message arrived
...
{
	/* ismod: set focus only when visible
	if (show)
		m_pParent->m_wndMessage.SetFocus();
	*/
	if (theApp.emuledlg->toolbar->IsButtonChecked(TBBTN_MESSAGES))
	{
		if (theApp.emuledlg->GetSafeHwnd() == CWnd::GetDesktopWindow()->GetForegroundWindow()->GetSafeHwnd())
		{
			m_pParent->m_wndMessage.SetFocus();
		}
	}
...
void CChatSelector::ProcessMessage(CUpDownClient* sender, const CString& message)
{
	...
	*/
	if (GetCurSel() == iTabItem && theApp.emuledlg->toolbar->IsButtonChecked(TBBTN_MESSAGES)
		&& theApp.emuledlg->GetSafeHwnd() == CWnd::GetDesktopWindow()->GetForegroundWindow()->GetSafeHwnd())
	{
	...
	else if (GetCurSel() != iTabItem)
	{
		...
		ci->notify = true;
		// ismod: activate timer - new unseen tab
		if(m_Timer == NULL) VERIFY( (m_Timer = SetTimer(20, 1500, 0)) != NULL );
	}
	else
	{
		ci->notify = true;
...
bool CChatSelector::SendMessage(const CString& rstrMessage)
{
	...
	ci->history_pos = (int) ci->history.GetCount();
...
void CChatSelector::OnTimer(UINT_PTR /*nIDEvent*/)
{
	...
	if (globalnotify) {
		...
		// ismod: kill timer - all tabs is seen
		if (m_Timer){
			KillTimer(m_Timer);
			m_Timer = NULL;
		}
