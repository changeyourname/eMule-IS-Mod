...
uint16_t CPPgWiz1Ports::GetTCPPort() {
	CString buffer;

	GetDlgItem(IDC_TCP)->GetWindowText(buffer);
	return (uint16_t)_tstoi(buffer);
}

uint16_t CPPgWiz1Ports::GetUDPPort() {
	uint16_t udp = 0;
	if (IsDlgButtonChecked(IDC_UDPDISABLE)==0) {
		CString buffer;
		GetDlgItem(IDC_UDP)->GetWindowText(buffer);
		udp = (uint16_t)_tstoi(buffer);
	}
	return udp;
}
...
void CPPgWiz1Ports::OnStartConTest() {
	...
	uint16_t tcp=GetTCPPort();
	uint16_t udp=GetUDPPort();
...
BOOL FirstTimeWizard()
{
	...
	*/
	...
	uint16_t oldtcpport=thePrefs.GetPort();
	uint16_t oldudpport=thePrefs.GetUDPPort();

	int iResult = static_cast<int>(sheet.DoModal());
	if (iResult == IDCANCEL) {
