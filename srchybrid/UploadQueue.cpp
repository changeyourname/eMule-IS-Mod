...
static uint32_t counter, sec, statsave;
static UINT s_uSaveStatistics = 0;
static uint32_t igraph, istats, i2Secs;
...
CUpDownClient* CUploadQueue::FindBestClientInQueue()
{
	...
	for (pos1 = waitinglist.GetHeadPosition();( pos2 = pos1 ) != NULL;)
	{
        ...
        {
		    // finished clearing
		    uint32_t cur_score = cur_client->GetScore(false);
...
void CUploadQueue::InsertInUploadingList(CUpDownClient* newclient) 
{
    ...
    // Add it last
    theApp.uploadBandwidthThrottler->AddToStandardList((uint32_t) uploadinglist.GetCount(), newclient->GetFileUploadSocket());
	uploadinglist.AddTail(newclient);
    newclient->SetSlotNumber((UINT) uploadinglist.GetCount());
}
...
bool CUploadQueue::AddUpNextClient(LPCTSTR pszReason, CUpDownClient* directadd){
	...
	if (!directadd)
	{
        ...
        if(newclient)
		{
		    ...
		    theApp.emuledlg->transferwnd->ShowQueueCount((uint32_t) waitinglist.GetCount());
        }
	}
	else 
		newclient = directadd;
...
void CUploadQueue::UpdateActiveClientsInfo(DWORD curTick) {
    ...
    uint32_t tempHighest = theApp.uploadBandwidthThrottler->GetHighestNumberOfFullyActivatedSlotsSinceLastCallAndReset();

    if(thePrefs.GetLogUlDlEvents() && theApp.uploadBandwidthThrottler->GetStandardListSize() > (uint32_t)uploadinglist.GetSize()) {
	...
	if(tempHighest > (uint32_t)uploadinglist.GetSize()+1) {
    ...
    while(!activeClients_tick_list.IsEmpty() && !activeClients_list.IsEmpty() && curTick-activeClients_tick_list.GetHead() > 20*1000) {
	    ...
	    uint32_t removed = activeClients_list.RemoveHead();
    ...
    if(activeClients_tick_list.GetSize() > 1) {
        uint32_t tempMaxActiveClients = m_iHighestNumberOfFullyActivatedSlotsSinceLastCall;
        uint32_t tempMaxActiveClientsShortTime = m_iHighestNumberOfFullyActivatedSlotsSinceLastCall;
        ...
        while(activeClientsListPos != NULL && (tempMaxRemoved > tempMaxActiveClients && tempMaxRemoved >= m_MaxActiveClients || curTick - activeClients_tick_list.GetAt(activeClientsTickPos) < 10 * 1000)) {
            ...
            uint32_t activeClientsSnapshot = activeClients_list.GetAt(activeClientsListPos);
...
void CUploadQueue::Process() {
	...
	uint64_t sentBytes = theApp.uploadBandwidthThrottler->GetNumberOfSentBytesSinceLastCallAndReset();
...
{
	uint32_t curUploadSlots = (uint32_t)uploadinglist.GetCount();
...
bool CUploadQueue::AcceptNewClient(uint32_t curUploadSlots){
	...
	*/
	if (curUploadSlots >= (uint32_t)thePrefs.GetMaxUploadSlots())
		return false;
...
bool CUploadQueue::ForceNewClient(bool allowEmptyWaitingQueue) {
	...
	*/
	...
	if ( MaxSpeed == UNLIMITED )
	{
		...
		uint16_t nMaxSlots;
		if (MaxSpeed > 12)
			nMaxSlots = (uint16_t)(((float)(MaxSpeed*1024)) / upPerClient);
		else if (MaxSpeed > 7)
...
//		AddLogLine(true,"maxslots=%u, upPerClient=%u, datarateslot=%u|%u|%u",nMaxSlots,upPerClient,datarate/UPLOAD_CHECK_CLIENT_DR, datarate, UPLOAD_CHECK_CLIENT_DR);
    ...
    if(m_iHighestNumberOfFullyActivatedSlotsSinceLastCall > (uint32_t)uploadinglist.GetSize()) {
...
CUpDownClient* CUploadQueue::GetWaitingClientByIP_UDP(uint32_t dwIP, uint16_t nUDPPort, bool bIgnorePortOnUniqueIP, bool* pbMultipleIPs){
...
CUpDownClient* CUploadQueue::GetWaitingClientByIP(uint32_t dwIP){
	for (POSITION pos = waitinglist.GetHeadPosition();pos != 0;){
		CUpDownClient* cur_client = waitinglist.GetNext(pos);
...
void CUploadQueue::AddClientToQueue(CUpDownClient* client, bool bIgnoreTimelimit)
{
	...
	if (reqfile != NULL && CCollection::HasCollectionExtention(reqfile->GetFileName()) && reqfile->GetFileSize() < (uint64_t)MAXPRIORITYCOLL_SIZE
    ...
    uint32_t softQueueLimit = thePrefs.GetQueueSize();
    uint32_t hardQueueLimit = thePrefs.GetQueueSize() + max<uint32_t>(thePrefs.GetQueueSize()/4, 200);

    // if soft queue limit has been reached, only let in high ranking clients
    if ((uint32_t)waitinglist.GetCount() >= hardQueueLimit ||
        (uint32_t)waitinglist.GetCount() >= softQueueLimit && // soft queue limit is reached
		/* ismod: unnecessary check
		(client->IsFriend() && client->GetFriendSlot()) == false && // client is not a friend with friend slot
		*/
		client->GetFriendSlot() == false && // client is not a friend with friend slot
	...
	// ismod: friend slot for all friends [start]
	else if (client->GetFriendSlot() && thePrefs.GetUseFriendSlotForAllFriends())
	{
		RemoveFromWaitingQueue(client, true);
		AddUpNextClient(_T("Friend slot for all friends."), client);
		return;
	}
	// ismod [end]
	else
...
uint32_t CUploadQueue::GetAverageUpTime(){
...
void CUploadQueue::RemoveFromWaitingQueue(POSITION pos, bool updatewindow){	
	...
	if (updatewindow) {
		...
		theApp.emuledlg->transferwnd->ShowQueueCount(static_cast<uint32_t>(waitinglist.GetCount()));
	}
...
void CUploadQueue::UpdateMaxClientScore()
{
	m_imaxscore=0;
	for(POSITION pos = waitinglist.GetHeadPosition(); pos != 0; ) {
		uint32_t score = waitinglist.GetNext(pos)->GetScore(true, false);
...
bool CUploadQueue::CheckForTimeOver(CUpDownClient* client){
	...
	if (!thePrefs.TransferFullChunks()){
		...
		const uint32_t score = client->GetScore(true, true);
...
{
	// ismod: check heap in debug builds
		...
		if (counter >= 10){
			...
			*/
			...
			if (thePrefs.GetTrafficOMeterInterval()>0) {
				...
				if (igraph >= (uint32_t)(thePrefs.GetTrafficOMeterInterval()) ) {
					igraph=0;
			...
			if (theApp.emuledlg->activewnd == theApp.emuledlg->statisticswnd && theApp.emuledlg->IsWindowVisible() )  {
				...
				if (thePrefs.GetStatsInterval()>0) {
					...
					if (istats >= (uint32_t)(thePrefs.GetStatsInterval()) ) {
						istats=0;
			...
			*/
			...
			// ismod: MoNKi: -Blinking tray icon on message received-
			// Update every second
			if (!theApp.emuledlg->IsTrayIconToFlash())
				theApp.emuledlg->ShowTransferRate();
			// ismod [end]
...
// END netfinity: Moved inline

uint32_t CUploadQueue::GetToNetworkDatarate() {
...
uint32_t CUploadQueue::GetWaitingUserForFileCount(const CSimpleArray<CObject*>& raFiles, bool bOnlyIfChanged)
{
...
uint32_t CUploadQueue::GetDatarateForFile(const CSimpleArray<CObject*>& raFiles) const
{
	uint32_t nResult = 0;
	for (POSITION pos = uploadinglist.GetHeadPosition(); pos != 0; )
	{
