...
BEGIN_MESSAGE_MAP(CClientDetailPage, CResizablePage)
	...
	// ismod
	ON_NOTIFY(NM_CLICK, IDC_MOD_IP_LINK, &CClientDetailPage::OnNmClickIPLink)
END_MESSAGE_MAP()
...
BOOL CClientDetailPage::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		*/
		if (!client->HasLowID() && client->GetConnectIP())
			GetDlgItem(IDC_MOD_IP_LINK)->SetWindowText(_T("<a>") + CString(ipstr(client->GetConnectIP())) + _T("</a>"));
		else
			GetDlgItem(IDC_MOD_IP_LINK)->SetWindowText(_T("?"));

		if (!client->GetClientModVer().IsEmpty())
			GetDlgItem(IDC_DSOFT)->SetWindowText(client->GetClientSoftVer()+_T(" [")+client->GetClientModVer()+_T("]"));
		else
			GetDlgItem(IDC_DSOFT)->SetWindowText(client->GetClientSoftVer());
		// ismod [end]
...
// ismod [start]
void CClientDetailPage::OnNmClickIPLink( NMHDR* /*pNMHDR*/, LRESULT *pResult )
{
	CString strCurIPAddr;
	*pResult = 0;

	GetDlgItem(IDC_MOD_IP_LINK)->GetWindowText(strCurIPAddr);
	if(strCurIPAddr == _T("?"))
		return;

	theApp.CopyTextToClipboard(strCurIPAddr.Mid(3, strCurIPAddr.GetLength()-7));
}
