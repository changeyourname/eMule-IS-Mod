...
CString CastItoXBytes(uint16_t count, bool isK, bool isPerSec, uint32_t decimal){
	return CastItoXBytes((double)count, isK, isPerSec, decimal);
}

CString CastItoXBytes(uint32_t count, bool isK, bool isPerSec, uint32_t decimal){
	return CastItoXBytes((double)count, isK, isPerSec, decimal);
}

CString CastItoXBytes(uint64_t count, bool isK, bool isPerSec, uint32_t decimal){
	return CastItoXBytes((double)count, isK, isPerSec, decimal);
}

#if defined(_DEBUG) && defined(USE_DEBUG_EMFILESIZE)
CString CastItoXBytes(EMFileSize count, bool isK, bool isPerSec, uint32_t decimal){
	return CastItoXBytes((double)count, isK, isPerSec, decimal);
}
#endif

CString CastItoXBytes(float count, bool isK, bool isPerSec, uint32_t decimal){
	return CastItoXBytes((double)count, isK, isPerSec, decimal);
}

CString CastItoXBytes(double count, bool isK, bool isPerSec, uint32_t decimal){
	if( count <= 0.0 )
	{
		if(isPerSec)
		{
...
CString CastItoIShort(uint16_t count, bool isK, uint32_t decimal){
	return CastItoIShort((double)count, isK, decimal);
}

CString CastItoIShort(uint32_t count, bool isK, uint32_t decimal){
	return CastItoIShort((double)count, isK, decimal);
}

CString CastItoIShort(uint64_t count, bool isK, uint32_t decimal){
	return CastItoIShort((double)count, isK, decimal);
}

CString CastItoIShort(float count, bool isK, uint32_t decimal){
	return CastItoIShort((double)count, isK, decimal);
}

CString CastItoIShort(double count, bool isK, uint32_t decimal){
	if( count <= 0.0 )
	{
		return _T("0");
	}
...
CString CastSecondsToHM(time_t tSeconds)
{
	...
	CString buffer;
	time_t count = tSeconds;
	if (count < 60)
		buffer.Format(_T("%u %s"), static_cast<unsigned int>(count), GetResString(IDS_SECS));
	else if (count < 3600)
		buffer.Format(_T("%u:%02u %s"), static_cast<unsigned int>(count/60), static_cast<unsigned int>(count - (count/60)*60), GetResString(IDS_MINS));
	else if (count < 86400)
		buffer.Format(_T("%u:%02u %s"), static_cast<unsigned int>(count/3600), static_cast<unsigned int>((count - (count/3600)*3600)/60), GetResString(IDS_HOURS));
	else {
		time_t cntDays = count/86400;
		time_t cntHrs = (count - cntDays*86400)/3600;
		buffer.Format(_T("%u %s %u %s"), static_cast<unsigned int>(cntDays), GetResString(IDS_DAYS), static_cast<unsigned int>(cntHrs), GetResString(IDS_HOURS));
	}
	return buffer;
}
...
CString CastSecondsToLngHM(time_t tSeconds)
{
	...
	time_t count = tSeconds;
	if (count < 60)
		buffer.Format(_T("%u %s"), static_cast<UINT>(count), GetResString(IDS_LONGSECS));
	else if (count < 3600)
		buffer.Format(_T("%u:%02u %s"), static_cast<UINT>(count/60), static_cast<UINT>(count - (count/60)*60), GetResString(IDS_LONGMINS));
	else if (count < 86400)
		buffer.Format(_T("%u:%02u %s"), static_cast<UINT>(count/3600), static_cast<UINT>((count - (count/3600)*3600)/60), GetResString(IDS_LONGHRS));
	else {
		UINT cntDays = static_cast<UINT>(count/86400);
		UINT cntHrs = static_cast<UINT>((count - cntDays*86400)/3600);
		if (cntHrs)
			buffer.Format(_T("%u %s %u:%02u %s"), cntDays, GetResString(IDS_DAYS2), cntHrs, static_cast<UINT>((count - (cntDays*86400) - (cntHrs*3600))/60), GetResString(IDS_LONGHRS));
		else
			buffer.Format(_T("%u %s %u %s"), cntDays, GetResString(IDS_DAYS2), static_cast<UINT>((count - (cntDays*86400) - (cntHrs*3600))/60), GetResString(IDS_LONGMINS));
	}
	return buffer;
} 
...
void ShellOpenFile(LPCTSTR name, LPCTSTR pszVerb)
{
	...
	theDesktop.ShellExecute(pszVerb, name);
}

bool ShellDeleteFile( LPCTSTR pszFilePath, bool bNoRecBin )
{
	if ( !PathFileExists(pszFilePath) )
		return true;
	if ( !thePrefs.GetRemoveToBin() || bNoRecBin )
...
CString URLDecode( const CString& inStr, bool bKeepNewLine )
{
	...
	for (int x = 0; x < inStr.GetLength(); x++)
	{
		...
		{
			TCHAR hexstr[3];
			_tcsncpy_s(hexstr, _countof(hexstr), inStr.Mid(x+1, 2), 2);
			hexstr[2] = _T('\0');
			x += 2;
...
WORD DetectWinVersion()
{
	...
	return _WINVER_XP_; // there should'nt be anything lower than this
}
...
uint32_t DecodeBase32(LPCTSTR pszInput, uchar* paucOutput, uint32_t nBufferLen)
{
	if (pszInput == NULL)
		return false;
	uint32_t nDecodeLen = static_cast<uint32_t>((_tcslen(pszInput)*5)/8);
	if ((_tcslen(pszInput)*5) % 8 > 0)
		nDecodeLen++;
	uint32_t nInputLen = static_cast<uint32_t>(_tcslen(pszInput));
	...
	for ( int nChars = nInputLen ; nChars-- ; pszInput++ )
	{
		...
		if ( nCount >= 8 )
		{
			*paucOutput++ = (BYTE)( (nBits >> ( nCount - 8 )) & 0xFF );
			nCount -= 8;
		}
...
int CWebServices::ReadAllServices()
{
		...
		while (!feof(readFile))
		{
			...
			if (iPos > 0)
			{
				...
				if (!strUrlTemplate.IsEmpty())
				{
					...
					svc.uMenuID = MP_WEBURL + static_cast<UINT>(m_aServices.GetCount());
	...
	return static_cast<int>(m_aServices.GetCount());
}
...
bool CWebServices::RunURL(const CAbstractFile* file, UINT uMenuID)
{
	for (int i = 0; i < m_aServices.GetCount(); i++)
	{
		...
		if (rSvc.uMenuID == uMenuID)
		{
			...
			return (INT_PTR) ShellExecute(NULL, NULL, strUrlTemplate, NULL, thePrefs.GetMuleDirectory(EMULE_APPDIR), SW_SHOWDEFAULT) > 32;
		}
	}
	return false;
}
...
CString DbgGetHexDump(const uint8_t* data, UINT size)
{
	...
	for(; i < size && i < 50; i++){
		if (i > 0)
			buffer += _T(" ");
		TCHAR temp[3];
		_sntprintf_s(temp, _countof(temp), ARRAYSIZE(temp), _T("%02x"), data[i]);
		buffer += temp;
	}
	buffer += (i == size) ? _T("]") : _T("..]");
	return buffer;
...
#pragma pack(pop)
	...
	__try
	{
		...
		do // the VS debugger truncates the string to 31 characters anyway!
		{
			lenBuf += 128;
			delete[] buffer;
			buffer = (char*)alloca(sizeof(char)*lenBuf);
			lenResult = _vsnprintf_s(buffer, lenBuf, lenBuf, szThreadName, args);
		} while (lenResult == -1);
		va_end(args);
...
bool IsGoodIP( uint32_t nIP, bool forceCheck )
{
	...
	if ( nIP==0 || static_cast<uint8_t>(nIP & 0xFF)==127 || static_cast<uint8_t>(nIP & 0xFF)>=224 ) {
#ifdef _DEBUG
	...
	if (!thePrefs.FilterLANIPs() && !forceCheck)
		return true;
	else
		return !IsLANIP(nIP);
}
...
bool IsLANIP(uint32_t nIP){
	...
	uint8_t nFirst = static_cast<uint8_t>(nIP & 0xFF);
	uint8_t nSecond = static_cast<uint8_t>((nIP >> 8) & 0xFF);
...
{
	TCHAR szVal[12];
	_ultot_s(ulVal, szVal, _countof(szVal), 10);
...
void DebugHexDump(const uint8_t* data, UINT lenData)
{
	...
	while (pos < lenData)
	{
		...
		CStringA single;
		line.Format("%08X ", pos);
		lenLine = min<int>((lenData - pos), 16);
...
void DebugRecv(LPCSTR pszMsg, const CUpDownClient* client, const uchar* packet, uint32_t nIP)
{
...
{
	TCHAR szIPPort[22];
	_sntprintf_s(szIPPort, _countof(szIPPort), ARRAYSIZE(szIPPort), _T("%s:%u"), ipstr(ntohl(ip)), port);
...
CString StripInvalidFilenameChars(const CString& strText)
{
	...
	for (int i = 0; i < _countof(apszReservedFilenames); i++)
	{
		...
		{
			if ((UINT)strDest.GetLength() == nPrefixLen) {
				// Filename is a reserved file name:
				// Append an underscore character
				strDest += _T("_");
				break;
			}
			else if (strDest[static_cast<int>(nPrefixLen)] == _T('.')) {
...
CString ipstr(uint32_t nIP)
{
	...
	CString strIP;
	strIP.ReleaseBuffer(_sntprintf_s(strIP.GetBuffer(3+1+3+1+3+1+3 +1), 16, 16, _T("%u.%u.%u.%u"), pucIP[0], pucIP[1], pucIP[2], pucIP[3]));
	return strIP;
}
...
CString ipstr(uint32_t nIP, uint16_t nPort)
{
	...
	const BYTE* pucIP = (BYTE*)&nIP;
	CString strIP;
	strIP.ReleaseBuffer(_sntprintf_s(strIP.GetBuffer(3+1+3+1+3+1+3+1+5 +1), 22, 22, _T("%u.%u.%u.%u:%u"), pucIP[0], pucIP[1], pucIP[2], pucIP[3], nPort));
	return strIP;
}
...
CStringA ipstrA(uint32_t nIP)
{
	const BYTE* pucIP = (BYTE*)&nIP;
	CStringA strIP;
	strIP.ReleaseBuffer(sprintf_s(strIP.GetBuffer(3+1+3+1+3+1+3), 15, "%u.%u.%u.%u", pucIP[0], pucIP[1], pucIP[2], pucIP[3]));
	return strIP;
}
...
uint16_t GetRandomUInt16()
{
	...
	return (uint16_t)(uRand0 | ((uRand1 >= RAND_MAX/2) ? 0x8000 : 0x0000));
#else
#error "Implement it!"
#endif
}
...
uint32_t GetRandomUInt32()
{
	...
	uint32_t uRand0 = GetRandomUInt16();
	srand(GetTickCount() | uRand0);
	uint32_t uRand1 = GetRandomUInt16();
...
HWND ReplaceRichEditCtrl(CWnd* pwndRE, CWnd* pwndParent, CFont* pFont)
{
	...
	if (pwndRE)
	{
		...
		if (GetClassNameA(*pwndRE, szClassName, _countof(szClassName)) && _stricmp(szClassName, "RichEdit20W")==0)
			return NULL;
...
void InstallSkin(LPCTSTR pszSkinPackage)
{
	...
	_tcslwr_s(szExt, _countof(szExt));
...
RC4_Key_Struct* RC4CreateKey(const uchar* const pachKeyData, const size_t nLen, RC4_Key_Struct* key, const bool bSkipDiscard){
	...
	uint8_t* const pabyState= &key->abyState[0];
	for (int i = 0; i < 256; i++)
		pabyState[i] = static_cast<uint8_t>(i);
	...
	for (int i = 0; i < 256; i++){
		tempi = pabyState[i];
		index2 = (pachKeyData[index1] + tempi + index2) & 0xFF;
		temp2 = pabyState[index2];
		pabyState[i] = temp2;
		pabyState[index2] = tempi;
		index1 = static_cast<uint8_t>(((index1 + 1) % nLen) & 0xFF);
	}
	if (!bSkipDiscard)
...
void RC4Crypt(const uchar* const pachIn, uchar* const pachOut, const size_t nLen, RC4_Key_Struct* const key){
	...
	{
		size_t i = 0;
		// netfinity: Unrolling a little bit to improve CPU pipeline performance
		uint32_t byXor;
		for (; i + 4 <= nLen;)
		{
			// Unrolled #0
			...
			tempY = pabyState[byY];
			pabyState[byY] = tempX;
			byXorIndex = (tempX + tempY) & 0xFF;
			pabyState[byX] = tempY;
			byXor = pabyState[byXorIndex];
			// Unrolled #1
			byX = (byX + 1) & 0xFF;
			tempX = pabyState[byX];
			byY = (tempX + byY) & 0xFF;
			...
			byXorIndex = (tempX + tempY) & 0xFF;
			pabyState[byX] = tempY;
			byXor = byXor << 8 | pabyState[byXorIndex] & 0xFF;
			// Unrolled #2
			byX = (byX + 1) & 0xFF;
			tempX = pabyState[byX];
			byY = (tempX + byY) & 0xFF;
			...
			*reinterpret_cast<uint32_t*>(&pachOut[i]) = *reinterpret_cast<const uint32_t*>(&pachIn[i]) ^ _byteswap_ulong(byXor);
			i+=4;
	    }
		for (; i < nLen; ++i)
		{
...
EFileType GetFileTypeEx(CShareableFile* kfile, bool checkextention, bool checkfileheader, bool nocached)
{
	...
	bool test4iso = (    !kfile->IsPartFile()
		              || ((uint64_t)pfile->GetFileSize() > 0x8000 + HEADERCHECKSIZE && pfile->IsComplete(0x8000, 0x8000 + HEADERCHECKSIZE, true)));
...
{
	uint32_t n1 = str1.GetLength();
	uint32_t n2 = str2.GetLength();

	uint32_t* p = new uint32_t[n2+1];
	uint32_t* q = new uint32_t[n2+1];
	uint32_t* r;
	...
	for(uint32_t i = 1; i <= n1; ++i)
	{
		...
		for(uint32_t j = 1; j <= n2; ++j )
		{
			uint32_t d_del = p[j] + 1;
			uint32_t d_ins = q[j-1] + 1;
			uint32_t d_sub = p[j-1] + (str1.GetAt(i-1) == str2.GetAt(j-1) ? 0 : 1);
...
bool _tmakepathlimit(TCHAR *path, const TCHAR *drive, const TCHAR *dir, const TCHAR *fname, const TCHAR *ext){
	...
	uint32_t nSize = 64; // the function should actually only add 4 (+1 nullbyte) bytes max extra
	if (drive != NULL)
...
uint8_t GetMyConnectOptions(bool bEncryption, bool bCallback){
	// Connect options Tag
	// 4 Reserved (!)
	// 1 Direct Callback
	...
	const uint8_t uSupportsCryptLayer	= (thePrefs.IsClientCryptLayerSupported() && bEncryption) ? 1 : 0;
	const uint8_t uRequestsCryptLayer	= (thePrefs.IsClientCryptLayerRequested() && bEncryption) ? 1 : 0;
	const uint8_t uRequiresCryptLayer	= (thePrefs.IsClientCryptLayerRequired() && bEncryption) ? 1 : 0;
	// direct callback is only possible if connected to kad, tcp firewalled and verified UDP open (for example on a full cone NAT)
	const uint8_t uDirectUDPCallback	= (bCallback && theApp.IsFirewalled() && Kademlia::CKademlia::IsRunning() && !Kademlia::CUDPFirewallTester::IsFirewalledUDP(true) && Kademlia::CUDPFirewallTester::IsVerified()) ? 1 : 0;
...
bool AddIconGrayscaledToImageList(CImageList& rList, HICON hIcon)
{
	...
	if (GetIconInfo(hIcon, &iinfo))
	{
		...
		{
			cxGray.GrayScale();
			HBITMAP hGrayBmp = cxGray.MakeBitmap();
...
// search substring like '+++XXX-XXX+++' at start of string
bool HasSubnetMark(const CString& rstr)
{
    ...
    for(j=0;j<3;++j) { if (rstr[i++] != _T('+')) return false; }
    j=0; while(rstr[i] >= _T('0') && rstr[i] <= _T('9') && j++<3 && len>i++);
    if (rstr[i++] != _T('-')) return false;
    j=0; while(rstr[i] >= _T('0') && rstr[i] <= _T('9') && j++<3 && len>i++);
    for(j=0;j<3;++j) { if (rstr[i++] != _T('+')) return false; }
    if (rstr[i] != _T(' ')) return false;
...
CString CreateBBCODEED2kLink(const CAbstractFile* f)
{
	CString strCode;
    strCode.Format(_T("[u][b][url=%s]%s[/url][/b][/u]"), CreateED2kLink(f), 
        StripInvalidFilenameChars(f->GetFileName()) );
	return strCode;
}
...
{
	CSafeFile file;
	if ( !file.Open(thePrefs.GetMuleDirectory(EMULE_LOGDIR)+_T("packet.log"), 
		CFile::modeNoTruncate|CFile::modeCreate|CFile::modeWrite|CFile::typeBinary) )
	{
		LogError(_T("Unable to open packetlog file!"));
		return;
	}
	file.SeekToEnd();
	file.WriteUInt32((uint32_t)time(NULL));
	file.WriteUInt32(src);
	file.WriteUInt32(dst);
	file.WriteUInt32(uPacketSize);
	file.Write(pPacket, uPacketSize);
	file.Close();
}
...
/**
 * Replace certain national characters in string
 */
void fixNationalChars( CString &rstr )
{
    LPCTSTR rplFrom =  _T("∏®");
    LPCTSTR rplTo   =  _T("Â≈");
    ASSERT(_tcslen(rplFrom) == _tcslen(rplTo));

    int length = _tcslen(rplFrom);
    for ( int i = 0; i < length; ++i ) {
        rstr.Replace(rplFrom[i], rplTo[i]);
    }
}
