...
void CFileDetailDlgStatistics::RefreshData()
{
	...
	UINT uRequests = 0;
	UINT uAccepted = 0;
	uint64_t uAllTimeTransferred = 0;
	...
	if (iFiles != 0)
	{
		...
		uint32_t nQueueCount = theApp.uploadqueue->GetWaitingUserForFileCount(*m_paFiles, !m_bDataChanged);
		if (nQueueCount != (-1))
		{
...
void CFileDetailDlgStatistics::OnTimer(UINT nIDEvent)
{
	...
	{
		if (!theApp.IsRunning() || !GetParent()->IsWindowVisible()
