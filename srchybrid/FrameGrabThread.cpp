...
IMPLEMENT_DYNCREATE(CFrameGrabThread, CSafeThread)
...
BOOL CFrameGrabThread::Run(){
	...
	CoInitialize(NULL);
	result->nImagesGrabbed = (uint8_t)GrabFrames();
	CoUninitialize();
...
void CFrameGrabThread::SetValues(const CKnownFile* in_pOwner, LPCTSTR in_strFileName,uint8_t in_nFramesToGrab, double in_dStartTime, bool in_bReduceColor, uint16_t in_nMaxWidth, void* in_pSender){
...
BEGIN_MESSAGE_MAP(CFrameGrabThread, CSafeThread)
END_MESSAGE_MAP()
