...
// ismod
#define DBL_ENTER_TIME 1000 // период двойного нажати€ Enter
	...
	if (pMsg->message == WM_KEYDOWN)
	{
		...
		*/
		...
		if ((pMsg->wParam == 'A' || pMsg->wParam == 'a') 
			&& pMsg->hwnd == m_wndMessage && (GetKeyState(VK_CONTROL) & 0x8000))
		{
			m_wndMessage.SetSel(0, -1);
		}

		if (pMsg->wParam == VK_RETURN && pMsg->hwnd == m_wndMessage)
		{
			//отправка по одинарному нажатию
			if(thePrefs.GetChatSendMethod()==SEND_ON_SINGLE_ENTER)
			{
				OnBnClickedSend();
				return TRUE;
			}

			// отправка по двойному Enter
			if ( (lastEnterClick+DBL_ENTER_TIME > GetTickCount()) 
				&& (thePrefs.GetChatSendMethod() == SEND_ON_DOUBLE_ENTER) )
			{
				OnBnClickedSend();
				return TRUE;
			}else{
				lastEnterClick = GetTickCount();
			}
...
void CChatWnd::OnBnClickedSend()
{
	...
	if (!strMessage.IsEmpty())
	{
		...
		*/
		...
		m_wndMessage.EnableWindow(FALSE);
		do 
		{
			uStartPos = uEndPos;
			uEndPos += MAX_CLIENT_MSG_LEN;
			if(uEndPos > (UINT)strMessage.GetLength())
				uEndPos = strMessage.GetLength();
			else
			{
				while((strMessage[(int)uEndPos] != _T(' ') 
					&& strMessage[(int)uEndPos] != _T('\n'))
					&& (uEndPos > uStartPos) )
					--uEndPos;

				//если сплошной текст, то шлем полный блок
			...
			UINT uSize = uEndPos-uStartPos;
			strMsgPart = strMessage.Mid(uStartPos, uSize);
			strMsgPart.Trim();

			if (chatselector.SendMessage(strMsgPart))
				m_wndMessage.SetWindowText(_T("")); // clear() не работает
		}while(uEndPos < (UINT)strMessage.GetLength());
		m_wndSend.EnableWindow(TRUE);
		m_wndMessage.EnableWindow(TRUE);
		// ismod [end]
	}
