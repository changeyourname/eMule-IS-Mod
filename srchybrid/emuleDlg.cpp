...
BEGIN_MESSAGE_MAP(CemuleDlg, CTrayDialog)
    ...
    // Version Check
	ON_MESSAGE(UM_VERSION_CHECK_RESPONSE, OnVersionCheckResponse)
    ON_MESSAGE(UM_UPDATE_DOWNLOAD_RESPONSE, OnUpdateDownloadResponse)
	...
	ON_MESSAGE(TM_FLUSHDONE,			OnFlushDone)
	ON_MESSAGE(TM_PARTFILE_IOERR,		OnPartFileIoErr)
	...
	ON_MESSAGE(TM_AUTHEVENT, OnAuthEvent)
...
CemuleDlg::CemuleDlg(CWnd* pParent /*=NULL*/)
	: CTrayDialog(IDD_EMULE_DIALOG, pParent)
{
	...
	// ismod[start]
	sctwnd = new CSctWnd;
	sctwnd->startCore(NULL);
	xcatwnd = new CXCatalogWnd;
	// ismod [end]
	m_hIcon = NULL;
...
#ifdef _DEBUG
	delete m_trabsferDebugWnd;
	m_trabsferDebugWnd = NULL;
#endif
}
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	*/
	SetWindowText(_T("eMule v") + theApp.m_strCurVersionLong + _T(" ") + theApp.m_strCurVersionLongMod);
	...
	*/
	// ismod [start]
	sctwnd->CreateWnd();
	DialogCreateIndirect(xcatwnd, IDD_MOD_XCATALOG);
	...
	if (thePrefs.GetRestoreLastMainWndDlg()){
		...
		*/
		case IDD_IRC:
			...
			break;
		case IDD_MOD_XCATALOG:
			SetActiveDialog(xcatwnd);
			break;
		case IDD_SCT:
			SetActiveDialog(sctwnd);
			break;
		}
	}
	...
	*/
	...
	// ismod [start]
	AddAnchor(*sctwnd,		TOP_LEFT, BOTTOM_RIGHT);
	AddAnchor(*xcatwnd,		TOP_LEFT, BOTTOM_RIGHT);
	// ismod [end]
...
// ismod [start]
	...
	if ( thePrefs.GetAlwaysShowTrayIcon() )
		TrayShow();

#ifdef _DEBUG
	CString strGenericClass = AfxRegisterWndClass(CS_HREDRAW|CS_VREDRAW|CS_NOCLOSE);
	m_trabsferDebugWnd = new TransferDebugWnd;
	RECT rc = {10,10,200,200};
	m_trabsferDebugWnd->CreateEx(WS_EX_APPWINDOW, strGenericClass, _T("TransferDebug"), WS_OVERLAPPEDWINDOW, rc, NULL, 0);
	m_trabsferDebugWnd->ShowWindow(SW_SHOWNORMAL);
#endif
// ismod [end]
...
{
    if ( !manual && thePrefs.lastVersionCheckTime() != 0 ) {
        struct tm tmTemp;
        time_t tLast = thePrefs.lastVersionCheckTime();
        time_t tNow  = safe_mktime(CTime::GetCurrentTime().GetLocalTm(&tmTemp));

        if ( (difftime(tNow,tLast) / 86400) < thePrefs.versionCheckDays() ){
            return;
        }
    }

    std::ostringstream * outStream = new std::ostringstream();
    emCurl::httpRequestAsync(EMULE_UPDATE_URL, "", outStream, m_hWnd, UM_VERSION_CHECK_RESPONSE);
}
		...
		switch(theApp.emuledlg->status){
			...
			case 4:{
				...
				if (!bError) // show the success msg, only if we had no serious error
					...
					*/
					AddLogLine(true, GetResString(IDS_MAIN_READY),theApp.m_strCurVersionLong + _T(" ") + theApp.m_strCurVersionLongMod);
...
void CemuleDlg::OnBnClickedConnect(){
	if (!theApp.IsConnected())
		...
		// ismod: ask user
		if (AfxMessageBox(_T("Разорвать соединение с сетью?"), MB_YESNO | MB_DEFBUTTON2) == IDNO)
		{
			return;
		}
...
void CemuleDlg::ShowMessageState(UINT iconnr)
{
	...
	// ismod: flash mesage
	toolbar->PressButton(TBBTN_MESSAGES, iconnr==2);
}
...
CString CemuleDlg::GetUpDatarateString(UINT uUpDatarate)
{
	...
	if (thePrefs.ShowOverhead()) {
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), _T("%.1f (%.1f)"), (float)m_uUpDatarate/1024, (float)theStats.GetUpDatarateOverhead()/1024);
		szBuff[_countof(szBuff) - 1] = _T('\0');
	}
	else {
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), _T("%.1f"), (float)m_uUpDatarate/1024);
		szBuff[_countof(szBuff) - 1] = _T('\0');
	}
	return szBuff;
}
...
CString CemuleDlg::GetDownDatarateString(UINT uDownDatarate)
{
	...
	TCHAR szBuff[128];
	if (thePrefs.ShowOverhead()) {
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), _T("%.1f (%.1f)"), (float)m_uDownDatarate/1024, (float)theStats.GetDownDatarateOverhead()/1024);
		szBuff[_countof(szBuff) - 1] = _T('\0');
	}
	else {
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), _T("%.1f"), (float)m_uDownDatarate/1024);
		szBuff[_countof(szBuff) - 1] = _T('\0');
	}
	return szBuff;
}
...
CString CemuleDlg::GetTransferRateString()
{
	TCHAR szBuff[128];
	if (thePrefs.ShowOverhead()) {
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), GetResString(IDS_UPDOWN),
		...
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), GetResString(IDS_UPDOWNSMALL), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024);
		szBuff[_countof(szBuff) - 1] = _T('\0');
	}
	return szBuff;
}
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	if (TrayIsVisible() || bForceAll)
	{
		...
		// ismod: upload procent
		int iUpRateProcent = (int)ceil((m_uUpDatarate/10.24) / thePrefs.GetMaxGraphUploadRate(false));
		UpdateTrayIcon(iDownRateProcent, iUpRateProcent);
		...
		if (theApp.IsConnected()) {
			...
			*/
			_sntprintf_s(buffer2, _countof(buffer2), _countof(buffer2), _T("eMule %s\r\n%s\r\n%s"), theApp.m_strCurVersionLongMod,GetResString(IDS_CONNECTED),strTransferRate);
			...
			*/
			_sntprintf_s(buffer2, _countof(buffer2), _countof(buffer2), _T("eMule %s\r\n%s\r\n%s"), theApp.m_strCurVersionLongMod, GetResString(IDS_DISCONNECTED), strTransferRate);
			buffer2[_countof(buffer2) - 1] = _T('\0');
		}
	...
	{
		/*  ismod: show extended info in title [start]
		TCHAR szBuff[128];
		_sntprintf_s(szBuff, _countof(szBuff), _countof(szBuff), _T("(U:%.1f D:%.1f) eMule v%s"), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong);
        szBuff[_countof(szBuff) - 1] = _T('\0');
        SetWindowText(szBuff);
		*/
		CString strBuffer;
        strBuffer.Preallocate(350);
        strBuffer.AppendFormat(_T("(U:%.1f D:%.1f) eMule v%s  "), (float)m_uUpDatarate/1024, (float)m_uDownDatarate/1024, theApp.m_strCurVersionLong + _T(" ") + theApp.m_strCurVersionLongMod);
		if (thePrefs.GetShowExtendedInfoInTitle())
		{
			if ( theApp.GetPublicIP() )
				strBuffer.AppendFormat(_T("%s - "), ipstr(theApp.GetPublicIP()));
            strBuffer.Append(_T(" "));
			strBuffer.Append(thePrefs.GetUserNick());
        }
...
void CemuleDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if (transferwnd != NULL && theApp.IsRunning())
...
{
	try {
		CString link2 = emUtils::decodeHtmlEntities(URLDecode(pszData).GetBuffer()).c_str();
...
LRESULT CemuleDlg::OnWMData(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (data->dwData == OP_ED2KLINK)
	{
		...
		if (clcommand==_T("status")) {
			CString strBuff;
			strBuff.Format(_T("%sstatus.log"), thePrefs.GetMuleDirectory(EMULE_LOGDIR));
...
LRESULT CemuleDlg::OnFileOpProgress( WPARAM wParam, LPARAM lParam )
{
	...
	UINT progress = static_cast<UINT>(lParam);

    if ( theApp.IsRunning() && theApp.downloadqueue->IsPartFile(owner) )
    {
		owner->SetFileOpProgress(progress);
		owner->UpdateDisplayedInfo(true);
	}
...
{
	UnknownFile_Struct *ufs = (UnknownFile_Struct*)lParam;

	if ( theApp.IsRunning() )  {
		theApp.sharedfiles->HashFailed(ufs);
	} else {
		delete ufs;
	}
...
{
	CPartFile *owner = (CPartFile*)wParam;
	uint16_t part = (uint16_t)lParam;

	if ( theApp.IsRunning() && theApp.downloadqueue->IsPartFile(owner) )
		owner->PartHashFinished(part, false);
...
LRESULT CemuleDlg::OnPartHashedCorrupt( WPARAM wParam, LPARAM lParam )
{
	...
	if ( theApp.IsRunning() && theApp.downloadqueue->IsPartFile(owner) )
		owner->PartHashFinished(part, true);
...
{
	if (theApp.IsRunning() && theApp.uploadqueue->IsDownloading((CUpDownClient*)lParam))
    {
		((CUpDownClient*)lParam)->SetReadBlockFromFileBuffer((byte*)wParam);
        return TRUE;
    }else if ((INT_PTR) wParam != INT_PTR(-1) && (INT_PTR) wParam != INT_PTR(-2) && wParam != NULL)
		delete[] (byte*)wParam;
	return 0;
}
...
LRESULT CemuleDlg::OnPartFileIoErr( WPARAM wParam, LPARAM lParam )
{
	CPartFile *owner = (CPartFile*)wParam;
	PartFile_Io_Error *pfioerr = (PartFile_Io_Error*)lParam;

	if ( theApp.IsRunning() && theApp.downloadqueue->IsPartFile(owner) ) {
		owner->IoExceptionHandler(pfioerr);
    } else {
		pfioerr->exception->Delete();
		delete pfioerr;
    }
...
{
	CPartFile*  owner = (CPartFile*)lParam;
	ASSERT( NULL != owner );
	
	owner->PerformFileCompleteEnd(static_cast<DWORD>(wParam));
...
{
	if (theApp.IsRunning() && thePrefs.IsConfirmExitEnabled() && !theApp.IsAutomaticShutdown() )
	{
...
void CemuleDlg::OnClose()
{
	...
	if (activewnd){
		...
		*/
		...
		// ismod [start]
		else if (activewnd->IsKindOf(RUNTIME_CLASS(CSctWnd)))
			thePrefs.SetLastMainWndDlgID(IDD_SCT);
		else if (activewnd->IsKindOf(RUNTIME_CLASS(CXCatalogWnd)))
			thePrefs.SetLastMainWndDlgID(IDD_MOD_XCATALOG);
		// ismod [end]
		else{
			ASSERT(0);
	...
	*/
	...
	// ismod: disable threads to execute new tasks
	theApp.m_threadlock.WriteLock();
	// ismod: wait for part file hashing thread
	WaitForSingleObject(theApp.m_hPartHashWaitEvent, INFINITE);
	CloseHandle(theApp.m_hPartHashWaitEvent);
	//ismod: close global wait event
	CloseHandle(theApp.m_globalWaitEvent);

	// ismod [start]
#ifdef _DEBUG
	m_trabsferDebugWnd->DestroyWindow();
	delete m_trabsferDebugWnd;
	m_trabsferDebugWnd = NULL;
#endif
	// ismod [end]
    ...
    // ismod
    try{
        TerminateThread(theApp.m_pFlushThread->m_hThread, 0);
        theApp.m_pFlushThread->Delete();
        TerminateThread(theApp.m_pReadBlockThread->m_hThread, 0);
        theApp.m_pReadBlockThread->Delete();
        TerminateThread(theApp.m_pHashThread->m_hThread, 0);
        theApp.m_pHashThread->Delete();
    }catch(...){}
...
void CemuleDlg::OnTrayRButtonUp(CPoint pt)
{
	...
	if (m_pSystrayDlg)
	{
		UINT nResult = static_cast<UINT>(m_pSystrayDlg->DoModal());
...
void CemuleDlg::AddSpeedSelectorMenus(CMenu* addToMenu)
{
	...
	{
		text.Format(_T("20%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphUploadRate(true)*0.2),GetResString(IDS_KBYTESPERSEC));	m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_U20,  text);
		text.Format(_T("40%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphUploadRate(true)*0.4),GetResString(IDS_KBYTESPERSEC));	m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_U40,  text);
		text.Format(_T("60%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphUploadRate(true)*0.6),GetResString(IDS_KBYTESPERSEC));	m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_U60,  text);
		text.Format(_T("80%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphUploadRate(true)*0.8),GetResString(IDS_KBYTESPERSEC));	m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_U80,  text);
		text.Format(_T("100%%\t%i %s"), (uint16_t)(thePrefs.GetMaxGraphUploadRate(true)),GetResString(IDS_KBYTESPERSEC));		m_menuUploadCtrl.AppendMenu(MF_STRING, MP_QS_U100, text);
	...
	{
		text.Format(_T("20%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphDownloadRate()*0.2),GetResString(IDS_KBYTESPERSEC));	m_menuDownloadCtrl.AppendMenu(MF_STRING|MF_POPUP, MP_QS_D20,  text);
		text.Format(_T("40%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphDownloadRate()*0.4),GetResString(IDS_KBYTESPERSEC));	m_menuDownloadCtrl.AppendMenu(MF_STRING|MF_POPUP, MP_QS_D40,  text);
		text.Format(_T("60%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphDownloadRate()*0.6),GetResString(IDS_KBYTESPERSEC));	m_menuDownloadCtrl.AppendMenu(MF_STRING|MF_POPUP, MP_QS_D60,  text);
		text.Format(_T("80%%\t%i %s"),  (uint16_t)(thePrefs.GetMaxGraphDownloadRate()*0.8),GetResString(IDS_KBYTESPERSEC));	m_menuDownloadCtrl.AppendMenu(MF_STRING|MF_POPUP, MP_QS_D80,  text);
		text.Format(_T("100%%\t%i %s"), (uint16_t)(thePrefs.GetMaxGraphDownloadRate()),GetResString(IDS_KBYTESPERSEC));		m_menuDownloadCtrl.AppendMenu(MF_STRING|MF_POPUP, MP_QS_D100, text);
...
void CemuleDlg::RestoreWindow()
{
	...
	*/
	...
	/* ismod: always show tray icon [start]
...
static int iOldUpPercent = 0;

void CemuleDlg::UpdateTrayIcon(int iDownPercent, int iUpPercent)
{
	// compute an id of the icon to be generated
	UINT uSysTrayIconCookie = (iDownPercent > 0) ? (16 - ((iDownPercent*15/100) + 1)) : 0;
	if (theApp.IsConnected()) {
	...
	// ismod: надо учитывать еще и мигающую иконку в трее
	uSysTrayIconCookie += m_iMsgIcon*100;

	// don't update if the same icon as displayed would be generated
	if ( (m_uLastSysTrayIconCookie == uSysTrayIconCookie) && (iOldUpPercent == iUpPercent) )
		return;
	m_uLastSysTrayIconCookie = uSysTrayIconCookie;
	iOldUpPercent = iUpPercent;
	...
	if (m_iMsgIcon!=0 && thePrefs.DoFlashOnNewMessage()==true ) {
		...
		if (m_iMsgBlinkState)
			/* ismod: new blink icon
			m_TrayIcon.Init(imicons[1], 100, 1, 1, 16, 16, thePrefs.GetStatsColor(11));
			*/
			m_TrayIcon.Init(imicons[2], 100, 2, 1, 16, 16, thePrefs.GetStatsColor(11));
	...
	if (!m_iMsgBlinkState) {
		if (theApp.IsConnected()) {
			if (theApp.IsFirewalled())
				m_TrayIcon.Init(m_icoSysTrayLowID, 100, 2, 1, 16, 16, thePrefs.GetStatsColor(11));
			else
				m_TrayIcon.Init(m_icoSysTrayConnected, 100, 2, 1, 16, 16, thePrefs.GetStatsColor(11));
		}
		else
			m_TrayIcon.Init(m_icoSysTrayDisconnected, 100, 2, 1, 16, 16, thePrefs.GetStatsColor(11));
	}

	// load our limit and color info
	static const int aiLimits[] = { 100, 100 }; // set the limits of where the bar color changes (low-high)
	COLORREF aColors[] = { thePrefs.GetStatsColor(11), thePrefs.GetStatsColor(15) }; // set the corresponding color for each level
	...
	int aiVals[] = { iDownPercent, iUpPercent };
...
HBRUSH CemuleDlg::GetCtlColor(CDC* /*pDC*/, CWnd* /*pWnd*/, UINT nCtlColor)
{
	...
	// use WM_CTLCOLOR...
	//
	//if (theApp.m_ullComCtrlVer >= MAKEDLLVERULL(6,16,0,0) && IsThemeActive() && IsAppThemed()) {
...
*/
		...
		case MP_HM_DIRECT_DOWNLOAD:{
			/* ismod: new clipboard handler [start]
			CDirectDownloadDlg dlg;
			dlg.DoModal();
			*/
			//if window is already created just bring it to top
			//CDirectDownloadDlg dlg;
			if (dlgDownload.GetSafeHwnd() == NULL)
			{
				dlgDownload.DoModal();
			}
			else
			{
				dlgDownload.SetForegroundWindow();
				dlgDownload.BringWindowToTop();
			}
			// ismod [end]
			break;
		}
	...
	if (wParam>=MP_WEBURL && wParam<=MP_WEBURL+99) {
		theWebServices.RunURL(NULL, static_cast<UINT>(wParam));
	}
	else if (wParam>=MP_SCHACTIONS && wParam<=MP_SCHACTIONS+99) {
		theApp.scheduler->ActivateSchedule(static_cast<int>(wParam)-MP_SCHACTIONS);
...
void StraightWindowStyles(CWnd* pWnd)
{
	...
	{
		if (_stricmp(szClassName, "Button") == 0)
			pWnd->ModifyStyle(BS_FLAT, 0);
		else if (   (_stricmp(szClassName, "EDIT") == 0 && (pWnd->GetExStyle() & WS_EX_STATICEDGE))
			     || _stricmp(szClassName, "SysListView32") == 0
			     || _stricmp(szClassName, "msctls_trackbar32") == 0
			    )
		{
...
void ApplySystemFont(CWnd* pWnd)
{
	...
	if (::GetClassNameA(*pWnd, szClassName, _countof(szClassName)))
	{
		if (   _stricmp(szClassName, "SysListView32") == 0
			|| _stricmp(szClassName, "SysTreeView32") == 0)
		{
...
void FlatWindowStyles(CWnd* pWnd)
{
	...
	if (::GetClassNameA(*pWnd, szClassName, _countof(szClassName)))
	{
		...
		else if (_stricmp(szClassName, "SysListView32") == 0)
		{
			pWnd->ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE);
		}
		else if (_stricmp(szClassName, "SysTreeView32") == 0)
		{
			pWnd->ModifyStyleEx(WS_EX_CLIENTEDGE, WS_EX_STATICEDGE);
		}
	}
...
{
    ASSERT( 0 != lParam );
    int statusCode = static_cast<int>(wParam);
    std::ostringstream * dataStream = reinterpret_cast<std::ostringstream*>(lParam);

    // in case of shutdown
    if ( !theApp.IsRunning() ) {
        if ( nullptr != dataStream)
            delete dataStream;
        return 0;
    }
    ...
    if ( 0 == statusCode ) {
        ...
        xmlDoc.Parse(dataStream->str().c_str(), nullptr, TIXML_ENCODING_UTF8 );
        delete dataStream;

        TiXmlElement * xmlUpdate = xmlDoc.FirstChildElement("update");
        if ( !xmlUpdate )
            return false;

        TiXmlElement * xmlEmule = xmlUpdate->FirstChildElement("emule");
        if ( !xmlEmule )
            return false;

        const char * versionString = xmlEmule->Attribute("version");
        if ( !versionString )
            return false;

        const char * updateUrl = xmlEmule->Attribute("url");
        if ( !updateUrl )
            return false;

        const char * changelogTextUtf8 = xmlEmule->GetText();
        if ( !changelogTextUtf8 )
            return false;

        CString changelogText = OptUtf8ToStr(changelogTextUtf8, _mbstrlen(changelogTextUtf8));

        CAppVersion theirVersion(versionString);
        CAppVersion ownVersion(IS_VERSION_MJR, IS_VERSION_MIN, IS_VERSION_UPD, IS_VERSION_BLD);

        thePrefs.updateLastVersionCheckTime(); // update last check time

        if ( theirVersion > ownVersion ) {
            // new version found
            SetActiveWindow();
            Log(LOG_SUCCESS|LOG_STATUSBAR, LTR(L"New version available!"));

            bool canRunUpdate = true;
            if ( thePrefs.askForUpdateInstall() ) {
                CString messageText;
                messageText.Format(LTR(L"Changes in new version:\r\n%s\r\n\r\nWould you like to apply this update?"), changelogText);
                canRunUpdate = MessageBoxW(messageText, LTR(L"New version available!"),  MB_YESNO) == IDYES;
            }
            if ( canRunUpdate ) {
                std::ofstream * outStream = new std::ofstream;
                CString updateFile = EMULE_UPDATE_FILE;
                ExpandEnvironmentStrings(updateFile);
                outStream->open(updateFile, std::ofstream::binary|std::ofstream::out);
                if ( !outStream->good() ) {
                    LogError(LOG_STATUSBAR, LTR(L"Failed to save program update (%s)"), updateFile);
                    // TODO: log error description to verbose
                } else {
                    emCurl::httpRequestAsync(updateUrl, "", outStream, this->GetSafeHwnd(), UM_UPDATE_DOWNLOAD_RESPONSE );
                    return 0;
                }
            }
        } else {
            // no new version found
            AddLogLine(true, LTR(L"No new version detected"));
        }

        return 0;
    }

    CString curlError = CA2W(emCurl::formatError(statusCode));
    LogWarning(LOG_STATUSBAR, LTR(L"Unable to check for update (%s)"), curlError);

    return 0;
}

LRESULT CemuleDlg::OnUpdateDownloadResponse( WPARAM wParam, LPARAM lParam )
{
    ASSERT( 0 != lParam );
    int statusCode = static_cast<int>(wParam);
    std::ofstream * outStream = reinterpret_cast<std::ofstream*>(lParam);

    // in case of shutdown
    if ( !theApp.IsRunning() && (nullptr != outStream) ) {
        delete outStream;
        return 0;
    }

    if ( 0 == statusCode ) {
        CString message = LTR(L"Update downloaded and will be installed at next startup");
        Log(LOG_SUCCESS|LOG_STATUSBAR, message);
        if ( thePrefs.askForUpdateInstall() )
            MessageBoxW(message, 0, MB_ICONINFORMATION);
        return 0;
    }
    else {
        CString curlError = CA2W(emCurl::formatError(statusCode));
        LogWarning(LOG_STATUSBAR, LTR(L"Unable to download update (%s)"), curlError);
        return 0;
    }
}
...
//#endif
			...
			lResult = theApp.OnIdle(static_cast<LONG>(lIdleCount));
		}
	}
...
int CemuleDlg::MapWindowToToolbarButton(CWnd* pWnd) const
{
	...
	*/
	// ismod: sct
	else if	(pWnd == sctwnd)		iButtonID = TBBTN_SCT;
	else if	(pWnd == xcatwnd)		iButtonID = TBBTN_XCAT;
	else ASSERT(0);
	return iButtonID;
}
...
CWnd* CemuleDlg::MapToolbarButtonToWindow(int iButtonID) const
{
	...
	switch (iButtonID)
	{
		...
		*/
		// ismod: sct
		case TBBTN_SCT:			pWnd = sctwnd;			break;
		case TBBTN_XCAT:		pWnd = xcatwnd;			break;
...
void CemuleDlg::HtmlHelp(DWORD_PTR dwData, UINT nCmd)
{
	...
	while (iTry++ < 2)
	{
		if (!AfxHtmlHelp(pWnd->m_hWnd, pApp->m_pszHelpFilePath, nCmd, dwData))
		{
			...
			HWND hwndResult = AfxHtmlHelp(pWnd->m_hWnd, NULL, HH_GET_LAST_ERROR, reinterpret_cast<DWORD_PTR>(&hhLastError));
			if (hwndResult != 0)
			{
	...
	if (bHelpError)
	{
		...
		{
			// ismod
			ShellExecute(NULL, NULL, _T("http://emule-project.net/home/perl/help.cgi"), NULL, thePrefs.GetMuleDirectory(EMULE_EXECUTEABLEDIR), SW_SHOWDEFAULT);
		}
	}
}
...
void CemuleDlg::CreateToolbarCmdIconMap()
{
	...
	*/
	// ismod: sct
	m_mapTbarCmdToIcon.SetAt(TBBTN_SCT, _T("IS74"));
	m_mapTbarCmdToIcon.SetAt(TBBTN_XCAT, _T("XCAT"));
...
UINT CemuleDlg::ShowPreferences(UINT uStartPageID)
{
	if (IsPreferencesDlgOpen())
	{
		preferenceswnd->SetForegroundWindow();
	...
	{
		if (uStartPageID != ~((UINT) 0))
			preferenceswnd->SetStartPage(static_cast<UINT>(uStartPageID));
		return static_cast<UINT>(preferenceswnd->DoModal());
	}
}
...
LRESULT CemuleDlg::OnWebAddDownloads(WPARAM wParam, LPARAM lParam)
{
	...
	if (link.GetLength()==32 && link.Left(4).CompareNoCase(_T("ed2k"))!=0) {
		...
		theApp.searchlist->AddFileToDownloadByHash(fileid,(uint8_t)lParam);
...
{
	theApp.downloadqueue->SetCatPrio(static_cast<UINT>(wParam),static_cast<uint8_t>(lParam));
	return 0;
}
...
void CemuleDlg::StartUPnP(bool bReset, uint16_t nForceTCPPort, uint16_t nForceUDPPort) {
...
LRESULT CemuleDlg::OnTaskbarBtnCreated ( WPARAM , LPARAM  )
{
	...
	if (thePrefs.GetWindowsVersion() >= _WINVER_7_ && theApp.IsRunning())
	{
		if (m_pTaskbarList)
			m_pTaskbarList.Release();
...
void CemuleDlg::SetTaskbarIconColor()
{
	...
	if (thePrefs.IsRunningAeroGlassTheme())
	{
		...
		if (hDWMAPI){
			...
			if (pfnDwmGetColorizationColor != NULL)
			{
				...
				{
					uint8_t byAlpha = (uint8_t)(dwGlassColor >> 24);
	...
	else
	{
		if (IsThemeActive() && IsAppThemed())
		{
			...
			VERIFY( SetWindowTheme(ptmpWnd->GetSafeHwnd(), L"TrayNotifyHoriz", NULL) == S_OK );
			HTHEME hTheme = OpenThemeData(ptmpWnd->GetSafeHwnd(), L"TrayNotify");
			if (hTheme != NULL)
			{

				if (!GetThemeColor(hTheme, TNP_BACKGROUND, 0, TMT_FILLCOLORHINT, &cr) == S_OK)
					ASSERT( false );
	...
	uint8_t iRed = GetRValue(cr);
	uint8_t iBlue = GetBValue(cr);
	uint8_t iGreen = GetGValue(cr);
	uint16_t iBrightness = (uint16_t)sqrt(((iRed * iRed * 0.241f) + (iGreen * iGreen * 0.691f) + (iBlue * iBlue * 0.068f)));
...
void CemuleDlg::OnDrawClipboard()
{
	...
	theApp.ClipboardText = theApp.CopyTextFromClipboard();
	::PostMessage(hwndNextViewer, WM_DRAWCLIPBOARD, (WPARAM)NULL, (LPARAM)NULL);
	if (thePrefs.WatchClipboard4ED2KLinks())
	{
		theApp.SearchClipboard();
	}
	theApp.ClipboardText.Empty();
	return;
}

// что мы делаем когда окно приложения станет активным
void CemuleDlg::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	if (theApp.IsRunning()){
		if (nState != WA_INACTIVE && activewnd == chatwnd)
			chatwnd->chatselector.ShowChat();
	}
...
LRESULT CemuleDlg::OnAuthEvent(WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(wParam);

	AuthProcess *auth = reinterpret_cast<AuthProcess*>(lParam);
	if ( !theApp.IsRunning() ) {
		delete auth;
		return 0;
	}

	auth->ProcessAuthResult();
	delete auth;
