...
static CSafeThread* s_pSocketThread = NULL;
...
{
	char szBuf[256];
	int nLen = _snprintf_s(szBuf, _countof(szBuf), _countof(szBuf), "%s\r\n", szReply);
	if (nLen > 0)
		SendData(szBuf, nLen);
}
					...
					while (WAIT_OBJECT_0 == WaitForMultipleObjects(2, pWait, FALSE, INFINITE))
					{
						for (;;)
						{
							...
							if(thePrefs.GetWSIsEnabled())
							{
								...
								CSafeThread* pAcceptThread = new CSafeThread(WebSocketAcceptedFunc, (LPVOID)pData);
...
void StartSockets(CWebServer *pThis)
{
	...
	if ((s_hTerminate = CreateEvent(NULL, TRUE, FALSE, NULL)) != NULL)
	{
		...
		//   use 'AfxBeginThread'
		s_pSocketThread = new CSafeThread(WebSocketListeningFunc, (LPVOID)pThis);
