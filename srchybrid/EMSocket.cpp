...
// BEGIN netfinity: Advanced download throttling
	downloadLimitMin = 0;
	downloadLimitLastSet = ::GetTickCount();
// END netfinity: Advanced download throttling
	...
	// ismod [start]
	::SetCriticalSectionSpinCount(sendLocker, 4000); // netfinity: To reduce task switching on multicore CPU's
	m_bDownloadLimitReallyDisabled = false;
}
...
void CEMSocket::OnReceive(int nErrorCode){
	...
	if(nErrorCode != 0 && nErrorCode != WSAESHUTDOWN){ // netfinity: Special case when socket is closing but data still in buffer, need to empty buffer or deadlock forever
		OnError(nErrorCode);
		return;
	}
...
// BEGIN netfinity: Advanced download throttling
	uint32_t limit = GetDownloadLimit();
	if (nErrorCode == WSAESHUTDOWN || m_bDownloadLimitReallyDisabled) // netfinity: Special case when socket is closing but data still in buffer, need to empty buffer or deadlock forever
...
// END netfinity: Advanced download throttling
	...
	uint32_t ret = Receive(GlobalReadBuffer + pendingHeaderSize, static_cast<int>(readMax));
	...
	// Bandwidth control
// BEGIN netfinity: Advanced download throttling
	uint32_t	uBytesUsed = GetRealReceivedBytes();
	if(globalDownloadLimitEnable == true){
		// Update limit
		globalDownloadLimit -= uBytesUsed;
	}
	if(downloadLimitEnable == true){
		// Update limit
		downloadLimit -= uBytesUsed;
	}
	// Update limit
	downloadLimitMin -= uBytesUsed;
	if (downloadLimitMin < -200)
		downloadLimitMin = -200;
// END netfinity: Advanced download throttling
	...
	if(pendingHeaderSize > 0) {
		...
		ret += static_cast<int>(pendingHeaderSize);
		pendingHeaderSize = 0;
	}
	...
	while ((rend - rptr >= PACKET_HEADER_SIZE) || ((pendingPacket != NULL) && (rend - rptr > 0)))
	{
		...
		ASSERT(rptr <= rend);
		size_t toCopy = ((pendingPacket->size - pendingPacketSize) < (uint32_t)(rend - rptr)) ? 
			             (pendingPacket->size - pendingPacketSize) : (uint32_t)(rend - rptr);
...
// BEGIN netfinity: Advanced download throttling
int32_t	CEMSocket::globalDownloadLimit = 0;
bool	CEMSocket::globalDownloadLimitEnable = false;
uint32_t	CEMSocket::globalDownloadLimitLastSet = ::GetTickCount();

void CEMSocket::SetGlobalDownloadLimit(uint32_t limit){	
	uint32_t	currentTick = ::GetTickCount();
	globalDownloadLimit += limit * min<int32_t>(1000, currentTick - globalDownloadLimitLastSet) / 1000;
	if (globalDownloadLimit < -((int32_t) limit))
		globalDownloadLimit = -((int32_t) limit);
	if (globalDownloadLimit > ((int32_t) limit))
		globalDownloadLimit = ((int32_t) limit);
	globalDownloadLimitLastSet = currentTick;
	globalDownloadLimitEnable = true;	
}

void CEMSocket::DisableGlobalDownloadLimit(){
	globalDownloadLimitLastSet = ::GetTickCount();
	globalDownloadLimitEnable = false;
}

uint32_t CEMSocket::GetDownloadLimit() const{
	uint32_t currentTick = ::GetTickCount();
	int32_t limit = 1000000000UL; // Extremly high! (1 GB/s)
	if(globalDownloadLimitEnable == true)
		limit = globalDownloadLimit;
	if(downloadLimitEnable == true && limit > downloadLimit)
		limit = downloadLimit;
	// If more than 500 ms since last update, refresh the minimum download limit
	if (currentTick - downloadLimitLastSet > 500)
	{
		// Minimum slot speed should be 200 bytes per second (download will timeout otherwise)
		downloadLimitMin += 200 * min<int32_t>(1000, currentTick - downloadLimitLastSet) / 1000;
		if (downloadLimitMin > 200)
			downloadLimitMin = 200;
		downloadLimitLastSet = currentTick;
	}
	return (uint32_t) max(0, max(limit, downloadLimitMin));
}

void CEMSocket::SetDownloadLimit(uint32_t limit){	
	uint32_t	currentTick = ::GetTickCount();
	downloadLimit += limit * min<int32_t>(1000, currentTick - downloadLimitLastSet) / 1000;
	if (downloadLimit < -((int32_t) limit))
		downloadLimit = -((int32_t) limit);
	if (downloadLimit > ((int32_t) limit))
		downloadLimit = ((int32_t) limit);
	// Minimum slot speed should be 200 bytes per second (download will timeout otherwise)
	downloadLimitMin += 200 * min<int32_t>(1000, currentTick - downloadLimitLastSet) / 1000;
 ...
 */
void CEMSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32_t actualPayloadSize, bool bForceImmediateSend){
...
uint64_t CEMSocket::GetSentBytesCompleteFileSinceLastCallAndReset() {
    sendLocker.Lock();

    uint64_t sentBytes = m_numberOfSentBytesCompleteFile;
    m_numberOfSentBytesCompleteFile = 0;
...
uint64_t CEMSocket::GetSentBytesPartFileSinceLastCallAndReset() {
    sendLocker.Lock();

    uint64_t sentBytes = m_numberOfSentBytesPartFile;
    m_numberOfSentBytesPartFile = 0;
...
uint64_t CEMSocket::GetSentBytesControlPacketSinceLastCallAndReset() {
    sendLocker.Lock();

    uint64_t sentBytes = m_numberOfSentBytesControlPacket;
    m_numberOfSentBytesControlPacket = 0;
...
uint64_t CEMSocket::GetSentPayloadSinceLastCallAndReset() {
    sendLocker.Lock();

    uint64_t sentBytes = m_actualPayloadSizeSent;
    m_actualPayloadSizeSent = 0;
 ...
 */
SocketSentBytes CEMSocket::Send(uint32_t maxNumberOfBytesToSend, uint32_t minFragSize, bool onlyAllowedToSendControlPacket) {
    ...
    uint32_t sentStandardPacketBytesThisCall = 0;
    uint32_t sentControlPacketBytesThisCall = 0;
             ...
             )
		{
                   ...
                   anErrorHasOccured == false)
			{
                ...
                if(!onlyAllowedToSendControlPacket || m_currentPacket_is_controlpacket) {
                    ...
                    uint32_t nextFragMaxBytesToSent = GetNextFragSize(sentStandardPacketBytesThisCall + sentControlPacketBytesThisCall, minFragSize);
		        ...
		        uint32_t result = CEncryptedStreamSocket::Send(sendbuffer+sent,tosend); // deadlake PROXYSUPPORT - changed to AsyncSocketEx
		        if (result == (uint32_t)SOCKET_ERROR){
...
uint32_t CEMSocket::GetNextFragSize(uint32_t current, uint32_t minFragSize) {
 ...
 * @author SlugFiller
 */
uint32_t CEMSocket::GetNeededBytes() {
	sendLocker.Lock();
	if (byConnected == ES_DISCONNECTED) {
		sendLocker.Unlock();
		return 0;
	...
	uint32_t sendgap = ::GetTickCount() - lastCalledSend;

	uint64_t timetotal = m_bAccelerateUpload?45000:90000;
	uint64_t timeleft = ::GetTickCount() - lastFinishedStandard;
	uint64_t sizeleft, sizetotal;
	...
	uint64_t decval = timeleft*sizetotal/timetotal;
	if (!decval)
		return (UINT)sizeleft;
	if (decval < sizeleft)
...
// BEGIN netfinity: Recover partial part packets
// WARNING! Code below only works on little-endian CPU's (Beware when porting to PPC and others)
void CEMSocket::RecoverPackets()
{
	uint64_t	start, stop;
	uint64_t	missingbytes;

	if (pendingPacket->prot == OP_EDONKEYPROT && pendingPacket->opcode == OP_SENDINGPART && pendingPacketSize > 24)
	{
		missingbytes = (pendingPacket->size - pendingPacketSize);
		start = *((unsigned long*) (&pendingPacket->pBuffer[16]));
		stop = *((unsigned long*) (&pendingPacket->pBuffer[20])) - 1;
		*((unsigned long*) (&pendingPacket->pBuffer[20])) = (uint32_t) (stop - missingbytes) + 1;
		pendingPacket->size -= (uint32_t) missingbytes;
		// Process recovered packet
		if (PacketReceived(pendingPacket))
			DebugLog(_T("EMSocket: Recovered %d bytes from incomplete part packet."), pendingPacket->size - 24);
		else
			DebugLogWarning(_T("EMSocket: Failed to recover an incomplete part packet."));
		// Create new header for remaining data
		*((unsigned long*) (&pendingPacket->pBuffer[16])) = (uint32_t) (stop - missingbytes + 1);
		*((unsigned long*) (&pendingPacket->pBuffer[20])) = (uint32_t) (stop) + 1;
		pendingPacket->size = (uint32_t) missingbytes + 24;
		pendingPacketSize = 24;
	}
}
// END netfinity: Recover partial part packets
