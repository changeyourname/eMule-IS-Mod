...
public:
	...
	UINT		GetServerCount() const { return (UINT) list.GetCount(); }
	...
	CServer*	GetServerByAddress(LPCTSTR address, uint16_t port) const;
	CServer*	GetServerByIP(uint32_t nIP) const;
	CServer*	GetServerByIPTCP(uint32_t nIP, uint16_t nTCPPort) const;
	CServer*	GetServerByIPUDP(uint32_t nIP, uint16_t nUDPPort, bool bObfuscationPorts = true) const;
	...
	void		GetStatus(uint32_t& total, uint32_t& failed, uint32_t& user, uint32_t& file, uint32_t& lowiduser, 
						  uint32_t& totaluser, uint32_t& totalfile, float& occ) const;
	void		GetAvgFile(uint32_t& average) const;
	void		GetUserFileStatus(uint32_t& user, uint32_t& file) const;
