...
BOOL CMiniMule::OnInitDialog()
{
	...
	*/
	SetWindowText(_T("eMule v") + theApp.m_strCurVersionLong + _T(" ") + theApp.m_strCurVersionLongMod+_T("\0"));
...
void CMiniMule::OnClose()
{
	...
	if (GetAutoClose()) { // ismod: AnimateWindow always available 
...
{
	if (theApp.IsRunning() && !theApp.emuledlg->IsWindowVisible())
	{
