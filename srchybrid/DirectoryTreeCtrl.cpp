...
void CDirectoryTreeCtrl::Init(void)
{
	...
	if (dwRet > 0 && dwRet < _countof(drivebuffer))
	{
		...
		while(*pos != _T('\0')){
			...
			// Copy drive name
			TCHAR drive[4];
		_tcsncpy_s(drive, _countof(drive), pos, _countof(drive));
			drive[_countof(drive) - 1] = _T('\0');

			// ismod: no floppy
			if(drive[0] == _T('a') || drive[0] ==_T('A') || drive[0] == _T('b') || drive[0] ==_T('B'))
				continue;
...
BOOL CDirectoryTreeCtrl::OnCommand(WPARAM wParam, LPARAM /*lParam*/)
{
	if (wParam < MP_SHAREDFOLDERS_FIRST)
	{
		...
		theDesktop.ShellExecute(_T("open"), m_strLastRightClicked);
	}
	else
	{
		...
		if (pos)
			...
			theDesktop.ShellExecute(_T("open"), m_lstShared.GetAt(pos));
	}
