	...
	if (m_pDirectoryFilter != NULL){
		...
		switch(m_pDirectoryFilter->m_eItemType){
			...
			case SDI_INCOMING:
				...
				break;
			// ismod [start] shared files by type
			case SDI_ED2KFILETYPE:
				if (m_pDirectoryFilter->m_nCatFilter == -1 || m_pDirectoryFilter->m_nCatFilter != GetED2KFileTypeID(file->GetFilePath()))
					return;
				break;
			// ismod [end] shared files by type
		}
	}
...
{
	CString str;
	CString strTotalSize = CastItoXBytes(theApp.sharedfiles->GetTotalShareSize());
	str.Preallocate(255);
	// ismod: no aich
	if (theApp.sharedfiles->GetHashingCount()) {
		str.Format(_T(" (%i - %s, %s %i"), theApp.sharedfiles->GetCount(), strTotalSize,
			GetResString(IDS_HASHING), theApp.sharedfiles->GetHashingCount() );
		if( NULL != theApp.sharedfiles->m_currentlyHashingFile )
			str += _T(" - ") + theApp.sharedfiles->m_currentlyHashingFile->strName;
		str += _T(")");
	}else
		str.Format(_T(" (%i - %s)"), theApp.sharedfiles->GetCount(), strTotalSize);
	theApp.emuledlg->sharedfileswnd->GetDlgItem(IDC_TRAFFIC_TEXT)
			->SetWindowText(GetResString(IDS_SF_FILES) + str);
}
				...
				switch (iColumn)
				{
					case 0: {
						...
						if (CheckBoxesEnabled())
						{
							...
							if (hTheme != NULL)
								DrawThemeBackground(hTheme, dc.GetSafeHdc(), BP_CHECKBOX, iState, &recCheckBox, NULL);
							else
					...
					case 8:
						if (pKnownFile != NULL && pKnownFile->partCount()) {
							cur_rec.bottom--;
							cur_rec.top++;
...
void CSharedFilesCtrl::GetItemDisplayText(const CShareableFile* file, int iSubItem, LPTSTR pszText, int cchTextMax) const
{
	...
	if (file->IsKindOf(RUNTIME_CLASS(CKnownFile))){
		...
		{			
			...
			case 8:
				_sntprintf(pszText, cchTextMax, _T("%s: %u"), GetResString(IDS_SHARED_STATUS), pKnownFile->partCount());
				break;
			...
			case 15:{
				uint32_t nMediaLength = pKnownFile->GetIntTagValue(FT_MEDIA_LENGTH);
				if (nMediaLength){
					CString buffer;
			...
			case 16:{
				uint32_t nBitrate = pKnownFile->GetIntTagValue(FT_MEDIA_BITRATE);
				if (nBitrate)
...
void CSharedFilesCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	m_SharedFilesMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_PrioMenu.m_hMenu) & UINT_MAX), (!bContainsShareableFiles && iSelectedItems > 0) ? MF_ENABLED : MF_GRAYED);
		...
		switch (wParam){
			...
			case MP_OPENFOLDER:
				if (file && !file->IsPartFile())
					...
					theDesktop.ShellExecute(_T("open"), file->GetPath());
				break; 
			case MP_RENAME:
			case MPG_F2:
			...
			case MPG_DELETE:
			{
				bool bNoRecBin = (GetKeyState(VK_DELETE) & 0x8000)!=0;
				...
				while (!selectedList.IsEmpty())
				{
					...
					bool delsucc = ShellDeleteFile(myfile->GetFilePath(), bNoRecBin);
					if (delsucc){
...
int CSharedFilesCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	if (bExtColumn)
	{
		...
		else if (item1->IsKindOf(RUNTIME_CLASS(CKnownFile)) && item2->IsKindOf(RUNTIME_CLASS(CKnownFile)))
		{
			...
			{
				case 3:{//prio
					uint8_t p1 = kitem1->GetUpPriority() + 1;
					if (p1 == 5)
						p1 = 0;
					uint8_t p2 = kitem2->GetUpPriority() + 1;
					if (p2 == 5)
						p2 = 0;
					iResult = p1 - p2;
					break;
...
{
	bool bSharedInKad;
	if ((uint32_t)time(NULL) < file->GetLastPublishTimeKadSrc()) {
			...
			if (!liToAddFiles.IsEmpty() || !liToAddDirs.IsEmpty())
			{
				...
				else if (!liToAddDirs.IsEmpty() && !bHaveFiles)
				{
					...
					if (liToAddDirs.GetCount() == 1)
						theApp.emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowDirectory(liToAddDirs.GetHead(), SDI_DIRECTORY);
					else
						theApp.emuledlg->sharedfileswnd->m_ctlSharedDirTree.ShowDirectory(_T(""), SDI_DIRECTORY);
				}
				else
				{
...
// ismod [start]
void CSharedFilesCtrl::GotoFile(const CKnownFile *pFile)
{
