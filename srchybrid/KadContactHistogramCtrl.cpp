...
//	UINT uHistSlot = h.ConvertToLong();
	...
	//DWORD dwHighestWord = *(DWORD*)KadUint128.GetData(); // netfinity: This breaks when words are in reverse ordered
	DWORD dwHighestWord = KadUint128.Get32BitChunk(0);
...
void CKadContactHistogramCtrl::OnPaint()
{
	...
	for (UINT s = 0; s <= uMax; s += uStep)
	{
		...
		TCHAR szLabel[12];
		int iLabelLen = _sntprintf_s(szLabel, _countof(szLabel), ARRAYSIZE(szLabel), _T("%u"), s);
