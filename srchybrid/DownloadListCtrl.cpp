...
bool CDownloadListCtrl::RemoveFile(const CPartFile* toremove)
{
	...
	// ismod [start]
	if ( theApp.emuledlg->transferwnd->GetDownloadBarFile() == toremove ) {
		theApp.emuledlg->transferwnd->SetDownloadBarFile(NULL);
	}
	// ismod [end]
...
// <-- ZZ:DownloadManager
		...
		case 11: // last receive
			if (lpPartFile->GetFileDate() != NULL && lpPartFile->GetCompletedSize() > (uint64_t)0)
	...
	switch (nColumn)
	{
		...
		case 5: {	// progress
			...
			if (pCtrlItem->dwUpdated + DLC_BARUPDATE < dwTicks || cx !=  iWidth || !pCtrlItem->dwUpdated)
			{
				...
				pPartFile->DrawSimpleStatusBar(&cdcStatus,  &rec_status, thePrefs.UseFlatBar());
				theApp.emuledlg->transferwnd->UpdateDownloadBar(); // ismod
			...
			if (thePrefs.GetUseDwlPercentage())
			{
				...
				*/
				_tcscpy_s(szItem, _countof(szItem), pPartFile->getProgressbarStatus());
...
void CDownloadListCtrl::DrawSourceItem(CDC *dc, int nColumn, LPCRECT lpRect, UINT uDrawTextAlignment, CtrlItem_Struct *pCtrlItem)
{
	...
	// ismod: client color highlight
	dc->SetTextColor(pClient->GetClientColor());
...
void CDownloadListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if ((cur_rec.top < rcClient.top || cur_rec.top > rcClient.bottom) 
		&&
		(cur_rec.bottom < rcClient.top || cur_rec.bottom > rcClient.bottom))
		return;
	...
	if (content->type == FILE_TYPE)
	{
		...
		/*if ((lpDrawItemStruct->itemState & ODS_SELECTED) == 0) { // ismod */
...
void CDownloadListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			m_FileMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_PrioMenu.m_hMenu) & UINT_MAX), iFilesNotDone > 0 ? MF_ENABLED : MF_GRAYED);
			...
			// ismod: goto file
			m_FileMenu.EnableMenuItem(MP_GOTOFILE, (iSelectedItems == 1 && file1->CanOpenFile()) ? MF_ENABLED : MF_GRAYED);
			...
			if (m_SourcesMenu && thePrefs.IsExtControlsEnabled()) {
				m_FileMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_SourcesMenu.m_hMenu) & UINT_MAX), MF_ENABLED);
				/* ismod: add sources to many files
				m_SourcesMenu.EnableMenuItem(MP_ADDSOURCE, (iSelectedItems == 1 && iFilesToStop == 1) ? MF_ENABLED : MF_GRAYED);
				*/
				m_SourcesMenu.EnableMenuItem(MP_ADDSOURCE, (iSelectedItems == iFilesToStop) ? MF_ENABLED : MF_GRAYED);
...
// <-- ZZ:DownloadManager
		...
		int total;
		m_FileMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_PrioMenu.m_hMenu) & UINT_MAX), MF_GRAYED);
		...
		// ismod: goto file
		m_FileMenu.EnableMenuItem(MP_GOTOFILE, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_CLEARCOMPLETED, GetCompleteDownloads(curTab,total) > 0 ? MF_ENABLED : MF_GRAYED);
		m_FileMenu.EnableMenuItem(thePrefs.GetShowCopyEd2kLinkCmd() ? MP_GETED2KLINK : MP_SHOWED2KLINK, MF_GRAYED);
		m_FileMenu.EnableMenuItem(MP_PASTE, theApp.IsEd2kFileLinkInClipboard() ? MF_ENABLED : MF_GRAYED);
		m_FileMenu.SetDefaultItem((UINT)-1);
		if (m_SourcesMenu)
			m_FileMenu.EnableMenuItem(static_cast<UINT>(reinterpret_cast<UINT_PTR>(m_SourcesMenu.m_hMenu) & UINT_MAX), MF_GRAYED);
	...
	if (iSel != -1)
	{
		...
		if (content != NULL && content->type == FILE_TYPE)
		{
			...
			switch (wParam)
			{
							...
							while (!selectedList.IsEmpty())
							{
								...
								switch (selectedList.GetHead()->GetStatus())
								{
									...
									case PS_COMPLETE:
										...
										RemoveFile(selectedList.RemoveHead());
										bRemovedItems = true;
										break;
									case PS_PAUSED:
										selectedList.RemoveHead()->DeleteFile();
				...
				case MP_ADDSOURCE: {
					CAddSourceDlg addSourceDlg;
					if (  addSourceDlg.DoModal() == IDOK ) {
                        for( auto it = addSourceDlg.sources.begin(); it != addSourceDlg.sources.end(); ++it ) {
                            // TODO: rewrite this code section, do better input data validation
                            CString & source = *it;
                            uint16_t port = 0;
			                int iColon = source.Find(_T(':'));
			                if ( iColon != -1 ) {
				                port = (uint16_t)_tstoi(source.Mid(iColon + 1));
				                source = source.Left(iColon);
			                }

			                uint32_t ip = inet_addr(CT2CA(source));
			                if ( IsGoodIPPort(ip, port) ) {
                                for ( POSITION pos = selectedList.GetHeadPosition(); pos != NULL; ) {
                                    CPartFile * file = selectedList.GetNext(pos);
               					    CUpDownClient* toadd = new CUpDownClient(file, port, ntohl(ip), 0, 0);
	            			        toadd->SetSourceFrom(SF_PASSIVE);
					                theApp.downloadqueue->CheckAndAddSource(file, toadd);
                                }
                            }
                        }
                    }
				...
				default:
					if (wParam>=MP_WEBURL && wParam<=MP_WEBURL+99){
						theWebServices.RunURL(file, static_cast<UINT>(wParam));
					}
						...
						thePreviewApps.RunApp(file, static_cast<UINT>(wParam));
					}
					break;
			}
		}
...
int CDownloadListCtrl::SortProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	...
	int dwOrgSort = static_cast<int>(lParamSort);
	int sortMod = 1;
	if (lParamSort >= 100)
	{
		sortMod = -1;
...
void CDownloadListCtrl::OnListModified(NMHDR *pNMHDR, LRESULT * /*pResult*/)
{
	...
	if ( pNMListView->uNewState&LVIS_SELECTED && !(pNMListView->uOldState&LVIS_SELECTED) ) {
		const CtrlItem_Struct* content = (CtrlItem_Struct*)GetItemData(pNMListView->iItem);
		if ( content && FILE_TYPE == content->type ) {
			file = static_cast<CPartFile*>(content->value);
		} else if ( content ) {
			file = content->owner;
		}
	...
	theApp.emuledlg->transferwnd->SetDownloadBarFile(file);
	// ismod [end]
...
int CDownloadListCtrl::Compare(const CPartFile *file1, const CPartFile *file2, LPARAM lParamSort)
{
	...
	switch (lParamSort)
	{
		...
		case 4: //speed asc
			/* ismod: compare by transferring srcs
			...
			*/
			...
			//downloading should be better and fix that issue wink.gif
...
void CDownloadListCtrl::CreateMenues()
{
	...
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	// ismod: goto file
	m_FileMenu.AppendMenu(MF_STRING, MP_GOTOFILE, GetResString(IDS_MOD_GOTOFILE), _T("SEARCHFILETYPE_ANY"));
	m_FileMenu.AppendMenu(MF_SEPARATOR);
	...
	for (ListItems::const_iterator it = m_ListItems.begin(); it != m_ListItems.end(); ++it)
	{
		...
		if (cur_item->type == FILE_TYPE)
		{
			...
			if (file->GetStatus() == PS_COMPLETE) {
				fsize += (uint64_t)file->GetFileSize();
			}
		}
	}
	return fsize;
...
void CDownloadListCtrl::MoveCompletedfilesCat(uint8_t from, uint8_t to)
{
	int mycat;
...
void CDownloadListCtrl::OnLvnGetInfoTip(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (pGetInfoTip->iSubItem == 0)
	{
		...
		if (content && pGetInfoTip->pszText && pGetInfoTip->cchTextMax > 0)
		{
			...
			_tcsncpy_s(pGetInfoTip->pszText, pGetInfoTip->cchTextMax, info, pGetInfoTip->cchTextMax);
...
bool CDownloadListCtrl::ReportAvailableCommands(CList<int>& liAvailableCommands)
{
	...
	// ismod
	liAvailableCommands.AddTail(MP_AUTOSHUTDOWN);

	return true;
}

// ismod [start]
void CDownloadListCtrl::GotoFile(const CPartFile *pFile)
{
	int Item = FindFile(pFile);

	if (Item != -1)
	{
		SetItemState(-1, 0, LVIS_SELECTED);
		SetItemState(Item, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
		SetSelectionMark(Item);	 // display selection mark correctly! 
		EnsureVisible(Item, FALSE);
		Update(Item);
	}
}

int CDownloadListCtrl::FindFile(const CPartFile* tofind)
{
	if (!theApp.IsRunning())
	{
		return -1;
	...
	for(ListItems::iterator it = m_ListItems.begin(); it != m_ListItems.end(); )
	{
		CtrlItem_Struct* findItem = it->second;
		if(findItem->owner == tofind || findItem->value == (void*)tofind)
		{
			// Remove it from the CListCtrl
			LVFINDINFO find;
			find.flags = LVFI_PARAM;
			find.lParam = (LPARAM)findItem;
