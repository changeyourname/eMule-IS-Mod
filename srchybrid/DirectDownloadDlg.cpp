...
CDirectDownloadDlg::CDirectDownloadDlg(CWnd* pParent /*=NULL*/)
	: CResizableDialog(IDD_DIRECT_DOWNLOAD, pParent)
{
	m_icnWnd = NULL;
}
...
BOOL CDirectDownloadDlg::OnInitDialog()
{
	...
	// ismod: shit [start]
	if (theApp.m_bClipboardLink)
	{		
		theApp.ClipboardText.Replace(_T("%7c"), _T("|"));
		theApp.ClipboardText.Replace(_T("%7C"), _T("|"));
		GetDlgItem(IDC_ELINK)->SetWindowText(theApp.ClipboardText);
		SetForegroundWindow();
		BringWindowToTop();
	}
	// ismod [end]
...
// ismod: shit [start]
void CDirectDownloadDlg::AddLink(CString strLinks)
{
	CString content;
	strLinks.Replace(_T("%7c"), _T("|"));
	strLinks.Replace(_T("%7C"), _T("|"));
	GetDlgItem(IDC_ELINK)->GetWindowText(content);
	if (content.Find(strLinks) != -1)
	{
		return;
	}
	while (content.Right(2) == _T("\r\n"))
	{
		content = content.Left(content.GetLength()-2);
	}
	if (content.IsEmpty())
	{
		content += strLinks;
