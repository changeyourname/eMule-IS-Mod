...
void AddLogTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	...
	_vsntprintf_s(szLogLine, _countof(szLogLine), _countof(szLogLine), pszLine, argptr);
	...
	else
	{
		...
		int iLen = _sntprintf_s(szFullLogLine, _countof(szFullLogLine), _countof(szFullLogLine), _T("%s: %s\r\n"), CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), szLogLine);
		if (iLen > 0)
		{
			if (!(uFlags & LOG_DEBUG))
			{
...
bool CLogFile::Logf(LPCTSTR pszFmt, ...)
{
	...
	TCHAR szMsg[1024];
	_vsntprintf_s(szMsg, _countof(szMsg), _countof(szMsg), pszFmt, argp);
	szMsg[_countof(szMsg) - 1] = _T('\0');

	TCHAR szFullMsg[1060];
	int iLen = _sntprintf_s(szFullMsg, _countof(szFullMsg), _countof(szFullMsg), _T("%s: %s\r\n"), CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()), szMsg);
	if (iLen <= 0)
		return false;
	va_end(argp);
...
void CLogFile::StartNewLogFile()
{
	...
	_tsplitpath_s(m_strFilePath, szDrv, _countof(szDrv), szDir, _countof(szDir), szNam, _countof(szNam), szExt, _countof(szExt));
