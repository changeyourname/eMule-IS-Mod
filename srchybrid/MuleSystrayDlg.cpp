...
// ismod
#include "sockets.h" // for severconnect
...
BEGIN_MESSAGE_MAP(CMuleSystrayDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	// ismod: show popups option
	ON_STN_CLICKED(IDC_MOD_SHOWPOPUP, OnStnClickedShowpopup)
END_MESSAGE_MAP()
...
BOOL CMuleSystrayDlg::OnInitDialog() 
{
	...
	if(p)
	{
		p->GetWindowRect(r);
		ScreenToClient(r);
		/* ismod: changed styles
		m_ctrlConnect.Create(NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_CONNECT);
		*/
		m_ctrlConnect.CreateEx(theApp.serverconnect->IsConnected() ? NULL : WS_EX_STATICEDGE, NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_CONNECT);
		...
		if(bValidFont)
		{
			...
			*/
			...
			if (theApp.serverconnect->IsConnected())
				lfFont.lfWeight += 200;			// make it bold
			m_ctrlConnect.m_cfFont.CreateFontIndirect(&lfFont);
		}
	}
	...
	if(p)
	{
		...
		*/
		m_ctrlDisconnect.CreateEx(theApp.serverconnect->IsConnected() ? WS_EX_STATICEDGE : NULL, NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_DISCONNECT);
		...
		*/
			...
			if (!theApp.serverconnect->IsConnected())
				lfFont.lfWeight += 200;			// make it bold
			m_ctrlDisconnect.m_cfFont.CreateFontIndirect(&lfFont);
		}
	}
	...
	// ismod: show popups button
	p = GetDlgItem(IDC_MOD_SHOWPOPUP);
	if(p)
	{
		p->GetWindowRect(r);
		ScreenToClient(r);
		m_ctrlShowpopup.CreateEx((thePrefs.notifierOnChat) ? WS_EX_STATICEDGE: NULL, NULL, NULL, WS_CHILD|WS_VISIBLE, r, this, IDC_MOD_SHOWPOPUP);
		m_ctrlShowpopup.m_nBtnID = IDC_MOD_SHOWPOPUP;
		//p->GetWindowText(m_ctrlPreferences.m_strText);
		m_ctrlShowpopup.m_strText = (thePrefs.notifierOnChat) ? _T("Отключить всплывающее окно") : _T("Включить всплывающее окно");
		m_ctrlShowpopup.m_strText.Remove(_T('&'));

		m_ctrlShowpopup.m_bUseIcon = true;
		m_ctrlShowpopup.m_sIcon.cx = 16;
		m_ctrlShowpopup.m_sIcon.cy = 16;
		m_ctrlShowpopup.m_hIcon = theApp.LoadIcon(_T("NOTIFICATIONS"), m_ctrlShowpopup.m_sIcon.cx, m_ctrlShowpopup.m_sIcon.cy);
		m_ctrlShowpopup.m_bParentCapture = true;
		if(bValidFont)
		{
			LOGFONT lfFont = lfStaticFont;
			if (thePrefs.notifierOnChat)
				lfFont.lfWeight += 200;			// make it bold
			m_ctrlShowpopup.m_cfFont.CreateFontIndirect(&lfFont);
		}
	}
	...
	/* ismod: 100 Kb/sec min
	m_ctrlUpSpeedSld.SetRange(0,m_iMaxUp);
	*/
	m_ctrlUpSpeedSld.SetRange(100,m_iMaxUp);
	...
	*/
	m_ctrlSidebar.SetWindowText(_T("eMule ") + theApp.m_strCurVersionLong + _T(" ") + theApp.m_strCurVersionLongMod);
...
// ismod [start]
void CMuleSystrayDlg::OnStnClickedShowpopup()
{
	thePrefs.notifierOnChat = !thePrefs.notifierOnChat;
	ReleaseCapture();
	EndDialog(m_nExitCode);
	m_bClosingDown = true;
}
