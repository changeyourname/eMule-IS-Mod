...
void CUpDownClient::DrawStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	if (!onlygreyrect && reqfile && m_abyPartStatus)
	{
		...
		if (m_nDownloadState == DS_DOWNLOADING){
			pcNextPendingBlks = new char[m_partCount];
			memset(pcNextPendingBlks, 'N', m_partCount); // do not use '_strnset' for uninitialized memory!
			for (POSITION pos = m_PendingBlocks_list.GetHeadPosition(); pos != 0; ){
				UINT uPart = (UINT)(m_PendingBlocks_list.GetNext(pos)->block->StartOffset / PARTSIZE);
				if (uPart < m_partCount)
					pcNextPendingBlks[uPart] = 'Y';
			}
		}

		for (uint16_t i = 0; i < m_partCount; i++){
			if (m_abyPartStatus[i]){
				uint64_t uEnd;
				if ( PARTSIZE*(uint64_t)(i+1) > reqfile->GetFileSize())
					uEnd = reqfile->GetFileSize();
				else
					uEnd = PARTSIZE*(uint64_t)(i+1);

				if (reqfile->IsPartComplete(i))
					s_StatusBar.FillRange(PARTSIZE*(uint64_t)i, uEnd, crBoth);
				else if (GetSessionDown() > 0 && m_nDownloadState == DS_DOWNLOADING && m_nLastBlockOffset >= PARTSIZE*(uint64_t)i && m_nLastBlockOffset < uEnd)
					s_StatusBar.FillRange(PARTSIZE*(uint64_t)i, uEnd, crPending);
				else if (pcNextPendingBlks != NULL && pcNextPendingBlks[i] == 'Y')
					s_StatusBar.FillRange(PARTSIZE*(uint64_t)i, uEnd, crNextPending);
				else
					s_StatusBar.FillRange(PARTSIZE*(uint64_t)i, uEnd, crClientOnly);
			}
		}
...
void CUpDownClient::SendFileRequest()
{
	...
	if (SupportMultiPacket())
	{
		...
		if (GetExtendedRequestsVersion() > 1)
			dataFileReq.WriteUInt16(reqfile->GetCompleteSourcesCount());

		// OP_SETREQFILEID
		if (thePrefs.GetDebugClientTCPLevel() > 0)
			DebugSend("OP__MPSetReqFileID", this, reqfile->GetFileHash());
		if (reqfile->partCount() > 1)
		...
		if (IsSourceRequestAllowed())
		{
			...
			if (SupportsSourceExchange2()){
				...
				const uint16_t nOptions = 0; // 16 ... Reserved
...
void CUpDownClient::SendStartupLoadReq()
{
	...
	// BEGIN SiRoB: Download Collision fix
	if (m_fFailedDownload == 1 && !GetSentCancelTransfer()) {
		if(thePrefs.GetLogUlDlEvents())
			DebugLog(_T("FIX CONNECTION COLLISION Failed download Successfully rescued client: %s"),DbgGetClientInfo());
		ProcessAcceptUpload();
		return;
	}
	// END SiRoB: Download Collision fix
	if (thePrefs.GetDebugClientTCPLevel() > 0)
...
void CUpDownClient::ProcessFileInfo(CSafeMemFile* data, CPartFile* file)
{
	...
	if (reqfile->partCount() == 1)
	{
		...
		m_partCount = reqfile->partCount();
		m_abyPartStatus = new uint8_t[m_partCount];
		memset(m_abyPartStatus,1,m_partCount);
		...
		if (thePrefs.GetDebugClientTCPLevel() > 0)
		{
			...
			char* psz = new char[m_partCount + 1];
			for (i = 0; i < m_partCount; i++)
				psz[i] = m_abyPartStatus[i] ? '#' : '.';
			psz[i] = '\0';
			Debug(_T("  Parts=%u  %hs  Needed=%u\n"), m_partCount, psz, iNeeded);
			delete[] psz;
		}
		UpdateDisplayedInfo();
		...
		if ( reqfile->hashsetShate() == PFHS_REQUIRED )
			SendHashSetRequest();
		else
			SendStartupLoadReq();
...
void CUpDownClient::ProcessFileStatus(bool bUdpPacket, CSafeMemFile* data, CPartFile* file)
{
	...
	uint16_t nED2KPartCount = data->ReadUInt16();
	delete[] m_abyPartStatus;
	m_abyPartStatus = NULL;
	...
	else
	{
		...
		while (done != m_partCount)
		{
			...
			for (UINT i = 0; i != 8; i++)
			{
				...
				{
					if (!reqfile->IsPartComplete(done)){
						bPartsNeeded = true;
						iNeeded++;
					}
				}
	...
	if (bUdpPacket ? (thePrefs.GetDebugClientUDPLevel() > 0) : (thePrefs.GetDebugClientTCPLevel() > 0))
	{
		TCHAR* psz = new TCHAR[m_partCount + 1];
		UINT i;
		for (i = 0; i < m_partCount; i++)
	...
	if (!bUdpPacket)
	{
		...
		else if ( reqfile->hashsetShate() == PFHS_REQUIRED ) //If we are using the eMule filerequest packets, this is taken care of in the Multipacket!
			SendHashSetRequest();
		else
			SendStartupLoadReq();
	}
...
void CUpDownClient::SetDownloadState(EDownloadState nNewState, LPCTSTR pszReason){
	if (m_nDownloadState != nNewState){
		...
		if (m_nDownloadState == DS_DOWNLOADING ){
			...
			//m_nDownDatarate = 0;
			m_nDownDatarate.Reset(); // netfinity: Transfer Rate BlackBox
...
void CUpDownClient::ProcessHashSet(const uchar* packet,uint32_t size)
{
	...
	if ( reqfile->LoadHashsetFromFile(&data,true) ) {
		reqfile->setHashestState(PFHS_OK);
		m_fHashsetRequesting = 0;
		reqfile->HashCompleteParts(); // rehash
	}
	else{
...
void CUpDownClient::CreateBlockRequests(int iMaxBlocks)
{
	...
	if (m_DownloadBlocks_list.IsEmpty())
	{
        ...
        if(iMaxBlocks > m_PendingBlocks_list.GetCount()) {
            count = (uint16_t)(iMaxBlocks - m_PendingBlocks_list.GetCount());
        } else {
            count = 0;
        }
	...
	while (m_PendingBlocks_list.GetCount() < iMaxBlocks && !m_DownloadBlocks_list.IsEmpty())
	{
		...
		pblock->block->data = new BYTE[(UINT)(pblock->block->EndOffset-pblock->block->StartOffset+1)];
...
void CUpDownClient::SendBlockRequests()
{
    ...
    int blockCount = 3;
    if(IsEmuleClient() && m_byCompatibleClient==0 && reqfile->GetFileSize()-reqfile->GetCompletedSize() <= (uint64_t)PARTSIZE*4) {
	...
	if (bI64Offsets){
		...
		for (uint32_t i = 0; i != 3; i++){
			if (pos){
				...
				uint64_t endpos = block->EndOffset+1;
		...
		for (uint32_t i = 0; i != 3; i++){
			if (pos){
				...
				pending->fZStreamError = 0;
				pending->fRecovered = 0;
				data.WriteUInt32((uint32_t)pending->block->StartOffset);
			}
			else
				data.WriteUInt32(0);
		}
		...
		for (uint32_t i = 0; i != 3; i++){
			if (pos){
				Requested_Block_Struct* block = m_PendingBlocks_list.GetNext(pos)->block;
				uint64_t endpos = block->EndOffset+1;
				data.WriteUInt32((uint32_t)endpos);
				if (thePrefs.GetDebugClientTCPLevel() > 0){
					CString strInfo;
					strInfo.Format(_T("  Block request %u: "), i);
					strInfo += DbgGetBlockInfo(block);
...
/* Barry - Originally this only wrote to disk when a full 180k block 
		   ...
		   fill a gap.
*/
void CUpDownClient::ProcessBlockPacket(const uchar *packet, uint32_t size, bool packed, bool bI64Offsets)
{
	if (!bI64Offsets) {
	    uint32_t nDbgStartPos = *((uint32_t*)(packet+16));
	    if (thePrefs.GetDebugClientTCPLevel() > 1) {
		    if (packed)
			    Debug(_T("  Start=%u  BlockSize=%u  Size=%u  %s\n"), nDbgStartPos, *((uint32_t*)(packet + 16+4)), size-24, DbgGetFileInfo(packet));
		    else
			    Debug(_T("  Start=%u  End=%u  Size=%u  %s\n"), nDbgStartPos, *((uint32_t*)(packet + 16+4)), *((uint32_t*)(packet + 16+4)) - nDbgStartPos, DbgGetFileInfo(packet));
	    }
	}

	// Ignore if no data required
	if (!(GetDownloadState() == DS_DOWNLOADING || GetDownloadState() == DS_NONEEDEDPARTS)){
		TRACE("%s - Invalid download state(Client:%d)\n", __FUNCTION__, DbgGetClientInfo());
		return;
	}
	...
	uint32_t uTransferredFileDataSize = size - nHeaderSize;
	...
	m_nDownDatarate.AddTransferred(uTransferredFileDataSize); // netfinity: Transfer Rate BlackBox
	if (credits)
	...
	for (POSITION pos = m_PendingBlocks_list.GetHeadPosition(); pos != NULL; )
	{
		...
		Pending_Block_Struct *pblock = m_PendingBlocks_list.GetNext(pos);
		if ((pblock->block->StartOffset <= nStartPos) && (pblock->block->EndOffset >= nStartPos))
		{
			// Found reserved block
			Requested_Block_Struct* reqblock = pblock->block;

			if (pblock->fZStreamError){
				if (thePrefs.GetVerbose())
				...
				reqfile->RemoveBlockFromList(reqblock->StartOffset, reqblock->EndOffset);
				return;
			}
			...
			if (!packed)
			{
				...
				if (nEndPos > reqblock->EndOffset){
					DebugLogError(_T("Received Blockpacket exceeds requested boundaries (requested end: %I64u, Part %u, received end  %I64u, Part %u), file %s, client %s"), pblock->block->EndOffset
						, (uint32_t)(reqblock->EndOffset / PARTSIZE), nEndPos, (uint32_t)(nEndPos / PARTSIZE), reqfile->GetFileName(), DbgGetClientInfo());
					reqfile->RemoveBlockFromList(reqblock->StartOffset, reqblock->EndOffset);
					return;
				}

                uint32_t offset = (uint32_t)(nStartPos - reqblock->StartOffset);
                uint32_t length = (uint32_t)(nEndPos-nStartPos+1);
                CopyMemory(reqblock->data + offset,
                    (void*)(packet + nHeaderSize),
                    length);
                reqblock->transferred += uTransferredFileDataSize;
			}
			else // Packed
			{
				ASSERT( (int)size > 0 );
				...
				// Don't get too big
				if (lenUnzipped > (EMBLOCKSIZE*3 + 300))
					lenUnzipped = (EMBLOCKSIZE*3 + 300);
				BYTE *unzipped = new BYTE[lenUnzipped];

				// Try to unzip the packet
				int result = unzip(pblock, packet + nHeaderSize, uTransferredFileDataSize, &unzipped, &lenUnzipped);
				...
				if (result == Z_OK && (int)lenUnzipped >= 0)
				{
					if (lenUnzipped > 0) // Write any unzipped data to disk
					{
						...
						nStartPos = reqblock->StartOffset + pblock->totalUnzipped - lenUnzipped;
						nEndPos = reqblock->StartOffset + pblock->totalUnzipped - 1;

						if (nStartPos > reqblock->EndOffset || nEndPos > reqblock->EndOffset){
				...
				else
				{
					...
					{
						CString strZipError;
						if (pblock->zStream && pblock->zStream->msg)
							strZipError.Format(_T(" - %hs"), pblock->zStream->msg);
						if (result == Z_OK && (int)lenUnzipped < 0){
							ASSERT(0);
            ...
            // full block received, write it to partfile
            if ( nEndPos == reqblock->EndOffset )
            {
                m_PendingBlocks_list.RemoveAt(posLast);
				uint32_t lenWritten = reqfile->WriteToBuffer(reqblock);
                m_nCurSessionPayloadDown += lenWritten;
                reqfile->RemoveBlockFromList(reqblock->StartOffset, reqblock->EndOffset);
                // data can be stolen by part file :)
                if(reqblock->data) delete[] reqblock->data;
                delete reqblock;
                // Not always allocated
                if (pblock->zStream){
                    inflateEnd(pblock->zStream);
...
int CUpDownClient::unzip(Pending_Block_Struct* block, const BYTE* zipped, uint32_t lenZipped, BYTE** unzipped, uint32_t* lenUnzipped, int iRecursion)
{
...
uint16_t CUpDownClient::GetAvailablePartCount() const
{
	UINT result = 0;
	for (uint16_t i = 0; i < m_partCount; i++){
...
{
	// BEGIN netfinity: UDP Reask sequrity check - Don't accept responses if we haven't asked (can be from bogus clients)
	if (!m_bUDPPending)
	{
		if (thePrefs.GetVerbose())
	...
	// END netfinity: UDP Reask sequrity check
	m_bUDPPending = false;
    ...
    if(!curFileisNNPFile && cur_file->GetSourceCount() < cur_file->GetMaxSources() ||
        curFileisNNPFile && cur_file->GetSourceCount() < cur_file->GetMaxSources()*.8) {
            ...
            if(ignoreSuspensions  || !IsSwapSuspended(cur_file, doAgressiveSwapping, curFileisNNPFile)) {
                ...
                uint32_t allNnpReaskTime = FILEREASKTIME*2*((uint32_t) m_OtherNoNeeded_list.GetSize() + ((GetDownloadState() == DS_NONEEDEDPARTS)?1:0)); // wait two reask interval for each nnp file before reasking an nnp file
...
uint32_t CUpDownClient::GetTimeUntilReask(const CPartFile* file, const bool allowShortReaskTime, const bool useGivenNNP, const bool givenNNP) const {
...
void CUpDownClient::ProcessEmuleQueueRank(const uchar* packet, UINT size)
{
	...
	uint16_t rank = PeekUInt16(packet);
	if (thePrefs.GetDebugClientTCPLevel() > 0)
...
uint32_t CUpDownClient::GetLastAskedTime(const CPartFile* partFile) const
{
...
void CUpDownClient::SendHashSetRequest()
{
	...
	reqfile->setHashestState(PFHS_REQUESTING);
}
