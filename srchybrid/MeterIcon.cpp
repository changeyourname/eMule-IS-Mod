...
{// begin DrawIconMeter
	// draw meter
	HBRUSH hBrush = CreateSolidBrush(RGB(00,255,00)/*GetMeterColor(nLevel)*/);
	if(hBrush == NULL)
	{// begin error check
		return false;
	}// end error check
	...
	if(!Rectangle(hDestDC, m_sDimensions.cx-nPos*2, m_sDimensions.cy-((nLevel*(m_sDimensions.cy-1)/m_nMaxVal)+1), m_sDimensions.cx-nPos*2-2, m_sDimensions.cy))
	{// begin error check
		return false;
	}// end error check
