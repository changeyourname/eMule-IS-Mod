...
BOOL CHttpDownloadDlg::OnInitDialog() 
{
	...
	m_pThread = CSafeThread::BeginThread(_DownloadThread, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	if (m_pThread == NULL)
	{
...
{
	CString sOf;
	sOf.Format(GetResString(IDS_HTTPDOWNLOAD_OF), CastItoXBytes((uint64_t)dwBytesRead, false, false), CastItoXBytes((uint64_t)dwFileSize, false, false));
