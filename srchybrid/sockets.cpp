	...
	if (connectionattemps.GetCount() < (thePrefs.IsSafeServerConnectEnabled() ? 1 : 2))
	{
		...
		if (next_server == NULL)
		{
			...
			//{
				...
				else if (m_idRetryTimer == 0)
				{
					...
					// ismod: check blacklist status
					if ( theApp.m_LastBlacklistedTime < time(NULL) )
					{
                    ...
                    // ismod: say about blacklisting
                    else
					{
						uint32_t uRetryTime = (uint32_t)(theApp.m_LastBlacklistedTime-time(NULL));
						LogWarning(LOG_STATUSBAR, _T("Сервер временно заблокировал Вас для доступа.\n У Вас либо слишком много закачек, либо Вы слишком часто производили поиск на сервере."));
						AddLogLine(false, GetResString(IDS_RECONNECT), uRetryTime);
						VERIFY( (m_idRetryTimer = SetTimer(NULL, 0, uRetryTime*1000, RetryConnectTimer)) != NULL );
						if (thePrefs.GetVerbose() && !m_idRetryTimer)
							DebugLogError(_T("Failed to create 'server connect retry' timer - %s"), GetErrorMessage(GetLastError()));
					}
				}
		...
		if (thePrefs.GetAutoConnectToStaticServersOnly())
		{
			if (next_server->IsStaticMember())
				ConnectToServer(next_server, false, !m_bTryObfuscated);
		}
		else
			ConnectToServer(next_server, false, !m_bTryObfuscated);
	}
...
void CServerConnect::ConnectToAnyServer(UINT startAt, bool prioSort, bool isAuto, bool bNoCrypt)
{
	...
	// ismod: random server at startup
	if (startAt == UINT_MAX)
		startAt = UINT((double(rand())/RAND_MAX)*theApp.serverlist->GetServerCount());
	theApp.serverlist->SetServerPosition(startAt);
...
void CServerConnect::ConnectToServer(CServer* server, bool multiconnect, bool bNoCrypt)
{
	...
	if ( theApp.m_LastBlacklistedTime > time(NULL) ) {
		LogWarning(LOG_STATUSBAR, _T("Сервер временно заблокировал Вас для доступа. Дождитесь окончания блокировки."));
		return;
	}
...
bool CServerConnect::IsSocketOpened(const CServerSocket* socket)
{
	POSITION pos = m_lstOpenSockets.GetHeadPosition();
	while ( pos ) {
		CServerSocket* tmpsock = m_lstOpenSockets.GetNext(pos);
		if(tmpsock == socket)
			return true;
	}
	return false;
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		// ismod: set server ip
		theApp.SetServerIP(sender->cur_server->GetIP());
	...
	else if (sender->GetConnectionState() == CS_CONNECTED)
	{
		...
		/* ismod: moved to far far away, where we get public ip
...
bool CServerConnect::SendUDPPacket(Packet* packet, CServer* host, bool delpacket, uint16_t nSpecialPort, BYTE* pRawPacket, uint32_t nLen){
...
void CServerConnect::ConnectionFailed(CServerSocket* sender)
{
	...
	{
		case CS_AUTHFAILED:
			LogError(LOG_STATUSBAR, _T("Ошибка авторизации на сервере %s"), pServer->GetAddress() );
			break;
		...
		case CS_SERVERFULL:
			/* ismod: small hack to notify user about his network/firewall settings.
			...
			*/
			LogError(LOG_STATUSBAR, _T("Невозможно установить входящее соединение! Пожалуйста, проверьте настройки вашего антивируса или брандмауэра."));
			break;
...
{ 
	...
	if (thePrefs.GetProxySettings().UseProxy)
		dwServerConnectTimeout = max<DWORD>(dwServerConnectTimeout, CONNECTION_TIMEOUT);
...
void CServerConnect::DestroySocket(CServerSocket* pSck){
	...
	{
		POSITION posDel = pos;
		CServerSocket* pTestSck = m_lstOpenSockets.GetNext(pos);
		if (pTestSck == pSck)
		{
	...
	__try{
		...
		if (gethostname(szHost, sizeof szHost) == 0){
			...
			if (pHostEnt != NULL && pHostEnt->h_length == 4 && pHostEnt->h_addr_list[0] != NULL)
				m_nLocalIP = *((uint32_t*)pHostEnt->h_addr_list[0]);
		}
	}
...
bool CServerConnect::AwaitingTestFromIP(uint32_t dwIP) const{
