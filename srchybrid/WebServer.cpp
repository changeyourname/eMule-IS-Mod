...
void CWebServer::ReloadTemplates()
{
	...
	if (file.Open(sFile, CFile::modeRead|CFile::shareDenyWrite|CFile::typeText))
	{
		...
		else
		{
			...
			m_Templates.iProgressbarWidth = (uint16_t)_tstoi(_LoadTemplate(sAll,_T("PROGRESSBARWIDTH")));
...
{
	CServer* server=theApp.serverlist->GetServerByAddress(sIP, (uint16_t)nPort);
	if (server!=NULL) 
...
void CWebServer::ProcessURL(ThreadData Data)
{
	...
	uint32_t myip= inet_addr(CT2CA(ipstr(Data.inadr)));
	...
	if(_IsLoggedIn(Data, lSession))
	{
		...
		else if (_ParseURL(Data.sURL, _T("w")) == _T("commentlist"))
		{
			...
			if (kf) {
				if (thePrefs.GetMaxWebUploadFileSizeMB() != 0 && kf->GetFileSize() > (uint64_t)thePrefs.GetMaxWebUploadFileSizeMB()*1024*1024) {
					...
					if(file.Open(kf->GetFilePath(), CFile::modeRead|CFile::shareDenyWrite|CFile::typeBinary))
					{
						...
						char szBuf[512];
						int nLen = _snprintf_s(szBuf, _countof(szBuf), _countof(szBuf), "HTTP/1.1 200 OK\r\nContent-Type: application/octet-stream\r\nContent-Description: \"%s\"\r\nContent-Disposition: attachment; filename=\"%s\";\r\nContent-Transfer-Encoding: binary\r\nContent-Length: %I64u\r\n\r\n", 
...
*/
    Out.Replace(_T("[VersionCheck]"), _T(""));
	...
	else if (theApp.serverconnect->IsConnected() && !disconnectissued)
	{
		...
		if (cur_server) {
			...
			if ( cur_server->GetMaxUsers() > 0 )
				_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f "), (static_cast<double>(cur_server->GetUsers()) / cur_server->GetMaxUsers()) * 100.0);
			else
				_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f "), 0.0);
	...
	uint32_t allUsers = 0;
	uint32_t allFiles = 0;
	for (uint32_t sc=0;sc<theApp.serverlist->GetServerCount();sc++)
	{
	...
	if(thePrefs.GetMaxUpload() == UNLIMITED)
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), static_cast<double>(100 * theApp.uploadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxGraphUploadRate(true));
	else
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), static_cast<double>(100 * theApp.uploadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxUpload());
	Out.Replace(_T("[UploadValue]"), HTTPHeader);

	if(thePrefs.GetMaxDownload() == UNLIMITED)
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), static_cast<double>(100 * theApp.downloadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxGraphDownloadRate());
	else
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), static_cast<double>(100 * theApp.downloadqueue->GetDatarate()) / 1024 / thePrefs.GetMaxDownload());
	Out.Replace(_T("[DownloadValue]"), HTTPHeader);

	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), (static_cast<double>(theApp.listensocket->GetOpenSockets()))/(thePrefs.GetMaxConnections())*100.0);
	Out.Replace(_T("[ConnectionValue]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), (static_cast<double>(theApp.uploadqueue->GetDatarate())/1024.0));
	Out.Replace(_T("[CurUpload]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.1f"), (static_cast<double>(theApp.downloadqueue->GetDatarate())/1024.0));
	Out.Replace(_T("[CurDownload]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), ARRAYSIZE(HTTPHeader), _T("%.0f"), (static_cast<double>(theApp.listensocket->GetOpenSockets())));
...
CString CWebServer::_CreateTransferList(CString Out, CWebServer *pThis, ThreadData* Data, void* _FilesArray, void* _UploadArray, bool bAdmin) {
	...
	uint32_t nNextScore = 0;	// highest score -> next upload user
	...
	if(pThis->m_Params.bShowUploadQueue)
	{
		...
		for(int i = 0; i < QueueArray.GetCount(); i++)
		{
			...
			if (QueueArray[i].sClientExtra == _T("none"))
			{
				...
				{
					_sntprintf_s(HTTPTempC, _countof(HTTPTempC), ARRAYSIZE(HTTPTempC), _T("%i") , QueueArray[i].nScore);
					pcTmp = HTTPTempC;
				}
...
//		uint16 nCountLo, nCountHi;
	...
	for(int i = 0; i < SharedArray.GetCount(); i++)
	{
		...
		if (!WSsharedColumnHidden[2])
		{
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), ARRAYSIZE(HTTPTempC), _T("%i"), SharedArray[i].nFileRequests);
			HTTPProcessData.Replace(_T("[FileRequests]"), HTTPTempC);
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), ARRAYSIZE(HTTPTempC), _T("%i"), SharedArray[i].nFileAllTimeRequests);
		...
		if (!WSsharedColumnHidden[3])
		{
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), ARRAYSIZE(HTTPTempC), _T("%i"), SharedArray[i].nFileAccepts);
			HTTPProcessData.Replace(_T("[FileAccepts]"), HTTPTempC);
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), ARRAYSIZE(HTTPTempC), _T("%i"), SharedArray[i].nFileAllTimeAccepts);
...
CString CWebServer::_GetGraphs(ThreadData Data)
{
	...
	for(int i = 0; i < WEB_GRAPH_WIDTH; i++)
	{
		if(i < pThis->m_Params.PointsForWeb.GetCount())
		{
			...
			// download
			strGraphDownload.AppendFormat(_T("%u"), (uint32_t)(pThis->m_Params.PointsForWeb[i].download*1024));

			// upload
			strGraphUpload.AppendFormat(_T("%u"), (uint32_t)(pThis->m_Params.PointsForWeb[i].upload*1024));

			// connections
			strGraphCons.AppendFormat(_T("%u"), (uint32_t)(pThis->m_Params.PointsForWeb[i].connections));
		}
	}
...
CString CWebServer::_GetDownloadGraph(ThreadData Data, CString filehash)
{
	...
	else
	{
		...
		for (uint16_t i=0;i<pThis->m_Templates.iProgressbarWidth;i++)
		{
...
CString	CWebServer::_GetSearch(ThreadData Data)
{
	...
	for (uint16_t i = 0; i < SearchFileArray.GetCount(); ++i)
	{
...
{
	int oldvalue=static_cast<int>(m_Params.Sessions.GetSize());
	...
	return static_cast<int>(m_Params.Sessions.GetCount());
}
