...
BOOL CZIPFile::File::Extract(LPCTSTR pszFile)
{
	...
	uint64_t nCompressed = 0, nUncompressed = 0;
	...
	if ( m_nCompression == Z_DEFLATED )
	{
		...
		while ( nCompressed < m_nCompressedSize || nUncompressed < m_nSize )
		{
			...
			{
				pStream.avail_in	= min<UINT>( (UINT)(m_nCompressedSize - nCompressed), BUFFER_IN_SIZE );
	...
	else
	{
		...
		{
			DWORD nChunk = min<DWORD>( (DWORD)(m_nSize - nUncompressed), BUFFER_OUT_SIZE );
			DWORD nProcess = 0;
