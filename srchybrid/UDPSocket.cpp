...
{
	SRawServerPacket(BYTE* pPacket, UINT uSize, uint16_t nPort) {
		m_pPacket = pPacket;
		m_uSize = uSize;
		m_nPort = nPort;
	}
...
bool CUDPSocket::ProcessPacket(const BYTE* packet, UINT size, UINT opcode, uint32_t nIP, uint16_t nUDPPort)
{
	try
	{
			...
			case OP_GLOBFOUNDSOURCES:{
 			...
 			case OP_GLOBSERVSTATRES:{
				...
				if (size < 12 || pServer == NULL)
					return true;
				uint32_t challenge = PeekUInt32(packet);
				...
				uint32_t cur_user = PeekUInt32(packet+4);
				uint32_t cur_files = PeekUInt32(packet+8);
				uint32_t cur_maxusers = 0;
				uint32_t cur_softfiles = 0;
				uint32_t cur_hardfiles = 0;
				uint32_t uUDPFlags = 0;
				uint32_t uLowIDUsers = 0;
				uint32_t dwServerUDPKey = 0;
				uint16_t nTCPObfuscationPort = 0;
				uint16_t nUDPObfuscationPort = 0;
				...
				if (pServer){
					...
					if (pServer->GetLastDescPingedCount() < 2)
					{
						...
						uint32_t uDescReqChallenge = ((uint32_t)GetRandomUInt16() << 16) + INV_SERV_DESC_LEN; // 0xF0FF = an 'invalid' string length.
...
void CUDPSocket::ProcessPacketError(UINT size, UINT opcode, uint32_t nIP, uint16_t nUDPPort, LPCTSTR pszError)
{
	if (thePrefs.GetVerbose())
	{
		CString strName;
...
SocketSentBytes CUDPSocket::SendControlData(uint32_t maxNumberOfBytesToSend, uint32_t /*minFragSize*/) // ZZ:UploadBandWithThrottler (UDP)
{
// ZZ:UploadBandWithThrottler (UDP) -->
...
int CUDPSocket::SendTo(BYTE* lpBuf, int nBufLen, uint32_t dwIP, uint16_t nPort)
{
	// NOTE: *** This function is invoked from a *different* thread!
...
void CUDPSocket::SendBuffer(uint32_t nIP, uint16_t nPort, BYTE* pPacket, UINT uSize)
{
// ZZ:UploadBandWithThrottler (UDP) -->
...
void CUDPSocket::SendPacket(Packet* packet, CServer* pServer, uint16_t nSpecialPort, BYTE* pInRawPacket, uint32_t nRawLen)
{
	...
	uint32_t nIP = inet_addr(pszHostAddressA);
	if (nIP == INADDR_NONE)
	{
