		...
		for (UINT i = 0; i < RecordsNumber; i++) {
			...
			if (!pRecord->LoadFromFile(&file)){
				TRACE(_T("*** Failed to load entry %u (name=%s  hash=%s  size=%I64u  parthashs=%u expected parthashs=%u) from known.met\n"), i, 
					pRecord->GetFileName(), md4str(pRecord->GetFileHash()), pRecord->GetFileSize(), pRecord->hashset().count(), pRecord->GetED2KPartCount());
				delete pRecord;
				pRecord = NULL;
				continue;
			}
	...
	if (!file.Open(fullpath, CFile::modeWrite|CFile::modeCreate|CFile::typeBinary|CFile::shareDenyWrite, &fexp)){
		...
		try{
			...
			if (!thePrefs.IsRememberingCancelledFiles()){
				...
				UINT nRecordsNumber = static_cast<UINT>(m_mapCancelledFiles.GetCount());
				file.WriteUInt32(nRecordsNumber);
				POSITION pos = m_mapCancelledFiles.GetHeadPosition();
				while( pos != NULL )
				{
					int dwDummy;
					CSKey key;
...
CKnownFile* CKnownFileList::FindKnownFile(LPCTSTR filename, time_t date, uint64_t size) const
{
	POSITION pos = m_Files_map.GetHeadPosition();
	while (pos != NULL)
	{
...
CKnownFile* CKnownFileList::FindKnownFileByPath(LPCTSTR sFilePath) const
{
	POSITION pos = m_Files_map.GetHeadPosition();
	while (pos != NULL)
	{
...
void CKnownFileList::CopyKnownFileMap(CRBMap<CCKey,CKnownFile*,CCKeyTrait> &Files_Map)
{
