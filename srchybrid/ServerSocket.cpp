...
void CServerSocket::OnConnect(int nErrorCode)
{
	...
	// ismod: no limits for server connection
	ReallyDisableDownloadLimit(true);
...
bool CServerSocket::ProcessPacket(const BYTE* packet, uint32_t size, uint8_t opcode)
{
	try
	{
		switch (opcode)
		{
			case OP_SERVERMESSAGE:{
				...
				while (!message.IsEmpty())
				{
					...
					if (_tcsnicmp(message, _T("server version"), 14) == 0){
						...
						// ismod: set blacklist time
						if (message.Find(_T("blacklisted")) != -1)
						{
                            theApp.m_LastBlacklistedTime = time(NULL) + 1000;
						}
					}
			...
			case OP_IDCHANGE:{
				...
				if (cur_server){
					if (size >= sizeof(LoginAnswer_Struct)+4){
						DWORD dwFlags = *((uint32_t*)(packet + sizeof(LoginAnswer_Struct)));
						if (thePrefs.GetDebugServerTCPLevel() > 0){
							CString strInfo;
				...
				uint32_t dwServerReportedIP = 0;
				uint32_t dwObfuscationTCPPort = 0;
				if (size >= 20){
					dwServerReportedIP = *((uint32_t*)(packet + 12));
					if (::IsLowID(dwServerReportedIP)){
						ASSERT( false );
						dwServerReportedIP = 0;
					}
					ASSERT( dwServerReportedIP == la->clientid || ::IsLowID(la->clientid) );
					dwObfuscationTCPPort = *((uint32_t*)(packet + 16));
					if (cur_server != NULL && dwObfuscationTCPPort != 0)
						cur_server->SetObfuscationPortTCP((uint16_t)dwObfuscationTCPPort);
					if (pServer != NULL && dwObfuscationTCPPort != 0)
						pServer->SetObfuscationPortTCP((uint16_t)dwObfuscationTCPPort);

				}

				if (la->clientid == 0)
				{
					uint8_t state = thePrefs.GetSmartIdState();
					if ( state > 0 )
					{
						if (state == 1)
                ...
                // ismod: offer shared files, moved here from far place
                theApp.sharedfiles->SendListToServer();
                
                theApp.downloadqueue->ResetLocalServerRequests();

				// ismod: remove from blacklist time
				theApp.m_LastBlacklistedTime = 0;
				// ismod: start kad
			...
			case OP_SEARCHRESULT:{
				...
				UINT uSearchResults = theApp.searchlist->ProcessSearchAnswer(packet, size, true/*pServer ? pServer->GetUnicodeSupport() : false*/, cur_srv ? cur_srv->GetIP() : 0, cur_srv ? cur_srv->GetPort() : (uint16_t)0, &bMoreResultsAvailable);
			...
			case OP_SERVERSTATUS:{
				...
				uint32_t cur_user = PeekUInt32(packet);
				uint32_t cur_files = PeekUInt32(packet+4);
				CServer* pServer = cur_server ? theApp.serverlist->GetServerByAddress(cur_server->GetAddress(), cur_server->GetPort()) : NULL;
				if (pServer){
			...
			case OP_SERVERIDENT:{
				...
				uint32_t nServerIP = data.ReadUInt32();
				uint16_t nServerPort = data.ReadUInt16();
				if (thePrefs.GetDebugServerTCPLevel() > 0)
				...
				if (pServer){
					...
					if (((uint32_t*)aucHash)[0] == 0x2A2A2A2A){
			...
			case OP_SERVERLIST:{
				...
				try{
					...
					while(count)
					{
						uint32_t ip = servers.ReadUInt32();
						uint16_t port = servers.ReadUInt16();
			...
			case OP_CALLBACKREQUESTED:{
				...
				if (size >= 6)
				{
					uint32_t dwIP = PeekUInt32(packet);
					...
					uint16_t nPort = PeekUInt16(packet+4);
	...
	catch(CFileException* error)
	{
		if (thePrefs.GetVerbose())
		{
			...
			error->GetErrorMessage(szError, _countof(szError));
...
void CServerSocket::ConnectTo(CServer* server, bool bNoCrypt)
{
	...
	if ( thePrefs.IsAuthDisabled() ) {
		ContinueConnectTo();
	} else if ( (AuthProcess::StartAuth(this, cur_server->GetAddress())) == NULL ) {
		SetConnectionState(CS_AUTHFAILED);
		serverconnect->DestroySocket(this);
	}
}

void CServerSocket::ContinueConnectTo()
{
	serverconnect->AddConnectionAttemp(this);

	// ismod: здесь стоило разобраться, какой порт используется, но у нас нет нигде обфускации.
	Log(_T("Cоединение с %s (%s:%u)..."), cur_server->GetListName(), cur_server->GetAddress(), cur_server->GetPort());

	if (!Connect(CStringA(cur_server->GetAddress()), cur_server->GetPort())){
		DWORD dwError = GetLastError();
		if (dwError != WSAEWOULDBLOCK){
			LogError(GetResString(IDS_ERR_CONNECTIONERROR), cur_server->GetListName(), cur_server->GetAddress(), cur_server->GetPort(), GetFullErrorMessage(dwError));
			SetConnectionState(CS_FATALERROR);
		}
	}
}
...
// ismod [start]
void CServerSocket::OnAuthFailed()
{
	OnConnect(0xDEADBEEF);
}
// ismod [end]
...
{
	// ismod [start]
	if(thePrefs.IsServerPacketLoggerEnabled())
		LogPacket(packet->GetPacket(), packet->GetRealPacketSize(), theApp.GetServerIP(), theApp.GetPublicIP());
	// ismod [end]
#ifndef _DEBUG
	try {
#endif
		...
		{
			uint32_t uComprSize = packet->size;
...
void CServerSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32_t actualPayloadSize, bool bForceImmediateSend){
	m_dwLastTransmission = GetTickCount();
	// ismod: log server packets [start]
	// TODO: log only real sent packed to server
	if(thePrefs.IsServerPacketLoggerEnabled())
		LogPacket(packet->GetPacket(), packet->GetRealPacketSize(), theApp.GetPublicIP(), theApp.GetServerIP());
	// ismod [end]
