...
void CCollection::SetCollectionAuthorKey(const byte* abyCollectionAuthorKey, uint32_t nSize)
{
	delete[] m_pabyCollectionAuthorKey;
	m_pabyCollectionAuthorKey = NULL;
	m_nKeySize = 0;
		...
		try
		{
			...
			if (m_pabyCollectionAuthorKey != NULL){
					...
					if (data.GetLength() > data.GetPosition()){
						...
						uint32_t nPos = (uint32_t)data.GetPosition();
						data.SeekToBegin();
	...
	else
	{
		...
		if(data.Open(sFilePath, CFile::modeCreate | CFile::modeReadWrite | CFile::shareDenyWrite | CFile::typeBinary))
		{
			try
			{
				...
				uint32_t dwVersion = COLLECTION_FILE_VERSION1_INITIAL;
				POSITION pos = m_CollectionFilesMap.GetStartPosition();
				CCollectionFile* pCollectionFile;
				CSKey key;
				while( pos != NULL ) {
				...
				//Total Files
				data.WriteUInt32(static_cast<uint32_t>(m_CollectionFilesMap.GetSize()));
				...
				if (pSignKey != NULL){
					...
					int nResult = (uint8_t)asink.TotalPutLength();
