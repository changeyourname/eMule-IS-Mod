...
{
	/* ismod: show real name for connected friends
	SetItemText(iItem, 0, pFriend->m_strName.IsEmpty() ? _T('(') + GetResString(IDS_UNKNOWN) + _T(')') : pFriend->m_strName);
	*/
	CUpDownClient *pLinkedClient = pFriend->GetLinkedClient();
	if ( pLinkedClient ) {
		SetItemText(iItem, 0, pLinkedClient->GetUserName());
	} else {
		SetItemText(iItem, 0, pFriend->m_strName.IsEmpty() ? _T('(') + GetResString(IDS_UNKNOWN) + _T(')') : pFriend->m_strName);
	}
...
bool CFriendListCtrl::RemoveFriend( CFriend* pFriend )
{
	// ismod: ask before delete
	if(AfxMessageBox(GetResString(IDS_MOD_DELFRIEND_ACK), MB_OKCANCEL) != IDOK)
		return false;
	pFriend->SetLinkedClient(NULL);
...
void CFriendListCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	...
	ClientMenu.SetDefaultItem(MP_MESSAGE);
	...
	switch (wParam)
	{
		...
		case MP_FRIENDSLOT:
			if (cur_friend)
			{
				...
				//if (!bIsAlready) // ismod: unset friend slot
				cur_friend->SetFriendSlot(!bIsAlready);
			}
			break;
		case MP_FIND:
			OnFindStart();
...
void CFriendListCtrl::OnNmDblClk(NMHDR* /*pNMHDR*/, LRESULT* pResult)
{
	int iSel = GetNextItem(-1, LVIS_SELECTED | LVIS_FOCUSED);
	if (iSel != -1) 
		/* ismod: start chat on double click
		ShowFriendDetails((CFriend*)GetItemData(iSel));
		*/
		theApp.emuledlg->chatwnd->StartSession(((CFriend*)GetItemData(iSel))->GetClientForChatSession());
