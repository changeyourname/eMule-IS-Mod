...
// ismod
#define AUTO_REDRAW_TIMER 32456
...
void CMuleListCtrl::SetColors()
{
	...
	if (!strBkImage.IsEmpty() )
	{
		...
		{
			_tcsncpy_s(szFullResPath, _countof(szFullResPath), strBkImage, _countof(szFullResPath));
...
BOOL CMuleListCtrl::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult) {
	...
	case LVM_SORTITEMS:
		m_dwParamSort = static_cast<DWORD>(wParam);
		m_SortProc = reinterpret_cast<PFNLVCOMPARE>(lParam);
	...
	case LVM_UPDATE:
		...
		if (wParam == static_cast<WPARAM>(UpdateLocation(static_cast<int>(wParam)))) { //no need to invalidate rect if item moved
			RECT rcItem;
			BOOL bResult = GetItemRect(static_cast<int>(wParam), &rcItem, LVIR_BOUNDS);
			if(bResult)
				InvalidateRect(&rcItem, FALSE);
			return (*pResult = bResult) != FALSE;
		}
		return (*pResult = TRUE) == TRUE;
		...
		if ((HWND)wParam == m_hWnd)
		{
			...
			if (ptMouse.x != -1 || ptMouse.y != -1)
			{
				...
				if (!rcClient.PtInRect(ptMouse))
					return (*pResult = DefWindowProc(message, wParam, lParam)) != FALSE;
			}
		}
		break;

	case WM_TIMER:
		if ( AUTO_REDRAW_TIMER == wParam ) {
			RECT rc;
			GetClientRect(&rc);
