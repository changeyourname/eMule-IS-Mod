...
CServer::CServer(uint16_t in_port, LPCTSTR i_addr)
{
	port = in_port;
...
bool CServer::AddTagFromFile(CFileDataIO* servermet)
{
	...
	case ST_TCPPORTOBFUSCATION:
		ASSERT( tag->IsInt() );
		if (tag->IsInt())
			m_nObfuscationPortTCP = (uint16_t)tag->GetInt();
		break;
	case ST_UDPPORTOBFUSCATION:
		ASSERT( tag->IsInt() );
		if (tag->IsInt())
			m_nObfuscationPortUDP = (uint16_t)tag->GetInt();
		break;
	default:
...
void CServer::SetIP(uint32_t newip)
{
	ip = newip;
	_tcscpy_s(ipfull, _countof(ipfull), ipstr(ip));
}
...
uint32_t CServer::GetServerKeyUDP(bool bForce) const{
...
void CServer::SetServerKeyUDP(uint32_t dwServerKeyUDP){
