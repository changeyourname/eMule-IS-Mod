...
public:
	...
	*/
	...
	UINT ShowPreferences(UINT uStartPageID = (UINT)-1);
	bool IsPreferencesDlgOpen() const;
	bool IsTrayIconToFlash()	{ return m_iMsgIcon!=0; }
	void SetToolTipsDelay(UINT uDelay);
	void StartUPnP(bool bReset = true, uint16_t nForceTCPPort = 0, uint16_t nForceUDPPort = 0);
	...
	// ismod virtual void HtmlHelp(DWORD_PTR dwData, UINT nCmd = 0x000F);
	...
	// ismod [start]
	CSctWnd             *sctwnd;
	CXCatalogWnd        *xcatwnd;
	// ismod [end]
	CWnd                *activewnd;
	uint8_t			status;

	// ismod: new clipboard handler
	CDirectDownloadDlg dlgDownload;
	HWND hwndNextViewer;
...
protected:
	...
	uint32_t			m_uUpDatarate;
	uint32_t			m_uDownDatarate;
...
protected:
	...
	void UpdateTrayIcon(int iDownPercent, int iUpPercent);
...
// ismod: fastspeed [start]
	...
	// END SiRoB: Flush Thread
// ismod: fastspeed [end]
	afx_msg LRESULT OnPartFileIoErr(WPARAM wParam,LPARAM lParam);
    ...
    afx_msg LRESULT OnUpdateDownloadResponse(WPARAM wParam, LPARAM lParam);
	...
	afx_msg LRESULT	OnAuthEvent(WPARAM wParam,LPARAM lParam);
	// ismod [end]	

// ismod [start]
#ifdef _DEBUG
	TransferDebugWnd *m_trabsferDebugWnd;
public:
	void updateTransferDebug( CPartFile *file ) { if(m_trabsferDebugWnd) m_trabsferDebugWnd->updateInfo(file); }
#endif
// ismod [end]
};
