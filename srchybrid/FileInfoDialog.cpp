...
class CGetMediaInfoThread : public CSafeThread
{
...
BOOL CFileInfoDialog::OnInitDialog()
{
	...
	// General Group - Labels
	AddAnchor(IDC_FD_XI2, TOP_LEFT);
	AddAnchor(IDC_STATICFI, TOP_LEFT);
	...
	// Video Group - Labels
	AddAnchor(IDC_FD_XI5, TOP_LEFT);
	AddAnchor(IDC_FD_XI7, TOP_LEFT);
	AddAnchor(IDC_FD_XI9, TOP_LEFT);
	AddAnchor(IDC_FD_XI14, TOP_LEFT);
	AddAnchor(IDC_FD_XI13, TOP_LEFT);
...
BOOL CFileInfoDialog::OnSetActive()
{
	...
	if (m_bDataChanged)
	{
		...
		CGetMediaInfoThread* pThread = (CGetMediaInfoThread*)CSafeThread::BeginThread(RUNTIME_CLASS(CGetMediaInfoThread), THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
		if (pThread)
		{
...
IMPLEMENT_DYNCREATE(CGetMediaInfoThread, CSafeThread)
...
LRESULT CFileInfoDialog::OnMediaInfoResult(WPARAM, LPARAM lParam)
{
	...
	for (int i = 0; i < paMediaInfo->GetSize(); i++)
	{
		...
		mi.InitFileLength();
		uTotalFileSize += (uint64_t)mi.ulFileSize;
		if (i == 0)
		{
			ami = mi;
		}
