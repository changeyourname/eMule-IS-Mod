...
CClientReqSocket::~CClientReqSocket()
{
	...
	if (client)
	{
		// netfinity: Extra safety check, by Maella
		if (client->socket == this)
		client->socket = 0;
		else if (client->socket != NULL)
			DebugLogWarning(_T(__FUNCTION__) _T("; Cross linking is corrupt!"));
	}
	client = 0;
...
{
	// netfinity: Clean up previous linkage
	if (client)
	{
		// netfinity: Extra safety check, by Maella
		if (client->socket == this)
...
bool CClientReqSocket::ProcessPacket(const BYTE* packet, uint32_t size, UINT opcode)
{
	try
	{
		try
		{
			...
			switch(opcode)
			{
					...
					if (size >= 16)
					{
						...
						if ( (reqfile = theApp.sharedfiles->GetFileByID(reqfilehash)) == NULL ){
							if ( !((reqfile = theApp.downloadqueue->GetFileByID(reqfilehash)) != NULL
								   && reqfile->GetFileSize() > (uint64_t)PARTSIZE) )
							{
						...
						if (reqfile->IsPartFile() && reqfile->GetFileSize() > (uint64_t)PARTSIZE)
						{
						...
						switch (client->GetDownloadState())
						{
							...
							case DS_NONEEDEDPARTS:
                                ...
                                if (!client->SwapToAnotherFile(_T("Source says it doesn't have the file. CClientReqSocket::ProcessPacket()"), true, true, true, NULL, false, false)) { // ZZ:DownloadManager
									...
									*/
									client->SetDownloadState(DS_NONEEDEDPARTS);
								}
							break;
						}
						break;
				...
				case OP_REQUESTPARTS:
				{
					...
					for (int i = 0; i < _countof(auStartOffsets); i++)
					{
				...
				case OP_CHANGE_CLIENT_ID:
				{
					...
					CSafeMemFile data(packet, size);
					uint32_t nNewUserID = data.ReadUInt32();
					uint32_t nNewServerIP = data.ReadUInt32();
					if (thePrefs.GetDebugClientTCPLevel() > 0)
				...
				case OP_MESSAGE:
				{
					...
					// ismod: если сообщение придет в ютф8, то его реальная длина нифига не в байтах должна мерица
					if (length > MAX_CLIENT_MSG_LEN*3){
						if (thePrefs.GetVerbose())
				...
				{	
					...
					if (thePrefs.CanSeeShares()==vsfaEverybody || (thePrefs.CanSeeShares()==vsfaFriends && client->IsFriend()))
					{
						CCKey bufKey;
						CKnownFile* cur_file;
						for (POSITION pos = theApp.sharedfiles->m_Files_map.GetHeadPosition();pos != 0;)
						{
					...
					uint32_t iTotalCount = (uint32_t) list.GetCount();
					CSafeMemFile tempfile(80);
					tempfile.WriteUInt32(iTotalCount);
					while (list.GetCount())
					{
						// ismod: skip files with bad words
						CKnownFile *file = (CKnownFile*)list.GetHead();
						if(!thePrefs.HasBadWords(file->GetFileName()))
							theApp.sharedfiles->CreateOfferedFilePacket(file, &tempfile, NULL, client);
						else
							iTotalCount--;
						list.RemoveHead();
					}
                ...
                case OP_ASKSHAREDFILESDIR:
				{
                    ...
                    if (thePrefs.CanSeeShares()==vsfaEverybody || (thePrefs.CanSeeShares()==vsfaFriends && client->IsFriend()))
					{
						AddDebugLogLine(true, GetResString(IDS_SHAREDREQ2), client->GetUserName(), client->GetUserIDHybrid(), strReqDir, GetResString(IDS_ACCEPTED));
						...
						else
						{
							...
							if (!strReqDir.IsEmpty())
							{
								...
								SharedDirsMap::const_iterator map_iter = theApp.sharedfiles->sharedDirs.find(strReqDir);
								if ( map_iter != theApp.sharedfiles->sharedDirs.end() ) {
									SharedDir* sharedDir = map_iter->second;
									KnownFileSet::const_iterator iter = sharedDir->getConstIteratorBegin();
									while ( iter != sharedDir->getConstIteratorEnd() ) {
										if(!(*iter)->IsLargeFile() || client->SupportsLargeFiles())
											list.AddTail(*iter);
										++iter;
									}
								}
						...
						//ismod: skip files with bad words [start]
						tempfile.SeekToBegin();
						tempfile.WriteString(strOrgReqDir, client->GetUnicodeSupport());
						tempfile.WriteUInt32(iTotalCount);
						tempfile.SeekToEnd();
				...
				case OP_ASKSHAREDDIRSANS:
				{
                    ...
                    if (client->GetFileListRequested() == 1)
					{
                        ...
                        for (UINT i = 0; i < uDirs; i++)
						{
							...
							AddDebugLogLine(true, GetResString(IDS_SHAREDANSW), client->GetUserName(), client->GetUserIDHybrid(), strDir);
                ...
                case OP_ASKSHAREDFILESDIRANS:
				{
					...
					{
						AddDebugLogLine(true, GetResString(IDS_SHAREDINFO1), client->GetUserName(), client->GetUserIDHybrid(), strDir);
				...
				case OP_ASKDIRCONTENTS:
				{
					...
					CKnownFile* collFile = theApp.sharedfiles->GetFileByID(hash);
					if ( !collFile ) {
						AddLogLine(true, _T("User '%s'(%d) requested contents of unknown collection."), client->GetUserName(), client->GetUserIDHybrid());
						return false;
					}
					SharedDir* sharedDir = theApp.sharedfiles->GetSharedDirByCollName(collFile->GetFileName());
					if ( !sharedDir ) {
						AddLogLine(true, _T("Ouch... can't find '%s' shared directory in list."), collFile->GetFileName());
						return false;
					}
					CSafeMemFile tempfile(80);
					tempfile.WriteHash16(hash);
					uint32_t iTotalCount = sharedDir->filesCount();
					tempfile.WriteUInt32(iTotalCount);
					KnownFileSet::const_iterator iter = sharedDir->getConstIteratorBegin();
					while ( iter != sharedDir->getConstIteratorEnd() )
					{
						CKnownFile *file = *iter;
						if(!thePrefs.HasBadWords(file->GetFileName()))
							theApp.sharedfiles->CreateOfferedFilePacket(file, &tempfile, NULL, client);
						else
							iTotalCount--;
					...
					Packet* replypacket = new Packet(&tempfile);
					replypacket->opcode = OP_ASKDIRCONTENTSANS;
					theStats.AddUpDataOverheadOther(replypacket->size);
					SendPacket(replypacket, true, true);
					break;
				}
                case OP_ASKDIRCONTENTSANS:
                {
                    theStats.AddDownDataOverheadOther(size);
                    CSafeMemFile data(packet, size);
                    if (client->GetDirContentsRequested() > 0)
                    {
                        client->ProcessDirContentsList(packet, size);
                    }
                    break;
                }
                // ismod [end]
...
bool CClientReqSocket::ProcessExtPacket(const BYTE* packet, uint32_t size, UINT opcode, UINT uRawSize)
{
	try
	{
		try
		{
			...
			switch(opcode)
			{
					...
					while (data_in.GetLength()-data_in.GetPosition() && !bAnswerFNF)
					{
						...
						switch (opcode_in)
						{
							...
							case OP_REQUESTSOURCES:
							{
								...
								uint8_t byRequestedVersion = 0;
								uint16_t byRequestedOptions = 0;
				...
				case OP_MULTIPACKETANSWER:
				{
					...
					if (reqfile==NULL){
						client->CheckFailedFileIdReqs(reqfilehash);
						TRACE(GetResString(IDS_ERR_WRONGFILEID) + _T(" (OP_MULTIPACKETANSWER; reqfile==NULL)"));
						break;
					}
					if (client->GetRequestFile()==NULL) {
						TRACE(GetResString(IDS_ERR_WRONGFILEID) + _T(" (OP_MULTIPACKETANSWER; client->GetRequestFile()==NULL)"));
						break;
					}
					if (reqfile != client->GetRequestFile()) {
						TRACE(GetResString(IDS_ERR_WRONGFILEID) + _T(" (OP_MULTIPACKETANSWER; reqfile!=client->GetRequestFile())"));
						break;
					}
					uint8_t opcode_in;
 				...
 				case OP_ANSWERSOURCES2:
				{
					...
					CSafeMemFile data(packet, size);
					uint8_t byVersion = data.ReadUInt8();
					uchar hash[16];
					data.ReadHash16(hash);
					CKnownFile* file = theApp.downloadqueue->GetFileByID(hash);
					if (file){
...
//					client->ProcessPeerCacheAcknowledge(packet, size);
				...
				case OP_CALLBACK:
				{
					...
					if(check == Kademlia::CKademlia::GetPrefs()->GetKadID())
					{
						...
						uint32_t ip = data.ReadUInt32();
						uint16_t tcp = data.ReadUInt16();
				...
				case OP_REASKCALLBACKTCP:
				{
					...
					CSafeMemFile data_in(packet, size);
					uint32_t destip = data_in.ReadUInt32();
					uint16_t destport = data_in.ReadUInt16();
					uchar reqfilehash[16];
					data_in.ReadHash16(reqfilehash);
					if (thePrefs.GetDebugClientTCPLevel() > 0)
					...
					if (sender)
					{
						...
						if (md4cmp(reqfilehash, sender->GetUploadFileID()) == 0)
						{
							...
							{
								uint16_t nCompleteCountLast= sender->GetUpCompleteSourcesCount();
								uint16_t nCompleteCountNew = data_in.ReadUInt16();
							...
							data_out.WriteUInt16((uint16_t)theApp.uploadqueue->GetWaitingPosition(sender));
							if (thePrefs.GetDebugClientUDPLevel() > 0)
					...
					else
					{
						if (!bSenderMultipleIpUnknown){
							if (((uint32_t)theApp.uploadqueue->GetWaitingUserCount() + 50) > thePrefs.GetQueueSize())
							{
								if (thePrefs.GetDebugClientUDPLevel() > 0)
	...
	// netfinity: If we don't catch it here eMule will quit
	catch(...)
	{
		DebugLogWarning(_T("Error: Unknown exception while processing eMule packet: opcode=%s  size=%u; %s"), DbgGetMuleClientTCPOpcode(opcode), size, DbgGetClientInfo());
		if (client)
			client->SetDownloadState(DS_ERROR, _T("ProcessExtPacket unknown exception."));
		Disconnect(_T("ProcessExtPacket unknown exception."));
		return false;
	}
	return true;
}

void CClientReqSocket::PacketToDebugLogLine(LPCTSTR protocol, const uchar* packet, uint32_t size, UINT opcode)
{
	if (thePrefs.GetVerbose())
	{
		CString buffer; 
		...
		for (i = 0; i < size && i < 50; i++){
			if (i > 0)
				buffer += _T(' ');
			TCHAR temp[3];
		    _sntprintf_s(temp, _countof(temp), ARRAYSIZE(temp), _T("%02x"), packet[i]);
			buffer += temp;
		}
		buffer += (i == size) ? _T("]") : _T("..]");
...
int FilterSE(DWORD dwExCode, LPEXCEPTION_POINTERS pExPtrs, CClientReqSocket* reqsock, Packet* packet)
{
	...
	//
	// 03-Jдn-2004 [bc]: Returning the execution to the catch-all handler in 'CAsyncSocketExHelperWindow::WindowProc'
...
SocketSentBytes CClientReqSocket::SendControlData(uint32_t maxNumberOfBytesToSend, uint32_t overchargeMaxBytesToSend)
{
...
SocketSentBytes CClientReqSocket::SendFileAndControlData(uint32_t maxNumberOfBytesToSend, uint32_t overchargeMaxBytesToSend)
{
...
void CClientReqSocket::SendPacket(Packet* packet, bool delpacket, bool controlpacket, uint32_t actualPayloadSize, bool bForceImmediateSend)
{
	ResetTimeOutTimer();
// BEGIN netfinity: Compress packets to save some bandwidth (TODO: make it depending on CPU utilization)
//	if (client->GetClientSoft() == SO_EMULE && client->GetVersion() >= MAKE_CLIENT_VERSION(0, 47, 1))
//		packet->PackPacket();
// END netfinity: Compress packets to save some bandwidth
...
CListenSocket::CListenSocket()
{
	...
	m_nHalfOpen = 0;
	m_nComp = 0;
	// netfinity: Precise sockets per 5 average
	m_AvgSocketsPerFive.SetSamplePeriod(100);
	m_AvgSocketsPerFive.SetSamples(50);
}
...
bool CListenSocket::StartListening()
{
	...
	// port!
	if (!Create(thePrefs.GetPort(), SOCK_STREAM, FD_ACCEPT, thePrefs.GetBindAddrA(), false))
	{
		LogError(_T("Unable to start listening %d port! (ErrCode: %d)"),thePrefs.GetPort(), WSAGetLastError());
		return false;
	}
	...
	if (!nErrorCode)
	{
		...
		while (m_nPendingConnections > 0)
		{
			...
			else
			{
				...
				// ismod: turn off limit for server connections
				if(theApp.GetServerIP() == SockAddr.sin_addr.S_un.S_addr) {
					newclient->ReallyDisableDownloadLimit(true);
				}
			}
...
void CListenSocket::AddConnection()
{
	...
	// netfinity: Precise sockets per 5 average
	m_AvgSocketsPerFive.AddTransferred(1);
}
...
bool CListenSocket::TooManySockets(bool bIgnoreInterval)
{
	if (   GetOpenSockets() > thePrefs.GetMaxConnections()
		...
		|| (m_AvgSocketsPerFive >= thePrefs.GetMaxConperFive() && !bIgnoreInterval) // netfinity: Precise sockets per 5 average
