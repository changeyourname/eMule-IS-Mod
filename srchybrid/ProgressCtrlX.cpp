...
{
	SetGradientColors(static_cast<COLORREF>(clrStart), clrEnd ? static_cast<COLORREF>(clrEnd) : static_cast<COLORREF>(clrStart));
...
{
	m_clrBk = static_cast<COLORREF>(clrBk);
	return CLR_DEFAULT;
}
...
{
	int nOldPos;
	if(SetSnakePos(nOldPos, static_cast<int>(newPos)))
		return nOldPos;
...
{
	int nOldPos;
	if(SetSnakePos(nOldPos, static_cast<int>(nIncrement), TRUE))
		return nOldPos;
...
BOOL CProgressCtrlX::SetSnakePos(int& nOldPos, int nNewPos, BOOL fIncrement)
{
	...
	nOldPos = static_cast<int>(DefWindowProc(PBM_SETPOS, nNewPos, 0));
	return TRUE;
}
