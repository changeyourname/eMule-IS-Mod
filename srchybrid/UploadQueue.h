...
public:
    ...
    uint32_t  GetToNetworkDatarate();

	bool	CheckForTimeOver(CUpDownClient* client);
	int		GetWaitingUserCount() const				{return static_cast<int>(waitinglist.GetCount());}
	int		GetUploadQueueLength() const			{return static_cast<int>(uploadinglist.GetCount());}
	uint32_t	GetActiveUploadsCount()	const			{return m_MaxActiveClientsShortTime;}
	uint32_t	GetWaitingUserForFileCount(const CSimpleArray<CObject*>& raFiles, bool bOnlyIfChanged);
	uint32_t	GetDatarateForFile(const CSimpleArray<CObject*>& raFiles) const;
	...
	CUpDownClient*	GetWaitingClientByIP_UDP(uint32_t dwIP, uint16_t nUDPPort, bool bIgnorePortOnUniqueIP, bool* pbMultipleIPs = NULL);
...
protected:
	...
	bool		AcceptNewClient(uint32_t curUploadSlots);
...
private:
	...
	uint32_t	GetMaxClientScore()						{return m_imaxscore;}
	...
	CList<uint64_t> avarage_dr_list;
    CList<uint64_t> avarage_friend_dr_list;
	CList<DWORD,DWORD> avarage_tick_list;
	CList<int,int> activeClients_list;
    CList<DWORD,DWORD> activeClients_tick_list;
	uint32_t	datarate;   //datarate sent to network (including friends)
    uint32_t  friendDatarate; // datarate of sent to friends (included in above total)
	// By BadWolf - Accurate Speed Measurement
	...
	uint32_t	m_dwRemovedClientByScore;

	uint32_t	m_imaxscore;

    DWORD   m_dwLastCalculatedAverageCombinedFilePrioAndCredit;
    float   m_fAverageCombinedFilePrioAndCredit;
    uint32_t  m_iHighestNumberOfFullyActivatedSlotsSinceLastCall;
    uint32_t  m_MaxActiveClients;
    uint32_t  m_MaxActiveClientsShortTime;

    DWORD   m_lastCalculatedDataRateTick;
    uint64_t  m_avarage_dr_sum;
