...
BOOL CSharedFilesWnd::OnInitDialog()
{
	...
	m_dlgDetails.SetWindowPos(NULL, rcFiles.left, rcFiles.bottom + 4, rcFiles.Width(), rcSpl.bottom - (rcFiles.bottom + 3), 0);
...
int CSharedFileDetailsModelessSheet::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	return CListViewPropertySheet::OnCreate(lpCreateStruct);
}
...
// ismod [start]
void CSharedFilesWnd::GotoFile(const CKnownFile *pFile)
{
	CString path;
	ESpecialDirectoryItems eItemType = SDI_DIRECTORY;
	
	// category or incoming
	for(int i = 0; i<thePrefs.GetCatCount(); ++i)
	{
		if(thePrefs.GetCategory(i)->strIncomingPath == pFile->GetPath())
		{
			eItemType = SDI_INCOMING;
			break;
		}
	}
	
	// incoming
	if ( eItemType == SDI_DIRECTORY || pFile->GetPath() != thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR) )
		path = pFile->GetPath();

	m_ctlSharedDirTree.ShowDirectory(path, eItemType);
	sharedfilesctrl.GotoFile(pFile);
}
